...
namespace UnitySweeper
{
    public class ShaderReferenceCollection : IReferenceCollection
    {
        ...
        private readonly Dictionary<string, string> shaderFileList = new();
        private List<CollectionData> references = new();
        ...
        private void CollectionShaderFiles()
        {
            ...
            foreach (var cgincPath in cgincFiles)
            {
                ...
                if (shaderFileList.ContainsKey(file) == false) shaderFileList.Add(file, cgincPath);
            }
        }
        ...
        private void CheckReference()
        {
            foreach (var shader in shaderFileList)
            {
                ...
                if (File.Exists(shaderFilePath) == false) continue;
                ...
                {
                    if (checkingShaderName == shader.Key) continue;
                        ...
                        shaderFileList.ContainsKey(checkingShaderName))
                    {
                        ...
                        if (referenceList.Contains(fileGuid) == false) referenceList.Add(fileGuid);
                    }
                }
            }
        }
