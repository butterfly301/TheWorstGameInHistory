...
public class ToonWaterInspector : ShaderGUI
{
    ...
    private static bool lightningFoldout;
    private static bool fresnelFoldout;
    private static bool refractionFoldout;
    private static bool foamFoldout;
    private static bool wavesFoldout;
    private static bool reflectionFoldout;
    ...
    public override void OnGUI(MaterialEditor editor, MaterialProperty[] properties)
    {
        ...
        if (colorsFoldout)
        {
            var waterDepth = FindProperty(WATER_DEPTH_ID, properties);
            editor.FloatProperty(waterDepth, "Water Depth");

            var shallowColor = FindProperty(SHALLOW_WATER_COLOR_ID, properties);
            editor.ColorProperty(shallowColor, "Shallow Water Color");

            var deepColor = FindProperty(DEEP_WATER_COLOR_ID, properties);
        ...
        if (lightningFoldout)
        {
            var specularPower = FindProperty(SPECULAR_ID, properties);
            editor.RangeProperty(specularPower, "Specular Power");

            var specularCutoff = FindProperty(SPECULAR_CUTOFF_ID, properties);
            editor.RangeProperty(specularCutoff, "Specular Cutoff");

            var specularEdgesSmoothnessFactor = FindProperty(SPECULAR_EDGES_SMOOTHNESS_FACTOR_ID, properties);
            editor.RangeProperty(specularEdgesSmoothnessFactor, "Specular Edges Smoothness Factor");

            var specularColor = FindProperty(SPECULAR_COLOR_ID, properties);
            editor.ColorProperty(specularColor, "Specular Color");

            var normalsStrenght = FindProperty(NORMALS_STRENGTH_ID, properties);
        ...
        if (fresnelFoldout)
        {
            var fresnelPower = FindProperty(FRESNEL_POWER_ID, properties);
            editor.RangeProperty(fresnelPower, "Fresnel Power");

            if (fresnelPower.floatValue <= 0)
                EditorGUILayout.HelpBox(
                    "When Fresnel Power is set to 0 effect is disabled and Planar Reflections will not be visible!",
                    MessageType.Info);
        }
        ...
        {
            var refractionStrenght = FindProperty(REFRACTION_STRENGTH_ID, properties);
            editor.RangeProperty(refractionStrenght, "Refraction Strength");

            var useRefraction = FindProperty(USE_REFRACTION_IN_DEPTH_BASED_WATER_COLOR, properties);
            useRefraction.floatValue =
                EditorGUILayout.Toggle("Use Refraction In Depth Based Water Color", useRefraction.floatValue > 0)
                    ? 1
                    : 0;
        }
        ...
        {
            var reflectionVisiblity = FindProperty(REFLECTION_VISIBILITY_ID, properties);
            editor.RangeProperty(reflectionVisiblity, "Reflection Visibility");

            var reflectionDistortion = FindProperty(REFLECTION_DISTORTION_STRENGHT, properties);
        ...
        if (foamFoldout)
        {
            var foamColor = FindProperty(FOAM_COLOR_ID, properties);
            editor.ColorProperty(foamColor, "Foam Color");

            var foamAmount = FindProperty(FOAM_AMOUNT_ID, properties);
            editor.FloatProperty(foamAmount, "Foam Amount");

            var foamCutoff = FindProperty(FOAM_CUTOFF_ID, properties);
            editor.FloatProperty(foamCutoff, "Foam Cutoff");

            var foamDirection = FindProperty(FOAM_DIRECTION_ID, properties);
            editor.VectorProperty(foamDirection, "Foam Direction");

            var foamScale = FindProperty(FOAM_SCALE_ID, properties);
        ...
        if (wavesFoldout)
        {
            var waveDir = FindProperty(WAVE_DIRECTION_ID, properties);
            editor.VectorProperty(waveDir, "Wave Direction");

            var waveSize = FindProperty(WAVE_SIZE_ID, properties);
