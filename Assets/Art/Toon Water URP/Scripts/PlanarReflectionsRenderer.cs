...
public class PlanarReflectionsRenderer : MonoBehaviour
{
    ...
    /// </summary>
    [Tooltip(
        "Select the resolution of reflection texture relative to screen resolution. Full means the best reflection quality but the worst performance. You should keep this setting set to Quarter or Third to get the best performance.")]
    public ResolutionScales ResolutionScale = ResolutionScales.Half;

    [Tooltip(
        "Skip every n frame to gain performance. For example this setting set to EverySecondFrame will make refelction render with half the fps of your game. However setting to anything over NoSkipFrame creates effect where objects are a bit off their position when changing camera position/rotation.")]
    public SkipFrames SkipEveryNFrame = SkipFrames.NoSkipFrame;

    [Tooltip(
        "Always make sure water layer is deselected (and your water plane is assigned to water layer)! Which layers of objects should we relfect? Usefull to skip in rendering f.e. small obejcts and make rendering reflections faster.")]
    public LayerMask LayersToReflect = -1;

    [Tooltip(
        "Render objects in reflection texture with shadows? Usually you want to turn it off because it's hard to notice that there is lack of shadows in reflections.")]
    public bool RenderWithShadows;

    [Tooltip(
        "Select to use occlusion culling when rendering objects to reflection texture. If you see some artifacts disable this option.")]
    public bool RenderWithOcclusionCulling = true;

    [Tooltip("Water plane")] public GameObject WaterPlane;

    [Tooltip("Offset reflection rendering to fix artifacts at geometry borders")]
    public float ClipPlaneOffset = 0.03f;

    [Tooltip(
        "Which renderer should this script use to render reflections texture? If you are not using any custom post processing/rendering with render features you probably want to leave it set to 0.")]
    ...
    private readonly int reflectionTextureId = Shader.PropertyToID("_WaterReflectionTexture");
    ...
    private void UpdateCamera(Camera src, Camera dest)
    {
        ...
        var data = dest.gameObject.GetComponent<UniversalAdditionalCameraData>();
        if (data != null) data.renderShadows = RenderWithShadows;
    }

    private void UpdateReflectionCamera(Camera realCamera)
    {
        //check if we allready created reflection camera
        if (reflectionCamera == null) reflectionCamera = CreateReflectionCamera();

        //get water plane informations
        var planePosition = new Vector3(0, 0, 0);
        ...
        var w = -Vector3.Dot(planeNormal, planePosition) - ClipPlaneOffset;
        var reflectionPlane = new Vector4(planeNormal.x, planeNormal.y, planeNormal.z, w);

        var reflection = Matrix4x4.identity;
        reflection *= Matrix4x4.Scale(new Vector3(1, -1, 1));

        CalculateReflectionMatrix(ref reflection, reflectionPlane);
        var oldPosition = realCamera.transform.position - new Vector3(0, planePosition.y * 2, 0);
        ...
        var clipPlane = WorldToCameraSpacePlane(reflectionCamera, planePosition - Vector3.up * 0.1f, planeNormal, 1.0f);
    ...
    {
        var go = new GameObject("Planar Reflections Camera", typeof(Camera));
    ...
    private void CreatePlanarReflectionTexture(Camera cam)
    {
        ...
        var textureRes =
            new Vector2((int)(cam.pixelWidth * UniversalRenderPipeline.asset.renderScale * GetScaleValue()),
                (int)(cam.pixelHeight * UniversalRenderPipeline.asset.renderScale * GetScaleValue()));
        if (reflectionTexture != null &&
            (reflectionTexture.width != textureRes.x || reflectionTexture.height != textureRes.y))
        {
