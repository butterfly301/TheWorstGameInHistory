...
namespace TravisGameAssets
{
    public class HitImpactEffectsPreview : MonoBehaviour
    {
        ...
        private readonly float maxFov = 90f;
        ...
        private void Start()
        {
            ...
            for (var i = 0; i < particlesPool.childCount; i++) hitEffects[i] = particlesPool.GetChild(i).gameObject;
        ...
        private void Update()
        {
            if (Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.LeftArrow))
                PreviousHit();
            else if (Input.GetKeyDown(KeyCode.D) || Input.GetKeyDown(KeyCode.RightArrow)) NextHit();
            ...
            if (Input.GetMouseButtonDown(2)) Camera.main.fieldOfView = initFov;

            if (cameraRotating) cameraPivot.Rotate(Vector3.up * (cameraRotationSpeed * Time.deltaTime));
        }
        ...
        {
            hitIndex++;
            if (hitIndex >= hitEffects.Length) hitIndex = 0;
            RefreshHitUI();
        }
        ...
        {
            hitIndex--;
            if (hitIndex < 0) hitIndex = hitEffects.Length - 1;
            RefreshHitUI();
        }
        ...
        private void RefreshHitUI()
        {
            ...
            hitIndexLabel.text =
                string.Format("{0}/{1}", (hitIndex + 1).ToString("00"), hitEffects.Length.ToString("00"));
        }
        ...
        {
            var spawnedHit = Instantiate(hitEffects[hitIndex]);
