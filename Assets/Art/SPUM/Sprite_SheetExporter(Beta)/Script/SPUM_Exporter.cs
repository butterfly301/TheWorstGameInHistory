...
public class SPUM_Exporter : MonoBehaviour
{
    ...
    public Vector2 _imageSize = new(128, 128);
    public Vector2 _fullSize = new(1024, 1024);
    ...
    public void CheckObjNow()
    {
        ...
        if (_objectPivot.childCount > 0) DestroyImmediate(_objectPivot.GetChild(0).gameObject);
    }
    ...
    public void MakeObjNow()
    {
        ...
        var tObj = Instantiate(_unitPrefab);
    ...
    public void LoadAnimationStateClip()
    {
        ...
        var _animNameNow = new List<string>();
        var IndexedClipNames = new List<string>();
        ...
        IndexedClipNames = new List<string>();
    ...
    private AnimationClip LoadAnimationClip(string clipPath)
    {
        ...
        if (clip == null) Debug.LogWarning($"애니메이션 클립 '{clipPath}'을 로드하지 못했습니다.");
    ...
    public List<Texture2D> _textSaveList = new();
    ...
    public int _gifRepeatNum; // 0 is repeats continuously, number is repeated as many times as the number.
    ...
    {
        ImageNumber++;
        var numX = (int)_fullSize.x / (int)_imageSize.x;
        var numY = (int)_fullSize.y / (int)_imageSize.y;
        var allSpriteNum = numX * numY;
        // Debug.Log(allSpriteNum);


        var resultImages = new List<Texture2D>();
        ...
        var resultImageNum = 0;
        var rISave = allSpriteNum;

        var numXSave = numX;

        var tYNum = 1;
        var tXNum = -1;
        var stateName = items[animNum - 1].name;
        for (var i = 0; i < _textSaveList.Count; i++)
        {
            ...
            var tTex = _textSaveList[i];
            tXNum++;

            for (var x = 0; x < tTex.width; x++)
            for (var y = 0; y < tTex.height; y++)
            {
                var bgColor = tTex.GetPixel(x, y);
                resultImages[resultImageNum].SetPixel(x + (int)_imageSize.x * tXNum,
                    (int)_fullSize.y - (int)_imageSize.y * tYNum + y, bgColor);
            }
        ...
        {
            var bytes = resultImages[i].EncodeToPNG();
            var tName = "";

            if (_imageName == "") _imageName = _unitPrefab.name;
            ...
            if (!Directory.Exists("Assets/SPUM/ScreenShots/")) Directory.CreateDirectory("Assets/SPUM/ScreenShots/");

            var filename = string.Format("{0}/SPUM/ScreenShots/{1}_{2}.png", Application.dataPath, tName, i);
...
// for gif exporter - preview version
    ...
    {
        var temp = new Texture2D((int)_imageSize.x, (int)_imageSize.y, TextureFormat.ARGB32, false);
        temp.hideFlags = HideFlags.HideAndDontSave;
        temp.wrapMode = TextureWrapMode.Clamp;
        temp.filterMode = FilterMode.Bilinear;
        temp.anisoLevel = 0;

        var frames = new List<GifFrame>(_textSaveList.Count);
        if (!Directory.Exists("Assets/SPUM/ScreenShots/GifExports"))
            Directory.CreateDirectory("Assets/SPUM/ScreenShots/GifExports");
        var filepath = Application.dataPath + "/SPUM/ScreenShots/GifExports/" + _unitPrefab.name + "_" + _sepaName +
                       ".gif";
        for (var i = 0; i < _textSaveList.Count; i++)
        {
            var frame = ToGifFrame(_textSaveList[i], temp);
            frames.Add(frame);
        }

        // Setup a worker thread and let it do its magic
        var encoder = new GifEncoder(_gifRepeatNum, _gifQuality);
        encoder.SetDelay((int)(_gifDelay * 1000));
        encoder.SetAlphaValue(_gifUseTransparancy, _gifAlphaBGColor);
        OnFileSaved += (i, t) => { Debug.Log(i + "/" + @"<a href=\file:///" + t + ">" + t + "</a>"); };
    ...
    private GifFrame ToGifFrame(Texture2D source, Texture2D target)
    {
        ...
        {
            var bgColor = source.GetPixel(x, y);
    ...
    {
        if (fillColor == null) fillColor = Color.clear;
        var fillPixels = new Color32[tex2D.width * tex2D.height];
        for (var i = 0; i < fillPixels.Length; i++) fillPixels[i] = (Color32)fillColor;
