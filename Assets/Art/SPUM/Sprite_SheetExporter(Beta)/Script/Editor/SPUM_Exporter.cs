...
public class SPUM_ExporterEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        //For Gif Exporter
        if (GUILayout.Button("RELOAD CLIP DATA")) SPB.LoadAnimationStateClip();
        EditorGUILayout.HelpBox("Extracting the checked clips.", MessageType.Info);
        if (SPB.items.Count > 0)
        {
            EditorGUILayout.BeginHorizontal();
            var newAllChecked = EditorGUILayout.Toggle(allChecked, GUILayout.Width(20));
            ...
            GUILayout.Space(-9);
            var horizontalLine = new GUIStyle(GUI.skin.horizontalSlider);
            EditorGUILayout.LabelField("", horizontalLine);
            GUILayout.Space(-9);
        }

        for (var i = 0; i < SPB.items.Count; i++)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            SPB.items[i].animationClip =
                (AnimationClip)EditorGUILayout.ObjectField(SPB.items[i].animationClip, typeof(AnimationClip), false);
        ...
        var xSize = Handles.GetMainGameViewSize().x;
        var ySize = Handles.GetMainGameViewSize().y;

        if (xSize < 1920f || ySize < 1080)
        {
            EditorGUILayout.PropertyField(_gifExportUse, new GUIContent("Disable Gif Export"));
            EditorGUILayout.HelpBox(
                "!!! Please set Game Windows size over the Full HD (1920x1080) to use export method !!!",
                MessageType.Error);
            if (GUILayout.Button("OK Change resolution now.", GUILayout.Height(50))) SetCaptureSize();
            return;
        }
    ...
    private void ProcessAnimation()
    {
        ...
        if (SPB._separated) SPB._sepaName = tAnimSave.name;
    ...
    private bool MoveToNextCheckedAnimation()
    {
        ...
        {
            SPB.animNum++;
            if (SPB.animNum >= SPB.items.Count) return false;
    ...
    private void ProcessNextAnimationClip()
    {
        ...
        var currentAnim = SPB.items[SPB.animNum].animationClip;
        var stateName = SPB.items[SPB.animNum].name;
    ...
    private void SetupNewAnimation(AnimationClip anim, string stateName)
    {
        ...
        Debug.Log(
            $"[[Generating : {stateName}_{tAnimSave.name} || Time Length : {tAnimSave.length} sec || Frame Numbers : {SPB._frameNumber}]]");
    ...
    {
        var idx = FindSize(GameViewSizeGroupType.Standalone, 1920, 1080);
        if (idx != -1)
            SetSize(idx);
    }
