...
public class SoonsoonData
{
    ...
    public SoonData _soonData2 = new();
    ...
    public static SoonsoonData Instance
    {
        get
        {
            if (instance == null) instance = new SoonsoonData();
            return instance;
        }
    }
    ...
    public void LoadProcess()
    {
        ...
        {
            var _str = PlayerPrefs.GetString("SoonsoonSave2");

            if (_str.Length > 0)
            {
                var _tmpStr = PlayerPrefs.GetString("SoonsoonSave2");
    ...
    public class SoonData
    {
        ...
        public List<string> _savedColorList = new(); // 작업 중인 색상 저장 
        public List<Dictionary<string, string>> packageList = new(); // 패키지 버전
    }
}