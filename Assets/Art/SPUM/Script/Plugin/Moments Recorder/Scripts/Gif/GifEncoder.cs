...
namespace Moments.Encoder
{
    public class GifEncoder
    {
        ...
        protected int m_FrameDelay; // Frame delay (milliseconds)
        protected bool m_HasStarted; // Ready to output frames
        protected int m_Height;
        protected byte[] m_IndexedPixels; // Converted frame indexed to palette
        protected bool m_IsFirstFrame = true;
        protected bool m_IsSizeSet; // If false, get size from first frame
        ...
        protected bool m_ShouldCloseStream; // Close stream when finished
        ...
        protected void GetImagePixels()
        {
            ...
            for (var th = m_CurrentFrame.Height - 1; th >= 0; th--)
            for (var tw = 0; tw < m_CurrentFrame.Width; tw++)
            {
        ...
        protected void AnalyzePixels()
        {
            ...
            if (useAlpha)
            {
                var tR = (int)(transparancy.r * 255f);
                var tG = (int)(transparancy.g * 255f);
                var tB = (int)(transparancy.b * 255f);
        ...
        protected int FindClosest(Color c)
        {
            ...
            var len = m_ColorTab.Length;
            for (var i = 0; i < len;)
            {
                var dr = r - (m_ColorTab[i++] & 0xff);
                var dg = g - (m_ColorTab[i++] & 0xff);
                var db = b - (m_ColorTab[i] & 0xff);
