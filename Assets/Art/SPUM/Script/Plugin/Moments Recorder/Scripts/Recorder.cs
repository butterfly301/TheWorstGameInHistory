...
namespace Moments
{
    ...
    public sealed class Recorder : MonoBehaviour
    {
        ...
        [SerializeField] [Min(8)] private int m_Width = 320;

        [SerializeField] [Min(8)] private int m_Height = 200;

        [SerializeField] private bool m_AutoAspect = true;

        [SerializeField] [Range(1, 30)] private int m_FramePerSecond = 15;

        [SerializeField] [Min(-1)] private int m_Repeat;

        [SerializeField] [Range(1, 100)] private int m_Quality = 15;

        [SerializeField] [Min(0.1f)] private float m_BufferSize = 3f;
        ...
        public void Pause()
        {
            ...
            {
                Debug.LogWarning(
                    "Attempting to pause recording during the pre-processing step. The recorder is automatically paused when pre-processing.");
                return;
            }
        ...
        private void Init()
        {
            ...
            if (string.IsNullOrEmpty(SaveFolder))
            {
                ...
                SaveFolder =
                    Application
                        .dataPath; // Defaults to the asset folder in the editor for faster access to the gif file
#else
        ...
        private void Flush(UnityObject obj)
        {
#if UNITY_EDITOR
        ...
        private IEnumerator PreProcess(string filename)
        {
            ...
            {
                var frame = ToGifFrame(m_Frames.Dequeue(), temp);
                frames.Add(frame);
                yield return null;
            }
            ...
            var encoder = new GifEncoder(m_Repeat, m_Quality);
