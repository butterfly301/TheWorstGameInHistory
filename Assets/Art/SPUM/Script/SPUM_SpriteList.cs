...
{
    public List<SpriteRenderer> _itemList = new();
    public List<SpriteRenderer> _eyeList = new();
    public List<SpriteRenderer> _hairList = new();
    public List<SpriteRenderer> _bodyList = new();
    public List<SpriteRenderer> _clothList = new();
    public List<SpriteRenderer> _armorList = new();
    public List<SpriteRenderer> _pantList = new();
    public List<SpriteRenderer> _weaponList = new();
    public List<SpriteRenderer> _backList = new();
    ...
    public List<string> _hairListString = new();
    public List<string> _clothListString = new();
    public List<string> _armorListString = new();
    public List<string> _pantListString = new();
    public List<string> _weaponListString = new();
    public List<string> _backListString = new();
    ...
    public void Reset()
    {
        ...
        for (var i = 0; i < _weaponList.Count; i++)
            if (_weaponList[i] != null)
                _weaponList[i].sprite = null;
        for (var i = 0; i < _backList.Count; i++)
            if (_backList[i] != null)
                _backList[i].sprite = null;
    }
    ...
    public void SyncPath(List<SpriteRenderer> _objList, List<string> _pathList)
    {
        for (var i = 0; i < _pathList.Count; i++)
            ...
            {
                var tPath = _pathList[i];
                tPath = tPath.Replace("Assets/Resources/", "");
                tPath = tPath.Replace(".png", "");

                var tSP = Resources.LoadAll<Sprite>(tPath);
                if (tSP.Length > 1)
                {
