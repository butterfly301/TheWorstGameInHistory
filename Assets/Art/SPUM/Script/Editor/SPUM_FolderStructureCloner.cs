...
{
    private readonly string newFolderPath = "Assets/SPUM/Resources/Addons/";
    private List<FolderNode> folderStructure = new();
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Copy the folder structure"))
        {
            ...
            if (!string.IsNullOrEmpty(selectedPath)) folderStructure = GetFolderStructure(selectedPath);
        }

        GUILayout.Space(10);
        GUILayout.Label("Initialize with the default folder structure.", EditorStyles.boldLabel);
        GUILayout.Space(10);
        if (GUILayout.Button("Initialize Structure")) InitFolderStructure();
        GUILayout.Space(10);
        ...
        if (folderStructure.Any())
        {
            ...
            if (GUILayout.Button("Create Folder Structure")) CreateAddonFolderStructure();
        }
    }
...
-2_Prefabs|true"
    ...
    public List<FolderNode> ParseFolderStructure(string input, char separator = '|', char indentChar = '-')
    {
        ...
        foreach (var line in lines)
        {
            ...
            // indentChar 개수 세기
            var indent = line.TakeWhile(c => c == indentChar).Count();
            ...
            var node = new FolderNode
            {
                ...
                IsExpanded = parts.Length > 1 && bool.TryParse(parts[1], out var isExpanded) && isExpanded,
                Children = new List<FolderNode>() // 명시적으로 Children 리스트 초기화
            };

            while (stack.Count > 0 && stack.Count > indent) stack.Pop();
    ...
    private void DrawFolderStructure(List<FolderNode> nodes, int indent)
    {
        for (var i = 0; i < nodes.Count; i++)
        {
            ...
            if (node.IsExpanded && node.Children.Any()) DrawFolderStructure(node.Children, indent + 1);
        }
    }
    ...
    private List<FolderNode> GetFolderStructure(string path)
    {
        ...
        var node = new FolderNode { Name = folderName, IsExpanded = true };

        foreach (var subDir in Directory.GetDirectories(path)) node.Children.AddRange(GetFolderStructure(subDir));
    ...
    private void CreateFolderStructure(List<FolderNode> nodes, string parentPath)
    {
        foreach (var node in nodes)
        {
            ...
            if (node.Children.Any()) CreateFolderStructure(node.Children, newPath);
        }
    }

    [MenuItem("SPUM/Print Sprite Renderer Names")]
    private static void PrintSpriteRendererNames()
    {
        // 선택된 오브젝트 가져오기
        ...
        // 스프라이트 랜더러 리스트 생성
        var spriteRendererNames = new List<string>();

        // 하위 오브젝트의 모든 스프라이트 랜더러 찾기
        var spriteRenderers = selectedObject.GetComponentsInChildren<SpriteRenderer>();
        var Text = "";
    ...
    private static void PrintPivotNames()
    {
        // 선택된 오브젝트 가져오기
        var selectedObject = Selection.activeGameObject;
        ...
        // 하위 오브젝트의 모든 스프라이트 랜더러 찾기
        var transforms = selectedObject.GetComponentsInChildren<Transform>();
        var Text = "";
