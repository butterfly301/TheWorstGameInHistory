...
public class SPUM_PaginationManager : MonoBehaviour
{
    ...
    private readonly Queue<int> deletedIndexes = new();
    private readonly int itemsPerPage = 10;
    private int nextAvailableIndex;
    private readonly SortedDictionary<int, SPUM_Prefabs> sortedItems = new();
    ...
    {
        var convertedPrefabs = new List<SPUM_Prefabs>();
        var indicesToRemove = new List<int>();

        foreach (var kvp in sortedItems)
        {
            var prefabIndex = kvp.Key;
            var savedPrefab = kvp.Value;
            Debug.Log(savedPrefab._code + " " + savedPrefab.EditChk);
            if (!savedPrefab.EditChk) continue;
            var needsConversion = savedPrefab._version < SPUM_Manager._version ||
                                  !SPUM_Manager.ValidateSpumFile(savedPrefab).Item2.Count.Equals(0) ||
                                  savedPrefab.EditChk;
        ...
        foreach (var index in indicesToRemove) DeleteUnit(index, sortedItems[index]);
    ...
    public void ApplyPresetAll()
    {
        ...
        var selectedIndex = presetDropdown.value;
        if (selectedIndex >= 0 && selectedIndex < presetData.Presets.Count)
        {
            var selectedPreset = presetData.Presets[selectedIndex];
    ...
    {
        var updatedPrefabs = new List<SPUM_Prefabs>();
        var indicesToRemove = new List<int>();
#if UNITY_EDITOR
        foreach (var kvp in sortedItems)
        {
    ...
    private void LoadItems()
    {
        ...
        foreach (var prefab in SavedPrefabs) AddItemToSortedDictionary(prefab);
    }

    private void AddItemToSortedDictionary(SPUM_Prefabs prefab)
    {
        int index;
    ...
    {
        if (sortedItems.Remove(index)) deletedIndexes.Enqueue(index);
    }
    ...
    private void DisplayPage()
    {
        ...
        var endIndex = Mathf.Min(startIndex + itemsPerPage, sortedItems.Count);
        var i = 0;
        foreach (var kvp in sortedItems)
        {
            if (i >= startIndex && i < endIndex)
            {
                var PreviewElement = Instantiate(itemPrefab, contentParent);
                var UnitName = PreviewElement.GetComponentInChildren<Text>();
                var PreviewData = PreviewElement.GetComponentInChildren<SPUM_Prefabs>();
                var SavedPrefab = kvp.Value;
                var prefabIndex = kvp.Key;
                ...
                foreach (var matchingElement in allMatchingElements)
                {
                    ...
                    {
                        var LoadSprite = SPUM_Manager.LoadSpriteFromMultiple(matchingTypeElement.ItemPath,
                            matchingTypeElement.Structure);
                ...
                ButtonPanel.CheckedButton.onValueChanged.AddListener(On => { SavedPrefab.EditChk = On; });
                SelectAllToggleButton.onValueChanged.AddListener(On =>
                {
                    if (ButtonPanel != null && ButtonPanel.CheckedButton != null) ButtonPanel.CheckedButton.isOn = On;
                });
                ...
                var isOldVersion = SavedPrefab._version < SPUM_Manager._version;

                var isInvalidClipPath = false;
                ...
                var IsActive = isOldVersion || isInvalidPath || !isInvalidClipPath;
