...
public class SpriteSyncManager : MonoBehaviour
{
    ...
    public List<SpriteSync> _syncList = new();
    public List<Vector3> _syncPos = new();
    ...
    private void SyncFistProcess(SpriteSync spriteSync, SpriteRenderer SP)
    {
        spriteSync.transform.position = SP.transform.position;
        var tV = new Vector3(SP.transform.position.x, SP.transform.position.y, SP.sortingOrder * 1f);
    }
#endif
}