...
using UnityEngine.UI;
    public class OpenIceBreaker : Open
    {
        ...
        {
            //播放bgm
            this.SendCommand(new PlayMusicCommand(AddressableKeys.Audio.BGM.Fruit_Machine_Mp3));
            //初始化管理器
        ...
        IEnumerator InitializeIceBreakerManager()
        {
            ...
            AddressablesManager.Instance.LoadAssetAsync<GameObject>(AddressableKeys.Prefabs.IceBreaker.IceBreakerManager_Prefab,
                (handle) =>
                {
        ...
        {
            bool initCompleted = false;
            AddressablesManager.Instance.LoadAssetAsync<GameObject>(AddressableKeys.Prefabs.IceBreaker.IceBreakerCamera_Prefab,
                (handle) =>
                {
                    ...
                    AddressablesManager.Instance.LoadAssetAsync<GameObject>(
                        AddressableKeys.Prefabs.IceBreaker.IceBreakerPlayer_Prefab,
                        (handle2) =>
                        {
                            ...
                            //初始化关卡
                            AddressablesManager.Instance.LoadAssetAsync<GameObject>(
                                AddressableKeys.Prefabs.IceBreaker.LevelGenerator_Prefab,
                                (handle3) =>
                                {
