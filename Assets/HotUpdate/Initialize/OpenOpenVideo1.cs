...
public class OpenOpenVideo1 : Open
{
    protected override IEnumerator InitializeSequence()
    {
        ...
        //初始化UI管理器
        AddressablesManager.Instance.LoadAssetAsync<GameObject>(AddressableKeys.Prefabs.UI.Playthrough1.Manager.UIManager1_Prefab,
            (handle) =>
            {
        ...
        //初始化视频管理器
        AddressablesManager.Instance.LoadAssetAsync<GameObject>(AddressableKeys.Prefabs.Video.VideoManager_Prefab,
            (handle) =>
            {
    ...
    {
        this.SendCommand(new LoadSceneCommand(AddressableKeys.Scenes.ToiletVillage_Unity, true,
