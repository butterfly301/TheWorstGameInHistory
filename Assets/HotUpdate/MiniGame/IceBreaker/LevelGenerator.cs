    ...
    public class LevelGenerator : MonoBehaviour
    {
        ...
        public void Init(Transform playerTransform)
        {
            ...
            AddressablesManager.Instance.LoadAssetAsync<GameObject>(
                AddressableKeys.Prefabs.IceBreaker.Level.PlatformPrefab_Prefab,
                handle =>
                {
                    platformPrefab = handle.Result;

                    AddressablesManager.Instance.LoadAssetAsync<GameObject>(
                        AddressableKeys.Prefabs.IceBreaker.Level.ObstaclePrefab_Prefab,
                        handle2 =>
                        {
                            obstaclePrefab = handle2.Result;
                            AddressablesManager.Instance.LoadAssetAsync<GameObject>(
                                AddressableKeys.Prefabs.IceBreaker.Level.EnemyPrefab_Prefab,
                                handle3 =>
                                {
                                    enemyPrefab = handle3.Result;
                                    AddressablesManager.Instance.LoadAssetAsync<GameObject>(
                                        AddressableKeys.Prefabs.IceBreaker.Level.GoalPrefab_Prefab,
                                        handle4 =>
                                        {
