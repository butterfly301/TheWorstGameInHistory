...
namespace TwoBitMachines.FlareEngine.Timeline
{
    ...
    {
        [SerializeField] public List<FieldStateList> record = new();

        public void RecordTracksOrigin(List<Track> track)
        {
            record.Clear();
            for (var i = 0; i < track.Count; i++)
            for (var j = 0; j < track[i].action.Count; j++)
            {
                var fieldChange = track[i].action[j].fieldChange;
                var component = fieldChange.component;
                if (component == null) continue;

                var componentType = fieldChange.component.GetType();
                var state = new FieldStateList();
                var fields = componentType.GetFields(BindingFlags.Instance | BindingFlags.Public);
                foreach (var fieldInfo in fields)
                    try
                    {
                        state.component = component;
                        state.state.Add(new FieldState(component, "Field", fieldInfo.Name,
                            fieldInfo.GetValue(component), true));
                    }
                    catch
                    {
                    }
                ...
                foreach (var propertyInfo in properties)
                    try
                    {
                        if (propertyInfo.CanRead && propertyInfo.PropertyType != typeof(Material) &&
                            propertyInfo.PropertyType != typeof(Material[]))
                        {
                            state.component = component;
                            state.state.Add(new FieldState(component, "Property", propertyInfo.Name,
                                propertyInfo.GetValue(component), false));
                        }
                    }
                    catch
                    {
            ...
            foreach (var component in components)
            {
                ...
                foreach (var fieldInfo in fields)
                    try
                    {
                        state.component = component;
                        state.state.Add(new FieldState(component, "Field", fieldInfo.Name,
                            fieldInfo.GetValue(component), true, false));
                    }
                    catch
                    {
                    }
                ...
                foreach (var propertyInfo in properties)
                    try
                    {
                            ...
                            propertyInfo.PropertyType != typeof(Material[]))
                        {
                            state.component = component;
                            state.state.Add(new FieldState(component, "Property", propertyInfo.Name,
                                propertyInfo.GetValue(component), false, false));
                        }
                    }
                    catch
                    {
        ...
        public bool RecordChange(FieldChange fieldChange)
        {
            ...
            {
                var component = record[i].component;
                var componentType = component.GetType();

                for (var j = 0; j < record[i].state.Count; j++)
                    try
                    {
                        var fieldState = record[i].state[j];
                        var oldValue = fieldState.value;
                        var memberInfo = fieldState.GetMemberInfo(componentType);

                        if (memberInfo is FieldInfo fieldInfo)
                        {
                            var newValue = fieldInfo.GetValue(component);

                            if (!Equals(oldValue, newValue))
                            {
                                Debug.Log($"Recorded change in {componentType.Name}\nField: {fieldInfo.Name}");
                                fieldChange.Set(fieldState.component, fieldState.fieldName,
                                    componentType.Name + ": " + fieldInfo.Name, newValue, fieldState.isField);
                                return true;
                            }
                        }
                        else if (memberInfo is PropertyInfo propertyInfo && propertyInfo.CanRead)
                        {
                            var newValue = propertyInfo.GetValue(component);

                            if (!Equals(oldValue, newValue))
                            {
                                Debug.Log($"Recorded change in {componentType.Name}\nProperty: {propertyInfo.Name}");
                                fieldChange.Set(fieldState.component, fieldState.fieldName,
                                    componentType.Name + ": " + propertyInfo.Name, newValue, fieldState.isField);
                                return true;
                            }
                        }
                    }
            ...
            for (var i = 0; i < record.Count; i++)
            {
                ...
                for (var j = 0; j < record[i].state.Count; j++)
                    try
                    {
                        ...
                        {
                            var newValue = fieldInfo.GetValue(component);
                            if (!Equals(oldValue, newValue)) fieldInfo.SetValue(component, oldValue);
                        }
                        else if (memberInfo is PropertyInfo propertyInfo && propertyInfo.CanRead)
                        {
                            var newValue = propertyInfo.GetValue(component);
                            if (!Equals(oldValue, newValue)) propertyInfo.SetValue(component, oldValue);
                        }
                    }
                    catch
                    {
    ...
    public class FieldStateList
    {
        ...
        [SerializeField] public List<FieldState> state = new();
    }
    ...
    public class FieldState
    {
        ...
        [SerializeField] public ExtraSerializeTypes extraType = new();

        // Need to Serialize manually
        public object value;

        public FieldState(Component component, string memberType, string fieldName, object value, bool isField,
            bool serialize = true)
        {
            ...
            if (serialize) ChronoTypes.Serialize(extraType, value, ref valueString);
        }

        public MemberInfo GetMemberInfo(Type targetType)
        {
            var memberInfo = (MemberTypes)Enum.Parse(typeof(MemberTypes), memberType);

            if (memberInfo == MemberTypes.Field)
                return targetType.GetField(fieldName, BindingFlags.Instance | BindingFlags.Public);

            if (memberInfo == MemberTypes.Property)
                return targetType.GetProperty(fieldName,
