...
namespace TwoBitMachines.FlareEngine.Timeline
{
    ...
    {
        [SerializeField] public List<Track> track = new();
        [NonSerialized] public float time;

        public void Reset()
        {
            for (var i = 0; i < track.Count; i++) track[i].Reset();
        }
        ...
        {
            for (var i = 0; i < track.Count; i++) track[i].Serialize();
#if UNITY_EDITOR
            play.Serialize();
#endif
        }
        ...
        {
            for (var i = 0; i < track.Count; i++) track[i].Deserialize();
#if UNITY_EDITOR
            play.Deserialize();
#endif
        }
...
#pragma warning disable 0414
        [SerializeField] public RecordObjectState recordState = new();
        [SerializeField] public ChronosPlay play = new();
...
#pragma warning restore 0414
    ...
    public class Track
    {
        ...
        [SerializeField] public List<Action> action = new();
        [NonSerialized] public bool hasCompleted;
        ...
        public void Reset()
        {
            time = 0;
            hasCompleted = false;
            for (var i = 0; i < action.Count; i++) action[i].Reset();
        }
        ...
        {
            for (var i = 0; i < action.Count; i++) action[i].Serialize();
#if UNITY_EDITOR
        ...
        {
            for (var i = 0; i < action.Count; i++) action[i].Deserialize();
#if UNITY_EDITOR
...
#pragma warning disable 0414
        ...
        [SerializeField] private ExtraSerializeTypes extraType = new();
        ...
        {
            for (var i = 0; i < action.Count; i++) action[i].Execute(time);
        }
#pragma warning restore 0414
#endif
    ...
    public class Action
    {
        [SerializeField] public FieldChange fieldChange = new();
        ...
        [NonSerialized] public bool actionUsed;
        [NonSerialized] public bool eventUsed;
        [NonSerialized] public bool initSet;

        [NonSerialized] public bool initStartValue;
        ...
        public bool Execute(float time)
        {
            if (isSetType)
            {
                if (time >= startTime && !initSet) ApplyState(time);
            }
        ...
        public void UseEvent()
        {
            ...
            if (onActionStay.GetPersistentEventCount() > 0) onActionStay.Invoke();
        }
    ...
    public class FieldChange
    {
        ...
        [NonSerialized] private PropertyInfo propertyInfo;
        [NonSerialized] private object startValue;
        public object value; // unity can't serialize system.object :(
        ...
        public void ApplyChange(Action action, float time, float duration)
        {
            ...
            {
                if (fieldInfo == null) fieldInfo = component.GetType().GetField(fieldName);
                if (fieldInfo != null)
                {
                    ...
                    else
                    {
                        ...
                        var newValue = ChronoInterpolate.Run(startValue, value, time, duration, action.tween);
            ...
            {
                if (propertyInfo == null) propertyInfo = component.GetType().GetProperty(fieldName);
