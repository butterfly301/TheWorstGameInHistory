...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Walk
    {
        ...
        [NonSerialized] private bool coolDownRun;
        [NonSerialized] private bool doubleTapMode;
        [NonSerialized] private int easeInWalkDirection;

        [NonSerialized] public float externalVelX;
        [NonSerialized] private float firstTapInputX;
        [NonSerialized] private float inputX;

        [NonSerialized] public bool isRunning;
        [NonSerialized] private bool keepPlayerDirection;
        [NonSerialized] private float lerpStop;
        [NonSerialized] private float onGroundCounter;
        [NonSerialized] private float runCoolDownCounter;
        [NonSerialized] private float runLimitCounter;

        [NonSerialized] private RunLimit runLimitState;
        [NonSerialized] public float runSmoothInVelocity;
        [NonSerialized] private float smoothIntoRunCounter;
        [NonSerialized] private bool stoppedRunning;
        [NonSerialized] private float switchCounter;
        [NonSerialized] private bool switchTimeActive;
        [NonSerialized] private float tapCounter;
        [NonSerialized] private float tapTimer;
        [NonSerialized] private float thresholdCounter;
        [NonSerialized] private bool toggleMode;

        [NonSerialized] private float velocityX;
        ...
        public void Execute(AbilityManager player, WorldCollision world, ref Vector2 velocity)
        {
            ...
            var previousDirection = player.playerDirection;
            ...
            if (!keepPlayerDirection && velocity.x < 0) player.playerDirection = -1;
            if (!keepPlayerDirection && velocity.x > 0) player.playerDirection = 1;

            if (previousDirection != player.playerDirection)
                onDirectionChanged.Invoke(ImpactPacket.impact.Set(directionChangedWE, world.transform,
                    world.box.collider, player.world.position, null, -world.box.right * player.playerDirection,
                    player.playerDirection, 0));
            if (!player.world.wasOnGround && player.world.onGround)
                onGroundHit.Invoke(ImpactPacket.impact.Set(groundHitWE, world.transform, world.box.collider,
                    player.world.position, null, world.box.up, player.playerDirection, 0));
            if (player.world.wasOnGround && !player.world.onGround)
                onNotOnGround.Invoke(ImpactPacket.impact.Set(notOnGroundWE, world.transform, world.box.collider,
                    player.world.position, null, world.box.down, player.playerDirection, 0));
            if (Clock.TimerExpired(ref onGroundCounter, 0.15f) && player.world.onGround && velocity.x != 0)
            {
                onWalkingOnGround.Invoke(ImpactPacket.impact.Set(walkingOnGroundWE, world.transform, world.box.collider,
                    player.world.position, null, -world.box.right * player.playerDirection, player.playerDirection, 0));
                onGroundCounter = 0;
            }
        }
        ...
        {
            var left = player.inputs.Holding("Left");
            var right = player.inputs.Holding("Right");
        ...
        public void HorizontalMove(AbilityManager player, ref Vector2 velocity)
        {
            if (smooth > 0 && smooth < 1f)
            {
                ...
                velocity.x =
                    Mathf.Abs(velocity.x) < 0.2f
                        ? 0
                        : velocity.x; // clamp or vel will continue to decrease without stopping
            }
            else
            {
        ...
        private void PlayerRun(AbilityManager player, WorldCollision world, ref Vector2 velocity)
        {
            ...
            if (stoppedRunning && smoothIntoRun && !isRunning) RunToWalkSmoothIn(player, ref velocity);
        ...
        {
            var inputXr = runFromStop && inputX == 0 ? player.playerDirection : inputX;
            if (!toggleMode && (inputXr == 0 || world.onWall)) return true;
            if (runType == RunType.Hold && !player.inputs.Holding(runButton)) return true;
            if (doubleTapMode && !player.inputs.Holding(runButton)) return true;
            if (toggleMode)
            {
                if (player.inputs.Pressed(runButton) || world.onWall) return true;
                ...
                if (switchTimeActive && Clock.Timer(ref switchCounter, 0.2f) && inputX == 0) return true;
            }
        ...
        {
            if (velocity.x != 0) tapCounter = tapTimer = 0;
            if (isRunning)
            {
                stoppedRunning = true;
                lerpStop = 0;
            ...
            {
                stoppedRunning =
                    false; // let player continue at normal speed if hit wall, or it will feel stuck for a moment during smoothIn.
                runSmoothInVelocity = 0;
            }
        ...
        private void RunToWalkSmoothIn(AbilityManager player, ref Vector2 velocity)
        {
            if (runSmoothInVelocity != 0)
            {
                ...
                if (inputX != 0) easeInWalkDirection = (int)Mathf.Sign(inputX);
            }
        ...
        private bool RunInputActive(AbilityManager player, WorldCollision world)
        {
            ...
            {
                if ((player.ground || isRunning) && player.inputs.Holding(runButton)) return true;
            }
            else if (runType == RunType.TimeThreshold)
            {
                if (!isRunning && !world.onGround) thresholdCounter = 0;
                if (inputX != 0 && Clock.TimerExpired(ref thresholdCounter, runThreshold)) return true;
            }
            else if (runType == RunType.StickThreshold)
            {
                if (inputX != 0)
                {
                    var left = player.inputs.Value("Left");
                    var right = player.inputs.Value("Right");
                    if (left >= runThreshold || right >= runThreshold) return true;
                }
            }
            else if (runType == RunType.Toggle)
            {
                if (toggleMode) return true;
        ...
        private void ApplyRunVelocity(AbilityManager player, ref Vector2 velocity)
        {
            var inputXr = runFromStop && inputX == 0 ? player.playerDirection : inputX;
            if (smoothIntoRun)
                velocity.x = runSmoothInVelocity = Compute.Lerp(velocity.x, inputXr * speed * speedBoost, lerpRunTime,
                    ref smoothIntoRunCounter);
            else
        ...
        {
            if (player.ground) player.UpdateVelocityGround();
            if (impedeChange > 0 && !player.ground)
            {
                if (player.airMomentumActive && inputX == 0) return;
                player.velocityOnGround = Mathf.MoveTowards(player.velocityOnGround, velocity.x,
                    Time.deltaTime * (5f - impedeChange) * 10f); // 5 is the max value for airResistance, invert value
                var scale = run ? speedBoost : 1f;
...
#pragma warning disable 0414
        ...
        public static void OnInspector(SerializedProperty parent, string[] inputList, Color barColor, Color labelColor)
        {
            ...
            {
                FoldOut.Box(3, FoldOut.boxColorLight, 5, -2);
                {
                    parent.Field("Speed", "speed");
                    parent.Slider("Smooth", "smooth", 0.95f);
                    parent.Slider("Damp Air Change", "impedeChange", 0, 5f);
                }

                if (FoldOut.FoldOutButton(parent.Get("eventFoldOut")))
                {
                    Fields.EventFoldOutEffect(parent.Get("onGroundHit"), parent.Get("groundHitWE"),
                        parent.Get("groundFoldOut"), "Ground Hit", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onNotOnGround"), parent.Get("notOnGroundWE"),
                        parent.Get("notGroundFoldOut"), "Not On Ground", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onWalkingOnGround"), parent.Get("walkingOnGroundWE"),
                        parent.Get("walkingOnGroundFoldOut"), "Walking On Ground", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onDirectionChanged"), parent.Get("directionChangedWE"),
                        parent.Get("directionFoldOut"), "Direction Changed", FoldOut.boxColorLight);
                }

                if (FoldOut.Bar(parent, FoldOut.boxColorLight).Label("Run", FoldOut.titleColor, false).BRE("run")
                    .FoldOut("runFoldOut"))
                {
                    var smoothIntoRun = parent.Bool("smoothIntoRun");
                    var runType = parent.Enum("runType");
                    var height = runType == 2 ? 2 : 0;
                    if (smoothIntoRun)
                        height += 1;
                    ...
                    FoldOut.Box(5 + height, FoldOut.boxColorLight, offsetY: -2);
                    {
                        ...
                        {
                            parent.FieldAndDropDownList(inputList, "Type", "runType", "runButton", runType < 3);
                            parent.FieldDouble("Type", "runType", "runThreshold", runType >= 3);
                            ...
                            Labels.FieldText("Ease Time", Layout.boolWidth + 4);
                            parent.Field("Ease Into Walk", "lerpWalkTime", smoothIntoRun);
                            parent.Field("Tap Threshold", "tapTime", runType == 2);
                ...
                if (parent.Bool("run") && FoldOut.Bar(parent, FoldOut.boxColorLight)
                        .Label("Run Time Limit", FoldOut.titleColor, false).BRE("runLimit").FoldOut("runLimitFoldOut"))
                {
                    ...
                    Fields.EventFoldOut(parent.Get("onRunTimeLimit"), parent.Get("onRunTimeLimitFoldOut"),
                        "On Run Time Limit", color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("onCoolDown"), parent.Get("onCoolDownFoldOut"), "On Run Cool Down",
