...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Player : Character
    {
        ...
        [SerializeField] public List<Ability> ability = new();
        [SerializeField] public List<string> priority = new();
        [SerializeField] public UserInputs inputs = new();
        [SerializeField] public AbilityManager abilities = new();
        [SerializeField] public bool isVehicle;
        ...
        {
            for (var i = 0; i < players.Count; i++) players[i]?.abilities.ResetAll();
        }

        public static void BlockAllInputs(bool value)
        {
            for (var i = 0; i < players.Count; i++) players[i]?.inputs.Block(value);
        }

        public static void BlockAllInputsRemember(bool value)
        {
            for (var i = 0; i < players.Count; i++) players[i]?.inputs.RememberBlock(value);
        }
        ...
        {
            for (var i = players.Count - 1; i >= 0; i--) players[i]?.PostAIExecute();
        }

        public static Vector2 PlayerPosition(Vector2 returnPosition)
        {
            return mainPlayer != null ? mainPlayer.transform.position : returnPosition;
        }
        ...
        public override void Execute()
        {
            ...
            world.box.Update();
            abilities.Execute();
            world.Move(ref abilities.finalVelocity, ref abilities.velocity.y, abilities.hasJumped, Time.deltaTime,
                ref abilities.onSurface);
        ...
        {
            world.box.Update();
            var velocity = new Vector2(speedX, 0);
