...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        private static CharacterSwapItems temp = new();
        ...
        [NonSerialized] private bool onSwapEventActive;

        private void Start()
        {
            Load();
            item.Initialize();
            if (resetToFirst) item.index = 0;
            Swap(item.index);
        ...
        public void Update()
        {
            if (buttonSwap != null && buttonSwap.Pressed()) MoveToNextCharacter();
        }
        ...
        {
            for (var i = item.index + 1; i < item.list.Count; i++)
        ...
        public void Swap(int newIndex)
        {
            ...
            {
                if (item.list[i].character == null || !item.list[i].character.gameObject.activeInHierarchy) continue;

                for (var j = 0; j < item.list.Count; j++)
                {
                    if (i == j || j != newIndex || item.list[j].character == null || item.list[j].locked) continue;
                    if (item.list[i].character != item.list[j].character) Swap(i, j);
                    return;
                }
            }
        }
        ...
        public void Swap(string name)
        {
            for (var i = 0; i < item.list.Count; i++)
            {
                ...
                for (var j = 0; j < item.list.Count; j++)
                {
                    if (i == j || item.list[j].name != name || item.list[j].character == null ||
                        item.list[j].locked) continue;
                    if (item.list[i].character != item.list[j].character) Swap(i, j);
                    return;
                }
            }
        ...
        {
            item.index = j;
            var oldChar = item.list[i].character;
            var newChar = item.list[j].character;
            ...
            var oldPlayer = oldChar.GetComponent<Player>();
            var player = newChar.GetComponent<Player>();
            if (player != null) Player.mainPlayer = player;
            if (player != null && oldPlayer != null) /// try to keep same air state
            {
                player.abilities.velocity.y = oldPlayer.abilities.velocity.y;
                player.abilities.world.onGround = oldPlayer.world.onGround;
                player.abilities.world.onCeiling = oldPlayer.world.onCeiling;
                player.abilities.onSurface = oldPlayer.abilities.onSurface;
            }
        ...
        public void Load()
        {
            ...
            temp = Storage.Load(temp, WorldManager.saveFolder, "CharacterSwap");
            item.index = temp.index;
            for (var i = 0; i < temp.list.Count; i++)
...
#pragma warning restore 0414
    ...
    public class CharacterSwapItems
    {
        ...
        [SerializeField] public List<CharacterSwapItem> list = new();

        public void Initialize()
        {
            for (var i = 0; i < list.Count; i++)
            {
                if (list[i].character != null) list[i].health = list[i].character.GetComponent<Health>();
                list[i].LockedEvents();
            }
        }
    }
