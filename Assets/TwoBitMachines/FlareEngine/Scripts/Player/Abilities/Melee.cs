...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    {
        [SerializeField] public List<FlareEngine.Melee> melee = new();
        [SerializeField] public float coolDown;
        [NonSerialized] public float coolDownCounter;
        ...
        {
            melee.Clear();
            var melees = gameObject.GetComponentsInChildren<FlareEngine.Melee>(true);
            for (var i = 0; i < melees.Length; i++) melee.Add(melees[i]);
            for (var i = 0; i < melee.Count; i++)
                if (melee[i] != null)
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            for (var i = 0; i < melee.Count; i++)
            {
                if (melee[i] == null || melee[i].pause || !melee[i].ActivateFromSleep()) continue;
            ...
            if (coolDown > 0 && Clock.TimerInverseExpired(ref coolDownCounter, coolDown)) meleeIndex = -1;

            if (meleeIndex >= 0 && meleeIndex < melee.Count && (melee[0] == null || melee[0] != melee[meleeIndex] ||
                                                                !melee[meleeIndex].gameObject.activeInHierarchy))
            {
                if (melee[0] != null) melee[0].ResetAll();
                ChangeMeleeAttack(meleeIndex);
                if (melee[0] != null) melee[0].SkipCoolDown();
            }
            ...
            bool isRunningAsException = false)
        {
            if (melee.Count > 0 && melee[0] != null)
                melee[0].Execute(player.signals, player.playerDirection, player.ground, player.crouching,
                    player.world.position, ref velocity); // only called if ability is active
        }

        public void CompleteAttack()
        {
            if (melee.Count > 0 && melee[0] != null) melee[0].CompleteAttack();
        }
        ...
        {
            var newMelee = melee[i];
            melee.RemoveAt(i);
            melee.Insert(0, newMelee);
            if (reset) newMelee.ResetAll();
            if (newMelee.gameObject != null) newMelee.gameObject.SetActive(true);
        }
        ...
        {
            if (melee.Count > 0 && melee[0] != null) return melee[0].inMelee;
            return false;
        }
