...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class HighJump : Ability
    {
        ...
        [NonSerialized] private float boostCounter;
        [NonSerialized] private bool escapeY;
        [NonSerialized] private Vector2 force;
        [NonSerialized] private Vector2 forceOrigin;
        [NonSerialized] private bool initialized;
        [NonSerialized] private Transform oldBoost;
        [NonSerialized] private float slowCounter;
        [NonSerialized] private float slowTimer;
        [NonSerialized] private float timer;
        [NonSerialized] private int type; // 1 = jump, 2 = wind
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (type == JUMP && player.world.onGround) Reset(player);
            if (type == WIND && !Found(player, velocity.y)) Reset(player);
            if (type == SLOW && !Found(player, velocity.y)) Reset(player);
            if (type == BOOST && !Found(player, velocity.y)) oldBoost = null;
        ...
        private void Boost(AbilityManager player, ref Vector2 velocity)
        {
            ...
            var time = timer <= 0 ? 1f : timer;
            force = Vector2.Lerp(forceOrigin, Vector2.zero, boostCounter / time);
            if (player.jumpButtonActive) escapeY = true;
            var mag = escapeY ? Mathf.Abs(force.x) : force.magnitude;
        ...
        private void Slow(AbilityManager player, ref Vector2 velocity)
        {
            ...
            var adjust = Mathf.Lerp(1f - force.x, 0, slowCounter / 0.75f);
            if (velocity.x != 0)
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "High Jump", barColor, labelColor))
            {
                ...
                {
                    Fields.EventFoldOutEffect(parent.Get("onTrampoline"), parent.Get("trampolineWE"),
                        parent.Get("trampolineFoldOut"), "On Trampoline");
                    Fields.EventFoldOutEffectAndRate(parent.Get("onWind"), parent.Get("windWE"), parent.Get("windRate"),
                        parent.Get("windFoldOut"), "On Wind");
                    Fields.EventFoldOutEffect(parent.Get("onSpeedBoost"), parent.Get("speedBoostWE"),
                        parent.Get("speedBoostFoldOut"), "On Speed Boost");
                    Fields.EventFoldOutEffectAndRate(parent.Get("onSlowDown"), parent.Get("slowDownWE"),
                        parent.Get("slowRate"), parent.Get("slowDownFoldOut"), "On Slow Down");
                }
            }
