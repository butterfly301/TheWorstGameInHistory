...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Hover : Ability // anim signal: hover
    {
        ...
        [SerializeField] public UnityEventEffect onThrust = new();
        [SerializeField] public UnityEventEffect onDescend = new();
        [SerializeField] public UnityEventFloat thrustCount = new();
        [SerializeField] public string thrustWE;
        [SerializeField] public string descendWE;


        [NonSerialized] public bool hovering;
        [NonSerialized] public int thrustCounter;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            var thrust = player.inputs.Pressed(this.thrust);
            ...
            bool isRunningAsException = false)
        {
            ...
            player.signals.Set("hover");
            HorizontalVelocity(player.inputX, player.speed, player.velocityX, 1f - airFrictionX,
                ref velocity); // reapply x velocity, will also override impede change (if it exists)

            if (player.inputs.Pressed(thrust) && ThrustLimitCounter())
            {
                velocity.y = player.maxJumpVel * hoverThrust;
                onThrust.Invoke(ImpactPacket.impact.Set(thrustWE, transform, player.world.boxCollider,
                    player.world.position, null, player.world.box.down, player.playerDirection, 0));
            }

            if (player.inputs.Pressed(descend))
            {
                velocity.y += player.gravityEffect * descendThrust;
                onDescend.Invoke(ImpactPacket.impact.Set(descendWE, transform, player.world.boxCollider,
                    player.world.position, null, player.world.box.down, player.playerDirection, 0));
            }
        ...
        {
            if (!thrustLimitEnable) return true;
        ...
        {
            if (thrustLimitEnable) thrustCount.Invoke(thrustCounter);
        }
...
#pragma warning disable 0414
        ...
        [SerializeField] private bool countFoldOut;
        [SerializeField] private bool eventsFoldOut;
        [SerializeField] private bool thrustFoldOut;
        [SerializeField] private bool descendFoldOut;
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Hover", barColor, labelColor))
            {
                ...
                {
                    parent.Slider("Thrust", "hoverThrust");
                    parent.Slider("Maintain", "maintainThrust", 0.5f);
                    ...
                    Fields.EventFoldOutEffect(parent.Get("onThrust"), parent.Get("thrustWE"),
                        parent.Get("thrustFoldOut"), "On Thrust", execute: eventOpen, color: FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onDescend"), parent.Get("descendWE"),
                        parent.Get("descendFoldOut"), "On Descend", execute: eventOpen, color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("thrustCount"), parent.Get("countFoldOut"), "Thrust Count",
                ...
                FoldOut.Box(2, FoldOut.boxColorLight);
                {
                    var exitType = parent.Enum("exitType");
                    parent.Field("Exit", "exitType", exitType == 0);
                    parent.FieldAndDropDownList(inputList, "Exit", "exitType", "exit", exitType > 0);
                    parent.Slider("Air Friction X", "airFrictionX");
                }
