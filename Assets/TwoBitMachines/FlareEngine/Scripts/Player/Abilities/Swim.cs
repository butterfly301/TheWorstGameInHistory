...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Swim : Ability //* animation signals: floating, swimming, inWater
    {
        ...
        [NonSerialized] private bool canJump;
        [NonSerialized] private float counterX;
        [NonSerialized] private float floatCounter;
        [NonSerialized] private bool inWater;
        [NonSerialized] private bool isFloating;
        [NonSerialized] private float jumpCounter;
        [NonSerialized] private bool jumpedFromWater;
        [NonSerialized] private float oldVelX;
        [NonSerialized] private int particleIndex;
        [NonSerialized] private float springVelocity;

        [NonSerialized] private bool submerged;

        //*Reset variables
        [NonSerialized] private SwimType swimType;
        [NonSerialized] private bool wasFloating;
        [NonSerialized] private bool wasInWater;
        [NonSerialized] private Water water;

        [NonSerialized] private Vector2 waveTopPoint;
        ...
        private void Floating(AbilityManager player, ref Vector2 velocity, bool canSwitch = false)
        {
            ...
            //* Spring Mechanics
            var acceleration = spring * (waveTopPoint.y - (player.world.box.center.y + player.world.box.sizeY * 0.1f)) -
            ...
            if (Time.deltaTime != 0 && velocity.y > 0 &&
                player.world.box.bottom + velocity.y * Time.deltaTime >= waveTopPoint.y)
            {
                velocity.y =
                    (waveTopPoint.y - 0.1f - player.world.box.bottom) / Time.deltaTime *
                    0.5f; // clamp player y velocity if being shot out of water too fast
                springVelocity = 0; //
            }
            ...
            if (velocity.x != 0)
                water.ApplyImpact(particleIndex + (int)Mathf.Sign(velocity.x),
                    waterImpact * 0.02f * Mathf.Abs(velocity.x * 0.1f));
            if (Clock.TimerExpired(ref jumpCounter, 0.1f) || player.world.box.top <= waveTopPoint.y) submerged = true;
            if (canSwitch && player.inputs.Holding(switchButton))
            {
                springVelocity +=
                    player.gravityEffect * 0.25f; // make player sink beneath water line before switching type
                if (player.world.box.top < waveTopPoint.y) swimType = SwimType.Swim;
            }
        ...
        private void Swimming(AbilityManager player, ref Vector2 velocity, bool canSwitch = false)
        {
            ...
            velocity.x -= water.wave.currentStrength;
            velocity.x *= 1f - waterResistanceX;
            var playerTop = player.world.box.top;

            SwimEasingX(player, ref velocity);
            if (playerTop <= waveTopPoint.y) submerged = canJump = true;
            ...
            if (submerged && player.world.box.bottom > waveTopPoint.y) ExitWater(player, ref velocity);
        }
        ...
        private void ExitWater(AbilityManager player, ref Vector2 velocity)
        {
            ...
            onExit.Invoke(ImpactPacket.impact.Set(exitWE, player.world.transform, player.world.boxCollider,
                waveTopPoint, null, Vector2.down, 1, 0));
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (bubbles != null) bubbles.Execute(water, inWater);
            ...
            if (jumpedFromWater && velocity.y > 0) return false;
        ...
        private bool FoundWater(BoxInfo box, bool onGround, ref Vector2 velocity)
        {
            ...
            if (onGround)
                canJump = false;
            var offset = wasFloating ? Vector2.down * 0.5f : Vector2.zero;
            var entryPoint = box.bottomCenter + offset;

            for (var i = 0; i < Water.water.Count; i++)
                if (Water.water[i] != null &&
                    Water.water[i].FoundWater(entryPoint, wasInWater, ref waveTopPoint, ref particleIndex))
                {
                    water = Water.water[i];
                    if (box.top > waveTopPoint.y && onGround) //   water became shallow
                        return false;
                    if (!wasInWater) EnterWater(box, water, ref velocity);
                    return true;
                }
        ...
        private void EnterWater(BoxInfo box, Water water, ref Vector2 velocity)
        {
            ...
            onEnter.Invoke(ImpactPacket.impact.Set(enterWE, transform, box.collider, waveTopPoint, null, Vector2.up, 1,
                0));
            ...
            if (water.swimType == SwimType.Swim) velocity.y *= 0.35f; //       slow down entry to prevent quick sinking
            ...
            bool isRunningAsException = false)
        {
            ...
            var canSwitch = water.canSwitch == SwitchSwimTypes.Yes;
            var swim = canSwitch ? swimType : water.swimType;
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Swim", barColor, labelColor))
            {
                FoldOut.Bar(FoldOut.boxColorLight).Label("Float", FoldOut.titleColor);
                {
                    ...
                    parent.Slider("Spring", "spring", 0.25f);
                ...
                FoldOut.Box(4, FoldOut.boxColorLight, 3);
                {
                    parent.Slider("Water Impact", "waterImpact", 0, 0.5f);
                    parent.Slider("Water Friction X", "waterResistanceX", 0.01f);
                    parent.Slider("Water Friction Y", "waterResistanceY", 0.01f);
                ...
                if (FoldOut.FoldOutButton(parent.Get("eventsFoldOut")))
                {
                    Fields.EventFoldOutEffect(parent.Get("onEnter"), parent.Get("enterWE"),
                        parent.Get("eventEnterFoldOut"), "On Enter Water", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onExit"), parent.Get("exitWE"),
                        parent.Get("eventExitFoldOut"), "On Exit Water", FoldOut.boxColorLight);
                }
            }
