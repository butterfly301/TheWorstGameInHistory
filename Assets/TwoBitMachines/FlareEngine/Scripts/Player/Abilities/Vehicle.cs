...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Vehicle : Ability
    {
        ...
        [NonSerialized] public Player passengerAbility;

        [NonSerialized] public Passenger passengerRef;
        [NonSerialized] public Player playerRef;
        ...
        {
            if (stopOnPassengerDeath && passengerRef != null && passengerRef.isDead) velocity = Vector2.zero;
        }
        ...
        public override void PostCollisionExecute(AbilityManager player, Vector2 velocity)
        {
            ...
            if (!Clock.TimerExpired(ref counter, mountTime)) player.signals.Set("vehicleMounted");

            Vector3 newHoldPosition = player.signals.characterDirection > 0
                ? passengerPosition
                : new Vector2(-passengerPosition.x, passengerPosition.y);
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Vehicle", barColor, labelColor))
            {
                ...
                Fields.EventFoldOut(parent.Get("onMounted"), parent.Get("eventFoldout"), "On Mount",
