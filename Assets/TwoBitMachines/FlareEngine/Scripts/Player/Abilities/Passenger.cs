...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Passenger : Ability
    {
        ...
        [NonSerialized] private Vehicle vehicle;
        [NonSerialized] private Collider2D vehicleCollider;
        ...
        public bool ExitCollider(AbilityManager player)
        {
            ...
            var length = Physics2D.Raycast(box.bottomCenter, -box.up, filter2D, results, 0.1f);
        ...
        {
            if (isExiting && (player.ground || ExitCollider(player))) isExiting = false;
            ...
            if (Search(player, velocity)) return true;
            return false;
        }
        ...
        public bool Search(AbilityManager player, Vector2 velocity)
        {
            ...
            var box = player.world.box;
            var magnitude = Mathf.Abs(v.y) + box.skin.y;
            var origin = box.bottomCenter + box.right * v.x;
            var hit = Physics2D.Raycast(origin, -box.up, magnitude, vehicleLayer);

            #region Debug

#if UNITY_EDITOR
            if (WorldManager.viewDebugger) Debug.DrawRay(origin, -box.up * magnitude, Color.blue);
#endif
            ...
            if (hit)
            {
                var vehicle = hit.transform.GetComponent<Vehicle>();
