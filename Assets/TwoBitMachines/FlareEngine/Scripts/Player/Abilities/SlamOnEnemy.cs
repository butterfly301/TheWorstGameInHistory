...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class SlamOnEnemy : Ability
    {
        ...
        {
            if (isSlamming && health != null) health.CanTakeDamage(true);
        ...
        {
            if (isSlamming) return true;
            if (pause || player.ground || velocity.y > 0) return false;
        ...
        public override void ExecuteAbility(AbilityManager player, ref Vector2 velocity,
            bool isRunningAsException =
                false) // since using EarlyExecute, this will ALWAYS execute. No need for priority.
        {
            player.signals.Set("slamOnEnemy");
            if (blockVelX) velocity.x = 0;

            if (begin)
            {
                begin = false;
                onSlamBegin.Invoke(ImpactPacket.impact.Set(slamBeginWE, transform, player.world.boxCollider,
                    player.world.position, transform, player.world.box.up, player.playerDirection, 0));
            }

            if (holding)
            {
                player.signals.Set("slamRecover");
                if (Clock.TimerExpired(ref groundCounter, groundTime)) Reset(player);
                return;
            }
            ...
            {
                var position = player.world.position;
                var hit = Compute.OverlapCircle(position, impactRadius, layer);
                if (Health.HitContactResults(transform, Compute.contactResults, hit, -damage, damageForce,
                        position)) // blast radius should be in all directions, derive direction from position
                    onSlam.Invoke(ImpactPacket.impact.Set(slamWE, transform, player.world.boxCollider,
                        player.world.position, transform, player.world.box.up, player.playerDirection, 0));
                if (ground)
                {
                    holding = true;
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            ...
            {
                FoldOut.Box(6, FoldOut.boxColorLight, 5, -2);
                ...
                {
                    Fields.EventFoldOutEffect(parent.Get("onSlamBegin"), parent.Get("slamBeginWE"),
                        parent.Get("slamBeginFoldOut"), "On Slam Begin", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onSlam"), parent.Get("slamWE"), parent.Get("slamFoldOut"),
                        "On Slam End", FoldOut.boxColorLight);
                }
            }
