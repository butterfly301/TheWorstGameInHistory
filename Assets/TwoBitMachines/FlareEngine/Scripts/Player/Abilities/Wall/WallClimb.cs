...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class WallClimb // anim signals: wall, wallHold, wallSlide, wallClimb
    {
        ...
        [NonSerialized] private bool autoCornerJump;
        [NonSerialized] private float barrierSign;
        [NonSerialized] private float barrierY;

        [NonSerialized] public bool clearBarrierY;
        [NonSerialized] private float climbCounter;
        [NonSerialized] private float cornerTopY;
        [NonSerialized] private float fallCounter;
        [NonSerialized] private bool fallToCorner;
        [NonSerialized] private bool isHolding;
        [NonSerialized] private int jumpCounter;
        [NonSerialized] private Vector2 jumpToVelocity;
        [NonSerialized] private bool monitorJumpHeight;
        [NonSerialized] private float onClimbRateCounter;
        [NonSerialized] private float slideAccel;
        [NonSerialized] private float slideRateCounter;
        [NonSerialized] private float slideStamp;
        [NonSerialized] private bool stickActive;
        [NonSerialized] private float stickInputTimeCounter;
        [NonSerialized] private float stickTimeCounter;

        [NonSerialized] private bool upJump;
        ...
        public void OnExit(AbilityManager player)
        {
            onExit.Invoke(ImpactPacket.impact.Set(exitWE, player.world.transform, player.world.boxCollider,
        ...
        {
            if (!clearBarrierY) return false;
            if (player.inputX != 0 && !Compute.SameSign(barrierSign, player.inputX))
                clearBarrierY = false;
            else if (player.world.position.y <= barrierY || WallGap(player, player.world.box)) clearBarrierY = false;
            return clearBarrierY;
        }

        private bool WallGap(AbilityManager player, BoxInfo box)
        {
            var corner = barrierSign > 0 ? box.bottomRight - box.skinX : box.bottomLeft + box.skinX;
            for (var i = 0; i < box.rays.x; i++)
            {
                var origin = corner + box.up * box.spacing.y * i;
                var hit = Physics2D.Raycast(origin, box.right * barrierSign, 0.25f, player.world.collisionLayer);
                if (!hit)
                    return true;
            }
        ...
        {
            if (!player.world.onWall || clearBarrierY || isJumping || (jumpingLimit && jumpCounter > jumpLimit) ||
                ClimbTimer(player)) return;
            if (holdType == HoldType.HoldButton && !player.inputs.Holding(hold)) return;
            if (holdType == HoldType.Automatic && !isHolding && velocity.y > 0)
            {
                velocity.y *=
                    0.9f; //                                        slow down velocity y if crashing into wall before latching on
                return;
            }

            var wallDirection = wall.Direction(player);

            if (player.ground && player.inputX != 0 && !Compute.SameSign(player.inputX, wallDirection)) return;

            if (!isHolding &&
                !DetectWall(player, wallDirection, velocity.x)) //  do not start climbing until player is below corner
            {
                if (velocity.y > 0)
            ...
            if (Jump(player, wall, player.inputX, wallDirection, ref velocity)) return;
            ClimbWall(player, wallDirection, ref velocity);
        }

        private bool ClimbTimer(AbilityManager player)
        {
            if (isJumping || player.ground) return false;
            if (enableClimbTimer && Clock.TimerExpired(ref climbCounter, climbTimer))
            {
                if (fallTimer > 0 && Clock.TimerExpired(ref fallCounter, fallTimer)) return true;
                if (fallTimer > 0) onFallLimit.Invoke(fallCounter / fallTimer);
            }

            if (enableClimbTimer && climbTimer > 0)
            {
                var percent = climbCounter / climbTimer;
                if (percent >= 0.70f) onClimbLimit.Invoke(percent);
            }
        ...
        private bool Jump(AbilityManager player, Wall wall, float inputX, int wallDirection, ref Vector2 velocity)
        {
            if (jumpOnInputChange)
            {
                ...
                if (stickActive)
                {
                    if (Time.time >= stickInputTimeCounter + 0.1f) ResetStickTime();
            ...
            {
                if (inputX == 0 || Compute.SameSign(inputX, wallDirection)) // jump up
                {
                    ...
                    clearBarrierY =
                        true; // must reset means the player can only jump up once to prevent wall jump spaming
                    OnExit(player);
                    return true;
                }
            ...
            {
                var jumpTo = new Vector2(player.world.position.x + player.world.box.sizeX * wallDirection, cornerTopY);
        ...
        {
            var climbY = cantClimb ? 0 : Inputs(player, wallDirection);
            if (climbY != 0) slideStamp = Time.time + 0.15f;

            if (climbUpOnly && climbY <= 0) return;
            ...
            {
                if (slideDown == WallSlideType.OnNoInput && climbY == 0 && Time.time >= slideStamp) sliding = true;
                if (slideDown == WallSlideType.OnNoInputUp && climbY <= 0) sliding = true;
                if (climbY > 0) slideAccel = 0;
                if (sliding)
                {
                    ...
                    if (slideRate > 0 && Clock.Timer(ref slideRateCounter, slideRate))
                    {
                        var impact = ImpactPacket.impact.Set(slideWE, player.world.transform, player.world.boxCollider,
                            player.world.position, null, player.world.box.right * wallDirection, player.playerDirection,
                            0);
            ...
            {
                ResetStickTime();
                onEnter.Invoke(ImpactPacket.impact.Set(enterWE, player.world.transform, player.world.boxCollider,
                    player.world.position, null, player.world.box.right * wallDirection, player.playerDirection, 0));
            }

            if ((!isHolding || velocity.y != 0) && onClimbRate > 0 && Clock.Timer(ref onClimbRateCounter, onClimbRate))
            {
                var impact = ImpactPacket.impact.Set(worldEffect, player.world.transform, player.world.boxCollider,
                    player.world.position, null, player.world.box.right * wallDirection, player.playerDirection, 0);
        ...
        {
            if (climbType == ClimbType.None) return 0;

            var useButtons = climbType == ClimbType.Button;
            var up = player.inputs.Holding(useButtons ? climbUp : "Left");
            var down = player.inputs.Holding(useButtons ? climbDown : "Right");

            if (!useButtons && !up && player.inputX < 0) up = true;

            if (!useButtons && !down && player.inputX > 0) down = true;

            if (useButtons || wallDirection == -1) return (up ? climbUpSpeed : 0) + (down ? -climbDownSpeed : 0);
        ...
        public void CompleteJump(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (fallToCorner)
            {
                if (player.ground || !Compute.SameSign(player.inputX, wallDirRef) ||
                    DetectWall(player, wallDirRef, velocity.x)) ResetJumpSignals();
            }
            else if (upJump)
            {
                if (player.ground || velocity.y < 0 ||
                    (player.inputX != 0 && !Compute.SameSign(player.inputX, wallDirRef))) ResetJumpSignals();
            }
            else if (autoCornerJump)
            {
                ...
                if (player.ground || Wall.CheckForGround(player.world.box, jumpToVelocity.x, velocity.y) ||
                    (player.inputX != 0 && !Compute.SameSign(player.inputX, wallDirRef)))
                    ResetJumpSignals();
                else
        ...
        {
            var shift = -box.size.y *
                        0.25f; // jump automatically to platform top if player is at least 25% above corner
            var origin = box.TopExactCorner(wallDirection) + box.up * shift + box.right * 0.1f * wallDirection;
            var hit = Physics2D.Raycast(origin, box.down, box.size.y + shift, WorldManager.collisionMask);

            if (!hit || hit.distance == 0) return false;
        ...
        {
            var origin = player.world.box.TopExactCorner(wallDirection);
            var direction = player.world.box.right * wallDirection;
            var length = Mathf.Abs(velX * Time.deltaTime);
...
#pragma warning disable 0414
        ...
        {
            var wallClimb = parent.Get("wallClimb");

            if (type == 1)
            {
                if (FoldOut.FoldOutButton(wallClimb.Get("eventsFoldOut")))
                {
                    Fields.EventFoldOutEffectAndRate(wallClimb.Get("onClimb"), wallClimb.Get("worldEffect"),
                        wallClimb.Get("onClimbRate"), wallClimb.Get("onClimbFoldOut"), "On Climb", Tint.Blue);
                    Fields.EventFoldOutEffectAndRate(wallClimb.Get("onSlide"), wallClimb.Get("slideWE"),
                        wallClimb.Get("slideRate"), wallClimb.Get("slideFoldOut"), "On Slide", Tint.Blue);
                    Fields.EventFoldOutEffect(wallClimb.Get("onEnter"), wallClimb.Get("enterWE"),
                        wallClimb.Get("enterFoldOut"), "On Enter", Tint.Blue);
                    Fields.EventFoldOutEffect(wallClimb.Get("onExit"), wallClimb.Get("exitWE"),
                        wallClimb.Get("exitFoldOut"), "On Exit", Tint.Blue);
                    Fields.EventFoldOut(wallClimb.Get("onClimbLimit"), wallClimb.Get("climbLimitFoldOut"),
                        "On Climb Limit", color: Tint.Blue);
                    Fields.EventFoldOut(wallClimb.Get("onFallLimit"), wallClimb.Get("fallLimitFoldOut"),
                        "On Fall Limit", color: Tint.Blue);
                }

                var holdType = wallClimb.Enum("holdType");
                var climbType = wallClimb.Enum("climbType");
                var slideType = wallClimb.Enum("slideDown");
                var slideSpeed = wallClimb.Enum("slideDownSpeed");
                var height = climbType == 0 ? 1 : 0;
                FoldOut.Box(2 + height, FoldOut.boxColorLight);
                {
                    wallClimb.Field("Buttons", "climbType");
                    wallClimb.DropDownDoubleList(inputList, "Up, Down", "climbUp", "climbDown",
                        wallClimb.Enum("climbType") == 0);
                    wallClimb.Field("Hold Wall", "holdType", holdType == 0);
                    wallClimb.FieldAndDropDownList(inputList, "Hold Wall", "holdType", "hold", holdType == 1);
                }
                ...
                FoldOut.Box(4 + height, FoldOut.boxColorLight);
                {
                    ...
                    wallClimb.FieldDouble("Slide Speed", "slideDownSpeed", "slideVelocity",
                        slideType > 0 && slideSpeed == 0);
                    wallClimb.FieldDouble("Slide Speed", "slideDownSpeed", "slideAccelerate",
                        slideType > 0 && slideSpeed == 1);
