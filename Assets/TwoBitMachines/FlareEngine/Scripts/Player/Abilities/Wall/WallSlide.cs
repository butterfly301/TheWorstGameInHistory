...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    {
        [NonSerialized] private const float stickTime = 0.2f;
        ...
        [SerializeField] public Vector2 jumpUp = new(10f, 15f);
        [SerializeField] public Vector2 jumpAway = new(10f, 15f);
        ...
        [NonSerialized] private float extraCounter;
        ...
        [NonSerialized] private float jumpStickCounter;
        [NonSerialized] private bool jumpTimerEnabled;

        [NonSerialized] private SlideJumpType jumpType;
        [NonSerialized] private bool monitorJumpHeight;
        [NonSerialized] private float onSlideRateCounter;
        [NonSerialized] private float slideCounter;
        [NonSerialized] private float stickCounter;
        [NonSerialized] private float velocityXRef;
        [NonSerialized] private int wallDirectionRef;
        ...
        {
            if (!player.world.onWall || isJumping || (jumpingLimit && jumpCounter > jumpLimit)) return;
            if (!canClimbUp && velocity.y >= 0) return;
            if (player.world.missedAHorizontal && !DetectWall(player, wall.Direction(player))) return;
            if (enableSlideTimer && Clock.TimerExpired(ref slideCounter, slideTimer)) return;
        ...
        private void SlideDownWall(AbilityManager player, int wallDirection, ref Vector2 velocity)
        {
            ...
            velocity.x = slideType == SlideType.Automatic || StickTime(player, wallDirection)
                ? 0.25f * wallDirection
                : velocity.x;
            ...
            {
                var impact = ImpactPacket.impact.Set(onSlideWE, player.world.transform, player.world.boxCollider,
        ...
        private bool Jump(AbilityManager player, int wallDirection, ref Vector2 velocity)
        {
            ...
            if (!player.jumpButtonPressed || (jumpingLimit && jumpCounter++ >= jumpLimit)) return false; // not jumping
            if (player.inputX == 0)
            {
        ...
        {
            if (slideType == SlideType.PressingInput &&
                player.inputX ==
                0) //            if player is not pressing any x input, stick to wall for a bit in case players wants to jump
            {
                if (!Clock.Timer(ref stickCounter, stickTime)) return true;
            }
            else
            {
                stickCounter = 0;
            ...
            if (player.inputX != 0 &&
                !Compute.SameSign(player.inputX,
                    wallDirection)) // when player is moving away from wall, stick to wall for a bit in case player wants to jump
            {
                if (!Clock.Timer(ref jumpStickCounter, stickTime)) return true;
            }
            else
            {
                jumpStickCounter = 0;
        ...
        public void CompleteJump(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (jumpType == SlideJumpType.JumpUp)
            {
                ...
                else // move away and then towards wall
                {
                    ...
                    velocityXRef = Mathf.Clamp(velocityXRef + player.speed * wallDirectionRef * Time.deltaTime * 5f,
                        -player.speed, player.speed);
                    player.signals.Set("wallSlideJump");
        ...
        {
            var origin = player.world.box.center;
...
#pragma warning disable 0414
        ...
        {
            var wallSlide = parent.Get("wallSlide");

            if (type == 0)
            {
                if (FoldOut.FoldOutButton(wallSlide.Get("eventsFoldOut")))
                {
                    Fields.EventFoldOutEffectAndRate(wallSlide.Get("onSlide"), wallSlide.Get("onSlideWE"),
                        wallSlide.Get("onSlideRate"), wallSlide.Get("onSlideFoldOut"), "On Slide",
                        FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(wallSlide.Get("onEnter"), wallSlide.Get("enterWE"),
                        wallSlide.Get("enterFoldOut"), "On Enter", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(wallSlide.Get("onExit"), wallSlide.Get("exitWE"),
                        wallSlide.Get("exitFoldOut"), "On Exit", FoldOut.boxColorLight);
                }

                var slideType = wallSlide.Enum("slideType");
                var height = slideType == 1 ? 1 : 0;
                FoldOut.Box(3 + height, FoldOut.boxColorLight);
                {
                    ...
                    wallSlide.FieldAndEnableHalf("Climb Up", "climbUpSpeed", "canClimbUp", slideType == 1);
                    Labels.FieldText("Speed", 17, slideType == 1);
                }
