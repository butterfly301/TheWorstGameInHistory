...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class RotateToWall
    {
        ...
        [NonSerialized] private float climbRateCounter;
        [NonSerialized] public bool isJumping;
        [NonSerialized] private float latchCounter;
        [NonSerialized] public bool latched;
        [NonSerialized] private float pressRateCounter;
        ...
        {
            if (latched || isJumping) player.world.transform.localEulerAngles = new Vector3(0, 0, 0);
        }
        ...
        {
            if (player.world.wallTransform != null && player.world.wallTransform.CompareTag("RotatedWall")) return true;
            if (player.world.verticalTransform != null && player.world.verticalTransform.CompareTag("RotatedWall"))
            {
                var box = player.world.box;
                var tempSign = player.playerDirection;
                var direction = box.right * tempSign;
                var hit = Physics2D.Raycast(box.center, direction, box.size.x * 1.25f, WorldManager.collisionMask);
                if (hit && hit.distance > 0 && hit.transform.CompareTag("RotatedWall")) return true;
            }
        ...
        public bool Rotate(Wall wall, AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (!latched && !RotatedSurface(player)) return false;

            var box = player.world.box;
            var tempSign = latched ? sign : player.playerDirection;
            var direction = box.right * tempSign;
            var hit = Physics2D.Raycast(box.center, direction, box.size.x * 1.25f, WorldManager.collisionMask);
            ...
            if (hit && hit.distance > 0)
            {
                ...
                var side = box.center + direction * box.size.x * 0.52f;
                ...
                {
                    var wallDirection = sign < 0 ? -1f : 1f;
                    var impact = ImpactPacket.impact.Set(climbWE, player.world.transform, player.world.boxCollider,
                        player.world.transform.position, null, player.world.box.right * wallDirection,
            ...
            if (latched && Jump(player, ref velocity)) return false;
            return latched;
        }

        private bool Jump(AbilityManager player, ref Vector2 velocity)
        {
            if (!canJump || player.inputX == 0 || !Compute.SameSign(-sign, player.inputX)) return false;
        ...
        private void RotatePlayer(AbilityManager player, Vector2 pivot, Vector2 direction, Vector2 slopeNormal,
            float rotateRate = 15f)
        {
            var angle = Vector2.Angle(direction, slopeNormal);
            var maxAngle = Mathf.Clamp(angle * Time.deltaTime * rotateRate, 0, angle);
        ...
        public void CompleteJump(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (Vector2.Angle(box.up, Vector2.up) == 0) Reset(player);
            if (velocity.y <= 0 &&
                (player.world.onGround ||
                 player.world.wasOnGround))
        ...
        public float Inputs(AbilityManager player, int wallDirection)
        {
            ...
            if (useButtons || wallDirection == -1) return (up ? speed : 0) + (down ? -speed : 0);
        ...
        private void GroundCheck(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (pressToClimb != "None")
            {
                ...
                if (player.inputs.Pressed(pressToClimb)) pressRateCounter = 0;
                if (pressRateCounter >= pressRate) return;
            }

            velocity.y = Inputs(player, this.sign);
            ...
            var magnitude = Mathf.Abs(velocity.y) * Time.deltaTime + box.skin.y;
            var corner = sign > 0 ? box.topLeft : box.bottomLeft;

            for (var i = 0; i < box.rays.y; i++)
            {
                var origin = corner + box.right * (box.spacing.x * i);
                var hit = Physics2D.Raycast(origin, box.up * sign, magnitude, world.collisionLayer);

                if (!hit || hit.transform.CompareTag("RotatedWall") ||
                    (hit.collider is EdgeCollider2D && world.IgnoreEdge2D(hit.transform, sign))) continue;
            ...
            if (hitSurface && sign < 0) Reset(player);
        }
...
#pragma warning disable 0414
        ...
        {
            var rotateToWall = parent.Get("rotateToWall");

            if (FoldOut.Bar(rotateToWall, FoldOut.boxColorLight).Label("Rotate To Wall", FoldOut.titleColor, false)
                .BRE().FoldOut())
            {
                var climbType = rotateToWall.Enum("climbType");
                ...
                FoldOut.Box(4 + height, FoldOut.boxColorLight, 5, -2);
                {
                    ...
                    rotateToWall.DropDownDoubleList(inputList, "Up, Down", "climbUp", "climbDown", climbType == 0);
                    rotateToWall.FieldAndDropDownList(inputList, "Press To Climb", "pressRate", "pressToClimb");
                    Labels.FieldText("Rate", Layout.contentWidth * 0.5f); // Press Rate
                ...
                if (FoldOut.FoldOutButton(rotateToWall.Get("eventsFoldOut")))
                    Fields.EventFoldOutEffectAndRate(rotateToWall.Get("onClimb"), rotateToWall.Get("climbWE"),
                        rotateToWall.Get("climbRate"), rotateToWall.Get("climbFoldOut"), "On Climb",
                        FoldOut.boxColorLight);
