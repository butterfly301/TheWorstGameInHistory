...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Wall : Ability
    {
        ...
        [SerializeField] public WallSlide wallSlide = new();
        [SerializeField] public WallClimb wallClimb = new();
        [SerializeField] public CornerGrab cornerGrab = new();
        [SerializeField] public CornerHang cornerHang = new();
        [SerializeField] public RotateToWall rotateToWall = new();
        ...
        {
            if (cornerGrab.isGrabbing && !player.pushBackActive) return false;
            Reset(player);
            return true;
        }
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            return WallIsActive(player) &&
                   (rotateToWall.Available(player) || WallIsSquare(player)); // only climb right angle walls
        }
        ...
        public override void EarlyExecute(AbilityManager player, ref Vector2 velocity)
        {
            ...
            else if (wallSlide.isJumping) wallSlide.CompleteJump(player, ref velocity);
            if (wallClimb.clearBarrierY) wallClimb.ClearBarrierY(player);
            if (rotateToWall.isJumping) rotateToWall.CompleteJump(player, ref velocity);
        }

        public override void ExecuteAbility(AbilityManager player, ref Vector2 velocity,
            bool isRunningAsException = false)
        {
            if (cornerGrab.Grab(this, player, ref velocity)) return;
            if (cornerHang.Hang(this, player, ref velocity)) return;
            if (rotateToWall.Rotate(this, player, ref velocity)) return;
            if (type == WallType.Climb) wallClimb.Climb(this, player, ref velocity);
            if (type == WallType.Slide) wallSlide.Slide(this, player, ref velocity);
        }

        private bool WallIsActive(AbilityManager player)
        {
            return (player.world.onWall || cornerGrab.isGrabbing || rotateToWall.Available(player)) &&
                   !IsWallJumping() && CanClimbWall(player);
        }
        ...
        {
            return player.world.wallTransform == null || (!player.world.wallTransform.CompareTag("NoClimb") &&
        ...
        public static bool CheckForGround(BoxInfo box, float velX, float velY)
        {
            ...
            {
                var origin = box.bottomLeft + box.right * (box.spacing.x * i + velX * Time.deltaTime);
                var hit = Physics2D.Raycast(origin, box.down, Mathf.Abs(velY) + box.skin.y, box.world.collisionLayer);
                if (hit && hit.distance > 0) return true;
            }
...
#pragma warning disable 0414
        ...
        {
            var cornerHang = parent.Get("cornerHang");
            var hangExitType = cornerHang.Enum("exitType");

            if (FoldOut.Bar(cornerHang, FoldOut.boxColorLight).Label("Corner Hang", FoldOut.titleColor, false)
                .BRE("cornerHang").FoldOut())
            {
                ...
                cornerHang.DropDownDoubleList(inputList, "Up, Down", "climbUp", "climbDown", hangExitType == 0);
                GUI.enabled = true;
                Layout.VerticalSpacing(3);
            }
        }
