...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class CornerGrabAnimation //anim signals: wall, wallCornerGrab
    {
        ...
        [SerializeField] public bool useGameObject;
        [SerializeField] public List<CornerGrabFrame> animation = new();
        [NonSerialized] private float cornerOffsetY;
        [NonSerialized] private bool holdingMP; // moving platforms
        [NonSerialized] private SpriteRenderer renderer;
        [NonSerialized] private SpriteEngineBase spriteEngine;
        [NonSerialized] private int spriteIndex;
        ...
        public void Initialize(Player player)
        {
            ...
            spriteEngine = player.GetComponent<SpriteEngineBase>();
        }

        public void Reset()
        {
            if (isGrabbing && renderer != null && spriteEngine != null) spriteEngine.pause = false;
            isGrabbing = false;
            holdingMP = false;
        }
        ...
        public bool IsPlaying(Wall wall, AbilityManager player, WorldCollision world, ref Vector2 velocity)
        {
            ...
            if (holdingMP) wall.cornerGrab.FindTopCorner(world.box, wallDirectionRef, velocity.y, ref topCorner);
        ...
        {
            if (renderer != null && spriteEngine != null) spriteEngine.pause = true;

            if (Clock.Timer(ref timer, animationTime / animation.Count)) //    play animation
            {
                if (spriteIndex < 0) onAnimationStart.Invoke();
        ...
        public void SetFinalPosition(WorldCollision world, bool value)
        {
            if (value)
            {
                world.transform.position =
                    new Vector3(
                        world.box.TopExactCorner(wallDirectionRef).x + world.box.sizeX * 0.5f * wallDirectionRef,
                        topCorner.y + 0.01f, 0);
        ...
        {
            var origin = box.topCenter;
            var hit = Physics2D.Raycast(origin, box.up, 0.25f,
                WorldManager.collisionMask); //    Debug.DrawRay (origin, box.up * 0.25f, Color.green);
