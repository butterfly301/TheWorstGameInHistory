...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class HighFall : Ability
    {
        ...
        {
            if (active && health != null && health.cantTakeDamage) health.CanTakeDamage(true);
            active = false;
            inAir = false;
            jumpPoint = 0;
            counter = 0;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (inAir && player.world.onGround && player.world.position.y < jumpPoint &&
                Mathf.Abs(jumpPoint - player.world.oldPosition.y) < height) inAir = false;
            if (inAir && player.world.onGround && player.world.position.y < jumpPoint &&
                Mathf.Abs(jumpPoint - player.world.oldPosition.y) >= height)
            {
                counter = 0;
                inAir = false;
                if (health != null)
            ...
            bool isRunningAsException = false)
        {
            ...
            if (Clock.Timer(ref counter, holdTime)) Reset(player);
        }
