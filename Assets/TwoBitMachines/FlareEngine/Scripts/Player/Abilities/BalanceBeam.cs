...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class BalanceBeam : Ability // beam should be an edge collider 2d!
    {
        ...
        [NonSerialized] private float beamDirection;
        [NonSerialized] private float counter;
        [NonSerialized] private ContactFilter2D filter2D;

        [NonSerialized] private bool isBalancing;
        [NonSerialized] private bool moveBackward;
        [NonSerialized] private bool moveForward;
        [NonSerialized] private readonly RaycastHit2D[] results = new RaycastHit2D[20];
        [NonSerialized] private float sign = 1f;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (isBalancing && notMoving && (!player.world.onGround || player.world.verticalTransform == null ||
                                             beamTag == "" ||
                                             !player.world.verticalTransform.gameObject.CompareTag(beamTag)))
            {
                Reset(player);
                return false;
            }

            if (isBalancing) return true;
            if (player.world.onGround && player.world.verticalTransform != null && beamTag != "" &&
                player.world.verticalTransform.gameObject.CompareTag(beamTag))
            {
                Reset(player);
                onBegin.Invoke();
            ...
            bool isRunningAsException = false)
        {
            ...
            if (moveForward)
            {
                ...
                if (Clock.Timer(ref counter, moveTime)) moveForward = moveBackward = false;
            }
        ...
        private bool IsRealGround(AbilityManager player)
        {
            ...
            var corner = box.bottomLeft;
            var magnitude = box.skin.y + 0.1f;

            for (var i = 0; i < box.rays.y; i++)
            {
                var origin = corner + box.right * (box.spacing.x * i);
                var length = Physics2D.Raycast(origin, -box.up, filter2D, results, magnitude);

                for (var j = 0; j < length; j++)
                {
                    var hit = results[j];
                    if (hit && !(hit.collider is EdgeCollider2D)) return true;
                }
            }
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Balance Beam", barColor, labelColor))
            {
                ...
                {
                    Fields.EventFoldOut(parent.Get("onBegin"), parent.Get("beginFoldOut"), "On Begin",
                        color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("onEnd"), parent.Get("endFoldOut"), "On End",
                        color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("sliderValue"), parent.Get("sliderFoldOut"), "Slider Value",
                        color: FoldOut.boxColorLight);
                }
        ...
        private void OnDrawGizmosSelected()
        {
            if (safeZoneUI != null)
            {
                var scale = safeZoneUI.transform.localScale;
                safeZoneUI.transform.localScale = new Vector3(safeZone, scale.y, scale.z);
                if (scale.x != safeZone) EditorUtility.SetDirty(safeZoneUI);
            }

            if (dangerZoneUI != null)
            {
                var scale = dangerZoneUI.transform.localScale;
                dangerZoneUI.transform.localScale = new Vector3(safeZone + dangerZone, scale.y, scale.z);
                if (scale.x != safeZone + dangerZone) EditorUtility.SetDirty(dangerZoneUI);
            }
        }
