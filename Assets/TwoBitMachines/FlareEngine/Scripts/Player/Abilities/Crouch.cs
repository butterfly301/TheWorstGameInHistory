...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Crouch : Ability
    {
        ...
        [NonSerialized] private float chargeCounter;
        [NonSerialized] private bool hasCrouched;
        [NonSerialized] private bool hasCrouchJumped;

        public override void Reset(AbilityManager player)
        {
            chargeCounter = 0;
            hasCrouchJumped = hasCrouched = false;
            if (player.world.boxCollider.size.y != player.world.box.boxSize.y) player.world.box.ColliderReset();
        }
        ...
        public override bool TurnOffAbility(AbilityManager player)
        {
            ...
            {
                if (hasCrouched) player.world.box.ColliderReset();
                chargeCounter = 0;
                hasCrouchJumped = hasCrouched = false;
                return true;
            }
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (player.inputs.Holding(button) && player.ground && !IsEdge2D(player.world)) return true;
            if (hasCrouched || hasCrouchJumped) return true;
            return false;
        }

        public static bool IsEdge2D(WorldCollision world)
        {
            var edge = world.verticalCollider;
            if (edge == null || !(edge is EdgeCollider2D)) return false;
            if ((world.skipDownEdge == WorldCollision.Edge2DSkip.OnDownHold && world.holdingDown) ||
                (world.skipDownEdge == WorldCollision.Edge2DSkip.OnDownAndJump && world.pressedDown))
        ...
        private void CanJump(AbilityManager player, ref Vector2 velocity)
        {
            ...
            {
                if (Clock.TimerExpired(ref chargeCounter, chargeTime) && player.jumpButtonActive &&
                    SafelyStandUp(player, player.world.box))
                {
                    hasCrouchJumped = false;
                ...
                if (chargeTime > 0) onHighJump.Invoke(chargeCounter / chargeTime);
            }
            else
            {
                chargeCounter = 0;
        ...
        {
            var length = Mathf.Abs(box.boxSize.y - box.collider.size.y) * box.collider.transform.localScale.y;
            for (var i = 0; i < box.rays.y; i++)
            {
                var origin = box.cornerTopLeft + box.right * (box.spacing.x * i);

                #region Debug

#if UNITY_EDITOR
                if (WorldManager.viewDebugger) Debug.DrawRay(origin, box.up * length, Color.white);
#endif

                #endregion

                var hit = Physics2D.Raycast(origin, box.up, length, WorldManager.collisionMask);
                if (hit && hit.distance == 0 && hit.transform.gameObject.layer == WorldManager.platformLayer) continue;
                if (hit) return false;
            }

            onExit.Invoke(ImpactPacket.impact.Set(exitWE, transform, box.world.boxCollider, box.world.position, null,
                box.world.box.down, player.playerDirection, 0));
            return true;
        }
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            ...
            {
                FoldOut.Box(2, FoldOut.boxColorLight, 5, -2);
                {
                ...
                if (FoldOut.FoldOutButton(parent.Get("entryFoldOut")))
                {
                    Fields.EventFoldOutEffect(parent.Get("onEnter"), parent.Get("enterWE"), parent.Get("enterFoldOut"),
                        "On Crouch", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onExit"), parent.Get("exitWE"), parent.Get("exitFoldOut"),
                        "On Exit", FoldOut.boxColorLight);
                }

                if (FoldOut.Bar(parent, FoldOut.boxColorLight).Label("Crawl", FoldOut.titleColor, false).BRE("crawl")
                    .FoldOut("crawlFoldOut"))
                {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColorLight).Label("Jump", FoldOut.titleColor, false)
                    .BRE("crouchJump").FoldOut("crouchFoldOut"))
                {
                    GUI.enabled = parent.Bool("crouchJump");
                    FoldOut.Box(1, FoldOut.boxColorLight, offsetY: -2);
                    parent.Slider("Jump Scale", "jumpScale", 0.1f);
                    GUI.enabled = true;
                    Layout.VerticalSpacing(3);
                }

                if (FoldOut.Bar(parent, FoldOut.boxColorLight).Label("High Jump", FoldOut.titleColor, false)
                    .BRE("canJumpBoost").FoldOut("jumpFoldOut"))
                {
                    ...
                    var eventOpen = FoldOut.FoldOutButton(parent.Get("eventsFoldOut"));
                    Fields.EventFoldOut(parent.Get("onHighJump"), parent.Get("eventFoldOut"), "On High Jump",
                        execute: eventOpen, color: FoldOut.boxColorLight);
                    GUI.enabled = true;
                    Layout.VerticalSpacing(3);
                }
            }
