...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class Jump : Ability
    {
        ...
        [SerializeField] public float airMomentum;
        ...
        [NonSerialized] private bool airGliding;

        [NonSerialized] private int airJumpCount;
        [NonSerialized] private float bufferTimer;
        [NonSerialized] private bool checkForAirJump;
        [NonSerialized] private bool checkForAirMomentum;
        [NonSerialized] private bool checkForFloat;
        [NonSerialized] private bool checkForMinJump;
        [NonSerialized] private bool checkRepress;
        [NonSerialized] private float coyoteTimer;
        [NonSerialized] private Gravity gravity;
        [NonSerialized] private bool groundFall;
        [NonSerialized] private float jumpInputXRef;
        [NonSerialized] private bool jumpRefreshed;
        [NonSerialized] private float maxJumpVel;
        ...
        public override void EarlyExecute(AbilityManager player, ref Vector2 velocity)
        {
            ...
            {
                if (player.checkForMomentum && airMomentum > 0 && Mathf.Abs(velocity.x) > 0) checkForAirMomentum = true;
            ...
            if (player.ground || player.world.onWall) player.dashAirJumpCheck = false;
            player.airJumpCount = airJumpCount;
        }

        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            player.jumpButtonActive =
                player.inputs.Active(buttonTrigger, "Jump"); //           read these values every frame
...
#if UNITY_EDITOR
            if (WorldManager.viewDebugger) player.jumpButtonActive = player.jumpButtonActive || forceJump;
#endif

            #endregion

            if (player.world.onGround && player.holdingDown && player.jumpButtonPressed &&
                player.world.verticalCollider != null && player.world.verticalCollider is EdgeCollider2D) return false;

            JumpBuffer(player);
            JumpFromFall(player);
            CoyoteTimer(player, velocity.y);

            return !pause && (player.jumpButtonActive || checkForMinJump || checkForAirJump || checkForFloat ||
                              checkForAirMomentum);
        }
            ...
            bool isRunningAsException = false)
        {
            if ((player.ground || coyote) && (player.jumpButtonActive || buffer))
            {
                ...
                onJump.Invoke(ImpactPacket.impact.Set(jumpWE, transform, player.world.boxCollider,
            ...
            if (checkForMinJump)
            {
                if (player.jumpButtonReleased && minJumpHeight > 0 && velocity.y > minJumpVel) velocity.y = minJumpVel;
                if (player.jumpButtonReleased) checkForMinJump = false;
            }

            if (checkForAirMomentum && !player.world.onWall &&
                (player.inputX == 0 || Compute.SameSign(player.inputX, jumpInputXRef)))
            {
                player.airMomentumActive = true;
                velocity.x = player.velocityOnGround * airMomentum;
                if (player.world.onWall) player.velocityOnGround = 0;
            }

            if (checkForAirJump)
            {
                if (player.jumpButtonPressed && airJumpCount < airJumps &&
                    airJumpCount++ < airJumps) // double check to keep airJump signal integrity
                {
                    player.dashAirJumpCheck = true;
                    velocity.y = maxJumpVel * airJumpScale;
                    onAirJump.Invoke(ImpactPacket.impact.Set(airJumpWE, transform, player.world.boxCollider,
                        player.world.position, null, player.world.box.down, player.playerDirection, 0));
                    airGliding = false;
                }
            ...
            // }
            if (checkRepress && !player.jumpButtonHold) jumpRefreshed = true;
            if ((velocity.y < 0 || (airGliding && velocity.y < player.maxJumpVel * glideBoost)) && checkForFloat &&
                (airJumpCount >= airJumps || glideImmediately) && player.inputs.Holding(glideButton) && jumpRefreshed)
            {
                velocity.y = !airGliding
                    ? player.maxJumpVel * glideBoost
                    : velocity.y - player.gravityEffect * glideScale;
                airGliding = true;
        ...
        {
            if (jumpBuffer > 0 && player.jumpButtonActive && !player.ground) bufferTimer = Time.time + jumpBuffer;
        }
        ...
        private void JumpFromFall(AbilityManager player)
        {
            ...
            if (player.ground) groundFall = true;
        ...
        public void SetAirJumpCurrentCount(int value)
        {
            airJumpCount = value;
        }
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Jump", barColor, labelColor))
            {
                ...
                {
                    var gravity = controller.Get("abilities").Get("gravity");
                    var jumpHeight = parent.Get("jumpHeight");
                ...
                {
                    Fields.EventFoldOutEffect(parent.Get("onJump"), parent.Get("jumpWE"), parent.Get("jumpFoldOut"),
                        "On Jump", FoldOut.boxColorLight);
                    Fields.EventFoldOutEffect(parent.Get("onAirJump"), parent.Get("airJumpWE"),
                        parent.Get("airJumpFoldOut"), "On Air Jumps", FoldOut.boxColorLight);
                }
