...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class PickAndThrow : Ability
    {
        ...
        [SerializeField] public Vector2 holdPosition = new(0, 2f);
        [SerializeField] public List<Vector2> pickUpPath = new();
        [NonSerialized] private readonly List<AIBase> blocks = new();

        [NonSerialized] private AIBase block;
        [NonSerialized] private BoxCollider2D blockCollider;
        [NonSerialized] private Transform blockTransform;
        [NonSerialized] private Vector2 currentPath;
        [NonSerialized] private float pickUpCounter;
        [NonSerialized] private int pickUpIndex;
        [NonSerialized] private BoxCollider2D postBlockCollider;
        [NonSerialized] private bool postContact;
        [NonSerialized] private PickState state;
        [NonSerialized] private float throwCounter;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            if (pause)
                return false;
            CheckPostContact(player);
            if (state == PickState.PickingUp || state == PickState.Holding || state == PickState.Throwing) return true;
            if (velocity.x != 0 || !player.ground) blockTransform = null;
            if (player.world.onWall) blockTransform = player.world.wallTransform;
            if (player.world.onGround && player.inputs.Holding("Down") && player.inputs.Holding(grabButton) &&
                block == null && blockCollider == null) // pick from on top of block
            ...
            if (player.inputs.Holding(grabButton) && block == null && blockCollider == null && blockTransform != null &&
                blockTransform.CompareTag("Block")) return BeginHold(blockTransform, player);
        ...
        private bool BeginHold(Transform transform, AbilityManager player)
        {
            ...
            var blockIsValid = block != null && blockCollider != null;
            if (blockIsValid)
            {
        ...
        private bool CheckPreviousBlocks(AbilityManager player)
        {
            for (var i = 0; i < blocks.Count; i++)
                if (blocks[i] != null && blocks[i].world.boxCollider != null &&
                    player.world.box.BoxTouchingAABB(blocks[i].world.boxCollider))
                {
        ...
        public override void PostCollisionExecute(AbilityManager player, Vector2 velocity)
        {
            switch (state)
            {
                ...
                case PickState.PickingUp:
                    ...
                    if (blockCollider == null || blockCollider.gameObject == null) break;

                    blockCollider.gameObject.layer = 2;
                    var time = pickUpTime / pickUpPath.Count;

                    if (pickUpLerp)
                    {
                        pickUpCounter = Mathf.Clamp(pickUpCounter + Time.deltaTime, 0, time);
                        var newPosition =
                            Compute.LerpNormal(currentPath, pickUpPath[pickUpIndex], pickUpCounter / time);
                        newPosition.x = player.playerDirection > 0
                            ? Mathf.Abs(newPosition.x)
                            : -Mathf.Abs(newPosition.x);
                    ...
                    else
                    {
                        ...
                        currentPath.x = player.playerDirection > 0
                            ? Mathf.Abs(currentPath.x)
                            : -Mathf.Abs(currentPath.x);
                ...
                case PickState.Holding:
                    ...
                    if (block == null || blockCollider == null ||
                        !block.gameObject.activeInHierarchy) // || BlockHitWall(velocity))
                    {
                        Reset(player);
                    }
                    else if (HoldBlock(player, velocity))
                    {
                        if (player.inputs.Pressed(grabButton) & (Time.deltaTime != 0))
                        {
                            ...
                            var jumpTo = block.GetComponent<Throw>();
                            if (jumpTo != null)
                            {
                ...
                case PickState.Throwing:
                    if (Clock.Timer(ref throwCounter, throwTime)) Reset(player);
                    break;
            }
        }
        ...
        private bool HoldBlock(AbilityManager player, Vector2 velocity)
        {
            ...
            var newHoldPosition = holdPosition;
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Pick And Throw", barColor, labelColor))
            {
                ...
                var array = parent.Get("pickUpPath");
