...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class LadderClimb : Ability
    {
        ...
        [SerializeField] private bool autoLatch;
        ...
        [SerializeField] public string ladderTopWE;
        [SerializeField] public UnityEventEffect onEnter;
        [SerializeField] public UnityEventEffect onExit;
        [SerializeField] public UnityEventEffect onLadderTop;

        [NonSerialized] private int autoDirection;
        [NonSerialized] private bool fallingToLadder;
        [NonSerialized] public FenceReverse fenceReverse = new();
        [NonSerialized] public bool hasFlipped;
        [NonSerialized] private bool jumpingFromLadder;

        [NonSerialized] private float jumpPoint;
        [NonSerialized] private Vector2 ladderPosition;
        [NonSerialized] private Ladder ladderRef;
        [NonSerialized] private bool onLadder;
        [NonSerialized] public float playerBottom;
        [NonSerialized] public Vector2 playerCenter;
        [NonSerialized] public float playerTop;
        ...
        {
            if (onLadder) OnExit(player);
            if (ladderRef != null) ladderRef.fenceFlip.Reset();
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (onLadder && ladder == null) Reset(player);

            if (onLadder && player.ground) Reset(player);


            if (fallingToLadder && (player.world.hitInteractable || player.ground)) fallingToLadder = false;

            if (onLadder)
            {
                if (!Ladder.Find(player.world, velocity, ref ladderRef))
                {
                    if (ladder.standOnLadder && jumpingFromLadder && player.inputX == 0 &&
                        !player.world.touchingASurface) return false;
            ...
            if ((autoLatch && velocity.y < 0 && Mathf.Abs(velocity.x) < 0.1f) || fallingToLadder ||
                player.inputs.Holding(up) || (player.inputs.Holding(down) && !player.ground))
                if (Ladder.Find(player.world, velocity, ref ladderRef))
                    return true;
            ...
            bool isRunningAsException = false)
        {
            ...
            if (ExitFromJump(velocity.y)) return;

            var wasOnLadder = onLadder;
            onLadder = true;
            jumpingFromLadder = false;
        ...
        {
            if (!jumpingFromLadder) return false;
            if (playerBottom >= ladder.top) return false;
            if (velocityY > 0) return true;
            if (fallPoint != 0 && playerBottom > jumpPoint + fallPoint) return true;
            return false;
        }

        private void AutomaticClimb(AbilityManager player, bool wasOnLadder, ref Vector2 velocity)
        {
            if (!wasOnLadder || autoDirection == 0) autoDirection = playerTop >= ladder.top ? -1 : 1;

            var autoUp = autoDirection == 1;
            var autoDown = autoDirection == -1;
        ...
        {
            if (ladderRef.fenceFlip.Flip(ladder, this, player, fenceFlip, ref velocity)) return;

            var climbUp = player.inputs.Holding(up);
            var climbDown = player.inputs.Holding(down);

            if (FallOnLadderTop(player, climbDown, ref velocity)) return;
        ...
        {
            if (!ladder.standOnLadder || Time.deltaTime == 0 || climbDown || velocity.y >= 0 ||
                player.jumpButtonActive) return false;
        ...
        {
            velocity.y = 0;
            if (climbUp) velocity.y += climbSpeed;
            if (climbDown && !player.ground) velocity.y -= climbSpeed;
            if (player.inputX != 0 && playerBottom + velocity.y * Time.deltaTime + 0.01f < ladder.top)
                velocity.x = sideSpeed * Mathf.Sign(player.inputX);

            var alignToCenter = ladder.alignToCenter && player.inputX == 0 && velocity.y != 0;
            var belowLadder = playerBottom + velocity.y * Time.deltaTime < ladder.top;

            if (alignToCenter && belowLadder && Time.deltaTime != 0)
            {
                var from = playerCenter.x;
                var to = ladder.CenterX();
                var speed = player.speed * Time.deltaTime * 0.10f;
                var moveTo = Mathf.MoveTowards(from, to, speed);
                velocity.x = (moveTo - from) / Time.deltaTime;
            }

            if (climbUp && playerBottom >= ladder.top) velocity.y = 0;
            player.signals.Set("ladderClimb");
        }

        private void ClimbToLadderTop(AbilityManager player, bool climbDown, ref Vector2 velocity)
        {
            if (!ladder.standOnLadder || Time.deltaTime == 0) return;
            if (climbDown) return;
        ...
        {
            if (!player.jumpButtonActive) return;
            if (ladder.stopSideJump && (player.world.box.bottomLeft.x <= ladder.left + 1f ||
                                        player.world.box.bottomRight.x >= ladder.right - 1f)) jumpingFromLadder = true;
            if (ladder.canJumpUp || (!ladder.stopSideJump && player.inputX != 0 && !player.world.onWall) ||
                !ladder.ContainsY(playerTop)) jumpingFromLadder = true;
        ...
        public override void PostAIExecute(AbilityManager player)
        {
            if (ladderRef != null && ladder != null && Time.deltaTime != 0)
            {
                ...
                var ladderVelocityAdjusted = Time.deltaTime != 0 ? ladderVelocity / Time.deltaTime : Vector2.zero;
        ...
        public void OnExit(AbilityManager player)
        {
            onExit.Invoke(ImpactPacket.impact.Set(exitWE, transform, player.world.boxCollider, player.world.position,
                null, player.world.box.down, player.playerDirection, 0));
        }

        public void OnLadderTop(AbilityManager player)
        {
            onLadderTop.Invoke(ImpactPacket.impact.Set(ladderTopWE, transform, player.world.boxCollider,
                player.world.position, null, player.world.box.down, player.playerDirection, 0));
        }
...
#pragma warning disable 0414
        ...
        [SerializeField] [HideInInspector] private bool ladderTopFoldOut;
            ...
            Color barColor, Color labelColor)
        {
            ...
            {
                var climb = parent.Enum("climb");
                if (climb == 0)
                {
                    FoldOut.Box(4, FoldOut.boxColorLight, 5, -2);
                    {
                    ...
                    {
                        Fields.EventFoldOutEffect(parent.Get("onEnter"), parent.Get("enterWE"),
                            parent.Get("enterFoldOut"), "On Enter", FoldOut.boxColorLight);
                        Fields.EventFoldOutEffect(parent.Get("onExit"), parent.Get("exitWE"), parent.Get("exitFoldOut"),
                            "On Exit", FoldOut.boxColorLight);
                        Fields.EventFoldOutEffect(parent.Get("onLadderTop"), parent.Get("ladderTopWE"),
                            parent.Get("ladderTopFoldOut"), "On Ladder Top", FoldOut.boxColorLight);
                    }
