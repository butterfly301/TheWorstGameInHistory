...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    {
        [NonSerialized] private int characterOrderLayer;
        [NonSerialized] private SpriteRenderer characterRenderer;
        [NonSerialized] private int characterSortingLayer;

        [NonSerialized] private float edgeCounter;
        [NonSerialized] private bool initialized;
        [NonSerialized] private bool isFlippingDown;
        [NonSerialized] private bool isFlippingLeft;
        [NonSerialized] private bool isFlippingRight;
        [NonSerialized] private bool isFlippingUp;
        [NonSerialized] private SpriteRenderer ladderRenderer;
        ...
        {
            if (!ladder.canFlip || Time.deltaTime == 0) return;
            if (player.world.touchingASurface) Reset(player);

            if (ladder.canFlipX) IsFlippingX(player, ladderClimb, ladder.reverseTime, ladder.ladder, ref velocity);
            if (ladder.canFlipY) IsFlippingY(player, ladderClimb, ladder.reverseTime, ladder.ladder, ref velocity);
        }

        private void IsFlippingX(AbilityManager player, LadderClimb ladderClimb, float reverseTime,
            LadderInstance ladder, ref Vector2 velocity)
        {
            var playerCenter = ladderClimb.playerCenter;
            var playerCenterOriginal = playerCenter;
            playerCenter += velocity * Time.deltaTime;

            if (!isFlippingX && ladderClimb.playerBottom + 0.1f >= ladder.top) return;
            ...
            {
                if (isFlippingLeft) HoldPositionLeft(ladder, playerCenterOriginal, ref velocity);

                if (isFlippingRight) HoldPositionRight(ladder, playerCenterOriginal, ref velocity);
        ...
        private void IsFlippingY(AbilityManager player, LadderClimb ladderClimb, float reverseTime,
            LadderInstance ladder, ref Vector2 velocity)
        {
            var playerCenter = ladderClimb.playerCenter;
            var playerCenterOriginal = playerCenter;
            ...
            {
                if (isFlippingUp) HoldPositionUp(ladder, playerCenterOriginal, ref velocity);

                if (isFlippingDown) HoldPositionDown(ladder, playerCenterOriginal, ref velocity);
        ...
        public void SetLayerOrder(AbilityManager player, LadderInstance ladder, bool reversed)
        {
            ...
            if (ladderRenderer == null || characterRenderer == null) return;
