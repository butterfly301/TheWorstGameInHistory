...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class YoshiTongue : Ability
    {
        ...
        [SerializeField] public List<Firearm> weapons = new();
        ...
        [NonSerialized] private AIBase aiTarget;
        [NonSerialized] private bool closedTriggered;
        [NonSerialized] private float counter;

        [NonSerialized] private float length;
        [NonSerialized] private float spriteWidth = 1f;
        [NonSerialized] private State state;
        [NonSerialized] private Collider2D targetCollider;

        [NonSerialized] private SpriteRenderer targetSprite;
        [NonSerialized] private Transform targetTransform;
        [NonSerialized] private CapturedByYoshi yoshiTarget;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (state == State.Start || state == State.Extract || state == State.Retract ||
                state == State.End) return true;
            if (player.inputs.Pressed(button))
            {
                counter = 0;
                state = State.Start;
            ...
            switch (state)
            {
                ...
                case State.Extract:
                {
                    if (stopVelX)
                        velocity.x = 0;
                    player.signals.Set("yoshiExtend");
                    var direction = Vector3.right * player.signals.characterDirection;
                    length = Mathf.MoveTowards(length, tongueLength, Time.deltaTime * speed);

                    var localLength = length / spriteWidth * direction.x * transform.localScale.x;
                    tongue.localScale = new Vector3(localLength, tongue.localScale.y, 1f);

                    var tongueLP = TonguePosition(direction.x);
                    tongueTip.localPosition = tongueLP + direction * length;

                    var hit = Physics2D.Linecast(tongue.position, tongueTip.position, layer);
                    ...
                    if (hit || length >= tongueLength) state = State.Retract;
                }
                    break;
                case State.Retract:
                {
                ...
                case State.End:
                {
                    ...
                    {
                        Reset(player);
                        if (weapons.Count > 0 && yoshiTarget != null && yoshiTarget.weapon >= 0) state = State.Shoot;
                    }
                }
                    break;
            }
        ...
        {
            var t = tongue.localPosition;
            return tongue.transform.localPosition =
                directionX > 0 ? new Vector3(Mathf.Abs(t.x), t.y) : new Vector3(-Mathf.Abs(t.x), t.y);
        }
...
#pragma warning disable 0414
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Yoshi's Tongue", barColor, labelColor))
            {
                ...
                Fields.EventFoldOut(parent.Get("onBegin"), parent.Get("onBeginFoldOut"), "On Begin",
                    color: FoldOut.boxColorLight);
                Fields.EventFoldOut(parent.Get("onEnd"), parent.Get("onEndFoldOut"), "On End",
                    color: FoldOut.boxColorLight);
                Fields.EventFoldOut(parent.Get("onClose"), parent.Get("onCloseFoldOut"), "On Close",
                    color: FoldOut.boxColorLight);
                Fields.EventFoldOut(parent.Get("onFail"), parent.Get("onFailFoldOut"), "On Fail",
                    color: FoldOut.boxColorLight);
                Fields.EventFoldOut(parent.Get("onShoot"), parent.Get("onShootFoldOut"), "On Shoot",
                    color: FoldOut.boxColorLight);
            }
