...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class FindAI : Ability
    {
        ...
        [NonSerialized] private readonly List<AIFSM> aiList = new();
        [NonSerialized] private float delayCounter;
        [NonSerialized] private bool delaySet;

        [NonSerialized] private bool found;
        [NonSerialized] private readonly Collider2D[] results = new Collider2D[200];
        [NonSerialized] private float signalCounter;
        [NonSerialized] private bool signalSet;
            ...
            bool isRunningAsException = false)
        {
            ...
            {
                for (var i = 0; i < aiList.Count; i++) aiList[i].ChangeState(newState);
                delaySet = true;
                aiList.Clear();
                onDelayComplete.Invoke();
            }

            if (delaySet && signalSet) Reset(player);
        }
        ...
        public void Search()
        {
            found = false;
            aiList.Clear();
            var hit = Physics2D.OverlapCircleNonAlloc(transform.position, radius, results);

            for (var i = 0; i < hit; i++)
            {
                var ai = results[i].GetComponent<AIFSM>();
                if (ai != null)
                {
                    found = true;
                    aiList.Add(ai);
            ...
            if (found) onFound.Invoke();
            if (!found) onFailed.Invoke();
        }
            ...
            Color barColor, Color labelColor)
        {
            if (Open(parent, "Find AI", barColor, labelColor))
            {
                FoldOut.Box(4, FoldOut.boxColorLight, 5, -2);
                {
                    ...
                    Labels.FieldText("Time Delay", 3);
                }

                if (FoldOut.FoldOutButton(parent.Get("eventFoldOut")))
                {
                    Fields.EventFoldOut(parent.Get("onFound"), parent.Get("foundFoldOut"), "On Found",
                        color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("onFailed"), parent.Get("failedFoldOut"), "On Failed",
                        color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("onDelayComplete"), parent.Get("delayCompleteFoldOut"),
                        "On Delay Complete", color: FoldOut.boxColorLight);
                    Fields.EventFoldOut(parent.Get("onSignalComplete"), parent.Get("signalCompleteFoldOut"),
                        "On Signal Complete", color: FoldOut.boxColorLight);
                }
            }
