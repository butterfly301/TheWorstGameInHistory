...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class CrouchSlide : Ability
    {
        ...
        [NonSerialized] private bool lowCeiling;
        [NonSerialized] private bool releasedEarly;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (Mathf.Abs(velocity.x) >= threshold && player.ground && !Crouch.IsEdge2D(player.world))
            {
                ...
                if (player.inputs.Pressed(button)) return true;
                if (player.inputs.Holding(button) &&
                    !player.world.wasOnGround) return true; // slide after a jump landing
            }
            ...
            bool isRunningAsException = false)
        {
            if (player.world.boxCollider.size.y != height) // CROUCH
            {
                ...
                onEnter.Invoke(ImpactPacket.impact.Set(enterWE, transform, player.world.boxCollider,
                    player.world.position, null, -player.world.box.right * player.playerDirection,
                    player.playerDirection, 0));
            }

            if (isSliding) Slide(player, ref velocity);
        }

        private void Slide(AbilityManager player, ref Vector2 velocity)
        {
            var oldVelX = velocity.x;
            var targetSpeed = type == SlideType.ConstantVelocity ? direction * player.speed : 0;
            ...
            if (!player.inputs.Holding(button) && lerpTime >= minTime) releasedEarly = true;
        ...
        public bool SafelyStandUp(BoxInfo ray)
        {
            ...
            for (var i = 0; i < ray.rays.y; i++)
            {
                ...
                var hit = Physics2D.Raycast(origin, ray.up, length, WorldManager.collisionMask);
