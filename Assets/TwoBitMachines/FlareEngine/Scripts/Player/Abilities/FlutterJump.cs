...
namespace TwoBitMachines.FlareEngine.ThePlayer
{
    ...
    public class FlutterJump : Ability
    {
        ...
        [NonSerialized] private bool isFluttering;

        [NonSerialized] private Jump jumpAbility;
        [NonSerialized] private bool mustReset;
        ...
        public override bool IsAbilityRequired(AbilityManager player, ref Vector2 velocity)
        {
            ...
            if (player.world.onGround) mustReset = false;
            if (isFluttering)
            {
            ...
            if (!mustReset && !player.world.onGround && player.inputs.Holding(flutterButton))
                if (jumpAbility == null || jumpAbility.minJumpHeight == 0 || velocity.y > jumpAbility.minJumpVel ||
                    velocity.y <= 0)
                {
                    counter = 0;
                    isFluttering = true;
