...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class MeleeAttack : Action
    {
        ...
        [NonSerialized] private readonly List<Collider2D> list = new();

        [NonSerialized] private ContactFilter2D filter;
        [NonSerialized] private bool success;

        public override NodeState RunNodeLogic(Root root)
        {
            if (colliderRef == null) return NodeState.Failure;
            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                ...
                if (enableCollider == MeleeCollider.EnableOnStart) colliderRef.enabled = true;
                if (velocity.y != 0)
                {
            ...
            if (velocity.x != 0) root.velocity.x = velocity.x * Mathf.Sign(root.direction);
            root.signals.Set("meleeCombo");
            root.signals.Set(animationSignal);

            var size = colliderRef.OverlapCollider(filter, list);
            for (var i = 0; i < size; i++)
            {
                if (list[i].transform == transform)
                    continue;
                var direction = colliderRef.transform.position.x < list[i].transform.position.x ? 1f : -1f;
                var newForceDirection = new Vector2(forceDirection.x * direction, forceDirection.y);
        ...
        public void FlipCollider(float direction, Transform transform)
        {
            transform.localPosition =
                Util.FlipXSign(transform.localPosition, direction); // change weapon position x depending on side
...
#pragma warning disable 0414
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(120,
                    "Perform a melee attack. If velocity is non zero, the y velocity will be treated as a jump force. CompleteAttack() must be called once the animation is complete, or the FSM will get stuck on this state. This method is available on the Melee Attack class. Signals: meleeCombo, customSignal" +
