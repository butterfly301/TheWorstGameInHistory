...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class ControlPlayer : Action
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (nodeSetup == NodeSetup.NeedToInitialize) player.BlockInput(true);
            if (Root.deltaTime != 0)
            {
                var destination = target.GetTarget().x;
                if (target.hasNoTargets) return NodeState.Running;
                var start = player.transform.position.x;
                var newP = Mathf.MoveTowards(start, destination, velocity * Root.deltaTime);
                var newVelocity = (newP - start) / Root.deltaTime;
