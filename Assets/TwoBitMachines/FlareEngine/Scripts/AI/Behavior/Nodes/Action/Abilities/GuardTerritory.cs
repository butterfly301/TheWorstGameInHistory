...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class GuardTerritory : Action
    {
        ...
        [NonSerialized] private bool wasFollowing;

        public override NodeState RunNodeLogic(Root root)
        {
            if (target == null || guardPoint == null || territory == null) return NodeState.Failure;
            ...
            var followTarget = target.GetTarget();
            var isFollowingTarget = territory.Contains(followTarget);
            var actualTarget = isFollowingTarget ? followTarget : guardPoint.GetTarget();

            if (target.hasNoTargets) return NodeState.Failure;

            if (!wasFollowing && isFollowingTarget) onEnter.Invoke();
            if (wasFollowing && !isFollowingTarget) onExit.Invoke();
            wasFollowing = isFollowingTarget;

            var newP = Mathf.MoveTowards(root.position.x, actualTarget.x, speed * Root.deltaTime);
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(80,
                    "The AI will follow the target if the target is inside the territory.Else, the AI moves to the guard point. This returns Success when the target is within the stop distance." +
                    "\n \nReturns Running, Success, Failure");

            FoldOut.Box(5, color, 5, -2);
            {
