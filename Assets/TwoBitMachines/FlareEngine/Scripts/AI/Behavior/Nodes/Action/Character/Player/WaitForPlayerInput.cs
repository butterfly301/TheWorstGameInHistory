...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class WaitForPlayerInput : Action
    {
        ...
        [NonSerialized] private WaitForState state;

        public override NodeState RunNodeLogic(Root root)
        {
            if (target == null || territory == null || inputButtonSO == null) return NodeState.Failure;
            if (nodeSetup == NodeSetup.NeedToInitialize) state = WaitForState.Wait;
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(65,
                    "Wait for the player to enter the territory and then for the button to be pressed. If enabled, this will return success on button pressed." +
            ...
            Fields.EventFoldOut(parent.Get("onPlayerEnter"), parent.Get("playerEnterFoldOut"), "On Player Enter",
                color: color, offsetY: -2);
            Fields.EventFoldOut(parent.Get("onPlayerExit"), parent.Get("playerExitFoldOut"), "On Player Exit",
                color: color, offsetY: -2);
            Fields.EventFoldOut(parent.Get("onButtonPressed"), parent.Get("buttonPressedFoldOut"), "On Button Pressed",
                color: color, offsetY: -2);
            return true;
        }
