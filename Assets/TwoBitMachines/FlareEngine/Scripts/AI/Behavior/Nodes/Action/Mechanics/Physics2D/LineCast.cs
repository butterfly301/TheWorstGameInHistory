...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class LineCast : Conditional
    {
        ...
        {
            var hit = Physics2D.Linecast(origin.GetTarget(), target.GetTarget(), layer);
            if (origin.hasNoTargets || target.hasNoTargets) return false;

            #region Debug

#if UNITY_EDITOR
            if (WorldManager.viewDebugger) Debug.DrawLine(origin.GetTarget(), target.GetTarget(), Color.red);
#endif
        ...
        public bool CastAll(Root root)
        {
            ...
            var i = Physics2D.Linecast(origin.GetTarget(), target.GetTarget(), Root.filter2D, Root.rayResults);
            if (origin.hasNoTargets || target.hasNoTargets) return false;
