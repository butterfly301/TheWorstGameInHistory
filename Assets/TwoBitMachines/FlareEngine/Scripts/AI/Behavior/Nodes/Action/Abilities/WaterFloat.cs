...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class WaterFloat : Action
    {
        ...
        [NonSerialized] private bool applyImpactVel;
        [NonSerialized] private float center;
        [NonSerialized] private float centerBottom;
        [NonSerialized] private float impactCounter;
        [NonSerialized] private int particleIndex;
        [NonSerialized] private int passengers;
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (root.movingPlatform.box != null) // moving platform
            {
                ...
                var height = root.movingPlatform.box.size.y * root.transform.localScale.y;
            ...
            if (water.FoundWater(new Vector2(x, centerBottom), false, ref waveTopPoint, ref particleIndex, true))
            {
                if (applyImpactVel)
                {
                    root.velocity.y = -Compute.Lerp(1f, 0, 1f, ref impactCounter, out var complete);
                    if (complete || center < waveTopPoint.y - 1.25f) applyImpactVel = false;
                }
                else
                {
        ...
        {
            var acceleration = buoyancy * (waveTopPoint.y - center) - springVelocity * damping;
            ...
            {
                velocity.y =
                    (waveTopPoint.y - 0.1f - centerBottom) / Root.deltaTime *
                    0.5f; // Clamp player y velocity if being shot out of water too fast by spring/buoyancy
                springVelocity = 0;
            }
        }
