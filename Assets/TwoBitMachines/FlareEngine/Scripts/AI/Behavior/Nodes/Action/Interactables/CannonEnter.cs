...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class CannonEnter : Conditional
    {
        ...
        [SerializeField] private bool needToClear;

        public override NodeState RunNodeLogic(Root root)
        {
            if (player == null || player.world.boxCollider == null) return NodeState.Failure;
            if (nodeSetup == NodeSetup.NeedToInitialize)
                if (cannonCollider == null)
                    cannonCollider = transform.GetComponent<Collider2D>();

            if (needToClear)
            {
                if (!cannonCollider.IsTouching(player.world.boxCollider)) needToClear = false;
                if (needToClear) return NodeState.Running;
            }

            if (cannonCollider == null) return NodeState.Failure;
        ...
        public bool SetPlayerState()
        {
            ...
            needToClear = true;
            cannon.Follow(transform);
            SetToCannonPosition(player.transform);
            var health = player.transform.GetComponent<Health>();
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(65,
                    "Enter a cannon. Place a Collider2D on the AI to detect players on the specified layer. Set it to IsTrigger. " +
