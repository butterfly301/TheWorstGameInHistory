...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class PlayerDeath : Action
    {
        ...
        [NonSerialized] private DeathState state;
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            {
                if (boxCollider2D == null) boxCollider2D = player.gameObject.GetComponent<BoxCollider2D>();
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(120,
                    "Once the player is killed, set the animation signal for the specified time. Transition time will continue to hold the player in the death state in case you want to activate a game transition once the regular time period is over. This will block player input and disable its collider." +
            ...
            Fields.EventFoldOut(parent.Get("onBegin"), parent.Get("beginFoldOut"), "On Begin", color: color,
                offsetY: -2);
            Fields.EventFoldOut(parent.Get("onTransitionBegin"), parent.Get("beginTransitionFoldOut"),
                "On Transition Begin", color: color, offsetY: -2);
            return true;
        }
