    ...
    public class Clamp : Action
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (clampX)
            {
                ...
                {
                    transform.position = new Vector3(left, p.y, p.z);
                    root.velocity.x = 0;
                }

                if (root.position.x >= right && root.velocity.x > 0)
                {
                    transform.position = new Vector3(right, p.y, p.z);
                    root.velocity.x = 0;
                }
            }
            ...
            if (clampY)
            {
                ...
                {
                    transform.position = new Vector3(p.x, down, p.z);
                    root.velocity.y = 0;
                }

                if (root.position.y >= up && root.velocity.y > 0)
                {
                    transform.position = new Vector3(p.x, up, p.z);
                    root.velocity.y = 0;
                }
            }
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(75,
                    "Clamp the AI's position. The limits will be determined by the distance from the player at scene start. Or the limits can be placed manually." +
                    "\n \nReturns Running");
