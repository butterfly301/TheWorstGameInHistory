...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class RayCast : Conditional
    {
        ...
        {
            var hit = Physics2D.Raycast(origin.GetTarget(), Direction(root.direction), length, layer);
        ...
        public bool CastAll(Root root)
        {
            ...
            var i = Physics2D.Raycast(origin.GetTarget(), Direction(root.direction), Root.filter2D, Root.rayResults,
                length);
            if (origin.hasNoTargets) return false;
