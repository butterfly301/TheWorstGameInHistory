...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    {
        [SerializeField] public List<Vector3> position = new();
        ...
        [NonSerialized] private float directionX;
        [NonSerialized] private float duration;
        [NonSerialized] private int index;
        [NonSerialized] private Vector2 playerPosition;
        ...
        private void MovePlayer()
        {
            ...
            var newPosition = Vector2.Lerp(playerPosition, targetPosition, Mathf.Clamp01(counter / duration));
            var velocity = newPosition - (Vector2)player.transform.position;
            velocity = Time.deltaTime == 0 ? Vector2.zero : velocity / Time.deltaTime;
            player.world.box.Update();
            player.world.Move(ref velocity, ref player.abilities.velocity.y, false, Time.deltaTime,
                ref player.abilities.onSurface);
        ...
        {
            var velX = velocity.x != 0;
            var velXLeft = velocity.x < 0;
            var velXRight = velocity.x > 0;
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(105,
                    "Move the player's transform along a set of points. These points are local to the Player. The Z value is the time duration. This will block player input and set a player animation signal." +
