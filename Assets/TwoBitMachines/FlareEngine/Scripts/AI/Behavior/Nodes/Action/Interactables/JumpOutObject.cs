...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class JumpOutObject : Conditional
    {
        ...
        {
            if (player == null || inputButtonSO == null) return NodeState.Failure;
            ...
            if (state != State.Waiting) root.signals.Set(signal);
            if (state == State.JumpOverTop)
            {
                ...
                oldPosition =
                    player.transform
                        .position; //     must remember old position because player controller will also apply gravity
