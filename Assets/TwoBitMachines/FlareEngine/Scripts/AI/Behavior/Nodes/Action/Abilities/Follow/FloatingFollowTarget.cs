    ...
    public class FloatingFollowTarget : Action
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                ...
                character = null;
                if (transform != null) character = transform.GetComponent<Character>();
            }

            if (Root.deltaTime != 0)
            {
                var followPosition = target.GetTarget();
                var followOffset = offset;
                if (target.hasNoTargets) return NodeState.Running;

                if (character != null)
                    followOffset.x = character.signals.characterDirection > 0
                        ? -Mathf.Abs(followOffset.x)
                        : Mathf.Abs(followOffset.x);
                followPosition += followOffset;
                followPosition.y += Compute.SineWave(0, wobbleSpeed, wobbleHeight);

                var newPosition = Vector2.Lerp(root.position, followPosition, speed * Root.deltaTime);
