...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class FollowPlatformWalls : Action
    {
        ...
        [NonSerialized] private Vector2 oldNormal = Vector2.zero;

        public override NodeState RunNodeLogic(Root root)
        {
            if (root.world.onGround && root.world.onWall)
            {
                var corner = root.world.box.topCenter;
                var hit = Physics2D.Raycast(corner, root.world.box.right * root.direction, root.world.box.size.x,
                    WorldManager.collisionMask);
                Debug.DrawRay(corner, root.world.box.right * root.direction * root.world.box.size.x, Color.red);
                if (ChangeDirection(hit, root, root.direction)) return NodeState.Success;
            }

            if (root.world.onGround && root.world.missedAVertical)
            {
                var corner = root.world.box.BottomCorner(root.direction) - root.world.box.skinY * 2f;
                var hit = Physics2D.Raycast(corner, -root.world.box.right * root.direction, root.world.box.size.x,
                    WorldManager.collisionMask);
                Debug.DrawRay(corner, -root.world.box.right * root.direction * root.world.box.size.x, Color.red);
                if (ChangeDirection(hit, root, -root.direction)) return NodeState.Success;
