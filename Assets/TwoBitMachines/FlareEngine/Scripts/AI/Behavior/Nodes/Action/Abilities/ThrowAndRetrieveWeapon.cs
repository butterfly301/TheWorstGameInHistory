...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class ThrowAndRetrieveWeapon : Action
    {
        ...
        [NonSerialized] private float decelerate;
        [NonSerialized] private float holdWeaponCounter;
        [NonSerialized] private bool isRetrieving;

        [NonSerialized] private RetrieveWeapon state;
        [NonSerialized] private float throwCounter;
        [NonSerialized] private float throwDirection;
        [NonSerialized] private float timeOut;
        [NonSerialized] private float waitCounter;
        [NonSerialized] private float weaponAngleOffset;
        [NonSerialized] private bool weaponBouncing;
        [NonSerialized] private bool weaponThrown;
        ...
        {
            if (weapon == null || firePoint == null) return NodeState.Failure;
            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                Reset();
                throwDirection = root.direction;
                var p = firePoint.localPosition;
            ...
            if (state == RetrieveWeapon.HoldWeapon)
            {
                if (isRetrieving)
                {
                    ...
                    var time = decelerate / 0.75f;
                    root.velocity.x = Mathf.Lerp(retrieveSpeed * throwDirection, 0, time);
                }

                root.signals.Set(holdWeaponSignal);
                if (TwoBitMachines.Clock.Timer(ref holdWeaponCounter, holdWeaponTime)) return NodeState.Success;
            }
        ...
        private void ThrowWeaponNow()
        {
            ...
            var throwDir = throwingDirection;
            var angle = Mathf.Atan2(throwDir.y, throwDir.x) * Mathf.Rad2Deg;
        ...
        {
            var caught = false;
            Debug.DrawRay(root.world.box.center, Vector2.right * throwingDirection * root.world.box.sizeX * 3.5f,
                Color.red);
            if (weapon.IsTouching(root.world.boxCollider) || root.world.onWall || Time.time > timeOut + 15f)
                caught = true;
            if (weaponType == WeaponWallType.StickToWall && Physics2D.Raycast(root.world.box.center,
                    Vector2.right * throwingDirection, root.world.box.sizeX * SlowDistance() * 0.5f,
                    WorldManager.collisionMask)) caught = true;
            if (throwDirection > 0 && weapon.transform.position.x < firePoint.position.x - 0.1f) caught = true;
            if (throwDirection < 0 && weapon.transform.position.x > firePoint.position.x + 0.1f) caught = true;
            if (caught)
            {
                weapon.gameObject.SetActive(false);
        ...
        {
            var stopTime = 0.75f;
            var decelerate = retrieveSpeed / stopTime;
...
#pragma warning disable 0414
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            var type = parent.Enum("weaponType");
            FoldOut.Box(4, color, offsetY: -2);
            {
                parent.Field("Weapon Collider2D", "weapon");
                parent.Field("Weapon Type", "weaponType", type == 0);
                parent.FieldDouble("Weapon Type", "weaponType", "stickOffset", type == 1);
                if (type == 1)
