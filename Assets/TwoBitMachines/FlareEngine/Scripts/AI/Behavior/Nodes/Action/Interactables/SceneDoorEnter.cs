    ...
    public class SceneDoorEnter : Conditional
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                isActive = false;
                hasJumped = false;
                if (player == null) player = Player.mainPlayer;
            }

            if (player == null || WorldManager.get == null) return NodeState.Failure;

            if (!isActive && player != null)
            {
                var currentIndex = WorldManager.get.save.sceneDoor;
                playerDirection = WorldManager.get.save.sceneDoorPlayerDirection;
                if (Mathf.Abs(currentIndex) != doorIndex) return NodeState.Failure;
            ...
            if (isActive && Root.deltaTime != 0)
            {
                ...
                {
                    var destination = transform.position.x + targetX * direction;
                    var start = player.transform.position.x;
                    var newPosition = Mathf.MoveTowards(start, destination, velocity * Root.deltaTime);
                    var newVelocity = (newPosition - start) / Root.deltaTime;
                ...
                else
                {
                    ...
                    if (exitType == DoorEnterExitType.EscapeRadius &&
                        (startPosition - currentPosition).magnitude >= escapeRadius)
                    {
                        player.BlockInput(false);
                        return NodeState.Success;
                    }
...
#pragma warning disable 0414
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(110,
                    "After entering a scene, the player will be placed at the door's position, then move towards target x with the specified velocity if the door has the correct door index. No door index should have a value of zero. If player is empty, the system will use the first player it finds." +
                    "\n \nReturns Running, Failure, Success");

            var height = type == DoorEnterType.Jumping ? 1 : 0;
            ...
            {
                parent.Field("Complete On", "exitType", exitType == DoorEnterExitType.HitGround);
                parent.FieldDouble("Complete On", "exitType", "escapeRadius",
                    exitType == DoorEnterExitType.EscapeRadius);
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            {
                Draw.GLStart();
                Draw.GLCircle(transform.position, 0.25f, Color.red);
                Draw.GLCircle(transform.position + Vector3.right * targetX, 0.25f, Color.green);
                Draw.GLCircle(transform.position - Vector3.right * targetX, 0.25f, Color.green);
                Draw.GLEnd();
            }
        }
