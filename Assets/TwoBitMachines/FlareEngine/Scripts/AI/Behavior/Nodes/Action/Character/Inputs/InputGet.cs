...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class InputGet : Conditional
    {
        ...
        {
            if (type == InputGetType.Key) return Input.GetKeyDown(key) ? NodeState.Success : NodeState.Failure;
...
#pragma warning disable 0414
        ...
        {
            if (parent.Bool("showInfo")) Labels.InfoBoxTop(35, "Get Input KeyDown or MouseDown.");

            var key = parent.Enum("type");
            FoldOut.Box(2, color, offsetY: -2);
            parent.Field("Type", "type");
            parent.Field("KeyCode", "key", key == 0);
            parent.Field("Mouse", "mouse", key == 1);
