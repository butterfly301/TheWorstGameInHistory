...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class LetsWiggle : Action
    {
        ...
        {
            if (wiggle == null) return NodeState.Failure;
            if (nodeSetup == NodeSetup.NeedToInitialize) wiggle.Wiggle();
            var position = transform.localPosition;
            var running = wiggle.Run();
            if (root.type == CharacterType.MovingPlatform)
            {
                Vector2 velocity = transform.localPosition - position;
