...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class ShootProjectileAtTarget : Action
    {
        ...
        {
            if (projectile == null || target == null || firePoint == null) return NodeState.Failure;
            ...
            {
                var rotation = GetDirectionToTarget(target.GetTarget());
        ...
        {
            var direction = target.x < transform.position.x ? -1f : 1f;
            var targetNormal = (target - (Vector2)transform.position).normalized;
            var angle = Compute.AngleDirection(transform.right * direction, targetNormal);
            var rotateAngle = direction < 0 ? -angle : angle;
            if (mirrorFirePoint) Compute.FlipLocalPositionX(firePoint, direction);
...
#pragma warning disable 0414
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            var rotate = parent.Enum("rotateTo");
            FoldOut.Box(5, color, offsetY: -2);
            {
                AIBase.SetRef(ai.data, parent.Get("target"), 0);
                parent.Field("Use AI Rotation", "rotateTo", rotate == 0);
                parent.FieldDouble("Use AI Rotation", "rotateTo", "offset", rotate == 1);
