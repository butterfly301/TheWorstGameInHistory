...
{
    [AddComponentMenu("")]
    public class OverlapCircle : Conditional
    {
        ...
        {
            var collider2D = Physics2D.OverlapCircle(origin.GetTarget(), radius, layer);
        ...
        public bool CastAll(Root root)
        {
            ...
            var i = Physics2D.OverlapCircle(origin.GetTarget(), radius, Root.filter2D, Root.colliderResults);
            if (origin.hasNoTargets) return false;
            return i > 0;
        }
