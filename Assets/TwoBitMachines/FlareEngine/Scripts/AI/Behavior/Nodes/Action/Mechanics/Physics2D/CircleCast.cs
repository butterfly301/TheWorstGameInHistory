...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class CircleCast : Conditional
    {
        ...
        {
            var hit = Physics2D.CircleCast(origin.GetTarget(), radius, Direction(root.direction), length, layer);

            if (origin.hasNoTargets) return false;
        ...
        public bool CastAll(Root root)
        {
            ...
            var i = Physics2D.CircleCast(origin.GetTarget(), radius, Direction(root.direction), Root.filter2D,
                Root.rayResults, length);
            if (origin.hasNoTargets) return false;
...
#pragma warning disable 0414
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            var type = parent.Enum("directionType");
