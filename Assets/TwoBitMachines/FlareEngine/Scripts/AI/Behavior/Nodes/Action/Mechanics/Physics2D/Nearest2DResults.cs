...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class Nearest2DResults : Action
    {
        ...
        {
            if (setTarget == null) return NodeState.Failure;
        ...
        private bool FindNearest(Root root)
        {
            ...
            var index = -1;
            for (var i = 0; i < Root.colliderResults.Count; i++)
            {
                if (Root.colliderResults[i] == null)
                    continue;
                var sqrMagnitude = (root.position - (Vector2)Root.colliderResults[i].transform.position).sqrMagnitude;
        ...
        private void SetValue(Collider2D collider2D)
        {
            ...
            else if (setTargetAs == SetResultAs.Vector3) setTarget.Set(collider2D.transform.position);
        }
