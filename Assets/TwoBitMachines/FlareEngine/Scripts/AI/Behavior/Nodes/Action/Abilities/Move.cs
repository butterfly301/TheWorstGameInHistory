...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class Move : Action
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (velocity.x != 0) root.velocity.x = flipVelX ? Mathf.Abs(velocity.x) * root.direction : velocity.x;
            if (velocity.y != 0) root.velocity.y = velocity.y;
            if (hasTimer && TwoBitMachines.Clock.Timer(ref counter, time)) return NodeState.Success;
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(65,
                    "This will move the AI with the specified velocity. This can have a time limit. If FlipVelX true, velocity x will flip to AI direction." +
