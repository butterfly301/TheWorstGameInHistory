...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class FollowGraphPoints : Action
    {
        ...
        [NonSerialized] private Vector2 currentTarget;

        [NonSerialized] private bool isActive;

        private Vector3 targetP;
        [NonSerialized] private TargetPoints targetPoints;
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            {
                isActive = true;
                currentTarget = targetPoints.GetNearestTarget(transform.position);
            }

            if (left != null && left.Pressed())
                FindTarget(root.position, lookLeft: true);
            if (right != null && right.Pressed())
                FindTarget(root.position, true);
            ...
            {
                var newPosition = Vector2.MoveTowards(root.position, currentTarget, Root.deltaTime * speed);
        ...
        private void FindTarget(Vector2 position, bool lookRight = false, bool lookLeft = false, bool lookUp = false,
            bool lookDown = false)
        {
            var point = targetPoints.point;
