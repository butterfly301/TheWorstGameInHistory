...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class RotateLookAt : Action
    {
        ...
        [SerializeField] public float lowerLimit;
        [SerializeField] public float upperLimit = 360f;
        [SerializeField] public float previousAngle;
        [SerializeField] public bool range;

        private void Start()
        {
            if (lookAt == null)
                return;
            var angle = Mathf.Clamp(Mathf.Repeat(Angle(), 360), lowerLimit, upperLimit);
        ...
        {
            if (lookAt == null) return NodeState.Failure;
            ...
            if (Root.deltaTime != 0)
            {
                ...
                {
                    var angle = Mathf.Repeat(Angle(), 360f);
                    if (Mathf.Abs(angle - previousAngle) > 180 &&
                        (angle > upperLimit ||
                         angle < lowerLimit)) //angle changed from 360 to 0 or vice versa and escaped limits
                    {
                    ...
                    var speed = Mathf.Abs(angle - previousAngle) > 45 ? 8f : 25f;
                    angle = Mathf.Clamp(angle, lowerLimit, upperLimit);
                    var lerpedAngle = Mathf.Lerp(previousAngle, angle, Root.deltaTime * speed);
        ...
        private float Angle()
        {
            var dir = (Vector3)lookAt.GetTarget() - transform.position;
