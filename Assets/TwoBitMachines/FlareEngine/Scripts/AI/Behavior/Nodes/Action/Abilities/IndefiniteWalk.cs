...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class IndefiniteWalk : Action
    {
        ...
        [NonSerialized] private bool hitAirGap;

        public override NodeState RunNodeLogic(Root root)
        {
            if (hitAirGap && root.onGround && !root.world.missedAVertical) hitAirGap = false;

            var oldSpeed = speed;

            if (root.world.leftWall) speed = Mathf.Abs(speed);

            if (root.world.rightWall) speed = -Mathf.Abs(speed);
            ...
            if (oldSpeed != speed) return NodeState.Success;
