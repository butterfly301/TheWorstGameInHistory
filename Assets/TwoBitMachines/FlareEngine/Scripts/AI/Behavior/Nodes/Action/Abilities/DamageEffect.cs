    ...
    public class DamageEffect : Action
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                ...
                if (damageRate != 0 && health == null) health = gameObject.GetComponent<Health>();
            }
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(105,
                    "Slow down the enemy, apply damage, and play the specified animation signal for the duration amount. Enable Start On Enter to activate this on state change, or call ActivateDamageEffect and use a gameObject's Tag." +
