...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class SceneDoorExit : Action
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (found)
            {
                if (playAnimation)
                {
                    ...
                    if (TwoBitMachines.Clock.TimerExpired(ref counter, signalTime)) LoadScene(root);
            ...
            if (player == null || manageScenes == null || WorldManager.get == null) return NodeState.Failure;
            if (!InsideArea(player.transform.position + Vector3.up * 0.1f)) return NodeState.Failure;
            if (onButtonPress && inputButtonSO != null && !inputButtonSO.Pressed()) return NodeState.Failure;
            found = true;
            return NodeState.Running;
        }
        ...
        {
            var direction = directional == SceneDoorDirection.Fixed
                ? 1
                : (int)Mathf.Sign(player.abilities.playerDirection);
        ...
        {
            Vector2 position = transform.position - Vector3.right * area.z;
            if (target.x > position.x + area.x * 0.5f || target.x < position.x - area.x * 0.5f) return false;
            if (target.y > position.y + area.y || target.y < position.y) return false;
            return true;
        }
...
#pragma warning disable 0414
        public static string[] sceneNames = { "Empty" };
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (onEnable)
            {
                ...
                if (sceneCount > 0)
                {
                    ...
                    for (var i = 0; i < sceneCount; i++) sceneNames[i] = Util.GetSceneName(i);
                }
                else
                {
            ...
            FoldOut.Box(7, color, offsetY: -2);
            {
                ...
                Labels.FieldDoubleText("Signal", "Time", rightSpacing: 18, show1: parent.String("signalName") == "",
                    show2: parent.Float("signalTime") == 0);
