...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class PathfinderRT : Action
    {
        ...
        [NonSerialized] private bool calculatePath;
        [NonSerialized] private bool mapInitializing;

        [NonSerialized] private TargetPathfindingRT path;
        [NonSerialized] private Vector2 previousPosition;
        [NonSerialized] private int targetGridY;

        private void Awake()
        {
            if (pathfinding != null) path = pathfinding as TargetPathfindingRT;
        }

        public override NodeState RunNodeLogic(Root root)
        {
            if (path == null || target == null) return NodeState.Failure;
            ...
            if (useFindDistance) // exit out if found to prevent the path from recalculating
            {
                if (findType == PathTargetFind.TargetWithinDistance && Found(root.position, target.GetTarget()) &&
                    path.CanExit() && !target.hasNoTargets) return NodeState.Success;

                if (findType == PathTargetFind.ReachedPathEnd && path.AtFinalTarget() && path.CanExit())
                    return NodeState.Success;
            }

            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                var cellSize = path.map != null ? path.map.cellSize : 1f;
            ...
            if (path.pathSafeToChagne)
            {
                ...
                if (target.hasNoTargets) return NodeState.Running;
                if (calculatePath || TargetPlaneChanged(targetPosition) ||
                    (previousPosition - targetPosition).sqrMagnitude > resetDistance * resetDistance)
                {
        ...
        {
            if (findDistance.x != 0 && (target.x > position.x + findDistance.x * 0.52f ||
                                        target.x < position.x - findDistance.x * 0.52f))
                return false;
            if (findDistance.y != 0 && (target.y > position.y + findDistance.y * 0.52f ||
                                        target.y < position.y - findDistance.y * 0.52f))
                return false;
            return true;
        }
        ...
        {
            var targetNode = path.map.PositionToNode(position + path.map.cellYOffset);
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(125,
                    "Follow a path to a target using the pathfinding algorithm. This algorithm takes gravity into account, making it ideal for platformers. When the target has changed its position by the reset distance amount, the path will recalculate. If Success On is enabled, success is returned when the specified setting is met." +
                    "\n \nReturns Running, Success, Failure");

            var index = (int)findType;
            var height = index == 1 ? 1 : 0;
            ...
            if (parent.Bool("useFindDistance"))
                parent.Field("Find Distance", "findDistance", index == 1);
