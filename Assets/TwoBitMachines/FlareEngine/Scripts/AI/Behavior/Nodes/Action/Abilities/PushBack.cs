...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class PushBack : Action
    {
        ...
        [NonSerialized] private float appliedForceXRef;
        [NonSerialized] private bool beginPushBack;
        [NonSerialized] private int characterDirection;
        [NonSerialized] private float flashCounter;
        [NonSerialized] private float flashTime;
        [NonSerialized] private bool flashToggle;
        [NonSerialized] private Vector2 forceDirection;
        [NonSerialized] private float holdCounter;
        [NonSerialized] private Color originColor;

        [NonSerialized] private Material originMaterial;
        [NonSerialized] private AIFSM parent;

        [NonSerialized] private bool pushBack;
        [NonSerialized] private int pushBackCalls;
        [NonSerialized] private float pushCounter;
        [NonSerialized] private int rootIndex;
        [NonSerialized] private Root rootRef;

        public void Start()
        {
            if (material != null) material = new Material(material);
            if (spriteRenderer != null) originMaterial = spriteRenderer.material;
        ...
        {
            if (packet.damageValue >= 0) return; // Only apply push back on negative values
            if (selectInterrupt && !rootRef.selectInterrupt) return;
            if (rootRef.aiState != null && rootRef.aiState.cantInterrupt) return;
            ...
            var totalTime = pushTime + holdTime;
            flashTime = flash <= 1 ? totalTime / 2f : totalTime / (flash * 2f);
        ...
        {
            rootIndex = root
                .interruptIndex; // weakness: this has the potential of failing on the first frame since it's not set, but that will be extremely rare
            if (!pushBack)
            ...
            if (beginPushBack)
            {
                if (pushBackCalls <= 1f)
                    characterDirection =
                        root.signals
                            .characterDirection; // pushbackcalls prevents resetting direction if pushback is already active
                beginPushBack = false;
                if (spriteEngine != null)
            ...
            {
                root.velocity.y = forceDirection.y > 0
                    ? jumpForce * forceDirection.y
                    : jumpForce * (forceDirection.y * 3f);
            ...
            if (useSignal)
            {
                var state = WorldFloatLogic.Compare(logic, variable.GetValue(), compareFloat);
...
#pragma warning disable 0414
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            var useSignal = parent.Bool("useSignal");
            FoldOut.Box(3, color, offsetY: -2);
            {
