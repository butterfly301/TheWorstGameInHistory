...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class RandomAnimation : Action
    {
        ...
        [NonSerialized] private int loopCounter;
        [NonSerialized] private SpriteEngine spriteEngine;
        ...
        {
            if (spriteEngine == null) return NodeState.Failure;
            ...
            if (!found)
            {
                if (spriteEngine.currentAnimation == targetAnimation)
                {
                    if (TwoBitMachines.Clock.Timer(ref counter, time))
                    {
                        ...
                        root.signals.Set(signalAnimation);
                    }
                }
                else
                {
...
#pragma warning disable 0414
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(70,
                    "When the specified animation has been playing for a set amount of time, a random animation will be triggered to play." +
