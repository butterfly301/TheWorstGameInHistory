...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class LaunchCharacter : Action
    {
        ...
        [NonSerialized] public Root root;
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            else if (target != null && (target.GetTarget() - root.position).sqrMagnitude <= range * range &&
                     !target.hasNoTargets)
            {
                root.movingPlatform.canBoostLaunch = true;
                root.movingPlatform.launchVelocity = launchVelocity;
        ...
        {
            if (root != null) root.movingPlatform.canBoostLaunch = false;
        }
...
#pragma warning disable 0414
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            var withinRange = parent.Bool("withinRange");
            FoldOut.Box(2 + (withinRange ? 1 : 0), color, offsetY: -2);
            {
                ...
                if (withinRange) AIBase.SetRef(ai.data, parent.Get("target"), 0);
            }
