...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class Jump : Action
    {
        ...
        [SerializeField] public Vector2 jump = new(5f, 5f);

        [NonSerialized] private bool isJumping;
        [NonSerialized] private float refVelX;

        public override NodeState RunNodeLogic(Root root)
        {
            if (nodeSetup == NodeSetup.NeedToInitialize) isJumping = false;

            if (!isJumping)
            {
                if (mustBeOnGround && !root.world.onGround) return NodeState.Running;

                isJumping = true;

                if (fullJump)
                {
                    var direction = root.world.rightWall ? -1 : root.world.leftWall ? 1f : root.direction;
            ...
            {
                if (fullJump) root.velocity.x = refVelX;
                if ((isHalfJump && root.velocity.y < 0) || root.world.onGround) return NodeState.Success;
            }
        ...
        {
            var velocity = Compute.ArchObject(root.position, jumpTarget, height, root.gravity.gravity);
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(80,
                    "Jump. If HalfJumpOnly is enabled, this returns success once the AI begins to fall. If MustBeOnGround is enabled, the AI must be on the ground to jump." +
                    "\n \nReturns Running, Success");

            var fullJump = parent.Bool("fullJump");
