...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class VariableLogic : Conditional
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (variableType == VariableType.Float)
            {
                var compareValue = compareTo == CompareTo.Value ? compareFloat :
                    compareVariable != null ? compareVariable.GetValue() : 0;
                return CompareFloat(variable.GetValue(), compareValue);
            }

            if (variableType == VariableType.String)
            {
                var compareValue = compareTo == CompareTo.Value ? compareString :
                    compareVariable != null ? (compareVariable as StringVariable).value : "";
                return CompareString((variable as StringVariable).value, compareValue);
            }

            if (variableType == VariableType.Bool) return CompareBool(variable.GetValue() == 1);
            if (variableType == VariableType.Vector2)
            {
                var compareValue = compareTo == CompareTo.Value ? compareVector2 :
                    compareVariable != null ? compareVariable.GetTarget() : Vector2.zero;
        ...
        {
            if (stringLogic == StringLogicType.Equal) return a == b ? NodeState.Success : NodeState.Failure;
            return NodeState.Failure;
        }

        public NodeState CompareBool(bool a)
        {
            if (boolLogic == BoolLogicType.IsTrue) return a ? NodeState.Success : NodeState.Failure;
            if (boolLogic == BoolLogicType.IsFalse) return !a ? NodeState.Success : NodeState.Failure;
            return NodeState.Failure;
        }
        ...
        public NodeState CompareVector2(Vector2 a, Vector2 b)
        {
            ...
            if (vector2Logic == Vector2LogicType.SameVector) return a == b ? NodeState.Success : NodeState.Failure;
            return NodeState.Failure;
        }
...
#pragma warning disable 0414
        ...
        {
            if (parent.Bool("showInfo")) Labels.InfoBoxTop(35, "Compare a float variable to a float value.");

            var variable = (VariableType)parent.Enum("variableType");
            var type = (CompareTo)parent.Enum("compareTo");
            ...
            {
                var logic = parent.Enum("stringLogic");
            ...
            {
                var logic = parent.Enum("boolLogic");
            ...
            {
                var logic = parent.Enum("vector2Logic");
