...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class Grid : Action
    {
        ...
        [SerializeField] public UnityEvent onGridSet = new();
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            if (nodeSetup == NodeSetup.NeedToInitialize)
            {
                ...
                saved = Storage.Load(saved, WorldManager.saveFolder, saveKey);
                if (setGrid) SetGrid();
            }
        ...
        {
            if (!saved.list.Contains(ID)) saved.list.Add(ID);
        }
        ...
        private void Navigate()
        {
            ...
            if (currentItem != previousCurrentItem && oldGridItem != null) oldGridItem.OnOutOfFocus();

            var gridItem = GetGridItem();
            ...
            if (gridItem != null) highlight.SetActive(gridItem.gridElement);
        }
        ...
        private void Move()
        {
            if (right != null && right.Pressed())
            {
                ...
                if (realIndex >= transform.childCount) index.x = 0;
                if (realIndex >= 0 && realIndex < transform.childCount) // skip deactivated gameobjects.
                    for (var i = realIndex; i < transform.childCount; i++)
                    {
                        var child = transform.GetChild(i);
            ...
            if (left != null && left.Pressed())
            {
                ...
                if (realIndex >= transform.childCount) index.x = transform.childCount - 1 - index.y * rowLength;
                if (realIndex >= 0 && realIndex < transform.childCount)
                    for (var i = realIndex; i >= 0; i--)
                    {
                        var child = transform.GetChild(i);
                        if (child != null && !child.gameObject.activeInHierarchy)
                        {
            ...
            if (down != null && down.Pressed())
            {
                ...
                if (realIndex >= transform.childCount) index.y = 0;
                if (realIndex >= 0 && realIndex < transform.childCount)
                    for (var i = realIndex; i < transform.childCount; i++)
                    {
                        var child = transform.GetChild(i);
            ...
            if (up != null && up.Pressed())
            {
                ...
                if (realIndex >= transform.childCount) index.y = rows - 2;
                if (realIndex >= 0 && realIndex < transform.childCount)
                    for (var i = realIndex; i >= 0; i--)
                    {
                        var child = transform.GetChild(i);
        ...
        {
            if (realIndex >= 0 && realIndex < transform.childCount) currentItem = transform.GetChild(realIndex);
        ...
        {
            var gridItem = currentItem.GetComponent<GridItem>();
            if (gridItem == null)
            ...
            {
                if (gridItem != null) gridItem.OnFocus();
        ...
        public void SetGrid()
        {
            ...
            onGridSet.Invoke();
            gridSet = true;
            var start = (Vector2)transform.position + offset;
            var advanced = Vector2.zero;
            var lengthIndex = 0;
            ...
            for (var i = 0; i < transform.childCount; i++)
            {
                ...
                var gridItem = child.GetComponent<GridItem>();
                if (gridItem != null)
                {
                    if (gridItem.gridElement) child.position = start + advanced;
        ...
        private void TurnOff()
        {
            ...
            for (var i = 0; i < transform.childCount; i++)
            {
                ...
                if (child == null) continue;
                var gridItem = child.GetComponent<GridItem>();
                if (gridItem != null) gridItem.ResetAll();
            }
        }
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(55,
                    "Create a 2D grid from all the child objects. Only active objects will be placed in the grid." +
                    "\n \nReturns Running");
