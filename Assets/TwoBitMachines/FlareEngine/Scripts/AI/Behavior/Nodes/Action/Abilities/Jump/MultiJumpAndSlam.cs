...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class MultiJumpAndSlam : Action
    {
        ...
        [SerializeField] public List<MultiJump> jump = new();
        ...
        [NonSerialized] private float airCounter;
        [NonSerialized] private bool airHold;
        [NonSerialized] private bool finalJump;
        [NonSerialized] private float groundCounter;
        [NonSerialized] private bool groundHold;
        [NonSerialized] private bool hasJumped;
        [NonSerialized] private float jumpDirection;
        [NonSerialized] private int jumpIndex;

        [NonSerialized] private Vector2 slamDirection;
        [NonSerialized] private float velX;
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            if (canSlam && finalJump)
            {
                if (airHold)
                {
                    ...
                    if (TwoBitMachines.Clock.Timer(ref airCounter, peakTime)) airHold = false;
                }

                if (groundHold)
                {
                    root.signals.Set(slamSignalHold);
                    if (TwoBitMachines.Clock.Timer(ref groundCounter, slamHoldTime)) return NodeState.Success;
                }

                if (!groundHold && !airHold)
                {
                    root.signals.Set(slamSignalAir);
                    if (slamTowardsTarget) root.velocity = slamDirection * Mathf.Abs(slamSpeed);
                    {
                    ...
                    if (root.world.onGround)
                    {
                        ...
                        if (slamHoldTime <= 0) return NodeState.Success;
                    }
                }
            }
            else
            {
                ...
                if (!hasJumped)
                {
                    ...
                    if (jumpIndex >= jump.Count) return NodeState.Success;
                }
            }
        ...
        public Vector2 SlamDirection(Vector2 direction)
        {
            ...
            var clampedAngle = angleInDegrees < 0 ? angleInDegrees + 360f : angleInDegrees;
        ...
        {
            var hit = Compute.OverlapCircle(root.position, slamRadius, slamLayer);
            Health.HitContactResults(transform, Compute.contactResults, hit, -slamDamage, slamForce,
                root.position); // blast radius should be in all directions, derive direction from position
        }
...
#pragma warning disable 0414
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            for (var i = 0; i < array.arraySize; i++)
            {
                ...
                FoldOut.Box(2, color);
                {
                    element.FieldAndDoubleButton("Jump", "height", "xsAdd", "xsMinus", out var Add, out var Minus);
                    Labels.FieldText("Height", Layout.contentWidth * 0.28f);
                    if (type == 0)
                    {
...
#pragma warning restore 0414
    ...
    public class MultiJump
    {
        ...
        public void Jump(Root root)
        {
            ...
            var velocity =
                Compute.ArchObject(root.position, root.position, height,
        ...
        {
            var distanceX = target.x - root.position.x;
            distanceX = Mathf.Abs(distanceX) < 1f ? Mathf.Sign(distanceX) : distanceX;
            distanceX = Mathf.Clamp(distanceX, -maxDistance, maxDistance);
            var jumpTo = Vector2.right * distanceX;
            var velocity = Compute.ArchObject(root.position, root.position + jumpTo, height, root.gravity.gravity);
            velocity.y +=
                root.gravity.gravity * Root.deltaTime *
                0.5f; // Added for jump precision, will more or less jump the correct archHeight
            root.velocity.y = velocity.y;
