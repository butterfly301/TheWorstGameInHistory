...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class JumpInObject : Conditional
    {
        ...
        public override NodeState RunNodeLogic(Root root)
        {
            ...
            {
                state = State.Waiting;
                if (playerRenderer == null) playerRenderer = player.transform.GetComponent<SpriteRenderer>();
                if (objectRenderer == null) objectRenderer = gameObject.GetComponent<SpriteRenderer>();
                if (playerHealth == null) playerHealth = player.transform.GetComponent<Health>();
            }
            ...
            {
                var velX = (transform.position.x - player.transform.position.x) * 0.25f;
...
#pragma warning disable 0414
        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(65,
                    "Player must first jump over the top point to enter this object. In this state, the player's hide flag goes true and can't take damage." +
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            Debug.DrawLine(p + Vector3.up * top - Vector3.right * width * 0.5f,
                p + Vector3.up * top + Vector3.right * width * 0.5f, Color.red);
        }
