    ...
    public class FindTarget : Conditional
    {
        ...
        private bool Search(Vector2 position, Vector2 target, Root root)
        {
            ...
            if (findType == AISearchType.Radius)
            {
                ...
                if ((target - (position + offset)).sqrMagnitude < radius * radius) return true;
            }
            else if (findType == AISearchType.Distance)
            {
                var offset = this.offset;
                offset.x = !mirror ? offset.x : Mathf.Abs(offset.x) * Mathf.Sign(root.direction);
                if (distance.x != 0 && (target.x > position.x + offset.x + distance.x * 0.5f ||
                                        target.x < position.x + offset.x - distance.x * 0.5f)) return false;
                if (distance.y != 0 && (target.y > position.y + offset.y + distance.y * 0.5f ||
                                        target.y < position.y + offset.y - distance.y * 0.5f)) return false;
                return true;
            }
...
#pragma warning disable 0414
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            ...
            var type = parent.Enum("findType");
            var height = type < 2 ? 2 : 0;
            FoldOut.Box(3 + height, color, offsetY: -2);
            AIBase.SetRef(ai.data, parent.Get("target"), 0);
            parent.Field("Type", "findType");
            parent.Field("Radius", "radius", type == 0);
            parent.Field("Distance", "distance", type == 1);
            if (type == 2)
            {
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            {
                var ai = editor.target as AIBase;
