        ...
        public override bool OnInspector(AIBase ai, SerializedObject parent, Color color, bool onEnable)
        {
            if (parent.Bool("showInfo"))
                Labels.InfoBoxTop(80,
                    "This will execute one node at a time and move into the next node when the current node returns Success. When the current node fails, it will return Failure. If all nodes complete, it will return Success. Otherwise, it will return Running.");
