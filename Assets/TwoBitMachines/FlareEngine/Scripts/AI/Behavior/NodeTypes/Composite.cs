...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public abstract class Composite : Node
    {
        ...
        [HideInInspector] public int currentChildIndex;

        public bool canInterruptSelf => canInterrupt == InterruptType.ThisNode ||
                                        canInterrupt == InterruptType.ThisAndLowerPriorityNodes;

        public bool canInterruptLowerNodes => canInterrupt == InterruptType.LowerPriorityNodes ||
                                              canInterrupt == InterruptType.ThisAndLowerPriorityNodes;
        ...
        public void InterruptCheck(Root root)
        {
            for (var i = 0; i < currentChildIndex; i++)
            {
                ...
                var composite = children[i] as Composite;
                if (composite.canInterruptLowerNodes && composite.InterruptLogic(root))
                {
            ...
            if (currentChildIndex > 0 && canInterruptSelf &&
                !InterruptLogic(root, true)) // interrupt on condition failure, not success
                Reset();
        }
        ...
        public virtual bool InterruptLogic(Root root, bool selfAbort = false)
        {
            ...
            if (children.Count == 0)
                return
                    selfAbort
                        ? true
                        : false; // failed, not a true fail, for self aborts we return true, since we are looking for a false to entre the state
            ...
            else if (child.isInterruptType) // composite 
            {
                ...
                (selfAbort) // if  not a conditional or interrupt, we encountered an action thus we exit if in self abort
            {
