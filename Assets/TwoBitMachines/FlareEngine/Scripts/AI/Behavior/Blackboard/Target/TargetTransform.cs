...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class TargetTransform : Blackboard
    {
        ...
        {
            return target != null ? target.position + (Vector3)offset : transform.position;
        }
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            if ((oldPosition.x != newPosition.x || oldPosition.y != newPosition.y) && target != null &&
                target != transform && !Application.isPlaying) // && TwoBitMachines.Editors.Mouse.ctrl)
                target.position += newPosition - oldPosition;
