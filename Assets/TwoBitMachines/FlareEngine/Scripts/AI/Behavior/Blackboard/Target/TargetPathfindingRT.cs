...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class TargetPathfindingRT : TargetPathfindingBase
    {
        ...
        [SerializeField] [HideInInspector] public FollowerState[] stateMachine =
        {
            new StateFollow(),
            new StateJump(),
            new StateCeiling(),
        ...
        [NonSerialized] public bool instant;
        [NonSerialized] public JobHandle jobHandle;
        [NonSerialized] public NativeList<Vector2Int> pathReference;
        ...
        public void GetPath()
        {
            if (!waitForPath || map == null || !jobHandle.IsCompleted) return;
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                if (i == length - 2 && map.map[pathReference[i]].Same(currentNode)) break;
            }
        ...
        {
            if (map == null || targetRef == null || currentNode == null) return;
            map.GridPosition(currentNode.position, out var startX, out var startY);
            map.GridPosition(targetRef.GetTarget() + map.cellYOffset, out var endX, out var endY);

            if (targetRef.hasNoTargets) return;

            jobHandle.Complete(); // ensure job is complete before doing a reset
            var calculateJob = new PathfindingJobRT
            {
                startGridX = startX,
                startGridY = startY,
                endGridX = endX,
        ...
        {
            bottomCenter = world != null ? world.box.bottomCenter : transform.position;
        ...
        public void RunPathFollower(ref Vector2 velocity)
        {
            ...
            GetPath();
            if (waitForPath || currentNode == null || map == null || world == null) return;
            ...
            if (WorldManager.viewDebugger)
            {
                ...
                if (futureNode != null) Draw.Circle(futureNode.position, 0.55f, Color.red, 1);
            }
#endif
        ...
        {
            var size = world == null ? Vector2.one : world.box.boxSize;
