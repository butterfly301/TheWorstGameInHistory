...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class TargetTransforms : Blackboard
    {
        ...
        [SerializeField] public List<Transform> transforms = new();
        ...
        public override Vector2 GetNearestTarget(Vector2 position)
        {
            ...
            for (var i = 0; i < transforms.Count; i++)
            {
                if (transforms[i] == null)
                    continue;
                var squareDistance = (position - (Vector2)transforms[i].position).sqrMagnitude;
        ...
        public override Transform GetRandomTransformTarget()
        {
            ...
            if (transforms.Count > 0)
            {
                var randomIndex = Randomize();
                return transforms[randomIndex] != null ? transforms[randomIndex] : transform;
            }
        ...
        public override NodeState NextTarget(ref Vector3 target, ref bool reversing, ref int index, Transform transform)
        {
            ...
            if (!loopSequence)
            {
                ...
                {
                    var complete = index + 1 >= transforms.Count;
        ...
        {
            var newRand = Random.Range(0, transforms.Count);
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            if (oldPosition == Vector3.zero) oldPosition = newPosition;
