...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class TargetBezierPath : Blackboard
    {
        ...
        [SerializeField] public List<CubicBezier> path = new();

        public bool endless =>
            onCompletePath == BezierPathType.Endless || onCompletePath == BezierPathType.EndlessReverse;
        ...
        public Vector2 FollowPath(Vector2 position, FollowBezierPath follower, ref int index, out NodeState complete)
        {
            ...
            if (index >= 0 && index < path.Count)
            {
                ...
                var t = follower.counter / follower.time;
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            parent.Update();
            {
                var path = parent.FindProperty("path");
                var newPosition = transform.position;
                if ((oldPosition.x != newPosition.x || oldPosition.y != newPosition.y) &&
                    !Application.isPlaying) // && TwoBitMachines.Editors.Mouse.ctrl)
                {
                    var move = (Vector2)(newPosition - oldPosition);

                    for (var i = 0; i < path.arraySize; i++)
                    {
                        var p = path.Element(i);
                        ...
                        Handles.DrawBezier(p.Get("start").vector2Value, p.Get("end").vector2Value,
                            p.Get("control1").vector2Value, p.Get("control2").vector2Value, Color.green,
                            Texture2D.whiteTexture, 2.5f);
                    }
                }
                else
                {
                    for (var i = 0; i < path.arraySize; i++)
                    {
                        ...
                        {
                            p.SetTrue("init");
                            p.Get("start").vector2Value = i > 0
                                ? path.Element(i - 1).Get("end").vector2Value
                                : SceneTools.SceneCenter(p.Get("start").vector2Value);
                        ...
                        if (i > 0) path.Element(i - 1).Get("end").vector2Value = p.Get("start").vector2Value;

                        p.Get("end").vector2Value =
                            SceneTools.MovePositionCircleHandle(p.Get("end").vector2Value, Color.red, 0.25f, 0.5f);
                        p.Get("start").vector2Value =
                            SceneTools.MovePositionCircleHandle(p.Get("start").vector2Value, Color.green, 0.25f, 0.5f);
                        p.Get("control1").vector2Value =
                            SceneTools.MovePositionCircleHandle(p.Get("control1").vector2Value, Color.blue, 0.25f,
                                0.35f);
                        p.Get("control2").vector2Value =
                            SceneTools.MovePositionCircleHandle(p.Get("control2").vector2Value, Color.blue, 0.25f,
                                0.35f);
    ...
    {
        public static List<Vector2> tempPoints = new();
