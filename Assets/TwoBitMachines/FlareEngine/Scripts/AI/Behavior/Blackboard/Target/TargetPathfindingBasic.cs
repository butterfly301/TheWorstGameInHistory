...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class TargetPathfindingBasic : Blackboard
    {
        ...
        [NonSerialized] public bool activeUnit;
        [NonSerialized] public PathfindingBasicJob calculateJob;
        [NonSerialized] public float counter;

        [NonSerialized] public BasicNode currentNode;
        [NonSerialized] public JobHandle jobHandle;
        [NonSerialized] public Stack<Vector2> path = new();
        [NonSerialized] public NativeList<Vector2> pathReference;
        [NonSerialized] public Vector2 shift;
        [NonSerialized] public BasicNode targetNode;
        [NonSerialized] public Vector2 targetOffset;
        ...
        public void GetPath()
        {
            ...
            var position = (Vector2)transform.position + map.cellYOffset;
        ...
        public void CalculatePath(Blackboard target)
        {
            ...
            var startNode = map.PositionToNode((Vector2)transform.position + map.cellYOffset);
            ...
            if (targetTransform != null) targetOffset = targetPosition - (Vector2)targetTransform.position;

            if (targetNode == null || startNode == null || target.hasNoTargets) return;
        ...
        {
            GetPath();
            if (waitForPath || map == null) return;
            ...
            if (path.Count > 0)
            {
                var checkSize = path.Count == 1 ? 0.01f : map.cellSize * 0.5f;
            ...
            {
                var newPosition = Vector2.MoveTowards(position, path.Peek(), followSpeed * Time.deltaTime);
                velocity = (newPosition - position) / Time.deltaTime;
            }

            if (path.Count == 0 && targetTransform != null)
            {
                var newPosition = Vector2.MoveTowards(position, targetTransform.position + (Vector3)targetOffset,
