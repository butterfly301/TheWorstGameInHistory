...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class Territory : Blackboard
    {
        ...
        {
            var randomX = Random.Range(0, bounds.size.x);
            var randomY = Random.Range(0, bounds.size.y);
        ...
        [SerializeField] [HideInInspector] public bool focus;
        ...
        public override void OnSceneGUI(Editor editor)
        {
            ...
            if (oldPosition.x != newPosition.x ||
                oldPosition.y != newPosition.y) // && TwoBitMachines.Editors.Mouse.ctrl)
            {
            ...
            if (dataName != "")
            {
                //   SceneTools.Label(bounds.position + Vector2.up * (bounds.size.y + 4f), dataName, Color.white);
            }
        }
#endif
