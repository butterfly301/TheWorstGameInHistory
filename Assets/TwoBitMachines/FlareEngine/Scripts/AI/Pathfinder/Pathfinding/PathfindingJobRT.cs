...
namespace TwoBitMachines.FlareEngine
{
    ...
    public struct PathfindingJobRT : IJob
    {
        ...
        public void Execute()
        {
            if (!CheckTarget(ref target, endGridX, endGridY)) return;
            if (!CheckTarget(ref start, startGridX, startGridY)) return;

            var openSet = new NativeList<Vector2Int>(Allocator.Temp);
            var closedSet = new NativeList<Vector2Int>(Allocator.Temp);
            var neighbors = new NativeList<PathNodeStruct>(Allocator.Temp);
            var nodeCost = new NativeHashMap<Vector2Int, NodeCost>(jobGrid.Count(), Allocator.Temp);
            ...
            var nearestTarget = Vector2Int.zero;
            ...
            while (openSet.Length > 0)
            {
                ...
                for (var i = 1; i < openSet.Length; i++)
                {
                    ...
                    if (nodeCost[openSetNode.cell].fCost() > nodeCost[node.cell].fCost()) continue;
                    if (nodeCost[openSetNode.cell].hCost < nodeCost[node.cell].hCost) node = openSetNode;
                }
                ...
                if (jobNeighbors.ContainsKey(node.cell))
                {
                    ...
                    while (values.MoveNext())
                        if (jobGrid.TryGetValue(values.Current, out var connectNode))
                        {
                ...
                for (var y = -1; y <= 1; y++)
                {
                    ...
                    if (block != 1 && block != -1) // don't test corners or middle nodes
                        continue;
                    var cell = new Vector2Int(node.x + x, node.y + y);
                    if (jobGrid.TryGetValue(cell, out var nextNode) && node.Path() && nextNode.Path())
                    {
                ...
                {
                    var neighbour = jobGrid[neighbors[i].cell];

                    var moveCostToNeighbour =
                        nodeCost[node.cell].gCost +
                        GetDistance(node.x, node.y, neighbour.x, neighbour.y); // nodeCost[node.gridID].penalty;
                    ...
                    if (moveCostToNeighbour < nodeCost[neighbour.cell].gCost || !openSet.Contains(neighbour.cell))
                    {
                        ...
                        var hCost = GetDistance(neighbour.x, neighbour.y, target.x, target.y);
                        ...
                        if (!openSet.Contains(neighbour.cell)) openSet.Add(neighbour.cell);
                    }
                }
            }
            ...
            if (canSearch && !foundPath)
            {
                if (start.ground &&
                    ...
                    nearestDistance) // star path is actually closer, stay here
        ...
        public int
            GetDistance(int cellaX, int cellaY, int cellbX, int cellbY) // don't pass struct since they are being copied
        {
            var dX = math.abs(cellaX - cellbX);
            var dY = math.abs(cellaY - cellbY);
        ...
        public void RetracePath(Vector2Int start, Vector2Int end, NativeHashMap<Vector2Int, NodeCost> nodeCost)
        {
            ...
            if (result.Length > 0 &&
                jobGrid[result[0]].edgeDrop)
                result.RemoveAt(
                    0); // if final target point is an edgeDrop, remove, since an edgeDrop is just air, ai should't move into air, edge drop useful for detecting ambiguous states and drops
        }

        public bool CheckTarget(ref PathNodeStruct nodeT, int cellX, int cellY)
        {
            for (var i = 0; i <= 25; i++)
                if (jobGrid.TryGetValue(new Vector2Int(cellX, cellY - i), out var node))
                {
                    nodeT = node;
                    return true;
                }
