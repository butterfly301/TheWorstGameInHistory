...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class PathNode
    {
        ...
        [SerializeField] public List<Vector2Int> neighbor = new();
        [NonSerialized] public TargetPathfindingBase unit;
        ...
        {
            return Mathf.Abs(position.x - node.position.x);
        }
        ...
        {
            return Mathf.Abs(position.y - node.position.y);
        }
        ...
        {
            if (!neighbor.Contains(child.cell)) neighbor.Add(child.cell);
            if (addToBoth)
    ...
    public struct PathNodeStruct
    {
        ...
        public bool Path()
        {
            return ground || jumpThroughGround || ladder || wall || moving || ceiling || bridge;
        }
    }
}