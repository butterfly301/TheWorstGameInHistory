...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class StateFollow : FollowerState
    {
        ...
        {
            if (ai.pauseAfterJumpActive && ai.pauseAfterJump > 0 &&
                TwoBitMachines.Clock.TimerInverse(ref ai.pauseCounter,
                    ai.pauseAfterJump))
            ...
            if (ai.state == PathState.Follow && ai.currentNode.onGround &&
                ai.currentNode.DistanceY(ai.nextNode) > 0) // not reachable
        ...
        {
            if (onGround && ai.state == PathState.Follow) StateLadder.FindLadderClimb(ai, ref velocity);
