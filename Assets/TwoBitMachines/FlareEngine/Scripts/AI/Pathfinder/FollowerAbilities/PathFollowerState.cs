...
#pragma warning restore 0414
        ...
        {
            var start = ai.bottomCenter;
            var end = ai.futureNode.position - ai.cellYOffset;
            var direction = end.x > start.x ? Vector2.right : Vector2.left;
            Vector2 control1, control2;
            var height = Mathf.Abs(end.y - start.y) + 0.5f;
        ...
        {
            MoveToTarget(ai.position.x, ai.futureNode == null ? ai.position.x : ai.futureNode.position.x,
                ai.followSpeed, ref velocity.x);
        ...
        {
            var newPosition = Mathf.MoveTowards(position, target, speed * Time.deltaTime);
            velocity = Time.deltaTime == 0 ? 0 : (newPosition - position) / Time.deltaTime;
        }

        public static bool MoveToCenterX(TargetPathfindingBase ai, PathNode node, ref float velocity, float corner = 0,
            bool arrived = true)
        {
            float direction = ai.futureNode.SameX(node) ? 0 : ai.futureNode.DirectionX(node);
            var target = corner == 0 ? node.position.x : node.position.x + direction * ai.cellSize * corner;
            if (direction != 0 &&
                ((ai.position.x >= target && direction > 0) || (ai.position.x <= target && direction < 0))) return true;
            var position = Mathf.MoveTowards(ai.position.x, target, ai.followSpeed * Time.deltaTime);
        ...
        {
            var newPosition = Mathf.MoveTowards(ai.position.x, target.position.x, ai.followSpeed * Time.deltaTime);
        ...
        {
            if (ai.nextNode != null && ai.futureNode != null && ai.currentNode.SameY(ai.nextNode) &&
                ai.nextNode.NextToY(ai.futureNode)) RemoveNode(ai);
        }

        public void RemoveOutOfOrderY(TargetPathfindingBase ai)
        {
            if (ai.futureNode != null && ai.futureNode.Below(ai.nextNode) &&
                ai.currentNode.Below(ai.nextNode)) // ai can sometimes be in incorrect order
                RemoveNode(ai);
