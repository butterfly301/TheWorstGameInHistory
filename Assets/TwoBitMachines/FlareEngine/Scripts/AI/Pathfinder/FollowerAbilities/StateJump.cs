...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    public class StateJump : FollowerState
    {
        ...
        {
            if (ai.nextNode.bridge && ai.futureNode.bridge) return;
            ...
            {
                if (MoveToCenterX(ai, ai.nextNode, ref velocity.x)) state.Jump(ai, ref velocity);
            }
            else if (ai.nextNode.DistanceX(ai.futureNode) <= 1f) //                          platform is below
            {
                if (MoveToCenterX(ai, ai.futureNode, ref velocity.x)) state.JumpFall(ai, ref velocity, true);
            }
        ...
        {
            return node.ladder ? PathState.Ladder :
                node.wall ? PathState.Wall :
                node.ceiling ? PathState.Ceiling : PathState.Follow;
        }
    }
