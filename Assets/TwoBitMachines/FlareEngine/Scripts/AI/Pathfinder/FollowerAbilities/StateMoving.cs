...
namespace TwoBitMachines.FlareEngine.AI.BlackboardData
{
    ...
    {
        [NonSerialized] public bool canFollow;
        ...
        public override void Execute(TargetPathfindingBase ai, ref Vector2 velocity)
        {
            ...
            if (ai.nextNode.moving && ai.futureNode != null && ai.futureNode.moving) RemoveNode(ai);
            ...
            if (ai.world.mp.velocity.sqrMagnitude < 0.001f) // platform not moving
            {
                if (canFollow && ai.futureNode != null)
                {
                    ...
                    else if (ai.nextNode.moving && !ai.futureNode.moving &&
                             InPlatformXrange(ai.nextNode.position.x)) // only move towards an exit node
        ...
        {
            if (ai.currentNode.moving) ai.state = PathState.Moving;
            // ai.canFollow = false;
        }
    }
}