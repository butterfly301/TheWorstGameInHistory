    ...
    public class FindCornersEditor : AIPathfindingEditor
    {
        ...
        public static void Execute(Pathfinding map)
        {
            ...
            for (var y = 0; y < map.linesY; y++)
            {
                ...
                if (tempNode == null || tempNode.air) continue;
        ...
        {
            if (IsCorner(tempNode, map, x, y, searchLeft)) tempNode.leftCorner = true;
            if (IsCorner(tempNode, map, x, y, searchRight)) tempNode.rightCorner = true;
        }

        private static bool IsCorner(PathNode tempNode, Pathfinding map, int x, int y, int sign)
        {
            var safetyCheckX = sign > 0 ? x + 1 < map.linesX : x - 1 >= 0;
            var safetyCheckY = y - 1 >= 0;

            if (!safetyCheckX || !safetyCheckY) return false;

            var nodeIsNextToWall = map.Node(x + sign, y) == null;
            var nodeIsACorner = map.Node(x + sign, y - 1) != null;

            return (nodeIsNextToWall || nodeIsACorner) && NotAGroundEdge(tempNode.jumpThroughGround, map, x, y, sign);
        }

        private static bool
            NotAGroundEdge(bool isGroundEdge, Pathfinding map, int x, int y, int sign) // this is a jumpthrough node
        {
            ...
            if (isGroundEdge)
            {
                var node = map.Node(x + sign, y);
                if (node != null &&
                    node.jumpThroughGround) // if there is a ground edge in the direction of the search, it means we are not at a corner
                    return false;
            }
