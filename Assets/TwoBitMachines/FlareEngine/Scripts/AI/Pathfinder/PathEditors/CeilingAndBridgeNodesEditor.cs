...
namespace TwoBitMachines.Editors
{
    public class CeilingAndBridgeNodesEditor : AIPathfindingEditor
    {
        ...
        {
            if (map.ceiling.Count != 0) Register(map);

            if (map.bridge.Count != 0) RegisterBridges(map);
        }
        ...
        private static void ConnectCeilingsToGround(PathNode ceiling, Pathfinding map, int x, int y)
        {
            ...
            for (var searchDown = 1;
                 ...
                 searchDown++) //                                 search for ladders from left to right
            {
                var ground = map.Node(x, y - searchDown);
                if (ground == null || ground.ceiling) return;
                jumpCount++;
