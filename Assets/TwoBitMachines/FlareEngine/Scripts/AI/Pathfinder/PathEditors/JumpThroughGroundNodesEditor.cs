    ...
    public class JumpThroughGroundNodesEditor : AIPathfindingEditor
    {
        public static void Execute(Pathfinding map, Vector2 cellOffset)
        {
            ...
            for (var y = 0; y < map.linesY; y++)
            {
                ...
                if (tempNode == null || !tempNode.ground) continue;
            ...
            {
                var tempNode = map.Node(x, y);
                if (tempNode == null || !tempNode.jumpThroughGround) continue;
        ...
        private static void SearchForGroundNodes(PathNode tempNode, Pathfinding map, Vector2 offset)
        {
            ...
            var origin = tempNode.position;
            for (var searchUp = 0; searchUp < 100; searchUp++) // arbitrary check limit
            {
                var hit = Physics2D.Raycast(origin, Vector2.up, map.linesY, map.layerWorld);
                if (!hit || !(hit.collider is EdgeCollider2D)) return; //no Edge Collider found, exit out
                origin = hit.point + Vector2.up * 0.01f; // update origin to test above edge incase edges are stacked
                var jumpTroughNode = map.PositionToNode(hit.point + Vector2.up * offset.y);
        ...
        private static void ConnectJumpThroughNodes(PathNode tempNode, Pathfinding map, int x, int y)
        {
            ...
            {
                var lowerPlatform = map.Node(x, y - searchDown);
                if (lowerPlatform == null) return;
        ...
        private static void ShowVisualConnections(Pathfinding map, int x, int y)
        {
            for (var searchDown = 1; searchDown < map.maxJumpHeight; searchDown++)
            {
                var connections = map.Node(x, y - searchDown);
                if (connections == null || connections.ground || connections.jumpThroughGround) return;
