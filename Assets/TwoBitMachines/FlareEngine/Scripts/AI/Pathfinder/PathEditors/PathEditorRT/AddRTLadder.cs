...
namespace TwoBitMachines.FlareEngine
{
    public class AddRTLadder
    {
        ...
        {
            for (var i = 0; i < map.ladder.Count; i++)
                if (map.Node(map.ladder[i].x, map.ladder[i].y, out var node))
                {
                    node.ladder = true;
                ...
                {
                    map.map[map.ladder[i]] = new PathNode
                    {
                        position = map.ladder[i] + map.cellOffset, x = map.ladder[i].x, y = map.ladder[i].y,
                        ladder = true
                    };
                }
        }
        ...
        {
            for (var i = 0; i < map.ladder.Count; i++)
                if (map.Node(map.ladder[i].x, map.ladder[i].y, out var node) && node.ladder)
                {
        ...
        {
            if (node.ground || map.ladder.Contains(new Vector2Int(node.x, node.y - 1))) return;
