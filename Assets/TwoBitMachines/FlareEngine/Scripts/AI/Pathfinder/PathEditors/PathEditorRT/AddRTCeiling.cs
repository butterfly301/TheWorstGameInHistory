...
namespace TwoBitMachines.FlareEngine
{
    public class AddRTCeiling
    {
        ...
        {
            if (map.ceiling.Count != 0) AddCeilingNodes(map);
        }

        public static void AddCeilingNodes(PathfindingRT map)
        {
            for (var i = 0; i < map.ceiling.Count; i++)
            {
                PathNode node;
                ...
                else
                {
                    node = new PathNode
                    {
                        position = map.ceiling[i] + map.cellOffset, x = map.ceiling[i].x, y = map.ceiling[i].y,
                        ceiling = true
                    };
