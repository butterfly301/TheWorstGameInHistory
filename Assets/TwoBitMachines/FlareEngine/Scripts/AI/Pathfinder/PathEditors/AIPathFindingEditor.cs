...
namespace TwoBitMachines.Editors
{
    public class AIPathfindingEditor
    {
        ...
        {
            if (!Application.isPlaying && map.foldOut) BeginGridCreation(map);
        }
        ...
        private static void BeginGridCreation(Pathfinding map)
        {
            if (map.createPaths && map.cellSize > 0 && map.bounds.size != Vector2.zero)
            {
                ...
                {
                    var node = map.grid[y * map.linesX + x];
        ...
        public static void DisplayMapAfterEditing(Pathfinding map, bool show)
        {
            ...
            {
                if (map.grid.Length == 0) map.connections.Clear();
                Draw.GLStart();
                for (var i = 0; i < map.connections.Count; i++)
                    Draw.GLCircle(map.connections[i].position, map.connections[i].size, map.connections[i].color,
                        map.connections[i].rays);
                Draw.GLEnd();
            }
        }
        ...
        private static void CreateGrid(Vector2 startingPosition, Vector2 gridSize, float cellSize, Pathfinding map)
        {
            ...
            var cellOffset = new Vector2(map.cellSize * 0.5f, map.cellSize * 0.5f);
            ...
            JumpThroughGroundNodesEditor.Execute(map,
                cellOffset); //                         sets jumpThroughGround (jTG), ground, air, -- reads ground
            NodePropertiesEditor.SetNodeHeadSpace(map); //                                    sets height
            FindCornersEditor
                .Execute(map); //                                                sets leftCorner, rightCorner   -- reads air, jTG

            LadderNodesEditor
                .Execute(map); //                                                sets ladder -- reads ground, jTG
            CeilingAndBridgeNodesEditor
                .Execute(map); //                                      sets ceiling, bridge, exact -- reads ground, jTG
            MovingPlatformNodesEditor
                .Execute(map); //                                        sets moving, also sets rightCorner, leftCorner
            WallNodesEditor
                .Execute(map); //                                                  sets wall, exact -- reads ground, jTG

            EdgesNodesEditor
                .Execute(map); //                                                 sets edgeOfCorner, bigDrop, exact -- reads leftCorner, rightCorner, air, ground, ladder, bridge, height, exact
            ConnectCornersEditor.Execute(map.grid,
                map); //                                   reads leftCorner, rightCorner, air, height
        ...
        {
            map.connections.Add(new Pathfinding.GridConnections
                { position = position, size = size, color = color, rays = rays });
        ...
        {
            var contain = false;
            for (var i = 0; i < map.neighbor.Count; i++)
        ...
        {
            var newDropNode =
                map.Node(dropNode.x + direction, dropNode.y); // let's shift drop node so that ai has more room to jump
            if (newDropNode != null && newDropNode.ground)
            {
                for (var searchUp = 1; searchUp < map.maxJumpHeight; searchUp++)
                {
                    var airNode = map.Node(newDropNode.x, newDropNode.y + searchUp);
                    if (airNode == null || airNode.ground) return;
                }
