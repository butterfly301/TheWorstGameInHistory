...
namespace TwoBitMachines.Editors
{
    ...
    {
        public static List<PathNode> airNode = new();

        public static void
            Execute(PathNode[] grid,
                Pathfinding map) //                       Connect edges so that ai can jump from platform to platform
        {
            for (var x = 0; x < map.linesX; x++)
            for (var y = 0; y < map.linesY; y++)
            {
                var cornerNode = grid[y * map.linesX + x];
        ...
        private static void SearchForCorners(PathNode[] grid, Pathfinding map, PathNode cornerA)
        {
            ...
            for (var y = 0; y < map.linesY; y++)
            {
                var cornerB = grid[y * map.linesX + x];
                ...
                {
                    airNode.Clear();
                    if (HorizontalBlock(grid, map, cornerA, cornerB, y) ||
                        HorizontalBlock(grid, map, cornerB, cornerA, y,
                            1)) // since ai has to jump to platform, make sure the jumping path is not blocked
                        continue;
                    if (VerticalBlock(grid, map, cornerA, y)) continue;
        ...
        {
            var withinDistanceX = Mathf.Abs(cornerA.x - cornerB.x) <= map.maxJumpDistance;
            var withinDistanceY = Mathf.Abs(cornerA.y - cornerB.y) <= map.maxJumpHeight;
        ...
        {
            if (cornerA.leftCorner && cornerB.rightCorner && cornerA.ShiftX(map, -1) == null &&
                cornerB.x < cornerA.x - 1)
                return true; // wall is between corners. Corner might already have a connection on said wall. Exit out.
            if (cornerA.rightCorner && cornerB.leftCorner && cornerA.ShiftX(map, 1) == null &&
                cornerB.x > cornerA.x + 1)
                return true;
            return false;
        }

        private static bool HorizontalBlock(PathNode[] grid, Pathfinding map, PathNode cornerA, PathNode cornerB, int y,
            int g = 0)
        {
            if (cornerA.rightCorner && cornerB.leftCorner)
                for (var searchX = cornerA.x; searchX <= cornerB.x; searchX++)
                {
                    var connectNode = grid[y * map.linesX + searchX];
                    ...
                    if (connectNode == null || connectNode.block || !connectNode.air || connectNode.height < 1)
                        return true;
                    if (cornerA.y == cornerB.y && connectNode.height <= 1) return true;
        ...
        private static bool VerticalBlock(PathNode[] grid, Pathfinding map, PathNode cornerA, int y)
        {
            ...
            {
                var connectNode = grid[searchY * map.linesX + cornerA.x];
                if (connectNode == null || connectNode.block || !connectNode.air || connectNode.height < 1) return true;
                airNode.Add(connectNode);
            }
