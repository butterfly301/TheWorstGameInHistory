...
namespace TwoBitMachines.FlareEngine.AI
{
    ...
    public class AIBase : Character
    {
        ...
        [SerializeField] public AIDamagePack damage = new();
        [SerializeField] public List<Blackboard> data = new();
        [NonSerialized] public bool initialized;
        [NonSerialized] public bool slowDownActive;
        [NonSerialized] public float timeScaleIntensity;
        ...
        public void Awake()
        {
            ...
            if (type == CharacterType.MovingPlatform) movingPlatform.Initialize(transform);
            if (type == CharacterType.Regular) world.Initialize(transform);
        ...
        public override void OnStart()
        {
            ...
            {
                var aIDamage = gameObject.AddComponent<AIDamage>();
#if UNITY_EDITOR
        ...
        public void Collision(Vector2 velocity, ref float gravity)
        {
            ...
            if (!turnOffSignals && !thisIsPlayer) signals.SetSignals(velocity, root.onSurface, world.onWallStop);
        ...
        {
            if (slowDownActive) Root.deltaTime = Time.deltaTime * timeScaleIntensity;
        }

        public void SlowDownReset()
        {
            if (slowDownActive) Root.deltaTime = Time.deltaTime;
        }
...
#pragma warning disable 0414
        [SerializeField] [HideInInspector] public List<Node> tempChildren = new();
        [SerializeField] [HideInInspector] public List<GameObject> units = new();
        ...
        [NonSerialized] private static readonly List<string> target = new();

        public static void AIType(Transform transform, SerializedObject parent, ref bool createUnits,
            bool isFSM = false)
        {
            FoldOut.Box(1, Tint.BoxTwo, 3);
            {
                parent.Field("AI Type ", "type");
            }

            if (!FoldOut.FoldOutButton(parent.Get("mainFoldOut"))) return;

            var type = (CharacterType)parent.Get("type").enumValueIndex;
            ...
            if (FoldOut.Bar(parent, Tint.BoxTwo).Label("Settings", FoldOut.titleColor, false)
                .FoldOut("settingsFoldOut"))
            {
                if (isFSM)
                {
            ...
            if (FoldOut.Bar(parent.Get("damage"), Tint.BoxTwo).Label("Damage", FoldOut.titleColor, false)
                .BRE("canDealDamage").FoldOut("damageFoldOut"))
            {
                var damage = parent.Get("damage");
        ...
        public static void CollisionSettings(SerializedObject parent, SerializedProperty world,
            SerializedProperty gravity, SerializedProperty signals)
        {
            if (FoldOut.Bar(parent, Tint.BoxTwo).Label("Collision", FoldOut.titleColor, false)
                .FoldOut("collisionSettings"))
            {
                var extraHeightS = world.Bool("climbSlopes") ? 3 : 0;
                ...
                FoldOut.Box(6, FoldOut.boxColor, 3);
                {
                    ...
                    if (FoldOut.FoldOutButton(world.Get("eventsFoldOut")))
                        Fields.EventFoldOutEffect(world.Get("onCrushed"), world.Get("crushedWE"),
                            world.Get("crushedFoldOut"), "Crushed By Platform");
                }
            }
        }

        public static void BlackboardDisplay(SerializedObject parent, SerializedProperty bar, AIBase ai,
            List<string> dataList, int size = 1)
        {
            if (bar.intValue <= size) return;

            var type = bar.intValue - (size + 1);
            var dataArray = parent.Get("data");
            for (var i = 0; i < dataArray.arraySize; i++)
            {
                ...
                var data = new SerializedObject(element.objectReferenceValue);

                var index = data.Enum("blackboardType");
                ...
                data.Update();
                {
                    ...
                    var name = element.objectReferenceValue.GetType().Name;
                    FoldOut.Bar(data).Label(Util.ToProperCase(name));
                    FoldOut.bar.BR("delete", "Delete");
                    var focus = name == "Territory" ? Bar.ButtonRight("Target", Color.white) : false;

                    if (FoldOut.bar.FoldOut())
                    {
                        if (focus)
                        {
                            var territory = element.objectReferenceValue as Territory;
                            Debug.Log(element.objectReferenceValue.GetType());
                            SceneTools.MoveSceneCamera(territory.bounds.position);
                        }

                        var fields = EditorTools.CountObjectFields(data);
                        if (fields != 0)
                            FoldOut.Box(fields, FoldOut.boxColor, offsetY: -2);
                        EditorTools.IterateObject(data, fields);
                        if (index == 1)
                        {
                            var create = data.Get("createPaths");
                            if (create != null && FoldOut.LargeButton("Create Paths +", Tint.Orange, Tint.White,
                                    Icon.Get("BackgroundLight"))) create.boolValue = true;
                        }
        ...
        public static void CreateUnits(AIBase ai)
        {
            ...
            for (var i = 0; i < newUnits.Count; i++)
            {
                ...
                {
                    // set transform
                    var oldT = ai.units[i].GetComponent<Transform>();
                    var newT = newUnits[i].GetComponent<Transform>();
                    ...
                    // set color
                    var oldS = ai.units[i].GetComponent<SpriteRenderer>();
                    var newS = newUnits[i].GetComponent<SpriteRenderer>();
                    var templateS = ai.GetComponent<SpriteRenderer>();
                    ...
                    //box sizes
                    var oldB = ai.units[i].GetComponent<BoxCollider2D>();
                    var newB = newUnits[i].GetComponent<BoxCollider2D>();
                ...
                newUnits[i].name = ai.name + "_" + (i + 2);

                var newTree = newUnits[i].GetComponent<AITree>();
                if (newTree != null)
                {
                    newTree.units.Clear(); //
                    newTree.tempUnits = 0;
                ...
                var nodes = newUnits[i].GetComponents<Node>();
                for (var k = 0; k < nodes.Length; k++) nodes[k].hideFlags = HideFlags.HideInInspector;

                var data = newUnits[i].GetComponents<Blackboard>();
                for (var k = 0; k < data.Length; k++) data[k].hideFlags = HideFlags.HideInInspector;

                var health = newUnits[i].GetComponents<WorldFloat>();
                for (var k = 0; k < health.Length; k++)
                    if (health[k].isHealth)
                        health[k].variableName = health[k].variableName + "_" + i;
            }

            for (var i = 0; i < ai.units.Count; i++)
                if (ai.units[i] != null)
                    DestroyImmediate(ai.units[i]);
            ai.units.Clear();
            for (var i = 0; i < newUnits.Count; i++) ai.units.Add(newUnits[i]);
        }

        public static void IterateObject(SerializedObject element, List<Blackboard> data, Node node = null,
            bool setReference = false)
        {
            ...
            while (iterator.NextVisible(true) && i++ < 1000) // prevent an infinite loop
            {
                ...
                if (prop != null)
                {
                    ...
                    if (setReference && SetRef(data, prop, i)) continue;
        ...
        public static bool SetRef<T>(List<Blackboard> data, SerializedProperty field, int index)
        {
            var typeName = field.type.Replace("PPtr<$", "").Replace(">", "");
            var typePath = "TwoBitMachines.FlareEngine.AI.BlackboardData." + typeName;
            if (typeName != "Pathfinding" && typeName != "PathfindingBasic" &&
                EditorTools.ValidateType(typePath)) // detect if field is blackboard data type
            {
                ...
                for (var j = 0; j < data.Count; j++)
                {
                    if (data[j] == null || data[j] is TreeVariable || data[j] is AIFSMVariable ||
                        !(data[j] is T)) continue;
                    target.Add(data[j].dataName);
                }

                for (var j = 0;
                     j < data.Count;
                     j++) // set ai tree data after main tree so that indexes don't get mixed up if removed
                    if (data[j] != null && (data[j] is TreeVariable || data[j] is AIFSMVariable))
                    {
                        var sharedTree = data[j].GetTree();
                        if (sharedTree != null)
                            for (var k = 0; k < sharedTree.data.Count; k++)
                                if (sharedTree.data[k] is T)
                                    target.Add(sharedTree.data[k].dataName);

                        var sharedAIFSM = data[j].GetAIFSM();
                        if (sharedAIFSM != null)
                            for (var k = 0; k < sharedAIFSM.data.Count; k++)
                                if (sharedAIFSM.data[k] is T)
                ...
                var refName = field.serializedObject.Get("refName");
                field.serializedObject.DropDownList(target.ToArray(), Util.ToProperCase(field.name),
                    GetRefName(refName, index));
                var chosenTarget = GetRefName(refName, index).stringValue;
                for (var j = 0; j < data.Count; j++) // set reference
                {
                    if (data[j] == null || data[j] is T)
                        continue;
        ...
        public static bool SetRef(List<Blackboard> data, SerializedProperty field, int index)
        {
                ...
                EditorTools.ValidateType(typePath)) // detect if field is blackboard data type
            {
                ...
                for (var j = 0; j < data.Count; j++)
                {
                    if (data[j] == null || data[j] is TreeVariable || data[j] is AIFSMVariable) continue;
                    target.Add(data[j].dataName);
                }
        ...
        {
            for (var i = 0; i < data.Count; i++) data[i].hideFlags = HideFlags.HideInInspector;
        }
