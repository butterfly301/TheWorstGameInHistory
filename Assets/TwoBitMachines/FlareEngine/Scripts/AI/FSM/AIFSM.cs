...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class AIFSM : AIBase
    {
        [SerializeField] public List<AIState> state = new();
        [SerializeField] public List<AIState> alwaysState = new();
        [SerializeField] public AIState resetState = new();
        [SerializeField] public bool resetToFirst;

        [NonSerialized] public AIState currentState;
        [NonSerialized] public AIState previousState;

        public override void Initialize()
        {
            if (state.Count > 0) currentState = state[0];
        }

        public override void Execute()
        {
            if (root == null || root.pause) return;
        ...
        private void RunStates()
        {
            ...
            if (currentState != null && root.RunFSM(currentState, position, ref velocity,
                    ref signals.characterDirection, out var nextState, true)) ChangeState(nextState);
            for (var i = 0; i < alwaysState.Count; i++)
                if (alwaysState[i].enabled)
                    root.RunFSM(alwaysState[i], position, ref velocity, ref signals.characterDirection, out var none,
                        false);
        }
        ...
        {
            if (currentState != null && currentState.cantInterrupt) return;
        ...
        {
            for (var i = 0; i < state.Count; i++) // change state
                if (state[i].stateName == stateName)
                {
                    if (currentState != null && currentState.stateName != stateName)
                        root.previousStateName = currentState.stateName;
        ...
        public override void ResetAI()
        {
            ...
            if (resetToActive && !gameObject.activeInHierarchy) gameObject.SetActive(true);
            ...
            root.RunFSM(resetState, transform.position, ref velocity, ref signals.characterDirection, out var none,
                true);
            transform.position += (Vector3)velocity * Time.deltaTime;

            if (state.Count > 0) currentState = state[0];
            for (var i = 0; i < alwaysState.Count; i++) alwaysState[i].Reset();
        }
    }
    ...
    {
        [SerializeField] public List<StateNode> action = new();
        [SerializeField] public string stateName = "State Name";
        [SerializeField] public string defaultSignal = "Signal";
        [SerializeField] public bool cantInterrupt;
        [SerializeField] public bool useSignal;
        [SerializeField] public bool enabled = true; // for always state only
        [SerializeField] public int childIndex;
        [SerializeField] public AIStateType type;

        public void Reset()
        {
            childIndex = 0;
            for (var i = 0; i < action.Count; i++) action[i].node.Reset(false, true);
        }

        public void ResetSkip()
        {
            childIndex = 0;
            for (var i = 0; i < action.Count; i++) action[i].node.Reset(true, true);
        }
...
#pragma warning disable 0414
        ...
        {
            for (var i = 0; i < action.Count; i++) action[i].node.hideFlags = HideFlags.HideInInspector;
        }
