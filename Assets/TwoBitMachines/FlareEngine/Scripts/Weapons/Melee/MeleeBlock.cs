...
#pragma warning restore 0414
        ...
        [NonSerialized] public bool needToRelease;
        [NonSerialized] private float recoilCounter;
        [NonSerialized] private int recoilDirection;
        ...
        public void Block(AnimationSignals signals, int direction, bool onGround, ref Vector2 velocity)
        {
            ...
            {
                if (velocity.x != 0) signals.ForceDirection((int)Mathf.Sign(velocity.x));
                velocity.x = 0;
            }

            if (health != null) health.block = canDeflect;
        ...
        {
            if (impact.attacker != null && recoilTag != "" && recoilTag != "Untagged" &&
                impact.attacker.gameObject.CompareTag(recoilTag))
            {
                isRecoiling = true;
                recoilCounter = 0;
        ...
        public void Recoil(AnimationSignals signals, ref Vector2 velocity)
        {
            ...
            if (isRecoiling)
            {
                ...
                if (Clock.Timer(ref recoilCounter, recoilDuration)) Reset();
            }
        }
    }
