...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class ChainedCombo
    {
        ...
        [SerializeField] public List<MeleeCombo> combo = new();
        ...
        [SerializeField] public bool cancelOtherAttacks;
        [NonSerialized] private bool blockOtherHits;
        [NonSerialized] private bool clearSignal;

        [NonSerialized] public Collider2D collider2DRef;
        [NonSerialized] private float earlyCounter;

        [NonSerialized] private ContactFilter2D filter;
        [NonSerialized] private bool firstFrame = true;
        [NonSerialized] private bool goToNextMelee;
        [NonSerialized] private int index;
        [NonSerialized] public bool inMelee;
        [NonSerialized] private float lateCounter;
        [NonSerialized] private List<Collider2D> list = new();
        [NonSerialized] private int originalDirection;
        [NonSerialized] private List<Transform> targetList = new();
        ...
        public bool Attack()
        {
            var mouseOverUI = EventSystem.current != null && EventSystem.current.IsPointerOverGameObject();
            if (mouseOverUI && EventSystem.current.currentSelectedGameObject != null &&
                EventSystem.current.currentSelectedGameObject.gameObject.CompareTag("UIControl"))
                mouseOverUI = false;
        ...
        public bool ExecuteMelee(AnimationSignals signals, int direction, bool onGround, bool crouching,
            Vector2 position, float coolDown, ref Vector2 velocity)
        {
            clearSignal = false;
            var isFirst = false;
            if (firstFrame)
            ...
            if (indexSafe && combo[index].exitOnStateChange && combo[index].condition != AttackCondition.Anytime &&
                combo[index].StateChanged(onGround, crouching)) return Reset();
            if (goToNextMelee &&
                SequenceFailed(isFirst || Attack() || earlyFlag, onGround, crouching,
                    coolDown)) // play after runmeleeattack so animation signals can clear for a least a frame or they might get stuck
                return true;
            if (clearSignal) return false;
            if (!goToNextMelee && indexSafe) RunMeleeAttack(signals, direction, onGround, position, ref velocity);
            return false;
        }

        private bool SequenceFailed(bool attack, bool onGround, bool crouching, float coolDown)
        {
            if (hitToContinue && indexSafe && !combo[index].readHitBox) return Reset();
            if (attack) //    go to next combo
            {
                ...
                if (indexSafe)
                {
                    var oldIndex = index;
                    if (combo[index]
                        .goToFirstAirAttack) // jump attack hit ground, reset to first, or it will move to the next air attack in sequence
                        for (var i = 0; i < combo.Count; i++)
                ...
                if (!GoToNextAttack(onGround, crouching, true)) return Reset();
                ...
                if (index >= combo.Count) // no more attacks
                    return Reset();
                if (previousIndex == index) clearSignal = true;
                ResetNext();
            }
            else if (combo.Count <= 1 || index >= combo.Count ||
                     Clock.Timer(ref lateCounter,
                         delayTime)) //  player did not press button in time to continue to next combo, exit
            {
                return Reset();
            }
        ...
        private bool GoToNextAttack(bool onGround, bool crouching, bool increment)
        {
            ...
            for (var i = index; i < combo.Count; i++)
        ...
        private void RunMeleeAttack(AnimationSignals signals, int direction, bool onGround, Vector2 position,
            ref Vector2 velocity)
        {
            combo[index].PlayAnimation(collider2DRef, signals, direction, originalDirection, onGround, position,
                ref velocity); // set animation signal and apply velocity

            if (Clock.TimerExpired(ref earlyCounter, earlyTime) && Attack()) combo[index].earlyFlag = true;
            if (combo[index].earlyNext && Clock.TimerExpired(ref combo[index].earlyCounter, combo[index].earlyTime) &&
                Attack()) // go to next attack early
            {
            ...
            {
                var hit = false;
                var size = collider2DRef.OverlapCollider(filter, list);
                for (var i = 0; i < size; i++)
                    if (!targetList.Contains(list[i].transform))
                    {
                        combo[index].recoil = true;
                        var newForceDirection = new Vector2(combo[index].forceDirection.x * direction,
                            combo[index].forceDirection.y);
                        if (Health.IncrementHealth(collider2DRef.transform, list[i].transform, -combo[index].damage,
                                newForceDirection))
                        {
                            hit = true;
                ...
                if (hit) combo[index].OnHit(collider2DRef, direction);
            }
        }
        ...
        {
            if (index >= 0 && index < combo.Count) combo[index].beginVelX = value;
        }

        public void BeginVelY(bool value)
        {
            if (index >= 0 && index < combo.Count) combo[index].beginVelY = value;
        }
...
#pragma warning restore 0414
    ...
    public class MeleeCombo
    {
        ...
        [NonSerialized] public bool alreadyJumped;
        [NonSerialized] public bool beginVelX;
        [NonSerialized] public bool beginVelY;
        [NonSerialized] public float earlyCounter;
        [NonSerialized] public bool earlyFlag;
        [NonSerialized] public bool firstFrame;
        [NonSerialized] public bool goToFirstAirAttack;
        [NonSerialized] public bool readHitBox;

        [NonSerialized] public bool recoil;
        [NonSerialized] public bool recoilYApplied;

        public bool isAirAttack => condition == AttackCondition.MustBeInAirOrExit ||
                                   condition == AttackCondition.MustBeInAirOrSkip;
        ...
        {
            onHit.Invoke(ImpactPacket.impact.Set(hitWE,
                collider.bounds.center + Vector3.right * collider.bounds.extents.x * direction,
                new Vector2(forceDirection.x * direction, forceDirection.y)));
        }

        public void PlayAnimation(Collider2D collider2D, AnimationSignals signals, int direction, int originalDirection,
            bool onGround, Vector2 position, ref Vector2 playerVelocity)
        {
            direction = keepInitialDirection ? originalDirection : direction;
            if (isAirAttack && onGround) goToFirstAirAttack = true;
            if (firstFrame)
            {
                firstFrame = false;
                var p = collider2D != null ? (Vector2)collider2D.bounds.center : position;
                onMeleeBegin.Invoke(ImpactPacket.impact.Set(meleeWE, p,
                    new Vector2(forceDirection.x * direction, forceDirection.y)));
            }

            signals.Set("meleeCombo");
            signals.Set(animationSignal);
        ...
        public void ApplyVelocity(AnimationSignals signals, bool onGround, ref Vector2 playerVelocity, int direction)
        {
            if (velXType != MeleeVelocityX.NoVelocity)
            {
                ...
                else if (beginVelX) playerVelocity.x = velX * direction;
            }
        ...
        {
            if ((condition == AttackCondition.MustBeInAirOrExit || condition == AttackCondition.MustBeInAirOrSkip) &&
                (onGround || crouching))
                return true;
            if ((condition == AttackCondition.MustBeOnGroundOrExit ||
                 condition == AttackCondition.MustBeOnGroundOrSkip) && (!onGround || crouching))
                return true;
            if ((condition == AttackCondition.MustBeCrouchingOrExit ||
                 condition == AttackCondition.MustBeCrouchingOrSkip) && (!onGround || !crouching))
                return true;
            return false;
        }
