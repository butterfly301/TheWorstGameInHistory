...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class Melee : MonoBehaviour
    {
        ...
        [SerializeField] public ChainedCombo melee = new();
        [SerializeField] public MeleeBlock block = new();
        ...
        [NonSerialized] private ThePlayer.Melee meleeRef;
        ...
        {
            if (attackNow) melee.input.ButtonPressed();
            if (attackNow) melee.input2.ButtonPressed();
            attackNow = false;

            if (pause) return false;

            FlipCollider(character.signals
                .characterDirection); // check to flip every frame in case there is a visual element to it
            if (!gameObject.activeInHierarchy) return false;
            var isBlocking = block.IsBlocking();
            if (!isBlocking && !melee.inMelee && collider2DRef != null && collider2DRef.enabled)
                collider2DRef.enabled = false;
            if (block.needToRelease && block.input.Released()) block.needToRelease = false;
            if (Clock.TimerInverseExpired(ref coolDownCounter, coolDown))
            {
                if (coolDown > 0) onCoolDown.Invoke(Mathf.Clamp(coolDownCounter / coolDown, 0, 1f));
                return false;
            }

            if (coolDown > 0) onCoolDown.Invoke(1f);
            return inMelee || isBlocking || (!block.needToRelease && !inputs.block && melee.Attack());
        }

        public void Execute(AnimationSignals signals, int direction, bool onGround, bool crouching, Vector2 position,
            ref Vector2 velocity)
        {
            if (block.canBlock && block.IsBlocking())
            {
                if (melee.inMelee && block.cancelCombo) melee.Reset();
                if (!melee.inMelee)
                {
            ...
            if (coolDownCounter >= coolDown && melee.ExecuteMelee(signals, direction, onGround, crouching, position,
                    coolDown, ref velocity)) ResetAll();
        }

        public void FlipCollider(float playerDirection)
        {
            transform.localPosition =
                Util.FlipXSign(transform.localPosition, playerDirection); // change weapon position x depending on side
        ...
        {
            for (var i = 0; i < melee.combo.Count; i++)
