...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class ProjectileShortRange : ProjectileBase
    {
        ...
        [NonSerialized] private ContactFilter2D contactFilter;
        [NonSerialized] private readonly List<Collider2D> contactResults = new();
        [NonSerialized] public Transform currentFirePoint;
        [NonSerialized] public Quaternion currentRotation;
        [NonSerialized] public bool deactivateActive;
        [NonSerialized] public float deactivateCounter;
        [NonSerialized] private FlameThrower flame;

        [NonSerialized] public float hitCounter;
        [NonSerialized] public SpriteEngine spriteEngine;
        [NonSerialized] public bool wasActivated;
        ...
        public void Start()
        {
            ...
            flame = gameObject.GetComponent<FlameThrower>();
        ...
        public override void Execute()
        {
            if (deactivateActive)
            {
                if (spriteEngine != null) spriteEngine.SetSignal("deactivateTimerOn");
            ...
            if (wasActivated && !activated && gameObject.activeInHierarchy && release != OnTriggerRelease.LeaveAsIs)
            {
        ...
        public override bool FireProjectile(Transform transform, Quaternion rotation, Vector2 playerVelocity)
        {
            ...
            if (!ammunition.Consume(pattern.projectileRate, inventory)) return false;
        ...
        public void AreaScan(Vector2 direction)
        {
            ...
            {
                var hits = boxCollider.OverlapCollider(contactFilter, contactResults);
                for (var i = 0; i < hits; i++)
                    if (contactResults[i] != null)
                        Health.IncrementHealth(transform, contactResults[i].transform, -damage,
                            direction * damageForce);
            }
        }
    }
}