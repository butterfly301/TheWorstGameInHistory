...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        public static List<ProjectileBase> projectiles = new();
        [SerializeField] public string projectileName;
        [SerializeField] public float fireRate;
        [SerializeField] public float damage = 1f;
        [SerializeField] public float damageForce = 1f;
        [SerializeField] public Ammunition ammunition = new();
        [SerializeField] public Pattern pattern = new();
        [NonSerialized] public ProjectileInventory inventory;

        [NonSerialized] public Vector2 playerVelocity;
        [NonSerialized] public bool triggerReleased = false;

        private void OnEnable()
        {
            if (!projectiles.Contains(this)) projectiles.Add(this);
            OnEnableNow();
        }

        private void OnDisable()
        {
            if (projectiles.Contains(this)) projectiles.Remove(this);
            OnDisableNow();
        }
        ...
        public void ChangeAmmo(ItemEventData itemEventData)
        {
            ...
            if (itemEventData.genericFloat > 0) ammunition.onAmmoReload.Invoke();
