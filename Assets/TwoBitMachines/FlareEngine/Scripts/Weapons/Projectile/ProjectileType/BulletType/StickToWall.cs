...
namespace TwoBitMachines.FlareEngine.BulletType
{
    ...
    public class StickToWall : BulletBase
    {
        ...
        [NonSerialized] private bool isStickingToWall;
        [NonSerialized] private Vector2 previousStickPosition;

        [NonSerialized] private float stickCounter;
        [NonSerialized] private Transform stickTransform;
        ...
        public override void Execute()
        {
            ...
            if (isStickingToWall)
            {
                ...
                {
                    var movement = (Vector2)stickTransform.position - previousStickPosition; // move holdpoint
                    previousStickPosition = stickTransform.position;
                    position += movement;
                }

                if (Clock.Timer(ref stickCounter, stickTimer) || stickTransform == null ||
                    !stickTransform.gameObject.activeInHierarchy)
                {
                    BlastRadius();
                    ReadyToSleep();
        ...
        public override bool CastRay(Vector2 origin, Vector2 velocityNormal, float velMagnitude, float size)
        {
            ...
            if (ray.distance == 0) return DealDamage(ray.transform, position, velocityNormal);
            ...
            if (Health.IncrementHealth(transform, ray.transform, -damage, velocityNormal * damageForce))
            {
