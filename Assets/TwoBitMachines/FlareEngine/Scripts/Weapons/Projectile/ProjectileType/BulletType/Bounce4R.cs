...
namespace TwoBitMachines.FlareEngine.BulletType
{
    ...
    public class Bounce4R : BulletBase
    {
        ...
        [NonSerialized] private float dissipate = 1;
        ...
        public override void Execute()
        {
            ...
            LifeSpanTimer();
            ApplyGravity(0.5f);
            if (bounceSpin == 0) ApplyRotation(transform);

            Vector3 actualVel = velocity * Time.deltaTime;
            var magnitude = actualVel.magnitude;
            var normal = actualVel / magnitude;
            Collision(position, Compute.Abs(velocity), normal, magnitude * Mathf.Abs(normal.x),
                magnitude * Mathf.Abs(normal.y), dissipate);
            transform.position = position;
            transform.rotation = rotation;
        }

        private void Collision(Vector3 origin, Vector3 originVel, Vector3 normal, float magnitudeX, float magnitudeY,
            float originDissipate)
        {
            var appliedVelocity = velocity;

            if (velocity.x != 0)
                for (var i = -1; i < 2; i += 2) // -1, and 1
                {
                    Vector2 corner = origin + Vector3.up * (bounceRadius - 0.01f) * i;
                    var ray = Physics2D.Raycast(corner, Vector2.right * Mathf.Sign(normal.x), bounceRadius + magnitudeX,
                        layer);
                    ...
                    if (WorldManager.viewDebugger)
                        Debug.DrawRay(corner, Vector2.right * Mathf.Sign(normal.x) * (bounceRadius + magnitudeX),
                            Color.green);
#endif
                    ...
                    if (ray && !IgnoreEdges(ray.collider))
                    {
                        ...
                        {
                            dissipate = originDissipate < 0.1f
                                ? 0f
                                : originDissipate * (1f - bounceFriction); // So it doesn't get applied twice
            ...
            if (velocity.y != 0)
            {
                ...
                for (var i = -1; i < 2; i += 2)
                {
                    Vector2 corner = origin + Vector3.right * (bounceRadius - 0.01f) * i +
                                     Vector3.right * appliedVelocity.x * Time.deltaTime;
                    var ray = Physics2D.Raycast(corner, Vector2.up * Mathf.Sign(normal.y), bounceRadius + magnitudeY,
                        layer);
...
#if UNITY_EDITOR
                    if (WorldManager.viewDebugger)
                        Debug.DrawRay(corner, Vector2.up * Mathf.Sign(normal.y) * (bounceRadius + magnitudeX),
                            Color.red);
#endif
