...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class ProjectileInstant : ProjectileBase
    {
        ...
        [NonSerialized] public float aliveCounter;
        ...
        {
            if (!ammunition.Consume(1, inventory)) return false;
...
#if UNITY_EDITOR
                    if (WorldManager.viewDebugger) Draw.GLCircleInit(ray.point, 0.25f, Color.red);
#endif
        ...
        private void TargetFound(Transform transform, Vector2 impactPoint, Vector2 direction)
        {
            if (Clock.Timer(ref hitCounter, hitRate))
                ...
                {
                    var impact = ImpactPacket.impact.Set(impactEffect, impactPoint, direction);
