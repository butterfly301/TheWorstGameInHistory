...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class WaitForAnimation
    {
        ...
        [NonSerialized] private AnimState state = AnimState.Ready;
            ...
            switch (state)
            {
                case AnimState.Ready:
                    if (fire)
                    {
                        ...
                        if (needToClear)
                            ClearSignal(
                                signals); // animation can get stuck if you shoot on last frame of animation since animation never gets reset and stays in loop once
                        needToClear = false;
                        return false;
                    }
        ...
        {
            if (state == AnimState.Ready) state = AnimState.Block;
        }
        ...
        public void
            ShootAndAnimationComplete() // charging state should really only use ShootAndAC or else animation will block discharge
        {
        ...
        private void SetSignal(AnimationSignals signals)
        {
            ...
            if (spriteEngine != null) spriteEngine.SetSignal(extraSignal);
        }

        private void ClearSignal(AnimationSignals signals)
        {
            signals.Set(weaponAnimation, false);
            if (spriteEngine != null) spriteEngine.SetSignalFalse(extraSignal);
        }
    }
}