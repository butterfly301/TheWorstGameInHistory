...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class Recoil
    {
        ...
        [SerializeField] public bool recoil;
...
#pragma warning restore 0414
        ...
        [NonSerialized] public bool canRecoil = true;
        [NonSerialized] private Vector2 characterDown;
        [NonSerialized] private bool isRecoiling;
        [NonSerialized] private bool offsetX;
        [NonSerialized] private float recoilCounter;
        [NonSerialized] private bool startRecoil;
        [NonSerialized] private Vector2 velOffset;
        [NonSerialized] private Vector2 weaponDirection;
        ...
        public void
            Execute(ref Vector2 velocity,
                AnimationSignals signals) // the amount of velocity applied to the character during recoil
        {
        ...
        private void RecoilShake(ref Vector2 velocity, AnimationSignals signals)
        {
            ...
            if (startRecoil)
            {
                ...
                signals.Set("recoilShake");
            }
            else if (Clock.Timer(ref recoilCounter, recoilTime)) // move character forward
            {
                var appliedForce = recoilDistance / Time.deltaTime * weaponDirection;
        ...
        private void RecoilSlideBack(ref Vector2 velocity, AnimationSignals signals)
        {
            ...
            if (hasGravity)
            {
                if (Vector2.Angle(weaponDirection, characterDown) <
                    35f) appliedForce.y *= 0.25f; //                   prevent player from flying via upward thrust
                velocity.x = -appliedForce.x;
                velocity.y -=
                    velocity.y > 0
                        ? 0
                        : appliedForce.y; // don't apply if character is moving up, helps prevent charcter from flying
            }
            else
            {
