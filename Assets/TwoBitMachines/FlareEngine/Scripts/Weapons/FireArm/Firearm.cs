...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class Firearm : Tool
    {
        ...
        [SerializeField] public DefaultProjectile defaultProjectile = new();
        [SerializeField] public ChargedProjectile chargedProjectile = new();
        [SerializeField] public LineOfSight lineOfSight = new();
        [SerializeField] public RotateWeapon rotate = new();
        ...
        [NonSerialized] private bool canStop;

        [NonSerialized] private bool fireNow;
        [NonSerialized] private float startStopCount;

        private void LateUpdate()
        {
            var externalShoot = fireNow;
            fireNow = false;
            if (pause || !EquipToolToParent()) return;
            if (firePoint == null || input == null || Time.timeScale == 0) return;

            transform.localPosition = localPosition;
            var mouseOverUI = MouseOverUI(input);
            var fire = (!mouseOverUI && input.Active(buttonTrigger) && HasParent()) || externalShoot;

            rotate.Execute(this, equipment, ref fire);
            lineOfSight.Sight(transform, firePoint, ref fire);

            if (chargedProjectile.Charge(mouseOverUI, input,
                    equipment.signals)) //               charge weapon will take priority if it exists and is active
        ...
        public override void ToolOnEnable()
        {
            ...
            {
                var fire = false;
                rotate.Execute(this, equipment,
                    ref fire); //                                  when firearm is toggled by inventory, set rotation immediately so it doesn't look glitchy.
            }
        }
        ...
        {
            if (projectile != null && Clock.TimerExpired(ref counter, projectile.fireRate) && fire &&
                CanUseWeapon(projectile))
                if (projectile.FireProjectile(firePoint, transform.rotation, equipment.setVelocity + recoil.Offset()))
                {
                    counter = 0;
                    defaultProjectile.onFireSuccess.Invoke(ImpactPacket.impact.Set("", firePoint.position,
        ...
        private bool CanUseWeapon(ProjectileBase projectile)
        {
            ...
            if (turnOffNearWall)
            {
                var hit = Physics2D.Linecast(transform.position, firePoint.position, WorldManager.collisionMask);
                if (hit && !(hit.collider is EdgeCollider2D)) return false;
            }
        ...
        public override void LateExecute(AbilityManager player, ref Vector2 velocity)
        {
            if (stopVelocity && player.world.onGround)
            {
                if (stopType == StopType.OnShoot && canStop)
                {
                    ...
                    if (Clock.Timer(ref startStopCount, stopTime)) canStop = false;
                }

                if (stopType == StopType.Always)
                {
                    if (velocity.x != 0) player.signals.ForceDirection((int)Mathf.Sign(velocity.x));
                    velocity.x = 0;
                    velocity.y = velocity.y > 0 ? 0 : velocity.y;
                }
            }
        ...
        {
            if (defaultProjectile.projectile != null) defaultProjectile.projectile.ChangeAmmo(value);
        }
