...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        [NonSerialized] private float highFrequencyMotor;
        [NonSerialized] private float highStep;
        [NonSerialized] private bool isMotorActive;

        [NonSerialized] private float lowFrequencyMotor; // 0f off, 1f max

        [NonSerialized] private float lowStep;
        [NonSerialized] private PlayerInput playerInput;
        [NonSerialized] private float pulseDuration;

        [NonSerialized] private float rumbleDuration;
        [NonSerialized] private float rumbleStep;
        [NonSerialized] private RumblePattern rumbleType;
        ...
        private Gamepad GetGamepad()
        {
            for (var i = 0; i < Gamepad.all.Count; i++)
            {
                ...
                for (var j = 0; j < playerInput.devices.Count; j++)
