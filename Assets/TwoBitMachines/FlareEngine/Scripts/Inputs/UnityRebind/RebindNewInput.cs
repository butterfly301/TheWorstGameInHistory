...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
        ...
        public class RebindNewInput : MonoBehaviour
    {
        ...
        [Tooltip("The binding label.")] [SerializeField]
        private TextMeshProUGUI m_BindingLabel;

        [Tooltip("Override button label.")] [SerializeField]
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            {
                currentIndex++;
                var currentBinding = action.actionMap.bindings[currentIndex];
                ...
                for (var i = 0; i < otherAction.bindings.Count; i++)
                {
                    var binding = otherAction.bindings[i];
                    if (binding.overridePath == newBinding.path) otherAction.ApplyBindingOverride(i, oldOverridePath);
                }
            }
        }
        ...
        public void CancelKeyRebind()
        {
            ...
            actionTemp?.Enable();
            m_RebindStopEvent?.Invoke(this, ongoingRebind);
            UpdateBindingDisplay();
        }
        ...
        private void UpdateButtonLabel()
        {
            ...
            {
                var action = m_Action?.action;
                if (action != null && overrideButtonLabel) m_ButtonLabel.text = action.name;
            }
        }
        ...
        [Serializable]
        public class
            InteractiveRebindEvent : UnityEvent<RebindNewInput, InputActionRebindingExtensions.RebindingOperation>
        {
        }
    }
}