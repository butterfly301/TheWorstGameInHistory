...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        public static List<Journal> journalList = new();
        ...
        [SerializeField] public List<JournalInventory> inventory = new();
        [NonSerialized] private bool initialized;
        [NonSerialized] private SaveJournal journal = new();

        [NonSerialized] private int rangeIndex;
        [NonSerialized] private readonly List<JournalSlot> slots = new();
...
#pragma warning restore 0414
        ...
        {
            for (var i = 0; i < slots.Count; i++) slots[i].RefreshCompleteStatus();
        }
        ...
        {
            journalList.Clear();
            for (var i = 0; i < parent.childCount; i++) parent.GetChild(i).GetComponent<Journal>()?.AddToSystem();
        }

        public static void AddToJournal(JournalObject journalObject, string journalName)
        {
            if (journalObject == null) return;
            for (var i = 0; i < journalList.Count; i++)
        ...
        private void Restore()
        {
            ...
            initialized = true;
            journal = Storage.Load(journal, WorldManager.saveFolder, saveKey);
        ...
        public void DeleteSavedData()
        {
            ...
            for (var i = 0; i < slots.Count; i++) slots[i].gameObject.SetActive(false);
            slots.Clear();
            journal.item.Clear();
            journal.index = 0;
            Save();
        ...
        {
            if (gameReset) gameObject.SetActive(false);
            if (!gameObject.activeInHierarchy) return;
            if (highlight != null && slots.Count > 0 && journal.index < slots.Count)
            {
                if (!highlight.gameObject.activeInHierarchy) highlight.gameObject.SetActive(true);
                highlight.anchoredPosition =
                    slots[journal.index].gameObject.GetComponent<RectTransform>().anchoredPosition + highlightOffset;
            }
        }
        ...
        private void RangeView()
        {
            if (hasRange)
            {
                ...
                for (var i = 0; i < slots.Count; i++) slots[i].gameObject.SetActive(Compute.IndexInListRange(i));
            }
        }
        ...
        public void CreateSlot(JournalObject newObject)
        {
            if (slot != null)
            {
                var newSlot = Instantiate(slot, Vector3.zero, Quaternion.identity, slotParent);
        ...
        {
            if (toggle != null && toggle.Pressed()) gameObject.SetActive(!gameObject.activeInHierarchy);
        }

        private void LateUpdate()
        {
            if (back != null && back.Pressed()) Back();
            if (forward != null && forward.Pressed()) Forward();
    ...
    public class SaveJournal
    {
        ...
        [SerializeField] public List<string> item = new();
        ...
        {
            if (item.Contains(journalObject.Name())) return true;
            return false;
        }
        ...
        public void CreateSlots(Journal journal)
        {
            ...
            for (var i = 0; i < journal.inventory.Count; i++)
                if (journal.inventory[i] is InventorySO)
                {
                    var list = journal.inventory[i].ItemList();
                ...
                {
                    var list = journal.inventory[i].QuestList();
                    for (var j = 0; j < list.Count; j++)
                        if (list[j] != null && item.Contains(list[j].Name()))
                            journal.CreateSlot(list[j]);
                }
