...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class WorldCollision
    {
        ...
        [SerializeField] public bool useMovingPlatformMomentum = true;
        ...
        [SerializeField] public BoxInfo box = new();
        [SerializeField] public RotateToType rotateTo;
        [NonSerialized] public BoxCollider2D boxCollider;
        [NonSerialized] public Dictionary<int, PlankState> bridge = new();
        [NonSerialized] public bool climbingSlopeDown;
        [NonSerialized] public bool climbingSlopeUp;
        [NonSerialized] public LayerMask collisionLayer;

        [NonSerialized] public Vector2 groundNormal;
        [NonSerialized] public bool hasJumped;
        [NonSerialized] public Health health;
        [NonSerialized] public bool healthInitialized;
        [NonSerialized] public bool hitInteractable;
        [NonSerialized] public bool holdingDown;
        [NonSerialized] public bool holdingJump;
        [NonSerialized] public bool ignoreYDown;
        [NonSerialized] public bool interactableWasHit;
        [NonSerialized] public bool isHiding;
        [NonSerialized] public bool isHidingExternal;
        [NonSerialized] public bool isReversed;

        [NonSerialized] private Vector2 jumpPoint;
        [NonSerialized] public bool leftWall;
        [NonSerialized] public bool missedAHorizontal;
        [NonSerialized] public bool missedAVertical;
        [NonSerialized] public MovingPlatformInfo mp = new();
        [NonSerialized] public Vector2 oldPosition;
        [NonSerialized] public Vector2 oldVelocity;
        [NonSerialized] public bool onBridge;
        [NonSerialized] public bool onCeiling;
        [NonSerialized] public bool onGround;
        [NonSerialized] public bool onSpikeX;
        [NonSerialized] public bool onSpikeY;
        [NonSerialized] public int originLayer;

        [NonSerialized] public bool pressedDown;
        [NonSerialized] public bool rightWall;

        [NonSerialized] public Transform transform;
        [NonSerialized] public Collider2D verticalCollider;
        [NonSerialized] public Transform verticalTransform;
        [NonSerialized] public Collider2D wallCollider;

        [NonSerialized] public Vector2 wallNormal;
        [NonSerialized] public Transform wallTransform;
        [NonSerialized] public bool wasHiding;
        [NonSerialized] public bool wasOnBridge;
        [NonSerialized] public bool wasOnCeiling;
        [NonSerialized] public bool wasOnGround;
        [NonSerialized] public bool wasOnPeak;
        [NonSerialized] public bool wasOnSlope;
        [NonSerialized] public bool wasOnSlopeUp;
        ...
        private void HorizontalCollision(ref Vector2 velocity, bool climbSlopes, bool stopSlopeMovement = true)
        {
            ...
            var magnitude = Mathf.Abs(velocity.x) + box.skin.x * 2f;
            var corner =
                signX > 0
                    ? box.bottomRight - box.skinX
                    : box.bottomLeft +
                      box.skinX; // x ray starts from inside collider, but skin stops outside of collider to allow y ray to start on collider edge,  to use perfect corners

            for (var i = 0; i < box.rays.x; i++)
            {
                var origin = corner + box.up * box.spacing.y * i;
                var hit = Physics2D.Raycast(origin, box.right * signX, magnitude, collisionLayer);

                #region Debug

#if UNITY_EDITOR
                if (WorldManager.viewDebugger) Debug.DrawRay(origin, box.right * signX * magnitude, Color.magenta);
#endif
                ...
                if (i == 0 && climbSlopes && (FindSlopeToClimbUp(ref velocity, hit, out var isJumping) || isJumping))
                {
                    if (isJumping)
                ...
                if (stopSlopeMovement &&
                    Vector2.Angle(hit.normal, box.up) <=
                    89.95f) // if on a sloped edge, stop x movement, since player can still move the corner into a slope
                {
                    velocity.x = 0;
                    SetWall(hit, signX);
                    return;
                ...
                magnitude +=
                    0.0001f; //                                                  add 0.0001f for next ray, or it will think it missed a horizontal
            }
        }
        ...
        private void VerticalCollision(ref Vector2 velocity, bool skip = false)
        {
            ...
            var magnitude = Mathf.Abs(velocity.y) + box.skin.y;
            var clampMag = magnitude;
            var corner = sign > 0 ? box.topLeft : box.bottomLeft;
            for (var i = 0; i < box.rays.y; i++)
            {
                var origin = corner + box.right * (box.spacing.x * i + velocity.x);
                var hit = Physics2D.Raycast(origin, box.up * sign, magnitude, collisionLayer);

                #region Debug

#if UNITY_EDITOR
                if (WorldManager.viewDebugger) Debug.DrawRay(origin, box.up * sign * magnitude, Color.yellow);
#endif
                ...
                if (ignoreYDown && sign < 0 && hit.distance == 0) continue;

                hit.distance =
                    Mathf.Min(hit.distance,
                        clampMag); //           fix missed a vertical issue, we only apply the smallest hit distance
                clampMag = hit.distance;
                magnitude = Mathf.Max(hit.distance,
                    box.skin.y + 0.0001f); //  add 0.0001f for next ray, or it will think it missed a vertical
                velocity.y = (hit.distance - box.skin.y) * sign;
                onGround = sign <= 0 ||
                           onGround; //                            since slope climb Up can set onGround true, keep true
        ...
        {
            if (sign > 0 && (holdingJump || jumpThroughEdge)) return !edge.CompareTag("Edge2DDownOnly");
            if (sign < 0 && ((skipDownEdge == Edge2DSkip.OnDownHold && holdingDown) ||
                             (skipDownEdge == Edge2DSkip.OnDownAndJump && pressedDown)))
        ...
        private bool FindSlopeToClimbUp(ref Vector2 velocity, RaycastHit2D slope, out bool isJumping)
        {
            isJumping = false;
            if (slope.distance <= 0 ||
                !Compute.Between(Compute.Angle(slope.normal, box.up, out var slopeAngle), 0, maxSlopeAngle))
                return false;
            if (!ValidateSlope(ref velocity, velocity.x.Abs(), slopeAngle, slope, ref isJumping)) return false;

            DiagonalClimbUp(ref velocity, box.BottomExactCorner(velocity.x));
            return true;
        }

        private bool ValidateSlope(ref Vector2 velocity, float magnitude, float slopeAngle, RaycastHit2D slope,
            ref bool isJumping)
        {
            var climbVelocity = Vector2.zero;
            var corner = box.BottomExactCorner(velocity.x) + box.up * 0.001f;

            for (var i = 0; i < 25; i++) // loop for organic slopes, most checks between 1-3
            {
                var slopeDirection = slope.normal.Rotate(90f * -Mathf.Sign(velocity.x));
                var newVelocity = Compute.LineCircleIntersect(corner, magnitude, slope.point, slopeDirection) - corner;
                climbVelocity = transform.InverseTransformVector(newVelocity);
                var hit = Physics2D.Raycast(corner, newVelocity.normalized, magnitude + 0.001f, collisionLayer);
                //Debug.DrawRay (corner, newVelocity.normalized * (magnitude + 0.001f), Color.red);

                if (hit && hit.distance > 0 && Compute.Angle(hit.normal, box.up, out var newAngle) > slopeAngle &&
                    newAngle <= maxSlopeAngle)
                {
                    slopeAngle = newAngle;
                    slope = hit;
                    continue;
        ...
        private void DiagonalClimbUp(ref Vector2 velocity, Vector2 corner)
        {
            ...
            var normal = velocity / magnitude;
            Vector2 direction = transform.InverseTransformVector(normal);
            magnitude += box.skin.x;

            for (var i = 1; i < box.rays.x; i++) // skip first since  already tested 
            {
                var origin = corner + box.up * (box.spacing.y * i + box.skin.y * 2f);
                var hit = Physics2D.Raycast(origin, direction, magnitude, collisionLayer);

                #region Debug

#if UNITY_EDITOR
                if (WorldManager.viewDebugger) Debug.DrawRay(origin, direction * magnitude, Color.black);
#endif
        ...
        {
            if (!climbSlopes || !wasOnGround || velocity.x == 0 || velocity.y >= 0) return;

            var corner = box.BottomCorner(-velocity.x);
            var magnitude = Mathf.Abs(velocity.x) + box.skin.y;
            var slope = Physics2D.Raycast(corner, box.down, magnitude, collisionLayer);

            if (slope && slope.distance > 0 && Compute.Between(Compute.Angle(box.up, slope.normal, out var slopeAngle),
                    0, maxSlopeAngle))
                if (Compute.Dot(box.right * Mathf.Sign(velocity.x), slope.normal) >
                    0) //      only  climb if pointing in the same direction
                {
        ...
        {
            if (!climbSlopes || velocity.x == 0 || onCeiling) return;
            if (climbingSlopeUp)
        ...
        private void FindWall(Vector2 origin, float magnitude, float signX)
        {
            for (var i = 0; i < 3; i++)
            {
                var direction = (box.right * -signX).Rotate(10f * i * signX);
                var slope = Physics2D.Raycast(origin, direction, magnitude + 0.1f + 0.1f * i + box.sizeX,
                    collisionLayer); //    Debug.DrawRay (origin, direction * (magnitude + 0.1f + 0.1f * i + box.sizeX), Color.black);

                if (!slope) continue;
                if (slope.distance > 0 && Vector2.Angle(slope.normal, box.up) <= maxSlopeAngle &&
                    slope.normal != groundNormal) Transition(magnitude, signX);
                return;
            }
        }
        ...
        {
            if (ModeDown(-velocity)) return;
            if (ModeToSide(velocity * -signX)) onGround = true;
        }

        private bool ModeDown(float velocity)
        {
            var velY = new Vector2(0, velocity);
        ...
        private void RotateToSlope(Vector2 velocity, float deltaTime)
        {
            ...
            if (onGround && (missedAVertical || climbingSlopeUp || box.angle != 0))
            {
                ...
                var left = Physics2D.Raycast(box.BottomCorner(-1) + box.skinY, box.down, box.size.x + box.skin.y * 2f,
                    collisionLayer);
                var right = Physics2D.Raycast(box.BottomCorner(1) + box.skinY, box.down, box.size.x + box.skin.y * 2f,
                    collisionLayer);

                if (rotateToWall && left.Valid() && !right.Valid()) // detect spikes
                    right = Physics2D.Raycast(box.BottomCorner(1) - box.up * box.size.x * 0.5f, -box.right, box.size.x,
                        collisionLayer);
                if (rotateToWall && right.Valid() && !left.Valid())
                    left = Physics2D.Raycast(box.BottomCorner(-1) - box.up * box.size.x * 0.5f, box.right, box.size.x,
                        collisionLayer);
            ...
            {
                var rotate = rotateTo == RotateToType.RotateUp ? Vector2.up : Util.GetNearest90Normal(box.up);
                if (wasOnGround)
                {
                    var sign = -box.up.CrossSign(rotate);
        ...
        {
            var angle = Vector2.Angle(box.up, slopeNormal);
            var maxAngle = Mathf.Clamp(angle * deltaTime * rotateRate * 10f, 0, angle);
            ...
            var size = new Vector2(box.size.x, box.sizeY * 0.8f);
            var origin = box.center + box.up * box.sizeY * 0.10f;
        ...
        private void HorizontalSpikeCheck(ref Vector2 velocity)
        {
            ...
            {
                var skin = box.size.x * 0.25f;
                var sign = Mathf.Sign(velocity.x);
                var magnitude = Mathf.Abs(velocity.x) + skin;
                var direction = box.right * sign;
                var size = new Vector2(box.size.x * 0.5f, box.size.y);
                var origin = box.center + (climbingSlopeUp ? box.up * velocity.y : Vector2.zero);
                var hit = Physics2D.BoxCast(origin, size, box.angle, direction, magnitude, collisionLayer);
        ...
        private void VerticalSpikeCheck(ref Vector2 velocity, bool force = false)
        {
            ...
            {
                var skin = box.sizeY * 0.25f;
                var magnitude = Mathf.Abs(velocity.y) + skin;
                var size = new Vector2(box.size.x - 0.025f, box.sizeY * 0.5f); // -0.02 minus contact offset
                var origin = box.center + box.right * velocity.x;
                var hit = Physics2D.BoxCast(origin, size, box.angle, box.down, magnitude, collisionLayer);
        ...
        {
            if (useMovingPlatform && useMovingPlatformMomentum && onGround && velocity.y > 0)
            {
