...
namespace TwoBitMachines.FlareEngine
{
    public class MovingPlatformInfo
    {
        ...
        [NonSerialized] public MPFollowType follow;
        [NonSerialized] public bool ignoreBlockCollision;
        [NonSerialized] public LatchMPType latch;
        [NonSerialized] public Vector2 launch;
        [NonSerialized] public bool launching;
        [NonSerialized] public MovingPlatform mp;

        [NonSerialized] public MovingPlatform mpJump;
        [NonSerialized] public Vector2 pushed;
        [NonSerialized] public Vector2 pushOutVelocity;
        ...
        {
            if (transform != null &&
                !transform.gameObject.CompareTag("Block")) // not a block, crushed by moving platform
                world.onCrushed.Invoke(ImpactPacket.impact.Set(world.crushedWE, transform, world.boxCollider,
                    world.position, null, Vector2.zero, world.box.directionX, 0));
            if (transform != null && transform.gameObject.CompareTag("Block")) ignoreBlockCollision = true;
        }

        public Vector2 LaunchVelocity()
        {
            return follow == MPFollowType.Follow && mpJump != null
                ? mpJump.velocity * Time.deltaTime
