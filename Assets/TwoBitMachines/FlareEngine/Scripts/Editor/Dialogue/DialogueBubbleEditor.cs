    ...
    public class DialogueBubbleEditor : UnityEditor.Editor
    {
        ...
        public void Settings()
        {
            if (FoldOut.Bar(parent).Label("Settings").FoldOut("settingsFoldOut"))
            {
                ...
                {
                    Fields.EventFoldOut(parent.Get("onEnter"), parent.Get("onEnterFoldOut"), "On Begin",
        ...
        public void Messengers()
        {
                ...
                .FoldOut("messengerFoldOut"))
            {
                ...
                for (var i = 0; i < messengers.arraySize; i++)
                {
                    ...
                    {
                        var bubbles = element.Get("bubbles");
                        if (bubbles.arraySize == 0) bubbles.arraySize++;

                        FoldOut.Box(1 + bubbles.arraySize, FoldOut.boxColor, offsetY: -2);
                        {
                            element.Field("Bubble", "bubble");

                            for (var j = 0; j < bubbles.arraySize; j++)
                ...
                if (Bar.FoldOpen(branch.Get("foldOut")))
                {
                    for (var j = 0; j < messages.arraySize; j++)
                    {
                        ...
                        if (
                            ...
                            .RightButton("options", "Reopen", "Options")
                            .BBR("Delete")
                        )
                        {
                        ...
                        if (message.Bool("options"))
                        {
                            ...
                            Fields.EventFoldOut(message.Get("onMessage"), message.Get("eventFoldOut"),
                                "On Message Complete", offsetY: -1, color: color);
                        }
