        ...
        public override void OnInspectorGUI()
        {
            ...
            parent.Update();
            {
                if (FoldOut.Bar(parent, Tint.Orange).Label("References").FoldOut("referenceFoldOut"))
                {
                    ...
                    {
                        var choices = parent.Get("choices");
                        var oldValue = Labels.useWhite;
                        Labels.useWhite = true;
                        if (parent.ReadBool("addOption")) choices.arraySize++;
                ...
                if (FoldOut.Bar(parent).Label("Events").FoldOut("eventsFoldOut"))
                {
                    ...
                    Fields.EventFoldOut(parent.Get("transitionOut"), parent.Get("outFoldOut"), "On Transition Out",
                        color: FoldOut.boxColor);
                    Fields.EventFoldOut(parent.Get("loadMessage"), parent.Get("loadFoldOut"), "On Load Message",
                        color: FoldOut.boxColor);
                }
            }
