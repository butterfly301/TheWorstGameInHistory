...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class InventoryEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var showItems = (ShowItems)slot.Enum("viewItems");

            FoldOut.Box(3, Tint.BoxTwo);
            {
                parent.Field("InventorySO",
                    "inventorySO"); //                                         unique name for saving
                slot.Field("View Items", "viewItems", showItems != ShowItems.KeyName);
                slot.FieldDouble("View Items", "viewItems", "keyName", showItems == ShowItems.KeyName);
            ...
            if (parent.Enum("navigation") > 0)
            {
                ...
                Labels.FieldText("Slot Index", 18);
            ...
            {
                Fields.EventFoldOut(parent.Get("onOpen"), parent.Get("onOpenFoldOut"), "On Open",
                    color: FoldOut.boxColor);
                Fields.EventFoldOut(parent.Get("onClose"), parent.Get("onCloseFoldOut"), "On Close",
                    color: FoldOut.boxColor);
                Fields.EventFoldOut(parent.Get("onMove"), parent.Get("onMoveFoldOut"), "On Move",
                    color: FoldOut.boxColor);
                Fields.EventFoldOut(parent.Get("onUseItem"), parent.Get("onUseItemFoldOut"), "On Use Item",
                    color: FoldOut.boxColor);
                Fields.EventFoldOut(parent.Get("onRemoveItem"), parent.Get("onRemoveItemFoldOut"), "On Remove Item",
                    color: FoldOut.boxColor);
            }

            parent.Get("loadSlots").boolValue =
                FoldOut.LargeButton("Refresh Slots", Tint.Blue, Tint.White, Icon.Get("BackgroundLight"));
            parent.Get("deleteInventory").boolValue = FoldOut.LargeButton("Delete Saved Data", Tint.Orange, Tint.White,
                Icon.Get("BackgroundLight"));
        ...
        public void ReloadSlots() // on draw gizmos only gets called if the scene view is open
        {
            if (main.transform.childCount != main.childCount || main.loadSlots)
            {
                ...
                var newSlots = main.transform.GetComponentsInChildren<InventorySlot>(true);

                for (var i = 0; i < newSlots.Length; i++) main.slot.Add(newSlots[i]);
