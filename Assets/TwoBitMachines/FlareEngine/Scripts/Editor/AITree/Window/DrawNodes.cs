...
using UnityEditor;
using UnityEngine;
using Action = TwoBitMachines.FlareEngine.AI.Action;
...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class DrawNodes
    {
        ...
        public Rect Draw(AITree tree, Node node, bool debug, Vector2 parentMoved, Vector2 gridOffset,
            Event currentEvent)
        {
            var childIsActive = false;
            var nTexture = node.nodeType == NodeType.Composite
                ? node is Decorator ? decoratorTexture : compositeTexture
                : nodeTexture;
            if (nTexture == null)
                nTexture = Texture2D.whiteTexture;
            node.rectSize = new Vector2(nTexture.width, nTexture.height);
            var nodeColorSelected = DebugNodeColor(node, debug, ref childIsActive);
            var visualRect = node.GetRect(gridOffset);

            if (node.inspectSelected)
            {
                var select = new Rect(visualRect)
                    { x = visualRect.x - 4, y = visualRect.y - 4, height = 38, width = 68 };
                select.DrawTexture(selectFrame, Tint.HardDark);
            }

            if (node.failed)
            {
                var select = new Rect(visualRect) { x = visualRect.x, y = visualRect.y - 10, height = 10, width = 10 };
            ...
            {
                var select = new Rect(visualRect)
                    { x = visualRect.x + 10, y = visualRect.y - 10, height = 10, width = 10 };

                var failed = node.nodeState == NodeState.Failure;
                var texture = failed ? failedTexture : checkTexture;
                var textureColor = failed ? Tint.Delete : Tint.PastelGreen;
                select.DrawTexture(texture, textureColor);

                if (Clock.Timer(ref node.interruptCounter, debugTimer)) node.interruptCheck = false;
            }
            ...
            if (node is Composite)
            {
                ...
                {
                    var select = new Rect(visualRect)
                        { x = visualRect.x + 22, y = visualRect.y - 12, height = 14, width = 14 };
                ...
                {
                    var select = new Rect(visualRect)
                        { x = visualRect.x + 24, y = visualRect.y - 17, height = 11, width = 11 };
                ...
                {
                    var select = new Rect(visualRect)
                        { x = visualRect.x + 24, y = visualRect.y - 19, height = 11, width = 11 };
        ...
        {
            var nodeColorSelected = nodeColor;

            if (node.copySelected) nodeColorSelected = Tint.Orange;
            if (debug && !Clock.TimerEditorExpired(ref node.selectCounter,
                    debugTimer)) // this will catch nodes that are active for less than a frame
            {
                childIsActive = true;
        ...
        public void DebugConnectionColor(Node node, bool childIsActive, Vector2 parentMoved, Vector2 gridOffset)
        {
            if (!childIsActive || node.nodeParent == null)
                return;
            ConnectBezierLine(node.nodeParent.GetRect(gridOffset),
                Compute.OffsetRect(node.GetRect(parentMoved), gridOffset), nodeSuccessColor, 4);
        }
        ...
        public void IconButton(Node node, Rect nodeRect)
        {
            ...
            var icon = icons.GetIcon(node.nameType); //
        ...
        public void ParentButton(AITree tree, Node node, Rect nodeRect)
        {
            ...
            var icon = node.Children().Count > 0 ? icons.GetIcon("Connected") : icons.GetIcon("Connect");
            var childSize = new Vector2(icon.width, icon.height);
            var buttonPosition = new Vector2(nodeRect.x + 3, nodeRect.y + nodeRect.height - childSize.y - 3);
            var buttonRect = new Rect(buttonPosition.x, buttonPosition.y, childSize.x, childSize.y);
            ...
            if (node.lookingForChild) buttonRect.DrawTexture(icons.GetIcon("ConnectLook"), Color.white);
        ...
        public void ChildButton(Node node, Rect nodeRect)
        {
            ...
            var icon = node.nodeParent == null ? icons.GetIcon("ConnectBlock") : icons.GetIcon("LookingToConnectBlock");
            var childSize = new Vector2(icon.width, icon.height);
            var buttonPosition = new Vector2(nodeRect.x + nodeRect.width * 0.5f - childSize.x * 0.5f, nodeRect.y - 16);
            var buttonRect = new Rect(buttonPosition.x, buttonPosition.y, childSize.x, childSize.y);

            if (buttonRect.Button(icon, connectionHookColor)) node.childActive = !node.childActive;
        }
        ...
        public void HideChildrenButton(Node node, Rect nodeRect)
        {
            ...
            var icon = node.hideChildren ? hide : unhide;
            var childSize = new Vector2(icon.width, icon.height);
            var buttonPosition = new Vector2(nodeRect.x + 3, nodeRect.y + 3);
            var buttonRect = new Rect(buttonPosition.x, buttonPosition.y, childSize.x, childSize.y);

            if (buttonRect.Button(icon, Color.white)) node.hideChildren = !node.hideChildren;
        }

        public void LeafButton(Node node, Rect nodeRect)
        {
            var icon = icons.GetIcon("Leaf");
            var childSize = new Vector2(icon.width, icon.height);
            var buttonPosition = new Vector2(nodeRect.x + 2, nodeRect.y + nodeRect.height - 19);
            var buttonRect = new Rect(buttonPosition.x, buttonPosition.y, childSize.x, childSize.y);
            var color = Color.white;
        ...
        public void DisplayNodeConnections(Node node, Rect nodeRect, Vector2 movedDelta, Vector2 gridOffset)
        {
            ...
            var parentLastDigit = Mathf.Abs(nodeRect.x) % 10;
            ...
            for (var i = children.Count - 1; i >= 0; i--)
            {
                ...
                ConnectBezierLine(nodeRect, Compute.OffsetRect(child.GetRect(movedDelta), gridOffset), connectionColor,
                    3);
            }
        }
        ...
        {
            var origin = new Vector2(parent.x + parent.width * 0.5f, parent.y + parent.height + 1);
            var offset = Vector2.up * thickness * 0.5f;
            var start = origin + Vector2.up * 15f;
            var end = new Vector2(child.x + child.width * 0.5f, child.y - 2f);
            var corner = new Vector2(end.x, start.y);
            var adjustedCorner = new Vector2(corner.x + Mathf.Sign(end.x - start.x) * thickness * 0.5f, corner.y);

            var normal = Handles.color;
            Handles.color = color;
            if (Mathf.Abs(end.x - start.x) > thickness) // if end point is below start point, no need to draw corner
                Handles.DrawBezier(start, adjustedCorner, adjustedCorner, adjustedCorner, color, Texture2D.whiteTexture,
                    thickness);
            else
            ...
            Handles.DrawBezier(origin, start + offset, start + offset, start + offset, color, Texture2D.whiteTexture,
                thickness);
