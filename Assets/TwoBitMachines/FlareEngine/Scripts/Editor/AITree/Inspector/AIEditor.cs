...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class AIEditor : UnityEditor.Editor
    {
        ...
        public static List<string> actionList = new();
        public AIFSM fsm;
        ...
        public List<string> stateNameTemp = new();
        public string[] barNames = { "States", "Reset", "Target", "Territory", "Variable" };
        ...
        private void OnEnable()
        {
            ...
            {
                Util.GetFolderStructure("TwoBitMachines", "/FlareEngine/Scripts/AI/Behavior/Nodes/Action", "Action",
                    actionList);
                Util.GetFolderStructure("", UserFolderPaths.Path(UserFolder.AINode),
                    UserFolderPaths.FolderName(UserFolder.AINode), actionList, false, true);
            }

            for (var i = 0; i < fsm.state.Count; i++) fsm.state[i].HideInInspector();
            for (var i = 0; i < fsm.alwaysState.Count; i++) fsm.alwaysState[i].HideInInspector();
        ...
        private void OnSceneGUI()
        {
            if (fsm == null) return;
            Mouse.Update();
            for (var i = 0; i < fsm.data.Count; i++)
            {
                fsm.data[i].OnSceneGUI(this);
                PrefabUtility.RecordPrefabInstancePropertyModifications(fsm.data[i]);
            }

            for (var i = 0; i < fsm.state.Count; i++)
            for (var j = 0; j < fsm.state[i].action.Count; j++)
            {
            ...
            for (var i = 0; i < fsm.alwaysState.Count; i++)
            for (var j = 0; j < fsm.alwaysState[i].action.Count; j++)
            {
            ...
            for (var i = 0; i < fsm.resetState.action.Count; i++)
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            parent.Update();
            {
                if (SafetyCheckFailed()) return;

                AIBase.AIType(fsm.transform, parent, ref createUnits, true);
                var bar = parent.Get("barIndex");
                FoldOut.TabBarString(Icon.Get("BackgroundLight"), Tint.Box, Tint.BoxTwo, barNames, bar,
                    LabelType.White);
                ...
                if (bar.intValue == 0)
                {
                    ...
                    {
                        if (previous != fsm.root.fsmChild) Repaint();
                ...
                if (bar.intValue == 1) ResetState(parent.Get("resetState"), Tint.Delete);
            }
            parent.ApplyModifiedProperties();
            Layout.VerticalSpacing(10);

            if (createUnits) AIBase.CreateUnits(fsm);
            onEnable = false;
        ...
        public bool SafetyCheckFailed()
        {
            ...
            {
                var tempRoot = fsm.GetComponent<Root>();
                if (tempRoot == null)
                {
            ...
            if (fsm.root != null) fsm.root.hideFlags = HideFlags.HideInInspector;
        ...
        {
            stateNameTemp.Clear();
            for (var x = 0; x < fsm.state.Count; x++) stateNameTemp.Add(fsm.state[x].stateName);
        ...
        public void ShowStates(SerializedProperty states, Color color, bool isAlways = false)
        {
            ...
            for (var i = 0; i < states.arraySize; i++)
            {
                ...
                var state = states.Element(i);

                FoldOut.Bar(state, color, 25)
                    .LabelAndEdit("stateName", "editName")
                    .RightButton(toolTip: "Add Node")
                    .RightButton("deleteAsk", "Delete", "Delete State")
                    .RightButton("delete", "Close", execute: state.Bool("deleteAsk"))
                    .RightButton("signal", "Signal", "Settings", execute: !isAlways && state.Bool("foldOut"))
                    .RightButton("changeState", "Lightning", "Change To this State", execute: isPlaying)
                    .RF("type", 16, -4, 3)
                    .BlockRight(20, color, 21)
                    .LabelRight(StateType(state.Enum("type")), offsetX: 22);
                var rectStart = Bar.barStart;
                rectStart.x = Bar.startXOrigin;
                rectStart.y += 2;

                if (ListReorder.Grip(parent, states, Bar.barStart.CenterRectHeight(), i, Tint.White)) return;

                var activeState = false;
                if (!isAlways && isPlaying && fsm.root.stateName == state.String("stateName"))
                {
                    activeState = true;
                    var rect = new Rect(Bar.barStart)
                    {
                        x = Bar.startXOrigin - 8f, y = Bar.barStart.y - 2, width = 3f, height = Bar.barStart.height + 5f
                    };
                ...
                if (Bar.FoldOpen(state.Get("foldOut")))
                {
                    if (!isAlways && state.Bool("signal"))
                    {
                        ...
                        {
                            state.FieldAndEnable("Default Signal", "defaultSignal", "useSignal", !isAlways);
                ...
                if (state.ReadBool("add")) NodeContextMenu(new Vector2(isAlways ? 1 : 0, i), rectStart);
        ...
        {
            var open = FoldOut.Bar(state, color)
                .Label("Reset State")
                .RightButton()
                .RF("type", 16, -4, 3)
                .BlockRight(20, color, 21)
            ...
            if (open) DisplayNodes(state, state.Get("action"), Tint.Orange, false, false);
            if (state.ReadBool("add")) NodeContextMenu(new Vector2(2, 0), rectStart);
        }

        private void DisplayNodes(SerializedProperty state, SerializedProperty nodes, Color color, bool isActive,
            bool showNextState = true)
        {
            if (nodes.arraySize == 0) Layout.VerticalSpacing(5);


            for (var i = 0; i < nodes.arraySize; i++)
            {
                var node = nodes.Element(i).Get("node");
                ...
                if (isActive && i == fsm.root.fsmIndex) childColor = Tint.Green;
                var element = new SerializedObject(node.objectReferenceValue);
                element.Update();
                {
                    var hideFlagsProp = element.FindProperty("m_ObjectHideFlags");
                    hideFlagsProp.intValue = (int)HideFlags.HideInInspector;

                    var name = node.objectReferenceValue.GetType().ToString().Split('.')[3];
                    var inspectNode = node.objectReferenceValue as Node;
                    var nodeElement = nodes.Element(i);
                    var isOpen = element.Bool("foldOut");
                    ...
                    {
                        GUI.enabled = isOpen;
                        var nextSateSet = nodeElement.Bool("goToSuccess") || nodeElement.Bool("goToFailure");
                        var nextColor = nextSateSet ? Tint.Blue : Tint.WarmWhite;
                    ...
                    if (ListReorder.Grip(state, nodes, Bar.barStart.CenterRectHeight(), i,
                            Tint.White)) return; // when reorder list changes, it should exit out
                    if (Bar.FoldOpen(element.Get("foldOut")))
                    {
                        if (inspectNode == null ||
                            !inspectNode.OnInspector(fsm, element, childColor,
                                onEnable)) // if not custom inspector, display it raw
                            AIBase.IterateObject(element, fsm.data, setReference: true);
                        if (showNextState && (inspectNode == null || inspectNode.HasNextState()) &&
                            element.Bool("openNext"))
                        {
        ...
        private void NodeContextMenu(Vector2 stateIndex, Rect rect)
        {
            if (Event.current.shift)
            {
                ...
                PopupWindow.Show(rect,
                    new SearchableContextMenu(actionList, selected => { SearchForState(selected); }));
                return;
            }
        ...
        private void SearchForState(object obj)
        {
            ...
            parent.Update();
            {
                var key = parent.Get("stateActionIndex").vector2Value;
                if (key.x == 2)
                {
                ...
                {
                    var states = key.x == 1 ? parent.Get("alwaysState") : parent.Get("state");
                    for (var i = 0; i < states.arraySize; i++)
                        if (key.y == i)
                        {
        ...
        private void DeleteState(SerializedProperty array)
        {
            ...
            {
                var state = array.Element(i);
                if (state.Bool("delete"))
                {
                    var actions = state.Get("action");
                    for (var j = 0; j < actions.arraySize; j++)
                    {
                        var node = actions.Element(j).Get("node");
                        if (node.objectReferenceValue != null) DestroyImmediate(node.objectReferenceValue);
                    }
        ...
        private void ExchangeListItems()
        {
            if (ListReorder.canSwap && ListReorder.listSwapObj == fsm) // this could affect other aIFSM
            {
                ...
                if (ListReorder.srcListIndex >= 0 && ListReorder.dstListIndex >= 0 &&
                    ListReorder.srcListIndex < fsm.state.Count && ListReorder.dstListIndex < fsm.state.Count)
                {
                    var stateSource = fsm.state[ListReorder.srcListIndex];
                    var stateDestination = fsm.state[ListReorder.dstListIndex];
    ...
    public class SearchableContextMenu : PopupWindowContent
    {
        ...
        public SearchableContextMenu(List<string> items, Action<string> onSelect)
        {
            Items = items;
            OnSelect = onSelect;
        }
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            IEnumerable<string> visibleItems = Items;

            if (!string.IsNullOrWhiteSpace(Search))
                visibleItems = Items.Where(i => i.IndexOf(Search, StringComparison.OrdinalIgnoreCase) >= 0);

            Scroll = EditorGUILayout.BeginScrollView(Scroll);
            foreach (var item in visibleItems)
            {
                var displayName = item.Contains("/") ? item.Split('/').Last() : item;
            ...
            // {
            //         Scroll = EditorGUILayout.BeginScrollView(Scroll);
            //         foreach (var item in Items.Where(i => i.IndexOf(Search, StringComparison.OrdinalIgnoreCase) >= 0))
            //         {
            //                 string displayName = item.Contains("/") ? item.Split('/').Last() : item;
