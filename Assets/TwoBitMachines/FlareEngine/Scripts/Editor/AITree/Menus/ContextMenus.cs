...
namespace TwoBitMachines.FlareEngine.Editors
{
    public class ContextMenus
    {
        ...
        {
            var rootPosition = editorRef.zoom.GroupRect;
            ...
            if (rootPosition.ContainsMouseRightDown())
            {
                ...
                for (var i = 0; i < AITreeWindowEditor.nodeList.Count; i++)
                {
                    var folder = AITreeWindowEditor.nodeList[i];
                    menu.AddItem(new GUIContent(folder), false, OnNodeMenu, folder);
                }

                if (templateRef != null)
                {
                    for (var i = 0; i < templateRef.template.Count; i++)
                    {
                        var folder = "Template/" + templateRef.template[i].templateKey;
                        menu.AddItem(new GUIContent(folder), false, OnNodeMenu, folder);
                    }

                    for (var i = 0; i < templateRef.template.Count; i++)
                    {
                        var folder = "Template/Delete/" + templateRef.template[i].templateKey;
                        menu.AddItem(new GUIContent(folder), false, OnNodeMenu, folder);
                    }
                }
        ...
        private void OnNodeMenu(object obj)
        {
            ...
            var delete = nameType.Length >= 2 ? nameType[1] : "none";
        ...
        private void OnNoteMenu(object obj)
        {
            ...
            for (var i = 0; i < nodeRef.message.Count; i++)
        ...
        private void OnUtilityMenu(object obj)
        {
            ...
            {
                var newMessage = new NodeMessage(nodeRef.rectPosition);
            ...
            {
                var newTemplate = new TemplateKey();
            ...
            {
                if (CreatedAndCopyNode(nodeRef.nameType, nodeRef.rectPosition + Vector2.up * 40f, nodeRef,
                        out var newNode))
                {
        ...
        private void CreateNode(List<Node> children, string nameType, string folder, string delete)
        {
            ...
            if (folder == "Template")
            {
                ...
                for (var i = 0; i < templateRef.template.Count; i++)
                    if (templateRef.template[i].templateKey == nameType)
                    {
                        ...
                        var headNode = CreateBranchFromTemplate(templateRef.template[i].node);
                        if (headNode != null)
                        {
            ...
            else if (CreateNode(nameType, editorRef.mousePosition - editorRef.gridOffset, out var newNode))
            {
            ...
            for (var i = children.Count - 1; i >= 0; i--)
            {
                if (children[i] == nodeDelete)
                {
                    if (children[i].CanHaveChildren())
                        for (var n = 0; n < nodeDelete.Children().Count; n++)
        ...
        private void CreateTemplate(TemplateNode template, Node node)
        {
            ...
            {
                var children = (node as Composite).children;
        ...
        private void CopyNodeChildren(Node parent, AITree tree)
        {
            ...
            var newChildren = new List<Node>(children); // copy since we are going to clear original list
            children.Clear(); //                                 clear list since content belongs to original node

            for (var i = 0; i < newChildren.Count; i++)
            {
                var child = newChildren[i];
                if (CreatedAndCopyNode(child.nameType, child.rectPosition + Vector2.up * 40f, child,
                        out var copiedNode))
                {
        ...
        public bool CreateNode(string nameType, Vector2 position, out Node newNode)
        {
            ...
            if (EditorTools.RetrieveType("TwoBitMachines.FlareEngine.AI." + nameType, out var type))
            {
                ...
                newNode.nodeType = newNode is Composite || newNode is Decorator || newNode is Root
                    ? NodeType.Composite
                    : NodeType.Action;
        ...
        private bool CreatedAndCopyNode(string nameType, Vector2 position, Node nodeToCopy, out Node newNode)
        {
            ...
            {
                newNode = Undo.AddComponent(editorRef.tree.gameObject, type) as Node;
                EditorTools.CopySerializedObject(new SerializedObject(newNode), new SerializedObject(nodeToCopy),
                    type); // Undo.AddComponent (editorRef.tree.gameObject, type) as Node;
                newNode.nameType = nameType;
                newNode.rectActive = true;
                newNode.rectPosition = position;
                newNode.nodeType = newNode is Composite || newNode is Decorator || newNode is Root
        ...
        {
            if (!CreateNode(template.nameType, template.position, out var newNode))
                return null;

            for (var i = 0; i < template.children.Count; i++)
            {
                var newChild = CreateBranchFromTemplate(template.children[i]);
        ...
        private void UpdateNodePositions(Node node, Vector2 distance)
        {
            ...
            for (var i = 0; i < node.message.Count; i++) node.message[i].position += distance;

            if (node.Children() == null)
                return;

            for (var i = 0; i < node.Children().Count; i++) UpdateNodePositions(node.Children()[i], distance);
        }
        ...
        {
            for (var i = 0; i < parent.message.Count; i++) parent.message[i].position.y += 40f;
        }
    }
}