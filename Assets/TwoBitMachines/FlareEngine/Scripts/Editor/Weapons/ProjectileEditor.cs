...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class ProjectileEditor : UnityEditor.Editor
    {
        ...
        public List<string> bulletTypes = new();
        ...
        private void OnEnable()
        {
            ...
            Util.GetFileNames("TwoBitMachines", "/FlareEngine/Scripts/Weapons/Projectile/ProjectileType/BulletType",
                bulletTypes);
        ...
        private SerializedObject ChooseType()
        {
            ...
            var projectile = main.gameObject.GetComponent<ProjectileBase>();
    ...
    public static class ProjectilesEditor
    {
        ...
        {
            var ammo = baseParent.Get("ammunition");
            var type = ammo.Enum("type");
            var height = type == 2 ? -1 : 0;
            if (ammo.String("saveKey") == "") ammo.Get("saveKey").stringValue = Guid.NewGuid().ToString();
            FoldOut.Box(5 + height, FoldOut.boxColor, 3);
            {
        ...
        public static void Setup(SerializedObject baseParent, List<string> bulletTypes)
        {
            ...
            {
                var pattern = baseParent.Get("pattern");
        ...
        public static void BulletType(Projectile main, SerializedObject baseParent, int projectileType)
        {
            if (main.transform.childCount > 0)
                baseParent.Get("projectileObject").objectReferenceValue =
                    main.transform.GetChild(0).gameObject; // This is the reference to the bullet gameobject
            ...
            var child = main.transform.GetChild(0);

            if (child == null) return;

            var bulletType = baseParent.Get("bulletType");
            var bulletBase = child.GetComponent<BulletBase>();
            if (bulletBase == null || bulletBase.nameType != bulletType.stringValue)
            {
                if (bulletBase != null)
                    MonoBehaviour.DestroyImmediate(bulletBase);
                var actionType = "TwoBitMachines.FlareEngine.BulletType." + bulletType.stringValue;
            ...
            if (bulletBase != null)
            {
                var bullet = new SerializedObject(bulletBase);
                bullet.Update();

                if (FoldOut.Bar(baseParent, Tint.Orange).Label("Properties").FoldOut("propertiesFoldOut"))
                {
                    var fields = EditorTools.CountObjectFields(bullet);
                    if (fields != 0)
        ...
        public static void Instant(SerializedObject baseParent, SerializedObject parent)
        {
            if (FoldOut.Bar(baseParent, Tint.Orange).Label("Properties").FoldOut("propertiesFoldOut"))
            {
                FoldOut.Box(7, FoldOut.boxColor, 3);
                ...
                var eventOpen = FoldOut.FoldOutButton(baseParent.Get("propertiesEventFoldOut"));
                Fields.EventFoldOut(baseParent.Get("onFire"), baseParent.Get("foldOutOnFire"), "On Fire",
                    execute: eventOpen);
                Fields.EventFoldOut(baseParent.Get("onImpact"), baseParent.Get("impactFoldOut"), "On Impact",
                    execute: eventOpen);
            }
        }
        ...
        public static void GrapplingHook(SerializedObject baseParent, SerializedObject parent)
        {
            if (FoldOut.Bar(baseParent, Tint.Orange).Label("Properties").FoldOut("propertiesFoldOut"))
            {
                ...
                FoldOut.Box(5, FoldOut.boxColorLight, 3);
                {
                    ...
                    baseParent.ClampInt("pointsInLine", 2, 1000);
                ...
                {
                    Fields.EventFoldOutEffect(baseParent.Get("onHook"), baseParent.Get("hookWE"),
                        baseParent.Get("hookFoldOut"), "On Hook", Tint.Blue);
                    Fields.EventFoldOut(baseParent.Get("onEnable"), baseParent.Get("onEnableFoldOut"), "On Enable",
                        color: Tint.Blue);
                    Fields.EventFoldOut(baseParent.Get("onMissed"), baseParent.Get("missedFoldOut"), "On Missed",
                        color: Tint.Blue);
                    Fields.EventFoldOut(baseParent.Get("offOnContact"), baseParent.Get("offOnContactFoldOut"),
                        "Off On Contact", color: Tint.Blue);
                    Fields.EventFoldOut(baseParent.Get("onEmergencyExit"), baseParent.Get("emergencyFoldOut"),
                        "On Emergency Exit", color: Tint.Blue);
                }
                ...
                if (FoldOut.Bar(baseParent, FoldOut.boxColorLight).Label("Retract", FoldOut.titleColor, false)
                    .BRE("retract").FoldOut("retractBarFoldOut"))
                {
                    FoldOut.Box(5, FoldOut.boxColorLight, 5, -2);

                    GUI.enabled = baseParent.Bool("retract");
                    {
                        var type = baseParent.Enum("retractType");
                        baseParent.Field("Type", "retractType", type == 0);
                        baseParent.FieldDouble("Type", "retractType", "retractButton", type == 1);
                        ...
                        if (FoldOut.FoldOutButton(baseParent.Get("retractMainFoldOut")))
                            Fields.EventFoldOut(baseParent.Get("onRetractComplete"), baseParent.Get("retractFoldOut"),
                                "On Retract Complete", color: Tint.Blue);
                    }
                    GUI.enabled = true;
                }

                if (FoldOut.Bar(baseParent, FoldOut.boxColorLight).Label("Grab Item", FoldOut.titleColor, false)
                    .BRE("canGrabItem").FoldOut("grabItemFoldOut"))
                {
                    ...
                    GUI.enabled = baseParent.Bool("canGrabItem");
                    {
                        ...
                        {
                            Fields.EventFoldOutEffect(baseParent.Get("onItemGrabComplete"),
                                baseParent.Get("grabCompleteWE"), baseParent.Get("grabItemCompleteFoldOut"),
                                "On Grab Item Complete", Tint.Blue);
                            Fields.EventFoldOut(baseParent.Get("onItemGrabFound"),
                                baseParent.Get("grabItemFoundFoldOut"), "On Grab Item Found", color: Tint.Blue);
                            Fields.EventFoldOut(baseParent.Get("onItemGrabFailed"),
                                baseParent.Get("grabItemFailedFoldOut"), "On Grab Item Failed", color: Tint.Blue);
                        }
                    }
                    GUI.enabled = true;
                }
