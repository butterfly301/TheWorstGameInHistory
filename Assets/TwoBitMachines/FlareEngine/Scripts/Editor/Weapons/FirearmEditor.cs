    ...
    public class FirearmEditor : UnityEditor.Editor
    {
        ...
        public void Firearm()
        {
            if (FoldOut.Bar(parent).Label("Firearm").FoldOut())
            {
                ...
                var projectile = parent.Get("defaultProjectile");
                var animation = projectile.Get("waitForAnimation");
                var extra = animation.Bool("wait") ? 1 : 0;

                FoldOut.Box(4 + extra, FoldOut.boxColor, 3);
                {
                    ...
                    Labels.FieldText("Signal Name", Layout.boolWidth, animation.String("weaponAnimation") == "");

                    if (extra > 0) animation.FieldDouble("Extra Animation", "spriteEngine", "extraSignal");
                }

                if (FoldOut.FoldOutButton(parent.Get("mainFoldOut")))
                {
                    Fields.EventFoldOut(parent.Get("onActivated"), parent.Get("eventFoldOut"), "On Enable",
                        color: eventColor);
                    Fields.EventFoldOut(projectile.Get("onFireSuccess"), projectile.Get("fireFoldOut"),
                        "On Fire Success", color: eventColor);
                    Fields.EventFoldOut(projectile.Get("onOutOfAmmo"), projectile.Get("outOfAmmoFoldOut"),
                        "On Out Of Ammo", color: eventColor);
                }


                if (FoldOut.Bar(parent, FoldOut.boxColor * Tint.WarmGrey).Label("Options", FoldOut.titleColor, false)
                    .FoldOut("optionsFoldOut"))
                {
                    FoldOut.Box(6, FoldOut.boxColor, offsetY: -2);
                    {
                        ...
                        var stopType = parent.Enum("stopType");
                        parent.FieldDoubleAndEnable("Stop Vel X", "stopType", "stopTime", "stopVelocity",
                            stopType == 0);
                        parent.FieldAndEnable("Stop Vel X", "stopType", "stopVelocity", stopType == 1);
                        Labels.FieldText("Time", 18, stopType == 0);
                        parent.FieldToggleAndEnable("Off Near Wall", "turnOffNearWall");
                        var rotate = parent.Get("rotate");
        ...
        public void Rotation()
        {
            ...
            if (FoldOut.Bar(parent).Label("Rotation").FoldOut("rotateFoldOut"))
            {
                ...
                var orientation = rotate.Enum("orientation");
                FoldOut.Box(2, FoldOut.boxColor, offsetY: -2);
                {
                    rotate.Field("Rotate With", "rotate");
                    rotate.Field("Point Towards", "orientation", orientation < 2);
                    rotate.FieldDouble("Point Towards", "orientation", "transformDirection", orientation > 1);
                }
                Layout.VerticalSpacing(3);
        ...
        public void LineOfSight()
        {
            ...
            {
                var lineOfSight = parent.Get("lineOfSight");

                var type = lineOfSight.Enum("lineOfSight");
                ...
                else if (type == 1)
                {
                    ...
                    if (FoldOut.FoldOutButton(lineOfSight.Get("eventsFoldOut"), -2))
                    {
                        Fields.EventFoldOut(lineOfSight.Get("onTargetHit"), lineOfSight.Get("onTargetHitFoldOut"),
                            "On Target Hit", color: eventColor);
                        Fields.EventFoldOut(lineOfSight.Get("onBeamHit"), lineOfSight.Get("onBeamHitFoldOut"),
                            "On Beam Hit", color: eventColor);
                        Fields.EventFoldOut(lineOfSight.Get("onNothingHit"), lineOfSight.Get("onNothingHitFoldOut"),
                            "On Nothing Hit", color: eventColor);
                    }
                }
                else if (type == 2)
                {
                    var rType = lineOfSight.Enum("reticleType") == 0;
                    ...
                    if (lineOfSight.Enum("reticleType") == 0) lineOfSight.Field("Distance", "reticleDistance");
                    Layout.VerticalSpacing(3);
                }
            }
        }
        ...
        public void ChargedProjectile()
        {
            ...
            if (FoldOut.Bar(charge).Label("Charge").SR(5).BRE("canCharge").FoldOut("chargeFoldOut"))
            {
                GUI.enabled = charge.Bool("canCharge");
                {
                    ...
                    if (FoldOut.FoldOutButton(charge.Get("eventsFoldOut"), -2))
                    {
                        Fields.EventFoldOut(charge.Get("onCharging"), charge.Get("onChargingFoldOut"), "On Charging",
                            color: eventColor);
                        Fields.EventFoldOut(charge.Get("onChargingBegin"), charge.Get("onChargingBeginFoldOut"),
                            "On Charging Begin", color: eventColor);
                        Fields.EventFoldOut(charge.Get("onChargingComplete"), charge.Get("onChargingCompleteFoldOut"),
                            "On Charging Complete", color: eventColor);
                        Fields.EventFoldOut(charge.Get("onDischarging"), charge.Get("onDischargingFoldOut"),
                            "On Discharging", color: eventColor);
                        Fields.EventFoldOut(charge.Get("onDischargingComplete"),
                            charge.Get("onDischargingCompleteFoldOut"), "On Discharging Complete", color: eventColor);
                        Fields.EventFoldOut(charge.Get("onDischargingFailed"), charge.Get("onDischargingFailedFoldOut"),
                            "On Discharging Failed", color: eventColor);
                        Fields.EventFoldOut(charge.Get("onCoolingDown"), charge.Get("onCoolDownFoldOut"),
                            "On Cooling Down", color: eventColor);
                    }

                    Recoil("Recoil", charge.Get("recoil"), charge, 1);
                }
                GUI.enabled = true;
            }
        }
        ...
        {
            var autoSeek = parent.Get("autoSeek");
            FoldOut.Box(4, FoldOut.boxColor, 3);
            {
            ...
            if (FoldOut.FoldOutButton(autoSeek.Get("eventsFoldOut")))
                Fields.EventFoldOut(autoSeek.Get("onFoundTarget"), autoSeek.Get("onFoundTargetFoldOut"),
                    "On Found New Target", color: eventColor);
        }

        public void Recoil(string title, SerializedProperty recoil, SerializedProperty discharge, int extraHeight = 0)
        {
            if (FoldOut.Bar(recoil, FoldOut.boxColor * Tint.WarmGrey).Label("Recoil", FoldOut.titleColor, false)
                .BRE("recoil").FoldOut())
            {
