...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class AudioManagerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var categories = parent.Get("categories");
            ...
            {
                var category = categories.Element(i);
                var audio = category.Get("audio");

                var header = Block.Header(category).Style(Tint.Orange)
                ...
                if (header.Build())
                {
                    ...
                    for (var j = 0; j < audio.arraySize; j++)
                    {
                        var element = audio.Element(j);
                        var childList = element.Get("childList");
                        ...
                        Block.Header(element).Style(Tint.SoftDark)
                            ...
                            .Space(4)
                            .Button(hide: false, tooltip: "Add audio variation")
                        ...
                        if (Header.SignalActive("Red")) StopAllClips();
                        if (Header.SignalActive("Add"))
                        {
                            childList.arraySize++;
                        ...
                        for (var k = 0; k < childList.arraySize; k++)
                        {
                            var childElement = childList.Element(k);
