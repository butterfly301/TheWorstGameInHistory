...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class CheckPointEditor : UnityEditor.Editor
    {
        ...
        {
            Layout.Update();
            for (var i = 0; i < main.checkPoints.Count; i++)
            {
                var bounds = main.checkPoints[i].bounds;
                SceneTools.DrawAndModifyBounds(ref bounds.position, ref bounds.size, Tint.Orange);
                SceneTools.Label(bounds.position + Vector2.up * (bounds.size.y + 1f),
                    "Check Point: " + main.checkPoints[i].index, Tint.WarmWhite);
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            parent.Update();
            {
                ...
                {
                    FoldOut.Box(4, FoldOut.boxColor, 5, -2);

                    var delete = parent.FieldAndButton("Name", "checkPointName", "Delete");
                    ...
                    if (delete) WorldManagerEditor.DeleteSavedData(main.checkPointName);

                    var array = parent.Get("checkPoints");
                    if (parent.ReadBool("add"))
                    {
                        array.arraySize++;
                        var element = array.LastElement();
                        element.Get("bounds").Get("position").vector2Value =
                            SceneTools.SceneCenter(main.transform.position);
                    ...
                    for (var i = 0; i < array.arraySize; i++)
                    {
                        ...
                        var open = FoldOut.Bar(element, FoldOut.boxColor, 25).Label(" Index: " + element.Int("index"))
                            .BR("delete", "Delete").FoldOut();
