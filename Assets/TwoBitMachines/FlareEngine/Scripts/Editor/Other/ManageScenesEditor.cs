...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class ManageScenesEditor : UnityEditor.Editor
    {
        ...
        public void SceneManagement()
        {
            ...
            var size = sceneNames == null || sceneNames.Count == 0 ? 2 : 4;
            ...
            {
                var step = steps.Element(i);
                var type = (LoadStepType)step.Enum("type");

                Block.Header().Style(FoldOut.boxColor).Grip(this, steps, i)
                    .Label(type.ToString(), 12, false, FoldOut.titleColor).Button("Delete").Build();
                ...
                else
                {
                    ...
                    if (Block.ExtraFoldout(step, "eventFoldOut"))
                    {
                        Fields.EventFoldOut(step.Get("onStart"), step.Get("startFoldOut"), "On Start", space: false);
                        Fields.EventFoldOut(step.Get("onComplete"), step.Get("completeFoldOut"), "On Complete");
                        Fields.EventFoldOut(step.Get("loadingProgress"), step.Get("loadingFoldOut"),
                            "Loading Progress Float");
                        Fields.EventFoldOut(step.Get("loadingProgressString"), step.Get("stringFoldOut"),
