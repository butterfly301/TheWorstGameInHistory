...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class PlayerEditor : UnityEditor.Editor
    {
        ...
        public static int abilitiesListSize;
        public static string[] abilityNamesList;
        public static string[] inputList = { "None" };
        private readonly List<string> availableList = new();
        private readonly Dictionary<string, SerializedObject> foundList = new();
        private Player main;
        ...
        {
            main = target as Player;
            so = serializedObject;
            mainGameObject = main.gameObject;
            Layout.Initialize();
            if (abilityNamesList == null || abilityNamesList.Length == 0 ||
            ...
            if (inputList == null || inputList.Length <= 1 || inputList.Length != main.inputs.inputSO.Count + 1)
                InputList(main);
            for (var i = 0; i < main.ability.Count; i++)
        ...
        public void InputList(Player main)
        {
            var size = main.inputs.inputSO.Count;
            inputList = new string[size + 1];
            inputList[0] = "None";

            for (var i = 0; i < main.inputs.inputSO.Count; i++) inputList[i + 1] = main.inputs.inputSO[i].buttonName;
        }

        public override void OnInspectorGUI()
        {
            Layout.Update();
            if (inputList.Length <= 1 || inputList.Length != main.inputs.inputSO.Count + 1) InputList(main);

            so.Update();
            {
                InputInspectorEditor.Display(so, so.Get("inputs").Get("inputSO"), main, this);
                var abilities = so.Get("ability");
                var serialObjList = GetAbilities(abilities);
        ...
        private void GetActiveAbilitiesList(List<SerializedObject> array, List<string> availableList,
            Dictionary<string, SerializedObject> foundList)
        {
            availableList.Clear();
            for (var i = 0; i < abilityNamesList.Length; i++)
            ...
            {
                var element = array[i]; //.Element (i);
                availableList.Remove(element.String("abilityName"));
                foundList.Add(element.String("abilityName"), element);
            }
        }

        private void DisplayAbilities(SerializedProperty abilities, Dictionary<string, SerializedObject> foundList,
            Color barColor, Color labelColor)
        {
            var open = so.Bool("abilityFoldOut");
            ...
            if (so.ReadBool("view")) AbilityView();

            Settings(so.Get("world"), so.Get("abilities").Get("gravity"), so.Get("signals"), barColor, labelColor);

            var viewIndex = so.Int("viewIndex");

            if (viewIndex <= 1)
                Walk.OnInspector(so.Get("abilities").Get("walk"), inputList, barColor,
                    labelColor); //  walk ability always displayed

            for (var i = abilities.arraySize - 1; i >= 0; i--)
            {
                var canView = viewIndex == 0 || viewIndex == i + 2;
                if (!canView)
                    continue;

                var obj = abilities.Element(i).objectReferenceValue;
                if (obj == null)
                    continue;
                var ability = (Ability)obj;
                var abilityObj = new SerializedObject(abilities.Element(i).objectReferenceValue);
                ...
                {
                    var name = abilityObj.String("abilityName");
                    if (Ability.Open(abilityObj, Util.ToProperCase(name), barColor, labelColor))
                    {
                        var fields = EditorTools.CountObjectFields(abilityObj);
                        if (fields != 0)
        ...
        private void Settings(SerializedProperty world, SerializedProperty gravity, SerializedProperty signals,
            Color barColor, Color labelColor)
        {
            if (so.Bool("settings"))
            {
                ...
                var extraHeight = world.Bool("climbSlopes") ? 3 : 0;
                ...
                FoldOut.Box(7, FoldOut.boxColor, 3);
                {
                    ...
                    world.FieldToggleAndEnable("Use Moving Platform Momentum", "useMovingPlatformMomentum");
                    world.FieldToggleAndEnable("Collide With World Only", "collideWorldOnly");
                    var eventOpen = FoldOut.FoldOutButton(world.Get("eventsFoldOut"));
                    Fields.EventFoldOutEffect(world.Get("onCrushed"), world.Get("crushedWE"),
                        world.Get("crushedFoldOut"), "Crushed By Platform", execute: eventOpen);
                }
            }
        }
        ...
        private void AbilityView()
        {
            ...
            for (var i = 0; i < main.ability.Count; i++)
                if (main.ability[i].pause)
                    menu.AddItem(new GUIContent(main.ability[i].abilityName + "  -- Paused"), viewIndex == i + 2,
                        AbilityView, i + 2);
                else
        ...
        private void CreateAbilityInstance(object obj)
        {
            ...
            var fullTypeName = "TwoBitMachines.FlareEngine.ThePlayer." + typeName;
        ...
        private void DeleteAbility(SerializedProperty abilities, List<SerializedObject> array)
        {
            for (var i = 0; i < array.Count; i++)
                if (array[i].ReadBool("delete"))
                    for (var j = abilities.arraySize - 1; j >= 0; j--)
        ...
        {
            var serialObjList = new List<SerializedObject>();
            for (var i = abilities.arraySize - 1; i >= 0; i--)
            {
