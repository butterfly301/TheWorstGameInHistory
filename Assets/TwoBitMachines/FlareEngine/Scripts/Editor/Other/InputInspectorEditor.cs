...
namespace TwoBitMachines.FlareEngine.Editors
{
    public class InputInspectorEditor
    {
        ...
        public static void Display(SerializedObject parent, SerializedProperty inputs, Player main,
            PlayerEditor pEditor)
        {
            if (inputs.arraySize == 0 &&
                !parent.Bool(
                    "initInputs")) // if no inputs detected, create base inputs. Only check once, or it will interfere if there is more than one player.
            {
                parent.SetTrue("initInputs");
                CreateBaseInput(inputs, main);
            }

            if (FoldOut.Bar(parent, Tint.Blue).Label("Inputs", Color.white)
                .RightButton("clearInputs", "Delete", "Clear Input Player Prefs").FoldOut("inputFoldOut"))
            {
                for (var i = 0; i < inputs.arraySize; i++)
                {
                    var element = inputs.Element(i);
                    ...
                    var input = new SerializedObject(element.objectReferenceValue);
                    input.Update();
                    {
                        FoldOut.BoxSingle(1, FoldOut.boxColor, 3);

                        var type = input.Enum("type");
                        if (type == 0 && Fields.Start(input, input.String("buttonName"), -5).F("type", S.CWB + 5)
                                .F("key", S.CWB).B("Delete", S.B))
                        {
                            inputs.DeleteArrayElement(i);
                            return;
                        }

                        if (type == 1 && Fields.Start(input, input.String("buttonName"), -5).F("type", S.CWB + 5)
                                .F("mouse", S.CWB).B("Delete", S.B))
                        {
                            inputs.DeleteArrayElement(i);
                            return;
                        }

                        if (type == 2 && Fields.Start(input, input.String("buttonName"), -5).F("type", S.CWB + 5)
                                .F("axisName", S.CWB).B("Delete", S.B))
                        {
                            inputs.DeleteArrayElement(i);
                            return;
                        }

                        if (type == 3 && Fields.Start(input, input.String("buttonName"), -5).F("type", S.CWB + 5)
                                .F("axisName", S.CWB).B("Delete", S.B))
                        {
                            inputs.DeleteArrayElement(i);
                            return;
                        ...
                        if (FoldOut.FoldOutButton(input.Get("foldOut"), -3))
                        {
                            var array = input.Get("bindings");
                            if (array.arraySize == 0)
                            {
                                FoldOut.BoxSingle(1, FoldOut.boxColor);
                                {
                                    ...
                                    if (Fields.ConstructButton("Add")) array.arraySize++;
                                }
                            ...
                            if (array.arraySize != 0)
                            {
                                ...
                                {
                                    var binding = array.Element(j);
                                    var inputSO = (InputButtonSO)binding.objectReferenceValue;
        ...
        private static InputButtonSO CreateScriptableObject(SerializedProperty inputs, string name, Player main)
        {
            var path = "Assets/TwoBitMachines/FlareEngine/AssetsFolder/Inputs/" + name + ".asset";
            var oldAsset = AssetDatabase.LoadAssetAtPath(path, typeof(InputButtonSO)) as InputButtonSO;
            var containsName = false;
            for (var i = 0; i < main.inputs.inputSO.Count; i++)
            {
            ...
            var asset = ScriptableObject.CreateInstance<InputButtonSO>();
        ...
        private static void CreateBaseInput(SerializedProperty inputs, Player main)
        {
        ...
        private static void SetInputsButton(InputButtonSO button, string name, KeyCode key, bool isMouse = false)
        {
            if (button == null)
                return;
            var serObj = new SerializedObject(button);
            serObj.Update();
        ...
        {
            var dropArea =
                Layout.CreateRect(Layout.longInfoWidth + 4, 27,
                    -13); //, texture : Texture2D.whiteTexture, color : Tint.Blue);
            Labels.Centered(dropArea, "-- Add Existing Input Here --", Color.white, 13);
