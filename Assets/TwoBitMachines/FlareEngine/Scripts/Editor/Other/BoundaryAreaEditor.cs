...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class BoundaryAreaEditor : UnityEditor.Editor
    {
        ...
        {
            var boundaryArea = (BoundaryArea)target;
            ...
            var position = boundaryArea.SimpleBounds.position;
            var size = boundaryArea.SimpleBounds.size;
            SceneTools.DrawAndModifyBounds(ref boundaryArea.SimpleBounds.position, ref boundaryArea.SimpleBounds.size,
                Color.green, 1f);
            ...
            if (components.Length < 4) boundaryArea.gameObject.AddComponent<BoxCollider2D>();
