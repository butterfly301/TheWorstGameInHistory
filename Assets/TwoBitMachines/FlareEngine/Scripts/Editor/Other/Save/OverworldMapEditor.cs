...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class OverworldMapEditor : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            if (main.mapSavekey == "") main.mapSavekey = Guid.NewGuid().ToString();
        ...
        public void OnSceneGUI()
        {
            ...
            var mainIndex = so.Int("editIndex");
            var list = so.Get("list");

            if (main.transform.childCount != list.arraySize) SetNodeChildren(list);

            for (var i = 0; i < list.arraySize; i++)
            {
                var node = list.Element(i);
                ...
                // set save key
                var nodeRef = node.objectReferenceValue as OverworldNode;
                var nodeObject = new SerializedObject(node.objectReferenceValue);
                nodeObject.Update();
                {
                    if (nodeObject.String("blockSaveKey") == "")
                        nodeObject.Get("blockSaveKey").stringValue = Guid.NewGuid().ToString();
                }
                nodeObject.ApplyModifiedProperties();

                // hide Flags
                var gameObject = nodeRef.gameObject;
                var isBasic = (OverworldNodeType)nodeObject.Enum("type") == OverworldNodeType.Basic;
                if (gameObject.hideFlags == HideFlags.HideInHierarchy) gameObject.hideFlags = HideFlags.None;
                ...
                // move transform
                var transformObject = new SerializedObject(nodeRef.transform);
                transformObject.Update();
                {
                    Vector2 position = transformObject.FindProperty("m_LocalPosition").vector3Value;
                    var insideCircle = (position - (Vector2)Mouse.position).sqrMagnitude <= 0.75f * 0.75f;

                    if (Mouse.down && insideCircle) so.Get("editIndex").intValue = mainIndex = i;
                    if (!selectPathActive && Mouse.down && insideCircle &&
                        (position - (Vector2)Mouse.position).sqrMagnitude > 0.3f * 0.3f)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            position = SceneTools.MovePositionCircleHandle(position, Vector2.zero,
                                mainIndex == i ? Tint.Delete * Tint.WarmGrey : Tint.Green, out var changed, 0.5f);
                            if (!Mouse.holding && insideCircle &&
                                (position - (Vector2)Mouse.position).sqrMagnitude > 0.3f * 0.3f)
                            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (Block.ExtraFoldout(so, "foldOut"))
            {
                ...
                Fields.EventFoldOut(so.Get("onEnteringLevel"), so.Get("enteringLevelFoldOut"),
            ...
            {
                var editNode = main.list[main.editIndex];
                if (editNode != null)
                {
                    Block.Header().Style(Tint.Delete)
                        .Label("Edit Node:  " + main.editIndex, 12, false, FoldOut.titleColor).Button("Delete").Build();
        ...
        {
            list.arraySize = 0;
            var newList = main.gameObject.GetComponentsInChildren<OverworldNode>();
        ...
        public static void DrawLevelPaths(OverworldMap main, OverworldNode[] level, bool delete = false)
        {
            ...
            {
                var parentNode = level[i];
                for (var j = 0; j < parentNode.path.Count; j++)
                {
                    var childNode = parentNode.path[j];
                    if (childNode == null)
                    {
                    ...
                    var isAblockedPath = parentNode.blockPath.Contains(childNode);
                    var nearLine = delete && MouseNeaerLine(parentNode.position, childNode.position, Mouse.position);
                    var color = nearLine ? Tint.Orange : isAblockedPath ? Tint.Delete : Tint.Green;
        ...
        public static bool MouseNeaerLine(Vector2 lineStart, Vector2 lineEnd, Vector2 point,
            float proximityThreshold = 0.5f)
        {
            if ((point - lineStart).sqrMagnitude <= 1f || (point - lineEnd).sqrMagnitude <= 1f) return false;
            ...
            var t = Mathf.Clamp01(Vector2.Dot(point - lineStart, lineDirection) / lineLengthSquared);

            var closestPoint = lineStart + t * lineDirection;
        ...
        private static void DrawWhenObjectIsNotSelected(OverworldMap main, GizmoType gizmoType)
        {
            Draw.GLStart();
            for (var i = 0; i < main.list.Length; i++)
            {
                var t = main.list[i].transform;
