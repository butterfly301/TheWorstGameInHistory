...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class OverworldNodeEditor : UnityEditor.Editor
    {
        ...
        public static void UpdateNode(SerializedObject so, OverworldNode main, List<string> sceneNames,
            bool space = true)
        {
            main.transform.localScale = Vector3.one;
            Layout.Update();
            if (space) Layout.VerticalSpacing(10);
            so.Update();

            var typeProperty = so.Get("type");
            var type = (OverworldNodeType)typeProperty.intValue;
            var typeRef = (int)type;
            ...
            else if (type == OverworldNodeType.Level)
            {
                Block.Box(4, FoldOut.boxColor, extraHeight: 3, noGap: !space);
                {
                    ...
                    var nextType = so.Enum("setNextNodeType");
                    so.Field_("Set Next Node", "setNextNodeType", nextType == 0);
                    so.FieldDouble_("Set Next Node", "setNextNodeType", "nextNode", nextType > 0);
                }
                if (Block.ExtraFoldout(so, "enterFoldOut"))
                {
                    Fields.EventFoldOut(so.Get("onEnterNode"), so.Get("onEnterFoldOut"), "On Enter Node", space: false);
