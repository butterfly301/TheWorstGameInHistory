...
namespace TwoBitMachines.FlareEngine.Editors
{
    ...
    public class HighJumpEditor : UnityEditor.Editor
    {
        ...
        {
            main.collider2DRef = main.GetComponent<Collider2D>();
            if (!Application.isPlaying) main.transform.position = Compute.Round(main.transform.position, 0.25f);
            ...
            if (main.collider2DRef != null)
            {
                ...
                {
                    var width = Mathf.Min(main.collider2DRef.bounds.size.x * 0.25f, 1.5f);
                    main.transform.rotation = Handles.Disc(main.transform.rotation, main.transform.position,
                        Vector3.forward, width, false, 0f);
                }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (FoldOut.Bar(parent, Tint.Blue).Label("High Jump", Color.white).FoldOut())
            {
                ...
                parent.FieldDouble("Force", "force", "timer", type == 2);
                if (type == 2)
        ...
        public static void DrawFollowGizmos(HighJump main, GizmoType gizmoType)
        {
            if (main.collider2DRef != null)
            {
                if (main.collider2DRef is BoxCollider2D)
                {
                    var boxCollider = main.collider2DRef as BoxCollider2D;
