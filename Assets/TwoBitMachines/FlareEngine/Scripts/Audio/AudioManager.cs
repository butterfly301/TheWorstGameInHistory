...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        [NonSerialized] public static AudioManager get;
        [NonSerialized] public static Audio tempAudio = new();
        ...
        [NonSerialized] private bool crossFade;
        [NonSerialized] private float fade;
        [NonSerialized] private float fadeCounter;
        [NonSerialized] private bool fadeIn;
        [NonSerialized] private bool fadeOut;
        [NonSerialized] private float lerpCounter;
        [NonSerialized] private bool lerpVolumeDown;
        [NonSerialized] private bool lerpVolumeUp;
        [NonSerialized] private float musicVolumeRef = 1f;
        [NonSerialized] private string newMusicName = "";
        [NonSerialized] private float oldValueMusicDown;
        [NonSerialized] private float oldValueMusicUp;
        [NonSerialized] private float oldValueSFXDown;
        [NonSerialized] private float oldValueSFXUp;

        [NonSerialized] private readonly Dictionary<string, Audio> playList = new();
        [NonSerialized] private float targetMusicDown;
        [NonSerialized] private float targetMusicUp;
        [NonSerialized] private float targetSFXDown;
        [NonSerialized] private float targetSFXUp;
        ...
        public void Awake()
        {
            ...
            if (audioManagerSO != null) audioManagerSO.Register(this);
        }
        ...
        private void Start()
        {
            if (playOnStart) PlayAudio(startMusic);
        }
        ...
        public void Update()
        {
            ...
            if (lerpVolumeDown)
            {
                ...
                if (lerpCounter >= 1) lerpVolumeDown = false;
            }
            ...
            if (lerpVolumeUp)
            {
                ...
                if (lerpCounter >= 1) lerpVolumeUp = false;
            }
        }
        ...
        private void LoadAudio()
        {
            ...
            {
                var audio = categories[i].audio;
                for (var j = 0; j < audio.Count; j++)
                {
                    if (audio[j].clip == null)
                        continue;
                    if (!playList.ContainsKey(audio[j].clip.name)) playList.Add(audio[j].clip.name, audio[j]);
                }
            }
        }
        ...
        {
            for (var i = 0; i < audio.childList.Count; i++)
            {
...
#pragma warning disable 0414
        [SerializeField] public bool loadAudio;
...
#pragma warning restore 0414
    ...
    {
        [SerializeField] public List<Audio> audio = new();
        [SerializeField] public string name = "";

#if UNITY_EDITOR
#pragma warning disable 0414
        [SerializeField] private bool foldOut;
        [SerializeField] private bool deleteAsk;
...
#pragma warning restore 0414
    ...
    public class Audio
    {
        ...
        [SerializeField] public List<AudioChild> childList = new();
