...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        [SerializeField] public List<Audio> sfx = new();
        [SerializeField] public AudioManager audioManager;
        [SerializeField] public bool attenuate;
        ...
        [NonSerialized] private float playStamp;
        private bool hasReference => audioManager != null || AudioManager.get != null;

        public void PlaySFXTimeRate(string name)
        {
            if (!hasReference || Attenuate(out var percent) || Time.time < playStamp) return;
        ...
        {
            if (!hasReference || Attenuate(out var percent)) return;
            for (var i = 0; i < sfx.Count; i++)
                if (sfx[i].clip.name == name)
                {
                    PlayAudio(sfx[i], percent);
        ...
        {
            if (Random.Range(0.0f, 1f) >= 0.5f) return;
            if (!hasReference || Attenuate(out var percent)) return;
            for (var i = 0; i < sfx.Count; i++)
                if (i == index)
                {
        ...
        {
            for (var i = 0; i < sfx.Count; i++) PlayAudio(sfx[i], 1f);
        }
        ...
        private bool Attenuate(out float percent)
        {
            ...
            var squareDistance = distance * distance;
            var diff = (WorldManager.gameCam.transform.position - transform.position).sqrMagnitude;

            if (diff > squareDistance) return true;
