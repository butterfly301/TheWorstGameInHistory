    ...
    public class Damage : MonoBehaviour
    {
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            if (Compute.ContainsLayer(layer, other.gameObject.layer))
                Health.IncrementHealth(transform, other.transform, -damage,
                    Direction(transform, other.transform, direction) * force);
        }

        private void OnTriggerStay2D(Collider2D collider)
        {
            if (Compute.ContainsLayer(layer, collider.gameObject.layer))
                Health.IncrementHealth(transform, collider.transform, -damage,
                    Direction(transform, collider.transform, direction) * force);
        }
        ...
        public static Vector3 Direction(Transform aiTransform, Transform transform, AttackDirection direction)
        {
            ...
            if (direction == AttackDirection.Up) return Vector3.up;
            if (direction == AttackDirection.Down) return Vector3.down;
            if (direction == AttackDirection.Left) return Vector3.left;
