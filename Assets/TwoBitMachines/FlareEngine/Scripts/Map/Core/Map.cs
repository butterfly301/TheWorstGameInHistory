...
namespace TwoBitMachines.MapSystem
{
    ...
    public class Map : MonoBehaviour
    {
        ...
        [SerializeField] public List<ZoneEvents> eventList = new();

        [NonSerialized] private string activeZone;
        [NonSerialized] private int currentRoomIndex = -1;
        ...
        private void Awake()
        {
            ...
            if (WorldManager.get.initialized) Initialized();
        }
        ...
        {
            if (map != null && map.pauseGame == MapPause.PauseGame) WorldManager.get.PauseNoInvoke();
            onOpen.Invoke();
        }

        private void OnDisable()
        {
            if (map != null && map.pauseGame == MapPause.PauseGame) WorldManager.get.UnpauseNoInvoke();
            onClose.Invoke();
        }
        ...
        private void GetZone()
        {
            ...
            for (var i = 0; i < map.zoneList.Count; i++)
        ...
        public void UpdateMesh(bool openRoomLists)
        {
            ...
            if (map != null) map.UpdateMesh(mesh, meshFilter, meshRenderer, eventList, openRoomLists);
        }
        ...
        private void UnlockRoom()
        {
            ...
            for (var z = 0; z < map.zoneList.Count; z++)
            {
                var zone = map.zoneList[z];
                if (zone.name != activeZone) continue;
                for (var r = 0; r < zone.roomList.Count; r++)
                    if (MapSO.IsPointInPoly(zone.roomList[r].pointList, target))
                    {
                        if (map.trackPlayerRealTime)
                        {
                            playerIcon.transform.position = player.position + (Vector3)zone.roomList[r].offset +
                                                            new Vector3(0, map.playerOffsetY, 0);
                            if (playerIcon != null && !playerIcon.activeInHierarchy) playerIcon.SetActive(true);
                        }
        ...
        public void UnlockRoom(string roomID)
        {
            ...
            if (parts.Length != 2 || map == null) return;

            var zoneName = parts[0];
            var roomNumber = int.Parse(parts[1]);
...
#pragma warning restore 0414
    ...
    {
        public List<RoomEvents> list = new();
    }
