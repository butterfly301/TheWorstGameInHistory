...
namespace TwoBitMachines.MapSystem
{
    ...
    {
        public static List<string> zoneNames = new();
        [SerializeField] public string mapName;
        [SerializeField] public MapPause pauseGame;
        [SerializeField] public InputButtonSO openInput;

        [SerializeField] public MeshData meshData = new();
        [SerializeField] public List<Zone> zoneList = new();
        [SerializeField] private SaveZoneList saveZones = new();
        ...
        {
            meshData.Clear();
            for (var i = 0; i < zoneList.Count; i++)
            {
                var zone = zoneList[i];
                for (var j = 0; j < zone.roomList.Count; j++)
                {
                    var room = zone.roomList[j];
        ...
        public void UpdateMesh(Mesh mesh, MeshFilter meshFilter, MeshRenderer meshRenderer, List<ZoneEvents> eventList,
            bool openRoomLists)
        {
            meshData.Clear();
            for (var i = 0; i < zoneList.Count; i++)
            ...
            for (var i = 0; i < zoneList.Count; i++)
            {
                ...
                {
                    var room = zone.roomList[j];
                    if (room.visible) meshData.Add(room.outlineMesh, room.offset);
                }
            }
        ...
        public void Save()
        {
            ...
            {
                var zone = zoneList[i];
                var saveZone = new SaveZone();
                saveZone.name = zone.name;
                saveZone.list = new List<bool>();
                saveZones.list.Add(saveZone);

                for (var j = 0; j < zone.roomList.Count; j++) saveZone.list.Add(zone.roomList[j].visible);
            }
        ...
        {
            saveZones.list.Clear();
            saveZones = Storage.Load(saveZones, WorldManager.saveFolder, key);

            for (var i = 0; i < saveZones.list.Count; i++)
            {
                var saveZone = saveZones.list[i];

                for (var j = 0; j < zoneList.Count; j++)
                    if (zoneList[j].name == saveZone.name)
                        for (var k = 0; k < zoneList[j].roomList.Count && k < saveZone.list.Count; k++)
        ...
        public static bool IsPointInPoly(List<Point> points, Vector2 offset, Vector2 point)
        {
            ...
            for (var i = 0; i < polyCorners; i++)
            {
                var a = points[j].position + offset;
                var b = points[i].position + offset;
        ...
        {
            for (var i = 0; i < zoneList.Count; i++) zoneList[i].SaveOriginalData();
        }
