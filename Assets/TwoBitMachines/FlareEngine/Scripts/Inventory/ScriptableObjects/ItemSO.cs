...
#pragma warning disable 0414
        ...
        {
            if (pickUpItem.isVendor) return pickUpItem.SellItem(this);
        ...
        public virtual void DropItem(Transform character, int droppedValue)
        {
            if (!consumable)
            {
                var equipment = character.GetComponent<Character>();
                if (equipment != null) equipment.DeactivateTool(itemName);
            }

            if (prefab != null)
            {
                var randomX = Random.Range(2f, 3f);
                var randomY = Random.Range(1f, 2f);
                var gameObject = Instantiate(prefab,
                    character.position + Vector3.up * randomY + Vector3.right * -randomX, Quaternion.identity);
                if (consumable)
                {
                    var item = gameObject
                        .GetComponent<Item>(); //    in case item is stacked, remember current stack value
                    if (item != null)
