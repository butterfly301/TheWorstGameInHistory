...
namespace TwoBitMachines
{
    ...
    {
        public static List<InventorySO> inventorySOList = new();
        [SerializeField] public List<ItemSO> referenceInventory = new();
        [SerializeField] public List<ItemSO> defaultItems = new();
        [NonSerialized] private readonly List<Inventory> inventories = new();
        [NonSerialized] public InventoryUI inventory = new();

        [NonSerialized] public PickUpItems pickUpItem; // This is the character
        [NonSerialized] public Dictionary<string, ItemSO> search = new();
...
#pragma warning disable 0414
        ...
        {
            for (var i = 0; i < inventorySOList.Count; i++) inventorySOList[i].Save();
        }
        ...
        public void Save()
        {
            Storage.Save(inventory, WorldManager.saveFolder, name);
        }
        ...
        public void Initialize(PickUpItems pickUpItem)
        {
            ...
            for (var i = 0; i < referenceInventory.Count; i++)
                if (referenceInventory[i] != null)
                    search.Add(referenceInventory[i].itemName, referenceInventory[i]);

            //* Retrieve saved data
            var savedInventory = Storage.Load(inventory, WorldManager.saveFolder, name);
        ...
        {
            for (var i = 0; i < inventory.itemList.Count; i++) inventory.itemList[i].slotID = -1;
        }
        ...
        {
            if (itemSO != null && inventory.AddItem(itemSO, droppedValue, out var newItemUI))
            {
        ...
        {
            if (search.TryGetValue(itemName, out var itemSO) && inventory.AddItem(itemSO, 0, out var newItemUI))
            {
                SlotsUpdate_NewItem(newItemUI, itemSO.icon);
                return true;
            }
        ...
        public void RemoveFromInventory(ItemUI removeItemUI)
        {
            ...
            {
                var itemSO = search[removeItemUI.name];
                if (itemSO != null && itemSO.isDroppable) itemSO.DropItem(pickUpItem.transform, removeItemUI.amount);
        ...
        public bool UseSlotItem(ItemUI useItemUI, out bool used, bool toggleTool = true)
        {
            ...
            if (useItemUI != null && pickUpItem != null && search.ContainsKey(useItemUI.name))
            {
                var itemSO = search[useItemUI.name];
                if (itemSO != null && itemSO.UseItem(pickUpItem, toggleTool))
                {
                    used = true;
                    if ((!pickUpItem.isVendor && itemSO.consumable) || (pickUpItem.isVendor &&
                                                                        itemSO.vendorItem ==
                                                                        VendorItemRemove.RemoveItemAfterSell))
                    {
        ...
        private void SlotsUpdate_NewItem(ItemUI newItemUI, Sprite icon)
        {
            ...
            {
                inventories[i].slot
                    .UpdateSlotTextAmount(); //        needs to be called in case it was a stacked consumable
                inventories[i].slot
                    .SetSlotItem(newItemUI, icon); //   will check for null in case new item was just a text update
            }
        }

        public void SlotsUpdate_ClearItem(ItemUI removeItemUI)
        {
            for (var i = 0; i < inventories.Count; i++) inventories[i].slot.ClearSlotItem(removeItemUI);
        }

        public void SlotsUpdate_Text()
        {
            for (var i = 0; i < inventories.Count; i++) inventories[i].slot.UpdateSlotTextAmount();
        }

        public void DeleteSavedData()
        {
            for (var i = 0; i < inventories.Count; i++) inventories[i].slot.ClearSlots();
    ...
    {
        [NonSerialized] public static List<ItemUI> temp = new();
        [SerializeField] public List<ItemUI> itemList = new();
        [SerializeField] public int barIndex;
        ...
        {
            newItemUI = null;
            var count = droppedValue > 0 ? droppedValue : 1;
            for (var i = 0; i < itemList.Count; i++)
                if (itemList[i].name == itemSO.itemName)
                {
                    if (itemSO.consumable)
                    {
                        ...
                        {
                            itemList[i].amount +=
                                count; //                            increase amount of existing item. If full, create another item
                            return true;
                        }
                    }
                    else
                    {
                        return
                            false; //                                                   already exists and is not consumable, do not add.
                    }
                }
