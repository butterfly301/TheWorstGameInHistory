...
namespace TwoBitMachines.FlareEngine
{
    public class InventorySlot : MonoBehaviour, ISelectHandler
    {
        ...
        public void OnSelect(BaseEventData eventData) // called automatically when inventory slot is selected
        {
            ...
            var oldID = inventory.slot.gridIndex;
            inventory.slot.gridIndex = ID;

            if (rectTransform != null) inventory.MoveSelectFrame(rectTransform.position);
            if (gameObject.activeInHierarchy && oldID != ID)
            {
                if (oldID >= 0)
                {
                    var oldItemUI = inventory.slot.Get(oldID).itemUI;
            ...
            if (inventory.itemCost != null) inventory.itemCost.text = empty ? "" : inventory.GetItemCost(itemUI.name);
            if (inventory.itemName != null) inventory.itemName.text = empty ? "" : itemUI.name;
            if (inventory.removeItem != null && !empty) inventory.removeItem.gameObject.SetActive(itemUI.droppable);
        ...
        {
            if (!empty && inventory != null) inventory.RemoveItem(itemUI);
        }
        ...
        public void UpdateTextAmount()
        {
            if (itemUI == null || textMesh == null)
                return;
            var enabled = itemUI.amount > 1 ? 10 : 0; // using enable was causing a huge lag spike
        ...
        public void OnBeginDrag()
        {
            ...
            if (inventory.slot.gridIndex == ID) inventory.MoveSelectFrame(rectTransform.position);
