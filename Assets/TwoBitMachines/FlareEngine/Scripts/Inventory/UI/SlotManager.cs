...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class SlotManager
    {
        ...
        public void SetSlotItem(ItemUI setItemUI, Sprite icon, bool initialize = false)
        {
            ...
            for (var i = 0; i < slots.Count; i++) // if resetting, we match item with slot id, or else we skip
                if (slots[i] != null && slots[i].empty &&
                    (!initialize || setItemUI.slotID == i ||
                     setItemUI.slotID ==
                     -1)) // If -1, it means the item is a default item and can be placed on the first empty slot
                {
        ...
        {
            for (var i = 0; i < slots.Count; i++) slots[i].Clear();
        }

        public void ChangeBarIndex(int totalItems, int direction)
        {
            var size = Mathf.Max(totalItems, count);
            if (direction > 0)
