...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        [NonSerialized] public static bool blockInventories;
        ...
        [SerializeField] public List<Inventory> block = new();
        ...
        [SerializeField] public SlotManager slot = new();
        [SerializeField] public float selectFrameSpeed = 0.25f;
        [SerializeField] public bool autoItemLoad;
        [SerializeField] public int autoLoadIndex;
        ...
        [NonSerialized] private GameObject lastSelect;
        [NonSerialized] private ItemUI previousItem;
        [NonSerialized] private bool skippedFirstFrame;
        ...
        private void LateUpdate()
        {
            ...
            {
                if (navigation == SlotNavigation.Manual && slot.count > 0) UseItem(slot.Get(0).itemUI);
                if (navigation == SlotNavigation.UnityNavigation && slot.count > 0 && slot.gridIndex >= 0 &&
                    slot.gridIndex < slot.count) UseItem(slot.Get(slot.gridIndex).itemUI);
            }
        ...
        {
            if (pauseType == PauseGameType.PauseGame) WorldManager.get.Pause();
            if (pauseType == PauseGameType.BlockPlayerInput) Player.BlockAllInputs(true);
            onOpen.Invoke();
            if (slot.count > 0 && slot.gridIndex <= 0) slot.Get(0).OnSelect(null);
        ...
        public void Initialize(PickUpItems pickUpItem)
        {
            ...
            for (var i = 0; i < inventorySO.list.Count; i++) // Put saved items, if any, back into slots
                slot.SetSlotItem(inventorySO.list[i], GetItemIcon(inventorySO.list[i].name), true);
            if (navigation == SlotNavigation.Manual) AutoItemLoad();
            if (slot.count > 0) slot.Get(0).OnSelect(null);
        }
        ...
        {
            if (slot.count > 0 && index < slot.count) UseItem(slot.Get(index).itemUI);
        }

        private void AutoItemLoad()
        {
            var newItemUI = slot.Get(autoLoadIndex).itemUI;
            ...
            if (autoItemLoad && slot.count > 0 && autoLoadIndex < slot.count) UseItem(newItemUI, false);
        }
        ...
        public void UseItem(ItemUI useItemUI, bool toggleTool = true)
        {
            ...
            {
                if (inventorySO.UseSlotItem(useItemUI, out var used, toggleTool)) ClearSelected();
...
#pragma warning restore 0414
        ...
        public void MoveSelectFrame(Vector2 position)
        {
            ...
            Wiggle.Target(selectFrame.gameObject).MoveTo2D(position, selectFrameSpeed, Tween.EaseInOut)
                .UnscaledTime(true);
        }
        ...
        private void MoveSlots(List<ItemUI> list, int left, int right)
        {
            ...
            var direction = slot.count >= totalItems ? left : right;
        ...
        {
            var list = inventorySO.ItemList(slot.viewItems, slot.keyName);
            var totalItems = list.Count;
            slot.barIndex = 0;
            if (offset != 0)
