...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    {
        public static List<HighJump> highJumps = new();
        ...
        [SerializeField] public bool moveWithParent;
        [SerializeField] public Vector2 windDirection = new(0, 1f);
        ...
        [NonSerialized] private float slowTimer;

        [NonSerialized] private float windTimer;
        ...
        {
            if (!highJumps.Contains(this)) highJumps.Add(this);
        }

        private void OnDisable()
        {
            if (highJumps.Contains(this)) highJumps.Remove(this);
        }
        ...
        {
            if (Clock.Timer(ref windTimer, windRate)) return true;
            return false;
        }

        public bool SlowTimer()
        {
            if (Clock.Timer(ref slowTimer, slowRate)) return true;
            return false;
        }

        public static bool Find(WorldCollision character, float characterVelocityY, ref int highJump, ref float timer,
            ref Vector2 force, ref Transform oldBoost)
        {
            for (var i = 0; i < highJumps.Count; i++)
                if (highJumps[i] != null && highJumps[i].DetectCharacter(character))
                {
                    if (highJumps[i].type == HighJumpType.Trampoline)
                    {
                        if (characterVelocityY > 0) return false;
                        character.hitInteractable = true;
                        force = highJumps[i].force * highJumps[i].transform.up;
                        highJumps[i].onTrampoline.Invoke(ImpactPacket.impact.Set(highJumps[i].trampolineWE,
                            character.position, Vector2.zero));
                        highJump = 1;
                        return true;
                    }

                    if (highJumps[i].type == HighJumpType.Wind)
                    {
                        character.hitInteractable = true;
                        Vector2 windDirection = highJumps[i].transform.up; //highJumps[i].windDirection;
                        var percentToTop =
                            Mathf.Clamp01(character.transform.position.y / (highJumps[i].bounds.top - 0.5f));
                        var windForce = Mathf.Lerp(0f, highJumps[i].force, 1f - percentToTop);
                        force = windDirection * windForce * Time.deltaTime * 10f;
                        if (highJumps[i].WindTimer())
                            highJumps[i].onWind.Invoke(ImpactPacket.impact.Set(highJumps[i].windWE, character.position,
                                Vector2.zero));
                        highJump = 2;
                        return true;
                    }

                    if (highJumps[i].type == HighJumpType.SpeedBoost)
                    {
                        if (highJumps[i].transform == oldBoost) return false;
                        force = highJumps[i].force * highJumps[i].transform.up;
                        timer = highJumps[i].timer;
                        highJumps[i].onSpeedBoost.Invoke(ImpactPacket.impact.Set(highJumps[i].speedBoostWE,
                            character.position, Vector2.zero));
                    ...
                    if (highJumps[i].SlowTimer())
                        highJumps[i].onSlowDown
                            .Invoke(ImpactPacket.impact.Set(highJumps[i].slowDownWE, character.position, Vector2.zero));
