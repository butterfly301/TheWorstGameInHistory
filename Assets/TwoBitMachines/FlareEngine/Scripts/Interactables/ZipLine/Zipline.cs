...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    public class Zipline : MonoBehaviour
    {
        ...
        {
            if (zipLines.Count == 0 || Time.deltaTime == 0) return;

            for (var i = zipLines.Count - 1; i >= 0; i--)
            ...
            if (zip.exitMomentum)
            {
                ...
                if (zip.exitMomentum) velocity.x = Compute.Lerp(zip.gravityMomentum, 0, 1f, ref zip.counter);
            }
        }
...
#pragma warning restore 0414
        ...
        private void Awake()
        {
            if (createOnAwake) CreateZipLine();
        }

        private void OnEnable()
        {
            if (!zipLines.Contains(this)) zipLines.Add(this);
        }

        private void OnDisable()
        {
            if (zipLines.Contains(this)) zipLines.Remove(this);
        }
        ...
        public void CreateZipLine()
        {
            ...
            var plankLength = distance / lines - 0f;
            var particles = lines + 1;
            ...
            if (line != null) line.positionCount = particles;
        }
        ...
        public void Ziplining(int index, AbilityManager player, Vector2 center, ZipInfo zip, ref Vector2 velocity)
        {
            ...
            else if (zip.state.TryGetValue(index, out var state) && state != PlankState.BeginSearch)
            {
        ...
        private int LinePointingUp(int index)
        {
            ...
            {
                var start = particle[stick[index].first].position;
                var end = particle[stick[index].second].position;
            ...
            out Vector2 intersectionPoint, out int index)
        {
            ...
            characterPosition.x =
                Mathf.Clamp(characterPosition.x, rect.x + 0.01f,
                    rect.x + rect.width -
                    0.01f); // keep center in bounds so that character always intersects with a bridge plank
            var characterTop = longSearch ? new Vector2(characterPosition.x, rect.max.y) : characterPosition;
            var characterBottom = longSearch
                ? new Vector2(characterPosition.x, rect.min.y)
                : characterPosition - Vector2.up * characterHeight +
                  Vector2.up *
                  (velY * Time.deltaTime -
                   0.2f); // 0.2f (arbitrary) extend length to find bridge quicker and prevent jump/fall state
        ...
        {
            if (!zip.state.ContainsKey(i)) zip.state.Add(i, PlankState.BeginSearch);

            var world = player.world;
            var velocityAdjust = world.box.right * velocity.x * Time.deltaTime;
            var characterTop = world.box.topCenter + velocityAdjust + Vector2.up * zip.yOffset;
            ...
            if (zip.state[i] == PlankState.BeginSearch)
                if (TetherIntersection(characterTop, true, 0, 0, out var intersectionPoint,
                        out var index)) //is character above or below current tether? If not, must pass threshold check before ziplining.
            ...
            if (zip.state[i] == PlankState.LerpToTether)
            {
                ...
                else if (velocity.y <= 0 && TetherIntersection(characterTop, false, world.box.sizeY * 0.3f, velocity.y,
                             out var intersectionPoint, out var index))
                {
                    var distance = intersectionPoint.y - characterTop.y;
            ...
            {
                zip.AutoSlideToCenter(LinePointingUp(zip.zipIndex), player.speed, ref velocity, out var goingUp);
                var friction = goingUp && upFriction > 0 ? upFriction : 1f;
                ...
                if (TetherIntersection(characterTop, true, 0, 0, out var intersectionPoint, out var index))
                {
                    if (zip.exitOnInput)
                    {
                    ...
                    else
                    {
                        ...
                        if (velocity.x != 0)
                            stick[index].ApplyAcceleration(particle,
                                Vector2.down * bounce * Mathf.Abs(velocity.x) * 0.03f);
                    }
                }
            }
        }
