...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    {
        public static List<Rope> ropes = new();
        ...
        [SerializeField] private List<Tether> tethers = new();
        ...
        private void FixedUpdate()
        {
            ...
            if (doubleAnchor) particle[last].SetPosition(transform.position + (Vector3)endOffset);
            ...
            for (var i = 0; i < tethers.Count; i++) tethers[i].RopeRotate(particle, stick[i].first, stick[i].second);
        }

        private void OnEnable()
        {
            if (!ropes.Contains(this)) ropes.Add(this);
        }

        private void OnDisable()
        {
            if (ropes.Contains(this)) ropes.Remove(this);
        }
        ...
        {
            if (particle.Length > 0) particle[last].anchor = false;
        ...
        {
            for (var i = ropes.Count - 1; i >= 0; i--) ropes[i]?.ResetAll();
        }

        public static bool HoldPoint(RopeSwing ropeSwing, Particle[] particle, Vector2 center, Vector2 right,
            float radius, ref Vector2 newHoldPoint)
        {
            var pointLeft = center - right * radius;
            var pointRight = center + right * radius;

            for (var i = 1; i < particle.Length; i++)
                if (Compute.LineIntersection(particle[i].position, particle[i - 1].position, pointLeft, pointRight,
                        out var intersect))
                {
        ...
        public static bool
            Find(Vector2 characterCenter, Vector2 noneGravityVel,
                RopeSwing ropeSwing) // Velocity that doesn't include the effect of gravity
        {
            for (var i = ropes.Count - 1; i >= 0; i--)
                if (ropes[i] == null)
                {
                    ropes.RemoveAt(i);
                }
        ...
        {
            if (type == RopeType.Swing) return LatchToRope(ropeSwing, characterCenter);
        ...
        {
            if (!isClimbable) return (ropeHandle.position - center).sqrMagnitude <= tetherRadius * tetherRadius;

            return HoldPoint(ropeSwing, particle, center, Vector2.right, tetherRadius,
                ref center); // ref center is dummy variable
        }
        ...
        private void PushThroughRope(Vector2 center, Vector2 noneGravityVel)
        {
            ...
            {
                var skipLast = doubleAnchor ? 1 : 0;
                for (var i = 1; i < particle.Length - skipLast; i++) // Start at one since first is always an anchor
                {
...
#pragma warning disable 0414
        ...
        public void CreateRope()
        {
            ...
            for (var i = 0; i < particle.Length; i++)
            {
                ...
                if (particle.Length - 1 == i) particle[i].Set(startPosition + endOffset, -gravity, doubleAnchor);
            }
        ...
        private void CreateRopeGameObjects(Vector2 direction)
        {
            ...
            for (var i = 0; i < tethers.Count; i++)
            {
                ...
                if (i == 0)
                {
                    var renderer = tethers[i].gameObject.GetComponent<SpriteRenderer>();
        ...
        private void OnDrawGizmos()
        {
            ...
            Draw.GLStart();
            if (type == RopeType.Idle) Draw.GLCircle(searchPoint, ropeRadius, Color.yellow, (int)ropeRadius * 2);
            for (var i = 0; i < particle.Length; i++)
            {
