...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    public class FoliageTextureArray
    {
        ...
        [SerializeField] public Vector2Int textureSize = new(16, 8);

        [SerializeField] public Mesh mesh;
        [SerializeField] public Material material;
        [SerializeField] public List<FoliageTexture> textures = new(); //*  all Textures MUST be the same size
        [SerializeField] public List<FoliageInstance> foliage = new();
        [SerializeField] public Matrix4x4[] matrixList = { };
        ...
        public MaterialPropertyBlock property; //                                 not serializable
        ...
        public bool GroupTextures()
        {
            ...
            for (var i = 0; i < textures.Count; i++)
            {
                ...
                {
                    Debug.LogWarning("Foliage could not create texture array. Texture " + texture.name +
                                     " is not of size " + textureSize + " but of size (" + texture.width + "," +
                                     texture.height + ")");
                    return false;
                }
            }
            ...
            var textureArray = CreateTextureArray(textures[0].texture);
        ...
        {
            var textureArray =
                new Texture2DArray(texture.width, texture.height, textures.Count + 1, texture.format, false);
        ...
        {
            var blankTexture =
                new Texture2D(texture.width, texture.height, texture.format,
                    false); //* create a clear texture to set as first texture for textureArray. This will avoid weird artifacts when rending other textures in scene view.
        ...
        {
            if (instance_orientation == null || instance_orientation.Length == 0) return;
        ...
        public void DrawMeshes()
        {
            if (SystemInfo.supportsInstancing && matrixList.Length > 0 && matrixList.Length < 1024 && material != null)
                Graphics.DrawMeshInstanced(mesh, 0, material, matrixList, matrixList.Length, property,
