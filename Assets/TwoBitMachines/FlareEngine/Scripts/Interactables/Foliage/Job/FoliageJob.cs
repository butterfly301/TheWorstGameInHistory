...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    public struct FoliageJob : IJobParallelFor
    {
        ...
        {
            var grassPosition = position[id] + size * 0.5f;

            for (var i = 0; i < playerPosition.Length; i++)
            {
                Vector2 currentPosition = playerPosition[i];
                var oldPosition = playerOldPosition[i];
                var interaction = interact[id].x;
                //                                                                                    interact x == interaction, Interact y == orientation
                if (interact[id].y == 1) //                                                           top vertices
                {
                    currentPosition +=
                        playerPosition[i].z *
                        Vector2.up; //                       offset player position with player height if testing for ceiling grass
                    oldPosition += playerPosition[i].z * Vector2.up; //                           z is player height
                }

                if (interaction > 0 &&
                    (currentPosition - grassPosition).sqrMagnitude < squareDistance) // Within grass range
                {
                    var collided = false;
                    var ID = interact[id].y;
                    var allOthers = interact[id].y != 1;
                    var isCeiling = interact[id].y == 1 && playerVelocity[i].y > 0;

                    if (ID == 0 || ID == 1) // bottom, top
                    {
                        if (CrossedThreshold(currentPosition.x, oldPosition.x, grassPosition.x))
                        {
                            var xStrength = math.clamp(math.abs(playerVelocity[i].x), 0, 0.25f);
                            velocity[id] = math.sign(playerVelocity[i].x) < 0
                                ? interaction * -xStrength
                                : interaction * xStrength;
                            collided = true;
                        }
                        ...
                        {
                            var yStrength = math.clamp(math.abs(playerVelocity[i].y), 0, 0.10f);
                            velocity[id] = currentPosition.x > grassPosition.x
                                ? interaction * -yStrength * 0.7f
                                : interaction * yStrength * 0.7f;
                            collided = true;
                        }
                    }
                    ...
                    if (ID == 2 || ID == 3) // left, right
                    {
                        if (CrossedThreshold(currentPosition.x, oldPosition.x, grassPosition.x))
                        {
                            var xStrength = math.clamp(math.abs(playerVelocity[i].x), 0, 0.3f);
                            velocity[id] = math.sign(playerVelocity[i].x) < 0
                                ? interaction * -xStrength
                                : interaction * xStrength;
                            collided = true;
                        ...
                        if (CrossedThreshold(currentPosition.y, oldPosition.y, grassPosition.y))
                        {
                            var yStrength = math.clamp(math.abs(playerVelocity[i].y), 0, 0.1f);
                            velocity[id] = currentPosition.y > grassPosition.y
                                ? interaction * yStrength
                                : interaction * -yStrength;
                            collided = true;
                        }
                    }
            ...
            //* spring physics
            var acceleration =
                -jiggle * vertexOffset[id] * deltaTime * 25f - velocity[id] * damping; // * deltaTime * 25f;
            ...
            //* Perlin noise
            var perlinNoise = noise.cnoise(new Vector2(this.perlinNoise[id],
                grassPosition.x * (1f - uniformity) + grassPosition.y));
