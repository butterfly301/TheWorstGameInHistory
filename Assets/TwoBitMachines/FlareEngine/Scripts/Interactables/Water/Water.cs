...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    {
        public static List<Water> water = new();
        ...
        [SerializeField] public Vector2 size = new(20f, 5f);
        [SerializeField] public WaterMesh waterMesh = new();
        [SerializeField] public WaterBatch waterBatch = new();
        [SerializeField] public WaveProperties wave = new();
        ...
        [NonSerialized] private Rect searchRect;

        private void Awake()
        {
            if (createOnAwake) CreateWaves();

            searchRect.Set(transform.position.x, transform.position.y - 1f, size.x,
                size.y * 2f); // subtract 1 from bottom to detect player even if sitting at the bottom

            if (isSquare) waterBatch.CreatePropertyBlock();
        }

        private void FixedUpdate()
        {
            var phase = wave.Execute(this, topY, size.y);
        ...
        {
            if (!water.Contains(this)) water.Add(this);
            if (isSquare) waterBatch.OnEnable();
        }

        private void OnDisable()
        {
            if (water.Contains(this)) water.Remove(this);
            if (isSquare) waterBatch.OnDisable();
        }
        ...
        public bool FoundWater(Vector2 entryPoint, bool wasInWater, ref Vector2 waveTopPoint, ref int index,
            bool force = false)
        {
            if (!(wasInWater || force ||
                  searchRect.Contains(
                      entryPoint))) //                               force is for AI (AI will always find the water)
                return false;

            entryPoint = wasInWater ? new Vector2(entryPoint.x, bottomY) : entryPoint;
            var near = Mathf.RoundToInt(Mathf.Abs(entryPoint.x - transform.position.x) /
                                        particleLength); //approximate landing point
            near = Mathf.Clamp(near, 1, wave.wave.Length - 1);

            for (index = near - 1; index < near + 1; index++)
            {
                if (index < 0 || index >= wave.wave.Length - 1) return false;
                if (Compute.LineIntersection(entryPoint, new Vector2(entryPoint.x, topHeight.y),
                        wave.wave[index].position, wave.wave[index + 1].position, out var intersectPoint))
                {
...
#pragma warning disable 0414
        ...
        [SerializeField] [HideInInspector] private bool bodyFoldOut;

        public void DrawBatches(SceneView sceneView)
        {
            if (!waterBatch.canRender) return;
            if (waterBatch.propertyBlockNull) waterBatch.CreatePropertyBlock();
