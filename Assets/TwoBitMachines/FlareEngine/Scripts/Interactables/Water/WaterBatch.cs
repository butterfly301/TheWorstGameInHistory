...
namespace TwoBitMachines.FlareEngine.Interactables
{
    ...
    {
        public static List<WaterBatch> waterBatch = new();
        [SerializeField] private float crestTaper;
        ...
        [SerializeField] private Color crestColor = new(250f / 255f, 250f / 255f, 250f / 255f, 240f / 255f);
        [SerializeField] private Color bodyColor = new(16f / 255f, 250f / 255f, 255f / 255f, 240f / 255f);
        [SerializeField] private Color bottomColor = new(0f / 255f, 219f / 255f, 238f / 255f, 240f / 255f);

        [SerializeField] private Mesh mesh;
        [SerializeField] private Material material;
        [SerializeField] private List<Vector4> colors = new();
        [SerializeField] private List<WaterWave> waves = new();
        [SerializeField] private List<Matrix4x4> tempData = new();
        ...
        private void AddWave(int particles, float particleLength, float bottomY, Point[] point, Color color,
            List<Vector4> colors)
        {
            for (var i = 0; i < particles; i++)
            {
                var waveParticle = new WaterWave
                {
                    scale = new Vector3(particleLength, 1, 1), position = new Vector3(point[i].x, bottomY, 0),
                    bottomY = bottomY
                };
                colors.Add(color);
        ...
        {
            if (!waterBatch.Contains(this)) waterBatch.Add(this);
        }

        public void OnDisable()
        {
            if (waterBatch.Contains(this)) waterBatch.Remove(this);
        }
        ...
        public static void Run()
        {
            ...
            for (var i = waterBatch.Count - 1; i >= 0; i--)
        ...
        public void BatchAndRender()
        {
            ...
            for (var i = 0; i < count; i += 1023)
            {
                ...
                if (i + 1023 < count)
                {
                    for (var ii = 0; ii < 1023; ii++) tempData.Add(waves[i + ii].TransformData());
                    Graphics.DrawMeshInstanced(mesh, 0, material, tempData, propertyBlock);
                }
                else //  Last batch
                {
                    for (var ii = 0; ii < count - i; ii++) tempData.Add(waves[i + ii].TransformData());
                    Graphics.DrawMeshInstanced(mesh, 0, material, tempData,
                        propertyBlock); // Graphics.DrawMeshInstaNced does not work in openGl2
                }
            }
        }
        ...
        public void SetWaterBatchHeights(WaveProperties wave, float topY, float height, float phase)
        {
            ...
            var bottomTopY = topY - amplitude;
            var baseLine = topY - height;
            var count = waves.Count / 3;
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var crest = waves[count * 2 + i];
                crest.position = body.position + Vector3.up * body.scale.y;
                var percent = amplitude <= 0
                    ? 0
                    : Mathf.Clamp((wave.wave[i].y - bottomTopY) / (amplitude * 2f), 1f - crestTaper, 1f);
                crest.scale = new Vector2(body.scale.x, crestThickness * percent);

                var bottom = waves[count + i];
                var phaseY = topY + Compute.SineWave(phase * (i + phaseShift), bottomSpeed, wave.amplitude);
                var newBaseY = wave.JoinWaves(topY, baseLine, phaseY, wave.dynamicWave[i].position.y);
                var scaleY = Math.Abs(newBaseY - body.bottomY) - bottomOffset;
