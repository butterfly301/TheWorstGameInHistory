...
namespace TwoBitMachines.FlareEngine
{
    ...
    {
        [NonSerialized] public static List<DialogueBubble> dialogues = new();
        ...
        {
            for (var i = 0; i < dialogues.Count; i++) dialogues[i]?.EndConversation();
        }
        ...
        public void EndConversation()
        {
            ...
            messageIndex = -1;
            TurnOffOtherBubbles();
            if (blockPlayerInput) Player.BlockAllInputs(false);
        }
        ...
        private void SetConversation(Conversation newConversation)
        {
            ...
            if (blockPlayerInput) Player.BlockAllInputs(true);
            messageIndex = 0;
        ...
        private void SetDialogueUI(Message message)
        {
            ...
            if (messenger != null && messenger.bubble != null)
            {
                ...
                if (message.type == MessageType.Message)
                    messenger.bubble.TransitionIn(message.message, fadeInTime, tweenIn,
                        MessageDirection(message.messageTo), null, this);
                else
                    for (var i = 0; i < message.choice.Count; i++)
                        if (i < messenger.bubbles.Count && messenger.bubbles[i] != null)
                        {
                            ...
                            var bubble = messenger.bubbles[i];
                            var choice = message.choice[i];
                            bubble.TransitionIn(choice.choice, fadeInTime, tweenIn, MessageDirection(message.messageTo),
                                choice, this);
                        }
            }
        }
        ...
        private void TurnOffOtherBubbles()
        {
            for (var i = 0; i < messengers.Count; i++)
            {
                ...
                if (messengers[i] != messenger && messengers[i].bubbles != null)
                    for (var j = 0; j < messengers[i].bubbles.Count; j++)
        ...
        {
            if (messenger == null || messenger.bubble == null || !messenger.bubble.isDirectional) return 1f;
            for (var i = 0; i < messengers.Count; i++)
                if (messengers[i].name == messageTo && messengers[i].bubble != null)
                    return messenger.bubble.transform.parent.transform.position.x <
                           messengers[i].bubble.transform.parent.transform.position.x
                        ? 1f
                        : -1f;
