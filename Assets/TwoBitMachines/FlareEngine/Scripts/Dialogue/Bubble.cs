...
namespace TwoBitMachines.FlareEngine
{
    ...
    public class Bubble : MonoBehaviour, ITypeWriterComplete
    {
        ...
        [SerializeField] public Vector2 maxSize = new(6f, 6f);
        [SerializeField] public Vector2 minSize = new(2f, 1f);
        [SerializeField] public Vector2 padding = new(0.5f, 0.5f);
        [SerializeField] public float directionXOffset;
        ...
        [SerializeField] public UnityEvent messageLoaded = new();
        [NonSerialized] private Choice choice;
        [NonSerialized] private RectTransform containerRect;
        [NonSerialized] private TextMeshProEffects effects;
        [NonSerialized] private DialogueBubble parent;

        [NonSerialized] private Vector2 size;
        ...
        public void TypingComplete()
        {
            ...
            if (parent != null && parent.message != null) parent.message.onMessage.Invoke();
        }
        ...
        public void TransitionIn(string message, float fadeTime, Tween tween, float messageDirection,
            Choice choiceRef = null, DialogueBubble parentRef = null)
        {
            choice = choiceRef;
            parent = parentRef;
            messageLoadingComplete = false;
            if (textRect == null) textRect = text?.GetComponent<RectTransform>();
            if (containerRect == null) containerRect = image?.GetComponent<RectTransform>();
            if (text != null && textRect != null && containerRect != null)
            {
                ...
                textRect.anchoredPosition =
                    new Vector2(0, size.y / 2 + offsetY); // Adjust Y position to align with container bottom
                // not working  size += padding;                                                          

                var parentRect = containerRect.parent as RectTransform;
                if (parentRect != null)
                {
                    var parentCenterY = parentRect.rect.height * 0.5f;
                    containerRect.anchoredPosition = new Vector2(0, parentCenterY);
                }
            }

            if (!gameObject.activeInHierarchy) gameObject.SetActive(true);
            if (effects == null) effects = gameObject.GetComponent<TextMeshProEffects>();
        ...
        private void Flip(float direction)
        {
            if (isDirectional && direction != 0)
            {
                ...
                transform.localPosition =
                    new Vector3(Mathf.Abs(size.x * 0.5f + directionXOffset) * direction, lp.y, lp.z);
            }
        }
