...
namespace TwoBitMachines
{
    ...
    public class TweenParent
    {
        ...
        [SerializeField] public bool useAnchors;
        [SerializeField] public Transform transform;
        [SerializeField] public RectTransform rectTransform;
        [SerializeField] public List<TweenChild> children = new();

        [SerializeField] private int loopLimit;
        ...
        public bool Run()
        {
            ...
            for (var i = 0; i < children.Count; i++)
            {
                ...
                if (child.active)
                {
                    ...
                    if ((foundParallel || stillActive) && !child.parallel) break;
                    if (foundParallel && child.parallel) child.Run(transform, rectTransform, useAnchors, unscaledTime);
                }
            }
        ...
        private TweenParent Add(float target, float time, float speed, Tween tween, Act act, Axis axis,
            Interpolate interpolate = Interpolate.Float)
        {
            var tweenChild = Wiggle.GetTweenChild();
            tweenChild.Set(Vector3.right * target, time <= 0 ? 1f : time, speed, act, axis, tween, interpolate);
            children.Add(tweenChild);
            return this;
        }

        private TweenParent Add(Vector3 target, float time, float speed, Tween tween, Act act, Axis axis,
            Interpolate interpolate = Interpolate.Float)
        {
            var tweenChild = Wiggle.GetTweenChild();
        ...
        {
            for (var i = 0; i < children.Count; i++) children[i].Reactivate();
        }
...
#pragma warning disable 0414
        [SerializeField] private bool activeR;
...
#pragma warning restore 0414
        ...
        {
            return Add(center, seconds, turnsPerSecond, Tween.Linear, Act.RotateAroundX, Axis.One,
                Interpolate.RotateAround);
        }

        public TweenParent RotateAroundY(Vector3 center, float turnsPerSecond, float seconds)
        {
            return Add(center, seconds, turnsPerSecond, Tween.Linear, Act.RotateAroundY, Axis.One,
                Interpolate.RotateAround);
        }

        public TweenParent RotateAroundZ(Vector3 center, float turnsPerSecond, float seconds)
        {
            return Add(center, seconds, turnsPerSecond, Tween.Linear, Act.RotateAroundZ, Axis.One,
                Interpolate.RotateAround);
        }
