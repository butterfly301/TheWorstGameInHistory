...
namespace TwoBitMachines
{
    ...
    public class TweenChild
    {
        ...
        [SerializeField] public UnityEvent onComplete = new();
        ...
        public void Set(Vector3 target, float time, float speed, Act act, Axis axis, Tween tween,
        ...
        public void Run(Transform T, RectTransform rectT, bool useAnchors, bool unscaledTime)
        {
            ...
            if (delay > 0 && Clock.TimerInverseExpired(ref delayCounter, delay)) return;
        ...
        public void InterpolateTween(float deltaTime, float easing, Vector3 target, Transform T, RectTransform rectT,
            bool useAnchors)
        {
            if (interpolate == Interpolate.Float)
            {
                if (axis == Axis.One)
                    SetTween.Set(act, Mathf.LerpUnclamped(from.x, target.x, easing), T, rectT,
                        useAnchors); // x can represent x,y, or z, jus a placeholder
            ...
            {
                SetTween.SetQuaternion(act,
                    Quaternion.LerpUnclamped(Quaternion.Euler(from), Quaternion.Euler(target), easing), T, rectT);
            }
        }
