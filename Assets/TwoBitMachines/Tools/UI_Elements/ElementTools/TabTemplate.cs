...
namespace TwoBitMachines.UIElement.Editor
{
    ...
    public class Tab : BindableElement
    {
        ...
        public Button TabButton(string background, string icon, Color backgroundColor, Color iconColor,
            int tabIndex = -1, SerializedProperty toggle = null, string iconOff = "", string tooltip = "")
        {
            ...
            if (toggle != null) image.SetImage(toggle.boolValue ? icon : iconOff);

            aTabWasPressed += () =>
            {
                button.style.unityBackgroundImageTintColor = index == tabIndex && tabIndex != -1
                    ? backgroundColor * Tint.activeTint
                    : backgroundColor;
            };
            button.clicked += () =>
            {
            ...
            {
                if (index != tabIndex || tabIndex == -1) button.style.unityBackgroundImageTintColor = backgroundColor;
            });
            return button;
        }
