...
namespace TwoBitMachines.Editors
{
    public static class Labels
    {
        ...
        public static GUIStyle fieldTextStyle = new();
        public static RectOffset rectZero = new(0, 0, 0, 0);
        ...
        public static void Label(string title, Rect rect, bool bold = false, bool whiteStyle = false)
        {
            EditorGUI.LabelField(rect, title,
                useWhite || whiteStyle ? labelStyle : bold ? EditorStyles.boldLabel : EditorStyles.label);
        }
        ...
        public static void Label(Rect rect, string title, Color color, float fontSize = 12, bool bold = true)
        {
            ...
            EditorGUI.LabelField(rect, title, textStyle);
            ResetTextStyle();
        }
        ...
        public static void Label(string title, Color color, float fontSize = 12, bool bold = true)
        {
            ...
            textStyle.clipping = TextClipping.Clip;
            EditorGUI.LabelField(Layout.CreateRectField(), title, textStyle);
            ResetTextStyle();
        }
        ...
        public static void LabelCenterWidth(string title, Rect rect, LabelType type, int yOffset = 0)
        {
            ...
            rect.y += yOffset;
            EditorGUI.LabelField(rect, title,
                type == LabelType.White ? labelStyle :
                type == LabelType.Bold ? EditorStyles.boldLabel : EditorStyles.label);
        }
        ...
        public static bool LabelAndButton(string title, string icon)
        {
            ...
            var buttonWidth = 15;
            Label(title, rect.Adjust(0, Layout.labelWidth + Layout.contentWidth - buttonWidth + 2));
            return rect.Adjust(Layout.labelWidth + Layout.contentWidth - buttonWidth + 2, buttonWidth)
        ...
        {
            var rect = Layout.CreateRectField();
            EditorGUI.LabelField(rect.Adjust(space, Layout.labelWidth), title,
                useWhite ? labelStyle : EditorStyles.label);
            var pressed = rect.Adjust(+Layout.labelWidth + Layout.contentWidth - buttonWidth - space, buttonWidth - 2)
                .Button(Icon.Get(icon), Tint.White, center: true);
            return pressed;
        }
        ...
        public static void Centered(Rect rect, string label, Color color, int fontSize = 11, int shiftY = 0)
        {
            ...
            var labelSize = style.CalcSize(new GUIContent(label));
        ...
        public static void Display(Rect rect, string label, float offsetX = 0, float offsetY = 0,
        ...
        {
            var rect = Layout.CreateRect(Layout.infoWidth, Layout.rectFieldHeight);
        ...
        public static void InfoBox(int height, string message, float offsetY = -1)
        {
            ...
            var rect = Layout.CreateRect(Layout.longInfoWidth, height, -11, offsetY);
        ...
        {
            var rect = Layout.GetLastRect(Layout.longInfoWidth, height, -11, -height + offsetY);
            rect.DrawRect(EditorGUIUtility.isProSkin ? Tint.SoftDark240 : Tint.WarmGrey225);
            EditorGUI.HelpBox(rect, message, type);
        }

        public static void FieldText(string title1, float rightSpacing = 2, bool execute = true, int space = 0,
            int yOffset = 2)
        {
            if (!execute)
                return;
            var rect = Layout.GetLastRect(Layout.infoWidth, Layout.rectFieldHeight);
            rect.y += yOffset;
            fieldTextStyle.normal.textColor = EditorGUIUtility.isProSkin ? Tint.WarmWhite : Tint.HardDark;
            var title1Width = fieldTextStyle.CalcSize(new GUIContent(title1)).x + 4f;
            EditorGUI.LabelField(rect.Adjust(rect.width - title1Width - rightSpacing, rect.width), title1,
                fieldTextStyle);
        }

        public static void FieldDoubleText(string title1, string title2, bool show1 = true, bool show2 = true,
            float rightSpacing = 2, bool execute = true, int space = 0)
        {
            ...
            rect.y += 2;
            var origin = new Rect(rect);
            var width = (Layout.contentWidth - rightSpacing) / 2f;
            fieldTextStyle.normal.textColor = EditorGUIUtility.isProSkin ? Tint.WarmWhite : Tint.HardDark;
            if (show2)
            {
                var title2Width = fieldTextStyle.CalcSize(new GUIContent(title2)).x + 4f;
                EditorGUI.LabelField(rect.Adjust(rect.width - title2Width - rightSpacing, width), title2,
                    fieldTextStyle);
            }
            ...
            if (show1)
            {
                rect = origin;
                var title1Width = fieldTextStyle.CalcSize(new GUIContent(title1)).x + 4f;
                EditorGUI.LabelField(rect.Adjust(rect.width - title1Width - rightSpacing - width, width), title1,
                    fieldTextStyle);
            }
        }

        public static void FieldDoubleText(string title1, string title2, float spacingA, float spacingB,
            bool show1 = true, bool show2 = true, bool execute = true, int space = 0)
        {
            if (!execute || (!show1 && !show2))
                return;
            var rect = Layout.GetLastRect(Layout.infoWidth, Layout.rectFieldHeight);
            rect.y += 3;
            var origin = new Rect(rect);
            var width = (Layout.contentWidth - spacingA) / 2f;
            fieldTextStyle.normal.textColor = EditorGUIUtility.isProSkin ? Tint.WarmWhite : Tint.HardDark;
            if (show2)
            {
                var title2Width = fieldTextStyle.CalcSize(new GUIContent(title2)).x + 4f;
            ...
            if (show1)
            {
                rect = origin;
                var title1Width = fieldTextStyle.CalcSize(new GUIContent(title1)).x + 4f;
                EditorGUI.LabelField(rect.Adjust(rect.width - title1Width - spacingA - spacingB - width, width), title1,
                    fieldTextStyle);
            }
        }
