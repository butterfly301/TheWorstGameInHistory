...
namespace TwoBitMachines.Editors
{
    public static class Layout
    {
        ...
        public static bool previousGUIEnabled;
        public static float rectTempWidth;
        ...
        {
            if (initiated) return; // save load time
        ...
        public static void EndGUIEnable(Rect rect = default, SerializedProperty toggle = null)
        {
        ...
        public static Rect CreateRectAndDraw(float width, float height, float offsetX = 0, float offsetY = 0,
            Texture2D texture = default, Color color = default)
        {
            ...
            var rect = Set(GUILayoutUtility.GetRect(0, height), width, height, offsetX, offsetY);
            Skin.Draw(rect, color == Color.clear ? Color.white : color, texture);
            return rect;
        }

        public static Rect CreateBoxRect(float width, float height, float offsetX = 0, float bottomSpace = 0,
            Texture2D texture = default, Color color = default)
        {
            // creating a gui rect that is near the same size as layout maxwidth will trigger a horizontal bar :( but this only happens if it's inside a guilayout area
            // to avoid all issues in any scenario, create gui with an initial witdh of zero, then reset to desired size. this seems to fix the problem.
            var rect = Set(GUILayoutUtility.GetRect(0, height + bottomSpace), width, height, offsetX, 0);
            Skin.Draw(rect, color == Color.clear ? Color.white : color, texture);
            return rect;
        }
        ...
        {
            var rect = Set(GUILayoutUtility.GetRect(0, rectFieldHeight), infoWidth, rectFieldHeight, offsetX - 3,
                offsetY);
            rect.height -= 2;
            rect.y += 1;
            return rect;
        ...
        public static Rect GetLastRectDraw(float width, float height, float offsetX = 0, float offsetY = 0,
            Texture2D texture = default, Color color = default)
        {
            var rect = Set(GUILayoutUtility.GetLastRect(), width, height, offsetX, offsetY);
            Skin.Draw(rect, color == Color.clear ? Color.white : color, texture);
            return rect;
        }
        ...
        {
            VerticalSpacing();
            var rect = CreateRectAndDraw(longInfoWidth, height, -11, offsetY, Icon.Get("BackgroundLight128x128"),
                color);
        ...
        {
            var moveX = rect.width / 2f - contentSize.x / 2f;
            var moveY = rect.height / 2f - contentSize.y / 2f;
            var x = rect.x;
            var y = rect.y;
            var w = rect.width;
            var h = rect.height;
