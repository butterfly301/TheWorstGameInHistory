...
namespace TwoBitMachines.Editors
{
    public class WiggleUtilEditor
    {
        ...
        {
            var act = (Act)child.Get("act").enumValueIndex;
            var height = standAlone ? 1 : 0;
            ...
            if (act == Act.Wait)
            {
                ...
                else if (FoldOut.FoldOutButton(child.Get("settingsFoldOut"))) ExtraSettings(child);
            }
        ...
        public static void Target2D(SerializedProperty child, string name)
        {
            ...
            child.Get("target").vector3Value = child.Get("placeHolderV2").vector2Value;
        }
    }
}
#endif