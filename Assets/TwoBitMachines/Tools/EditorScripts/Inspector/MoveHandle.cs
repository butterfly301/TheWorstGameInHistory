...
namespace TwoBitMachines.Editors
{
    public class MoveHandle
    {
        ...
        public static Vector2Int intID = new(-1, -1);
        ...
        public static bool Move<T>(T objectInput, int id, Vector2 position, Color color, float handleSize,
            float snapSize, out Vector2 newPosition, HandleType type = HandleType.SolidCircle, bool isFirst = false)
        {
            newPosition = position;
            var contains = clickedObject != null && clickedObject.Equals(objectInput) && clickedIndex == id;
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.Repaint:
                    ...
                    if (isFirst) SceneTools.Circle(position, handleSize * 0.35f, color * 0.7f);
                    break;
        ...
        public static bool MoveID(int idA, int idB, Vector2 position, Color color, float handleSize, float snapSize,
            out Vector2 newPosition, HandleType type = HandleType.Circle, bool isFirst = false)
        {
            newPosition = position;

            var controlID = new Vector2Int(idA, idB);

            if (intID == controlID) GUI.FocusControl(null);
            switch (Event.current.type)
            {
                case EventType.MouseUp:
                    if (intID == controlID)
        ...
        public static bool Move(Vector2 position, float handleSize, out Vector2 newPosition, ref bool active,
            HandleType type = HandleType.None, bool isFirst = false)
        {
            newPosition = position;
            var controlID = GUIUtility.GetControlID(1400, FocusType.Passive);
        ...
        {
            var controlID = GUIUtility.GetControlID(1100, FocusType.Passive);
        ...
        {
            var controlID = GUIUtility.GetControlID(1200, FocusType.Passive);
        ...
        public static bool Drag(Vector2 position, Color color, float handleSize,
            HandleType type = HandleType.SolidCircle,
            bool isFirst = false) // works better if handle points have a fixed number
        {
            var controlID = GUIUtility.GetControlID(1300, FocusType.Passive);
            ...
            switch (Event.current.GetTypeForControl(controlID))
            {
                case EventType.Repaint:
                    ...
                    DrawHandle(type, position, color, handleSize);
                    if (isFirst) SceneTools.Circle(position, handleSize * firstSize, color * 0.7f);
                    break;
                case EventType.MouseUp:
                    if (GUIUtility.hotControl == controlID)
                    {
        ...
        public static bool ButtonDown(Vector2 position, float handleSize)
        {
            if (Event.current.type == EventType.MouseDown &&
                (Mouse.mousePosition - position).sqrMagnitude <= handleSize * handleSize && !disable && !blockSelect)
            {
                Event.current.Use();
                return disable ? false : true;
            }
