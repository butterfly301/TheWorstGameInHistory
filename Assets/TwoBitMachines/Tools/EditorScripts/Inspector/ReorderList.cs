...
namespace TwoBitMachines.Editors
{
    public class ListReorder
    {
        ...
        public static bool Reorder(Rect rect, Texture2D icon, int itemIndex, SerializedProperty array,
            SerializedProperty index, SerializedProperty active, Color color)
        {
            switch (Event.current.type)
            {
                case EventType.Repaint:
                    if (icon != null)
                        rect.DrawTexture(icon, index.intValue == itemIndex && active.boolValue ? Tint.Grey150 : color);
                    break;
                ...
                case EventType.MouseDrag:
                    ...
                    if (isActive && canExchangeItemsFromOtherLists && !active.boolValue && rect.ContainsMouse() &&
                        activeArray != null && array != null && activeArray.propertyPath != array.propertyPath)
                    {
        ...
        public static bool Grip(SerializedObject property, SerializedProperty array, Rect rect, int index,
            Color gripColor, string signalIndex = "signalIndex", string active = "active", int yOffset = 0)
        {
            return Grip(property.Get(signalIndex), property.Get(active), array, rect, index, gripColor, 0, yOffset);
        }

        public static bool Grip(SerializedProperty property, SerializedProperty array, Rect rect, int index,
            Color gripColor, string signalIndex = "signalIndex", string active = "active", int yOffset = 0)
        {
            return Grip(property.Get(signalIndex), property.Get(active), array, rect, index, gripColor, 0, yOffset);
        }

        public static bool Grip(SerializedProperty signalIndex, SerializedProperty active, SerializedProperty array,
            Rect rect, int index, Color gripColor, float xOffset = 0, int yOffset = 0)
        {
            if (signalIndex == null || active == null)
                return false;
            var grip = new Rect(rect) { x = 12 + xOffset, y = rect.y + 2 + yOffset, width = 12, height = 12 };
            return Reorder(grip, Icon.Get("Grip"), index, array, signalIndex, active, gripColor);
        }

        public static bool GripRaw(SerializedObject property, SerializedProperty array, Rect rect, int index,
            string signalIndex = "signalIndex", string active = "active")
        {
            return GripRaw(property.Get(signalIndex), property.Get(active), array, rect, index);
        }

        public static bool GripRaw(SerializedProperty signalIndex, SerializedProperty active, SerializedProperty array,
            Rect rect, int index)
        {
            if (signalIndex == null || active == null)
                return false;
