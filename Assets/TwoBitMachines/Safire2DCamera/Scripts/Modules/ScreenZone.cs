...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class ScreenZone
    {
        ...
        {
            if (size == Vector2.zero || isUser) return Vector3.zero;

            Reset();
            Vector2 velocityClamp = Vector3.zero;
            Vector2 cameraPosition = camera.transform.position;
            var zone = new Vector2(camera.Width() * size.x, camera.Height() * size.y);

            if (zone.x > 0 && target.x < cameraPosition.x - zone.x)
            {
                surpassedZoneX = true;
                velocityClamp.x = target.x - (cameraPosition.x - zone.x);
            }
            ...
            if (zone.y > 0 && target.y < cameraPosition.y - zone.y)
            {
                surpassedZoneY = true;
                velocityClamp.y = target.y - (cameraPosition.y - zone.y);
            }
