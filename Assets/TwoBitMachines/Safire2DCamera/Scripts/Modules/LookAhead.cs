...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class LookAhead
    {
        ...
        [NonSerialized] private Vector2 lookAhead;
        [NonSerialized] private Vector2 newTarget;
        ...
        public Vector2 mouseViewPort => follow.camera.ScreenToViewportPoint(Input.mousePosition);
        ...
        public Vector2 Position(Vector2 target, Camera camera, bool isUser)
        {
            ...
            var desiredMovement = target - previousTarget;
            if (desiredMovement.magnitude >=
                2f) /// 2 represent the amount of distance the player instantly skipped. if so, force a smooth follow.
            {
                if (radiusCircle)
                {
                ...
                if (distanceX > 0) follow.ForceTargetSmooth(y: false);
                if (distanceY > 0) follow.ForceTargetSmooth(false);
            }

            var velocity = Time.deltaTime <= 0 ? Vector2.zero : (target - previousTarget) / Time.deltaTime;
            newTarget = Vector2.zero;
            previousTarget = target;

            var movingX = Mathf.Abs(velocity.x) > 0.001f;
            var movingY = Mathf.Abs(velocity.y) > 0.001f;
            var moving = movingX || movingY;
            if (movingX)
            ...
            else if (isCircle)
            {
                if (moving)
                    direction = velocity
                        .normalized; // this will remember previous direction instead of resetting it to zero.
                newTarget = Look(camera.ShortestLength() * direction * radius, velocity, threshold, lookAhead, moving);
            }
            else
            {
                newTarget.x = Look(camera.Width() * distanceX * direction.x, velocity.x, threshold.x, lookAhead.x,
                    movingX);
                newTarget.y = Look(camera.Height() * distanceY * direction.y, velocity.y, threshold.y, lookAhead.y,
                    movingY);
            }
            ...
            if (radiusCircle)
            {
                ...
                follow.ForceTargetClampConditional(false);
                follow.ForceTargetClampConditional(y: false);
            }

            if (distanceX > 0) follow.ForceTargetClampConditional(y: false);
            if (distanceY > 0) follow.ForceTargetClampConditional(false);
            lookAhead = Compute.Lerp(lookAhead, newTarget, smooth);

            return target + new Vector2(distanceX > 0 || radiusCircle ? lookAhead.x : 0,
                distanceY > 0 || radiusCircle ? lookAhead.y : 0);
        }

        private Vector2 MouseTarget(Vector2 cameraSize)
        {
            var mouse = mouseViewPort * 2f - Vector2.one; // will make the center of the screen the relative point
            mouse = new Vector2(Mathf.Clamp(mouse.x, -1f, 1f), Mathf.Clamp(mouse.y, -1f, 1f));

            if (isCircle)
            {
                var shortestLength = cameraSize.y < cameraSize.x ? cameraSize.y : cameraSize.x;
                return mouse.normalized * radius *
                       Mathf.Clamp(mouse.magnitude * shortestLength, -shortestLength, shortestLength);
            }
        ...
        public float Look(float newTarget, float velocity, float threshold, float lookAhead, bool moving)
        {
            ...
            else if (type == ForwardFocusType.Threshold) newTarget = Mathf.Abs(velocity) >= threshold ? newTarget : 0;
            return newTarget;
        }
