...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class UserPan
    {
        ...
        [NonSerialized] private bool usingDesktop;
        [NonSerialized] private Vector2 velocityCurrent;
        [NonSerialized] public Vector2 velocityTarget;
        ...
        public Vector3 Velocity(Vector2 cameraPosition, Follow follow, bool UI)
        {
            ...
            if (useKeyboard)
            {
                var speed = Vector2.one * follow.speed;
            ...
            if (useTouch && Input.touchCount == 1 && !UI && InsideCamera(follow, cameraPosition, follow.Touch()))
            {
                ...
                if (useEdge)
                    velocityTarget =
                        EdgePan(follow, follow.zoom, cameraPosition, follow.Touch()); // this should return a velocity
            }
            else if (mouseActive && !UI && Input.touchCount == 0 &&
                     InsideCamera(follow, cameraPosition, follow.MousePosition()))
            {
                if (pan == Pan.Drag && Input.GetMouseButton((int)mouseButton))
                {
                    ...
                    if (Input.GetMouseButton((int)mouseButton))
                    {
                        ...
                        {
                            // Orthographic camera
                            var orthoSize = follow.camera.orthographicSize;
                            var aspectRatio = follow.camera.aspect;
                        ...
                        {
                            // Perspective camera
                            var distance = follow.camera.transform.position.z;
                            var screenPoint1 = new Vector3(Screen.width / 2, Screen.height / 2, distance);
                            var screenPoint2 = new Vector3(Screen.width / 2 + mouseDelta.x,
                                Screen.height / 2 + mouseDelta.y, distance);
                            var worldPoint1 = follow.camera.ScreenToWorldPoint(screenPoint1);
                            var worldPoint2 = follow.camera.ScreenToWorldPoint(screenPoint2);
                ...
                if (useEdge) velocityTarget = EdgePan(follow, follow.zoom, cameraPosition, follow.MousePosition());
            }

            var x = follow.useUnscaledTime
                ? Compute.LerpUnscaled(0, velocityTarget.x, follow.smoothX)
                : Compute.Lerp(0, velocityTarget.x, follow.smoothX);
            var y = follow.useUnscaledTime
                ? Compute.LerpUnscaled(0, velocityTarget.y, follow.smoothY)
                : Compute.Lerp(0, velocityTarget.y, follow.smoothY);
        ...
        private Vector2 EdgePan(Follow follow, Zoom zoom, Vector3 cameraPosition, Vector3 mousePosition)
        {
            ...
            var cameraHeight = zoom.realTimeHeight;
            var cameraWidth = zoom.realTimeWidth;
            var topBorder = cameraPosition.y + cameraHeight * (1 - topEdge);
            var leftBorder = cameraPosition.x - cameraWidth * (1 - leftEdge);
            var rightBorder = cameraPosition.x + cameraWidth * (1 - rightEdge);
            var bottomBorder = cameraPosition.y - cameraHeight * (1 - bottomEdge);
        ...
        {
            var width = follow.zoom.realTimeWidth;
            var height = follow.zoom.realTimeHeight;
            return target.x >= cameraPosition.x - width && target.x <= cameraPosition.x + width &&
                   target.y >= cameraPosition.y - height && target.y <= cameraPosition.y + height;
        }
...
#pragma warning disable 0414
        ...
        public static void CustomInspector(SerializedProperty parent, Color barColor, Color labelColor)
        {
            ...
            if (Follow.Open(parent, "User Pan", barColor, labelColor))
            {
                ...
                var guiCurrent = GUI.enabled;

                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Pan Touch", FoldOut.titleColor).BRE("useTouch")
                    .FoldOut("touchFoldOut"))
                {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Pan Mouse", FoldOut.titleColor).BRE("useMouse")
                    .FoldOut("mouseFoldOut"))
                {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Pan Keyboard", FoldOut.titleColor).BRE("useKeyboard")
                    .FoldOut("keyboardFoldOut"))
                {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Pan InputButtonSO", FoldOut.titleColor)
                    .BRE("useInputButtonSO").FoldOut("inputFoldOut"))
                {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Pan Edge", FoldOut.titleColor).BRE("useEdge")
                    .FoldOut("edgeFoldOut"))
                {
...
#pragma warning restore 0414
    ...
    public class UserZoom
    {
        ...
        [NonSerialized] public bool pause;
        [NonSerialized] private float zoomScale;
        ...
        public void Execute(Zoom zoom, Follow follow, bool UI)
        {
            ...
            var time = follow.useUnscaledTime ? Time.unscaledDeltaTime : Time.deltaTime;

            if (useTouch && !UI && Input.touchCount == 2 && Input.GetTouch(0).phase == TouchPhase.Moved &&
                Input.GetTouch(1).phase == TouchPhase.Moved)
            {
                var delta = (tA.position - tB.position).magnitude -
                            (tA.position - tA.deltaPosition - (tB.position - tB.deltaPosition))
                            .magnitude; // current position - previous position
            ...
            if (zoomScale != 0)
                zoom.Set(zoomScale = Mathf.Clamp(zoomScale, maxZoomIn, maxZoomOut), timeScale: !follow.useUnscaledTime,
                    speed: smooth, isTween: false);
        }
...
#pragma warning disable 0414
        ...
        public static void CustomInspector(SerializedProperty parent, Color barColor, Color labelColor)
        {
            ...
            if (Follow.Open(parent, "User Zoom", barColor, labelColor))
            {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Zoom Keyboard", FoldOut.titleColor).BRE("useKeyboard")
                    .FoldOut("keyboardFoldOut"))
                {
                    GUI.enabled = parent.Bool("useKeyboard") && guiCurrent;
                    FoldOut.Box(2, FoldOut.boxColor, offsetY: -2);
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Zoom InputButtonSO", FoldOut.titleColor)
                    .BRE("useInputButtonSO").FoldOut("inputFoldOut"))
                {
                    GUI.enabled = parent.Bool("useInputButtonSO") && guiCurrent;
                    FoldOut.Box(4, FoldOut.boxColor, offsetY: -2);
...
#pragma warning restore 0414
    ...
    public class UserRotate
    {
        ...
        [NonSerialized] private float angle;
        [NonSerialized] public bool pause;
        ...
        public void Reset(Transform camera)
        {
            ...
            if (enable)
            {
                var a = camera.localEulerAngles;
                camera.localEulerAngles = new Vector3(a.x, a.y, 0);
            }
        }

        public void
            Execute(Follow follow,
                bool overUI) // Execute all input inside LateUpate. Fixed Update can miss input signals.
        {
            if (!enable || pause)
                return;
            ...
            if (useTouch && !overUI && Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved)
                target -= touchAxis == Axis1D2.Horizontal
                    ? Input.GetTouch(0).deltaPosition.x * speed * time
                    : Input.GetTouch(0).deltaPosition.y * speed *
                      time; // do not multiply speed by factor, touch is already fast
            ...
            {
                angle = follow.useUnscaledTime
                    ? Compute.LerpUnscaled(angle, target, smooth)
                    : Compute.Lerp(angle, target, smooth);
                var a = follow.cameraTransform.transform.localEulerAngles;
                follow.cameraTransform.transform.rotation = Quaternion.AngleAxis(angle, Vector3.forward);
                follow.cameraTransform.transform.localEulerAngles =
                    Tooly.SetPosition(a.x, a.y,
                        follow.cameraTransform.transform.localEulerAngles); // Do not override other angles, reset them.
            }
        }
...
#pragma warning disable 0414
        ...
        public static void CustomInspector(SerializedProperty parent, Color barColor, Color labelColor)
        {
            ...
            if (Follow.Open(parent, "User Rotate", barColor, labelColor))
            {
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Rotate Touch", FoldOut.titleColor).BRE("useTouch")
                    .FoldOut("touchFoldOut"))
                {
                    GUI.enabled = parent.Bool("useTouch") && guiCurrent;
                    FoldOut.Box(1, FoldOut.boxColor, offsetY: -2);
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Rotate Mouse", FoldOut.titleColor).BRE("useMouse")
                    .FoldOut("mouseFoldOut"))
                {
                    GUI.enabled = parent.Bool("useMouse") && guiCurrent;
                    FoldOut.Box(1, FoldOut.boxColor, offsetY: -2);
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Rotate Keyboard", FoldOut.titleColor)
                    .BRE("useKeyboard").FoldOut("keyboardFoldOut"))
                {
                    GUI.enabled = parent.Bool("useKeyboard") && guiCurrent;
                    FoldOut.Box(2, FoldOut.boxColor, offsetY: -2);
                ...
                if (FoldOut.Bar(parent, FoldOut.boxColor).Label("Rotate InputButtonSO", FoldOut.titleColor)
                    .BRE("useInputButtonSO").FoldOut("inputFoldOut"))
                {
                    GUI.enabled = parent.Bool("useInputButtonSO") && guiCurrent;
                    FoldOut.Box(4, FoldOut.boxColor, offsetY: -2);
