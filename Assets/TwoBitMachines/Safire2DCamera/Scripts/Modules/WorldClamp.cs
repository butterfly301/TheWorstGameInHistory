...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class WorldClamp
    {
        ...
        [SerializeField] public bool clampCenterOnly;
        ...
        public void Clamp(Vector3 previousCamera)
        {
            ...
            var camSizeX = clampCenterOnly ? 0 : camera.Width();
            var camSizeY = clampCenterOnly ? 0 : camera.Height();
...
#pragma warning disable 0414
        ...
        public static void DrawTrigger(Safire2DCamera main)
        {
            ...
            {
                var bounds = main.worldClamp.bounds;
                SceneTools.Square(bounds.position, bounds.size, Tint.Delete);
            }
            else
            {
                var bounds = main.worldClamp.bounds;
                var copyPos = bounds.position;
                var copySize = bounds.size;
                SceneTools.DrawAndModifyBounds(ref bounds.position, ref bounds.size, Tint.Delete, 1f);
                if (copyPos != bounds.position || copySize != bounds.size) EditorUtility.SetDirty(main.gameObject);
            }
        }
