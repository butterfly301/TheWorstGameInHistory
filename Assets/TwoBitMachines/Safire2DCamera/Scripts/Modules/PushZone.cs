...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    {
        [NonSerialized] private const float timeLimit = 1.25f;
        ...
        [NonSerialized] private float counterY;
        ...
        {
            if (!enabledX && !enabledY) return cameraVelocity;

            var pushVelocity = new Vector3(enabledX ? 0 : cameraVelocity.x, enabledY ? 0 : cameraVelocity.y);
            Vector2 cameraPosition = camera.transform.position;
            var size = new Vector2(camera.Width() * zoneX, camera.Height() * zoneY);

            if (isUser)
            {
                if ((right && cameraVelocity.x > 0) || (left && cameraVelocity.x < 0))
                    pushVelocity.x = cameraVelocity.x;
            ...
            {
                Clock.TimerExpired(ref counterX, timeLimit);
            ...
            {
                Clock.TimerExpired(ref counterY, timeLimit);
