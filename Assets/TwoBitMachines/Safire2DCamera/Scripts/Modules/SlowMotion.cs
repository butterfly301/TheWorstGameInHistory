...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class SlowMotion
    {
        ...
        [NonSerialized] private bool constant;

        [NonSerialized] private float counter;
        [NonSerialized] private float currentTimeScale = 1;
        [NonSerialized] private float duration = 1f;
        ...
        public void Execute()
        {
            ...
            if (timeScaleWasPaused && slowMotion) Time.timeScale = currentTimeScale;

            timeScaleWasPaused = false;
            currentTimeScale = Time.timeScale;

            if (Time.timeScale == 1 || constant || !slowMotion) return;
            counter += Time.unscaledDeltaTime;
            Time.timeScale = Mathf.Lerp(startValue, 1f, counter / duration);
            if (Time.timeScale >= 1) Reset();
        }
