...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class WorldBounds
    {
        ...
        [SerializeField] public List<WorldBound> bounds = new();
        [NonSerialized] private Vector2 previousTarget;
        ...
        public void Clamp(Camera camera, Vector3 previousCamera, Follow follow)
        {
            ...
            {
                if (bounds[i].pause) continue;
                if (!bounds[i].InRange(target) && bounds[i].BreakClamp(target)) continue;
                ...
                if (bounds[i].canClamp || bounds[i].CanClamp(camCenter, width, height))
                {
                    ...
                    bounds[i].holding = false;
                    camCenter = bounds[i].Clamp(camCenter, previousCamera.x, width, height,
...
#pragma warning disable 0414
        ...
        public static void CustomInspector(SerializedProperty parent, Color barColor, Color labelColor)
        {
            ...
            if (Follow.Open(parent, "World Bounds", barColor, labelColor, true, canView: true))
            {
                ...
                if (array.arraySize == 0) Layout.VerticalSpacing(5);

                var select = parent.Int("select");
                for (var i = 0; i < array.arraySize; i++)
                {
                    ...
                    {
                        if (i == select)
                            Layout.GetLastRectDraw(Layout.longInfoWidth, Layout.rectFieldHeight + 4, -11,
                                color: Tint.Delete180);
                        ...
                        var pause = element.Get("pause");
                        if (Layout.GetLastRect(20, 20, 10).Button(Icon.Get("Pause"),
                                pause.boolValue ? Tint.PastelGreen : Tint.Delete, center: true)) pause.Toggle();

                        var block = Layout.GetLastRect(Layout.longInfoWidth, 18, -11);
                        if (block.ContainsMouseDown()) parent.Get("select").intValue = i;
                    }
        ...
        public static void DrawBounds(Safire2DCamera main, Editor editor)
        {
            if (!main.worldBounds.view || !main.worldBounds.enable)
                return;

            var bounds = main.worldBounds.bounds;

            for (var i = 0; i < bounds.Count; i++)
            {
                var previousColor = Handles.color;
                var bound = bounds[i];
                var color = Tint.Delete;
                Handles.color = color;
                ...
                var aChanged = oldA != bound.a;
                var bChanged = oldB != bound.b;
                ...
                var center = (bound.a + bound.b) * 0.5f;
                if (main.worldBounds.select == i)
                    SceneTools.SolidDisc(center, Vector3.forward, 1.5f,
                        bound.pause ? Tint.PastelGreen100 : Tint.Delete100);
                if (bound.restrict)
                ...
                SceneTools.Line(center,
                    center - bound.Direction() *
                    (bound.horizontal ? main.cameraRef.Width() * 2f : main.cameraRef.Height() * 2f),
                    Tint.WhiteOpacity100);
...
#pragma warning restore 0414
    ...
    public class WorldBound
    {
        ...
        [NonSerialized] private float buffer = 2.5f;

        [NonSerialized] public bool canClamp;
        [NonSerialized] public bool clamping;
        [NonSerialized] private float graceDistance = 2f;
        [NonSerialized] private float graceRate = 4f;
        [NonSerialized] private float height;
        [NonSerialized] private Vector2 hold;
        [NonSerialized] public bool holding;
        [NonSerialized] private Vector2 oldCenter;
        [NonSerialized] private float rate = 0.98f;
        [NonSerialized] private float width;
        ...
        {
            if (boundsType == BoundsType.Left) return position.x >= a.x && InsideY(position.y);
            if (boundsType == BoundsType.Right) return position.x <= a.x && InsideY(position.y);
            if (boundsType == BoundsType.Top) return position.y <= a.y && InsideX(position.x);
            if (boundsType == BoundsType.Bottom) return position.y >= a.y && InsideX(position.x);
            return false;
        }
        ...
        {
            if (boundsType == BoundsType.Left) return center.x - width >= a.x - 0.035f;
            if (boundsType == BoundsType.Right) return center.x + width <= a.x + 0.035f;
            if (boundsType == BoundsType.Top) return center.y + height <= a.y + 0.035f;
            if (boundsType == BoundsType.Bottom) return center.y - height >= a.y - 0.035f;
            return false;
        }

        public Vector3 Hold(Vector2 target, Vector3 center, Vector3 oldCenter, float width, float height)
        {
            if (canClamp || clamping) return center;
            ...
            if (boundsType == BoundsType.Right)
                ...
                {
                    holding = true;
                    if (nearRight) oldCenter.x += (a.x - width - oldCenter.x) * Time.deltaTime * graceRate;
                    return new Vector3(oldCenter.x, center.y, center.z);
                }
            ...
            if (boundsType == BoundsType.Top)
                ...
                {
                    holding = true;
                    if (nearTop) oldCenter.y += (a.y - height - oldCenter.y) * Time.deltaTime * graceRate;
            ...
            if (boundsType == BoundsType.Bottom)
                ...
                {
                    holding = true;
                    if (nearBottom) oldCenter.y += (a.y + height - oldCenter.y) * Time.deltaTime * graceRate;
                    return new Vector3(center.x, oldCenter.y, center.z);
                }
        ...
        {
            if (boundsType == BoundsType.Left) return Vector2.left;
            if (boundsType == BoundsType.Right) return Vector2.right;
            if (boundsType == BoundsType.Top) return Vector2.up;
            if (boundsType == BoundsType.Bottom) return Vector2.down;
            return Vector2.one;
        }
