...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class Peek
    {
        ...
        [NonSerialized] private float distanceDown;

        [NonSerialized] private float distanceLeft;
        [NonSerialized] private float distanceRight;
        [NonSerialized] private float distanceUp;

        [NonSerialized] private bool goLeft, goRight, goUp, goDown;
        [NonSerialized] private bool xActive;
        [NonSerialized] private bool yActive;
        ...
        private Vector3 Move(Follow follow)
        {
            ...
            var speedUp = time == 0 ? 0 : Mathf.Abs(directionA.y * distance / time);
            var speedDown = time == 0 ? 0 : Mathf.Abs(directionB.y * distance / time);
            var speedLeft = time == 0 ? 0 : Mathf.Abs(directionA.x * distance / time);
            var speedRight = time == 0 ? 0 : Mathf.Abs(directionB.x * distance / time);

            if (directionA.x < 0)
            {
                signLeft = Input.GetKey(buttonA) || goLeft ? -1 : 1 * 1.5f;
                distanceLeft = Mathf.Clamp(distanceLeft + Time.deltaTime * speedLeft * signLeft,
                    directionA.x * distance, 0);
            }
            else if (directionA.x > 0)
            {
                signLeft = Input.GetKey(buttonA) || goLeft ? 1 : -1 * 1.5f;
                distanceLeft = Mathf.Clamp(distanceLeft + Time.deltaTime * speedLeft * signLeft, 0,
                    directionA.x * distance);
            }

            if (directionB.x < 0)
            {
                signRight = Input.GetKey(buttonB) || goRight ? -1 : 1 * 1.5f;
                distanceRight = Mathf.Clamp(distanceRight + Time.deltaTime * speedRight * signRight,
                    directionB.x * distance, 0);
            }
            else if (directionB.x > 0)
            {
                signRight = Input.GetKey(buttonB) || goRight ? 1 : -1 * 1.5f;
                distanceRight = Mathf.Clamp(distanceRight + Time.deltaTime * speedRight * signRight, 0,
                    directionB.x * distance);
            }
            ...
            if (directionB.y < 0)
            {
                ...
                distanceDown = Mathf.Clamp(distanceDown + Time.deltaTime * speedDown * signDown,
                    directionB.y * distance, 0);
            }
            else if (directionB.y > 0)
            {
                signDown = Input.GetKey(buttonB) || goDown ? 1 : -1 * 1.5f;
                distanceDown = Mathf.Clamp(distanceDown + Time.deltaTime * speedDown * signDown, 0,
                    directionB.y * distance);
            }

            var appliedPeek = new Vector2(distanceLeft + distanceRight, distanceUp + distanceDown);
            if (appliedPeek.x != 0)
                xActive = true;
            if (appliedPeek.y != 0)
                yActive = true;
