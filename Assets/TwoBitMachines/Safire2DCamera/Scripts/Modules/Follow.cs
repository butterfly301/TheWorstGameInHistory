...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class Follow
    {
        ...
        [SerializeField] public Rails rails = new();
        [SerializeField] public Regions regions = new();
        [SerializeField] public DeadZone deadZone = new();
        [SerializeField] public PushZone pushZone = new();
        [SerializeField] public LookAhead lookAhead = new();
        [SerializeField] public ScreenZone screenZone = new();
        [SerializeField] public DetectWalls detectWalls = new();
        [SerializeField] public FollowBlocks followBlocks = new();
        [SerializeField] public HighlightTarget highlightTarget = new();

        [SerializeField] public UserPan userPan = new();
        [SerializeField] public UserZoom userZoom = new();
        [SerializeField] public UserRotate userRotate = new();
        [NonSerialized] public Camera camera;
        [NonSerialized] public Transform cameraTransform;
        [NonSerialized] public bool forceClampX;
        [NonSerialized] public bool forceClampY;
        [NonSerialized] public bool forceSmoothX;
        [NonSerialized] public bool forceSmoothY;
        [NonSerialized] public bool pauseAutoscroll;
        [NonSerialized] public Peek peek;
        [NonSerialized] public PeekPlus peekPlus;

        [NonSerialized] public PixelPerfect pixelPerfect;
        [NonSerialized] public Vector3 previousTarget;
        [NonSerialized] public Rooms rooms;

        [NonSerialized] public int roomSmoothX;
        [NonSerialized] public int roomSmoothY;
        [NonSerialized] public Transform targetTransform;
        [NonSerialized] public bool usingAutoRail;
        [NonSerialized] public Zoom zoom;
        ...
        {
            targetTransform = main.targetTransform;
        ...
        public void Execute()
        {
            ...
            usingAutoRail = false;
            var overUI = isUser ? IsPointerOverGameObject() : false;
            ...
            var realSmoothX = forceSmoothX ? Mathf.Min(smoothX, returnSmooth) : forceClampX ? 1f : smoothX;
            var realSmoothY = forceSmoothY ? Mathf.Min(smoothY, returnSmooth) : forceClampY ? 1f : smoothY;
            forceClampX = forceClampY = false; // have to keep setting true

            var newCameraPosition = Compute.LerpToTarget(cameraTransform.position, target, ref previousTarget,
                realSmoothX, realSmoothY, speed);
            ...
            var cameraVelocity = newCameraPosition - cameraTransform.position;
            ...
            cameraVelocity += peekPlus.Velocity(this, isUser);
            ...
            if (forceSmoothX && Mathf.Abs(cameraTransform.position.x - target.x) < 0.1f) forceSmoothX = false;
            if (forceSmoothY && Mathf.Abs(cameraTransform.position.y - target.y) < 0.1f) forceSmoothY = false;
        ...
        {
            if (x && !forceSmoothX) forceClampX = true;
            if (y && !forceSmoothY) forceClampY = true;
        }
        ...
        public Vector2 MousePosition()
        {
            ...
            if (!camera.orthographic) mousePosition.z = zoom.currentCameraDepth;
            return camera.ScreenToWorldPoint(mousePosition);
        }

        public Vector2 MousePositionRay()
        {
            var mousePosition = camera.ScreenPointToRay(Input.mousePosition).origin;
            if (!camera.orthographic) mousePosition.z = zoom.currentCameraDepth;
        ...
        public Vector2 Touch()
        {
            ...
            if (!camera.orthographic) touchPosition.z = zoom.currentCameraDepth;
        ...
        public static bool IsPointerOverGameObject()
        {
            ...
            var overUI = EventSystem.current.IsPointerOverGameObject(); // Check mouse
            for (var i = 0; i < Input.touchCount; i++) //                     Check touches
...
#pragma warning disable 0414
        ...
        public static void CustomInspector(SerializedProperty parent, bool isUser, Color barColor, Color labelColor)
        {
            if (Open(parent, "Follow", barColor, labelColor, false, false, true))
            {
                ...
                parent.Slider("Return Smooth", "returnSmooth", 0.01f, 0.99f);
        ...
        {
            var index = detectWalls.Enum("direction");
            detectWalls.Field("Detect Walls", "direction", index == 0);
            detectWalls.FieldDouble("Detect Walls", "direction", "layerMask", index > 0);
        }

        private static void PushZone(SerializedProperty pushZone)
        {
            var activeX = (PushHorizontal)pushZone.Get("horizontal").enumValueIndex;
            pushZone.Field("Push Zone X", "horizontal", activeX == PushHorizontal.DontPush);
            pushZone.FieldDouble("Push Zone X", "horizontal", "zoneX", activeX != PushHorizontal.DontPush);
            pushZone.Clamp("zoneX");

            var activeY = (PushVertical)pushZone.Get("vertical").enumValueIndex;
            pushZone.Field("Push Zone Y", "vertical", activeY == PushVertical.DontPush);
            pushZone.FieldDouble("Push Zone Y", "vertical", "zoneY", activeY != PushVertical.DontPush);
            pushZone.Clamp("zoneY");
        }

        public static bool Open(SerializedProperty parent, string name, Color barColor, Color labelColor,
            bool add = false, bool checkClose = true, bool canView = false)
        {
            var open = FoldOut.Bar(parent, barColor, -3)
