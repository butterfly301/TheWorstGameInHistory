...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    public class Rooms
    {
        ...
        [NonSerialized] private bool clampedX;
        [NonSerialized] private bool clampedY;
        [NonSerialized] private Follow follow;

        [NonSerialized] public bool ignoreClamps;
        [NonSerialized] private int index = -1;
        [NonSerialized] private bool inTwoRooms;
        [NonSerialized] private int previousIndex = -1;
        [NonSerialized] private bool transition;
        ...
        public bool Execute(Vector3 previousCamera)
        {
            ...
            if (roomsFound == 1) inTwoRooms = false;
        ...
        {
            if (!enable || !transition || !roomExists) return false;
            Vector2 targetPosition = follow.TargetPosition();
            var exit = false;
            if (rooms[index].canExit && !rooms[index].bounds.DetectBounds(targetPosition)) exit = true;
            for (var i = 0; i < rooms.Count; i++)
        ...
        public Vector3 RoomTarget(Vector3 target, Follow follow)
        {
            ...
            if (Mathf.Abs(follow.cameraTransform.position.x - roomTarget.x) < 0.25f &&
                Mathf.Abs(follow.cameraTransform.position.y - roomTarget.y) < 0.25f) ignoreClamps = false;
...
#pragma warning disable 0414
        ...
        public static void CustomInspector(SerializedProperty parent, Color barColor, Color labelColor)
        {
            ...
            if (Follow.Open(parent, "Rooms", barColor, labelColor, true, canView: true))
            {
                ...
                for (var i = 0; i < array.arraySize; i++)
                {
                    ...
                    if (CustomInspectorRoom(element, array, i)) break;
                }
        ...
        public static bool CustomInspectorRoom(SerializedProperty element, SerializedProperty array, int index)
        {
            FoldOut.BoxSingle(1, Tint.Blue);
            {
                ...
                if (array != null && Fields.ConstructButton("Target")) Follow.Select(array, index);
            ...
            var eventOpen = FoldOut.FoldOutButton(element.Get("eventsFoldOut"));
            Fields.EventFoldOut(element.Get("onEnter"), element.Get("enterFoldOut"), "On Enter", color: Tint.Blue,
                execute: eventOpen);
            Fields.EventFoldOut(element.Get("onExit"), element.Get("exitFoldOut"), "On Exit", color: Tint.Blue,
                execute: eventOpen);
            Fields.EventFoldOut(element.Get("onTransitionStart"), element.Get("startFoldOut"), "On Transition Start",
                color: Tint.Blue, execute: eventOpen);
            Fields.EventFoldOut(element.Get("onTransitionComplete"), element.Get("completeFoldOut"),
                "On Transition Complete", color: Tint.Blue, execute: eventOpen);
        ...
        public static void DrawRooms(Safire2DCamera main)
        {
            ...
            for (var i = 0; i < main.rooms.rooms.Count; i++)
            {
                var room = main.rooms.rooms[i];
        ...
        public static void DrawRooms(Room room, Safire2DCamera main, int index, bool standAlone = false)
        {
            ...
            DrawRoom(room, ref bounds.position, ref bounds.size, room.select == index ? Tint.PastelGreen : Tint.Blue,
                0.5f, standAlone);
            SceneTools.ModifyPercentH(ref bounds.detectLeft, bounds.center, bounds.size,
                bounds.detectLeft == 0 ? Tint.PastelGreen : Tint.Delete, -1f);
            SceneTools.ModifyPercentH(ref bounds.detectRight, bounds.center, bounds.size,
                bounds.detectRight == 0 ? Tint.PastelGreen : Tint.Delete);
            SceneTools.ModifyPercentV(ref bounds.detectTop, bounds.center, bounds.size,
                bounds.detectTop == 0 ? Tint.PastelGreen : Tint.Delete);
            SceneTools.ModifyPercentV(ref bounds.detectBottom, bounds.center, bounds.size,
                bounds.detectBottom == 0 ? Tint.PastelGreen : Tint.Delete, -1f);

            if (SceneTools.UpperRightButton(bounds.position, bounds.size, Tint.PastelGreen, 0.5f))
                if (Camera.main != null)
                    bounds.size = Camera.main.CameraSize();
            if (main != null && Mouse.down && bounds.DetectRaw(Mouse.position))
            {
                for (var j = 0; j < main.rooms.rooms.Count; j++)
                {
        ...
        public static void DrawRoom(Room room, ref Vector2 position, ref Vector2 size, Color color,
            float handleSize = 0.25f, bool standAlone = false)
        {
            SceneTools.DrawAndModifyBounds(ref position, ref size, color, handleSize, false, !standAlone);
...
#pragma warning restore 0414
    ...
    public class Room
    {
        [NonSerialized] private static bool isClampedX;
        [NonSerialized] private static bool isClampedY;
        ...
        [SerializeField] public bool holdTarget;
        [SerializeField] public bool lazyClamp;
        [SerializeField] public float transitionTime = 1f;
        [SerializeField] public float delay;
        [SerializeField] public Tween tween;
        [SerializeField] public MultipleTargets multipleTargets = new();
        ...
        [SerializeField] [HideInInspector] public Bounds bounds = new();
        [NonSerialized] private float delayCounter;
        [NonSerialized] private bool insideRestrictArea;
        [NonSerialized] private bool lazyClampBottom;
        [NonSerialized] private bool lazyClampLeft;
        [NonSerialized] private bool lazyClampRight;
        [NonSerialized] private bool lazyClampTop;
        [NonSerialized] private Vector3 startPoint;
        [NonSerialized] private Vector3 targetEntry;
        [NonSerialized] private float transitionCounter;

        [NonSerialized] public Vector3 transitionPoint;
        [NonSerialized] private bool transitionStart;
        [NonSerialized] private bool transitionX;
        [NonSerialized] private bool transitionY;
        ...
        public bool SetTransitionPoint(Vector2 targetEntryPoint, Transform camera, Follow follow, Zoom zoom,
            bool setZoom = true)
        {
            var foundX = false;
            var foundY = false;
            ...
            var camSizeX = !canZoom ? zoom.realTimeWidth : zoom.originalWidth * scale;
            var camSizeY = !canZoom ? zoom.realTimeHeight : zoom.originalHeight * scale;
        ...
        public bool Transition(Camera camera, Vector3 previousCamera, Zoom zoom, Follow follow)
        {
            ...
            var ease = EasingFunction.Run(tween, transitionCounter, transitionTime);
            var x = Mathf.Lerp(startPoint.x, transitionPoint.x, ease);
            var y = Mathf.Lerp(startPoint.y, transitionPoint.y, ease);
            ...
            follow.SetCameraPosition(new Vector3(transitionX ? x : camera.transform.position.x,
                transitionY ? y : camera.transform.position.y));
        ...
        public void Clamp(Camera camera, Vector3 previousCamera, bool clampX = true, bool clampY = true)
        {
            ...
            var origin = camCenter;

            if (clampX)
            {
                Vector2 velocity = camCenter - previousCamera;
                var camSizeX = camera.Width();
                //float buffer = 2.5f;
                // float rate = 0.98f;

                if (lazyClamp)
                {
                    if (clampLeft && !lazyClampLeft && camCenter.x - camSizeX >= bounds.left) lazyClampLeft = true;
                    if (clampRight && !lazyClampRight && camCenter.x + camSizeX <= bounds.right) lazyClampRight = true;
                }
            ...
            if (clampY)
            {
                var camSizeY = camera.Height();

                if (lazyClamp)
                {
                    if (clampTop && !lazyClampTop && camCenter.y + camSizeY <= bounds.top) lazyClampTop = true;
        ...
        {
            return canZoom
                ? zoomScale == ZoomScale.Zoom ? zoomValue :
                zoomScale == ZoomScale.MatchHeight ? bounds.height / zoom.originalHeight :
                bounds.width / zoom.originalWidth
                : 1f;
        }
        ...
        public void RestrictTarget(Transform target)
        {
            ...
            if (!insideRestrictArea && bounds.DetectBounds(p, 1f)) insideRestrictArea = true;
        ...
        public void ForceSmooth(Vector2 target, Follow follow)
        {
            ...
            if (target.x < bounds.left && canClampLeft) follow.ForceTargetSmooth();
            if (target.x > bounds.right && canClampRight) follow.ForceTargetSmooth();
            if (target.y < bounds.bottom && canClampBottom) follow.ForceTargetSmooth();
            if (target.y > bounds.top && canClampTop) follow.ForceTargetSmooth();
        }
