...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    {
        [SerializeField] public Room room = new();
        ...
        private void Start()
        {
            ...
            if (safireCamera == null) return;

            room.bounds.Initialize();
            room.multipleTargets.Initialize(safireCamera.zoom, room.bounds);
            safireCamera.rooms.rooms.Add(room);
            var index = safireCamera.rooms.rooms.Count - 1;

            if (!safireCamera.rooms.enable) return;

            Vector2 targetPosition = safireCamera.follow.TargetPosition();
            if (room.bounds.DetectBounds(targetPosition)) safireCamera.rooms.EnterRoom(room, targetPosition, index);
        }
    }
}