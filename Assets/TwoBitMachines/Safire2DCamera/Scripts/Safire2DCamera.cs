...
namespace TwoBitMachines.Safire2DCamera
{
    ...
    {
        [NonSerialized] public static List<Safire2DCamera> cameras = new();

        public static Safire2DCamera mainCamera;
        [SerializeField] public Transform targetTransform;
        [SerializeField] [HideInInspector] public Camera cameraRef;

        [SerializeField] public Zoom zoom = new();
        [SerializeField] public Peek peek = new();
        [SerializeField] public Rooms rooms = new();
        [SerializeField] public Shakes shake = new();
        [SerializeField] public Follow follow = new();
        [SerializeField] public Parallax parallax = new();
        [SerializeField] public PeekPlus peekPlus = new();
        [SerializeField] public ParallaxFinite parallaxFinite = new();
        [SerializeField] public ResolutionScaling resolutionScaling = new();
        [SerializeField] public SpeedZoom speedZoom = new();
        [SerializeField] public Cinematics cinematics = new();
        [SerializeField] public WorldBounds worldBounds = new();
        [SerializeField] public WorldClamp worldClamp = new();

        [SerializeField] public PixelPerfect pixelPerfect;
        [SerializeField] public bool enablePixelPerfect;
        [SerializeField] [HideInInspector] public bool pixelFoldOut;

        [SerializeField] public ZoomTrigger zoomTrigger = new();
        [SerializeField] public BasicTrigger basicTrigger = new();
        [SerializeField] public SlowMotionTrigger slowMotionTrigger = new();
        [NonSerialized] private bool pauseFollow;
        [NonSerialized] private Vector3 previousCamera;
        [SerializeField] public SlowMotion timeManager = new();
        ...
        {
            if (!cameras.Contains(this)) cameras.Add(this);
        }

        private void OnDestroy()
        {
            if (cameras.Contains(this)) cameras.Remove(this);
            follow.userZoom.SaveZoomLevel();
        }

        public static void ResetCameras()
        {
            for (var i = 0; i < cameras.Count; i++) cameras[i]?.ResetAll();
        }
        ...
        public void CameraUpdate()
        {
            ...
            if (!cinematics.Execute() && !pauseFollow && !rooms.Execute(previousCamera))
            {
                ...
                peekPlus.Velocity(cameraRef, follow);
            }
        ...
        public void Module(string moduleName, bool enable)
        {
            if (moduleName == "Follow")
                ...
                peek.enable = enable;
            else if (moduleName == "PeekPlus")
                peekPlus.enable = enable;
        ...
        public void SetCameraPosition(Vector3 position)
        {
            if (follow.cameraTransform != null)
            {
                ...
                shake.previousPosition = transform.position;
                shake.previousAngle = transform.eulerAngles;
            }
            else
            {
                transform.position = new Vector3(position.x, position.y, transform.position.z);
                follow.previousTarget = transform.position;
                shake.previousPosition = transform.position;
                shake.previousAngle = transform.eulerAngles;
            }
        }
        ...
        {
            if (key == 0) follow.ForceTargetSmooth();
        ...
        {
            cinematics.TriggerCinematic(cinematicName, transform.position);
        }
        ...
        public void ShakeRandom(float duration, Vector3 Amplitude, float strength = 1, float speed = 1,
            bool useTimeScale = true, bool constant = false)
        {
            shake.Set(ShakeType.Random, Mathf.Clamp(speed, 0, 1), Mathf.Clamp(strength, 0, 1),
                Mathf.Clamp(duration, 0, Mathf.Infinity), constant,
                useTimeScale ? TimeScale.TimeScale : TimeScale.TimeUnscaled, Amplitude);
        }

        /// <summary> Create a Perlin shake. The shake algorithm will be random but smooth. </summary>
        public void ShakePerlin(float duration, Vector3 Amplitude, float strength = 1, float speed = 1,
            bool useTimeScale = true, bool constant = false)
        {
            shake.Set(ShakeType.Perlin, Mathf.Clamp(speed, 0, 1), Mathf.Clamp(strength, 0, 1),
                Mathf.Clamp(duration, 0, Mathf.Infinity), constant,
                useTimeScale ? TimeScale.TimeScale : TimeScale.TimeUnscaled, Amplitude);
        }

        /// <summary> Create a Sine shake. The shake algorithm will be sinusoidal. </summary>
        public void ShakeSine(float duration, Vector3 Amplitude, float strength = 1, float speed = 1,
            bool useTimeScale = true, bool constant = false)
        {
            shake.Set(ShakeType.Sine, Mathf.Clamp(speed, 0, 1), Mathf.Clamp(strength, 0, 1),
                Mathf.Clamp(duration, 0, Mathf.Infinity), constant,
                useTimeScale ? TimeScale.TimeScale : TimeScale.TimeUnscaled, Amplitude);
        }

        /// <summary>
        ///     Create a One-Shot shake. This is a quick shake, displacing the camera in the specified direction. It's
        ///     basically a recoil shake for bullets.
        /// </summary>
        public void ShakeOneShot(float duration, Vector2 direction, float strength = 1, bool useTimeScale = true)
        {
            shake.Set(ShakeType.OneShot, 1, strength, Mathf.Clamp(duration, 0, Mathf.Infinity), false,
                useTimeScale ? TimeScale.TimeScale : TimeScale.TimeUnscaled, direction);
        }

        /// <summary>
        ///     A Single shake should be used for frequent shakes with short durations such as bullet impacts. Numerous shakes can
        ///     still occur -- it is only called Single because the shakes are grouped together and executed concurrently in a
        ///     single state.
        ///     To work effectively, the shakes in this group should have strengths much lower than 1.
        /// </summary>
        public void ShakeIsSingle(float duration, Vector3 Amplitude, float strength = 0.1f, float speed = 1,
            bool useTimeScale = true)
        {
            shake.Set(ShakeType.SingleShake, Mathf.Clamp(speed, 0, 1), Mathf.Clamp(strength, 0, Mathf.Infinity),
                Mathf.Clamp(duration, 0, Mathf.Infinity), false,
                useTimeScale ? TimeScale.TimeScale : TimeScale.TimeUnscaled, Amplitude);
        }
        ...
        {
            timeManager.Set(Mathf.Clamp(intensity, 0, 1f), 0.75f);
        }

        /// <summary>
        ///     Camera control will switch from the player to the specified position and will follow with the specified
        ///     speed.
        /// </summary>
        ...
        ///     speed.
        /// </summary>
        public void HighlightTarget(Transform transform, float duration, float speed, float yOffset = 0,
            float range = 0, bool stay = false, bool ignoreClamps = true)
        {
            follow.highlightTarget.Set(TargetTypes.Transform, transform, Vector3.zero, duration, speed, yOffset, range,
                stay, ignoreClamps, follow);
        }
        ...
        ///     speed.
        /// </summary>
        public void HighlightTarget(Vector2 position, float duration, float speed, float yOffset = 0, float range = 0,
            bool stay = false, bool ignoreClamps = true)
        {
            follow.highlightTarget.Set(TargetTypes.Position, null, position, duration, speed, yOffset, range, stay,
                ignoreClamps, follow);
        }
