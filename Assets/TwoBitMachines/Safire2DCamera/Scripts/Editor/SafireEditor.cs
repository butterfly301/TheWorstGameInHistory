...
namespace TwoBitMachines.Safire2DCamera.Editors
{
    ...
    public class CameraEngineEditor : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            var monoScript = MonoScript.FromMonoBehaviour(main); // Execute script last.
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (main.cameraRef == null) main.cameraRef = main.gameObject.GetComponent<Camera>();

            parent.Update();
            {
                var barColor = Tint.BoxTwo;
                var labelColor = Color.black;
                var follow = parent.Get("follow");

                var type = follow.Enum("followType");
                var height = type == 0 ? 1 : 0;
                var is3D = main.cameraRef != null && !main.cameraRef.orthographic ? 1 : 0;

                FoldOut.Box(1 + height + is3D, barColor);
                {
                    follow.Field("Follow", "followType", bold: true);
                    parent.Field("", "targetTransform", height == 1);
                    parent.Get("zoom").Field("3D Zoom", "type", is3D == 1);
                }
                Layout.VerticalSpacing(20);

                Follow.CustomInspector(follow, type == 1, barColor, labelColor);
                Peek.CustomInspector(parent.Get("peek"), barColor, labelColor);
                PeekPlus.CustomInspector(parent.Get("peekPlus"), barColor, labelColor);
        ...
        {
            var foldOut = FoldOut.Bar(parent, barColor, -3)
            ...
            if (parent.Bool("enablePixelPerfect"))
            {
                ...
                {
                    var pixelPerfect = main.transform.gameObject.AddComponent<PixelPerfect>();
                    parent.Get("pixelPerfect").objectReferenceValue = pixelPerfect;
                    pixelPerfect.hideFlags = HideFlags.HideInInspector;
                }

                if (foldOut)
                    PixelPerfect.CustomInspector(new SerializedObject(parent.Get("pixelPerfect").objectReferenceValue),
                        barColor, labelColor);
            }
        }
        ...
        private void CreateDragAndDropArea()
        {
            ...
            var dropArea = Layout.CreateRectAndDraw(Layout.longInfoWidth, 27, -11, texture: Icon.Get("RoomExit"),
                color: Color.white);
            Labels.Centered(dropArea, "MODULE +", Color.white);
        ...
        {
            var module = (string)obj;
            serializedObject.Update();
            var property = serializedObject.Get(module);
            if (property != null)
            {
            ...
            {
                property = serializedObject.Get("follow").Get(module);
                if (property != null)
                    property.SetTrue("edit");
            }
        ...
        {
            var cameraPosition = Tooly.SetDepth(main.cameraRef.transform.position, 0);
            var height = main.cameraRef.Height();
            var width = main.cameraRef.Width();

            // Position and Dead Zone
            if (main.follow.view && main.follow.enable && main.targetTransform != null &&
                main.follow.followType == FollowType.Target)
            {
                main.follow.targetTransform = main.targetTransform;
                var playerPosition = main.targetTransform.position + (Vector3)main.follow.offset;
            ...
            {
                var screenZone = main.follow.screenZone.size;
                Handles.color = Color.red;
                var offsetY = screenZone.y == 0 ? 0 : (1 - screenZone.y) * height;
                var offsetX = screenZone.x == 0 ? 0 : (1 - screenZone.x) * width;
                if (screenZone.x > 0)
                    SceneTools.DottedLine(cameraPosition + V * (height - offsetY) + H * width * screenZone.x,
                        cameraPosition - V * (height - offsetY) + H * width * screenZone.x);
                if (screenZone.x > 0)
                    SceneTools.DottedLine(cameraPosition + V * (height - offsetY) - H * width * screenZone.x,
                        cameraPosition - V * (height - offsetY) - H * width * screenZone.x);
                if (screenZone.y > 0)
                    SceneTools.DottedLine(cameraPosition - V * height * screenZone.y - H * (width - offsetX),
                        cameraPosition - V * height * screenZone.y + H * (width - offsetX));
                if (screenZone.y > 0)
                    SceneTools.DottedLine(cameraPosition + V * height * screenZone.y - H * (width - offsetX),
                        cameraPosition + V * height * screenZone.y + H * (width - offsetX));
            }
            ...
            //Push Zone
            if (main.follow.view && main.follow.enable &&
                (main.follow.pushZone.enabledX || main.follow.pushZone.enabledY))
            {
                Handles.color = Color.magenta;
                var distanceX = main.follow.pushZone.zoneX;
                var distanceY = main.follow.pushZone.zoneY;
                if (main.follow.pushZone.right)
                    SceneTools.DottedLine(cameraPosition + V * height + H * (width * distanceX),
                        cameraPosition - V * height + H * (width * distanceX));
                if (main.follow.pushZone.left)
                    SceneTools.DottedLine(cameraPosition + V * height - H * (width * distanceX),
                        cameraPosition - V * height - H * (width * distanceX));
                if (main.follow.pushZone.up)
                    SceneTools.DottedLine(cameraPosition + V * (height * distanceY) - H * width,
                        cameraPosition + V * (height * distanceY) + H * width);
            ...
            if (main.follow.lookAhead.enable && main.follow.lookAhead.view && main.follow.enable)
            {
                ...
                var distanceY = main.follow.lookAhead.distanceY;
                var distanceX = main.follow.lookAhead.distanceX;

                if (main.follow.lookAhead.zoneType == ZoneShape.Circle)
                {
                    var shortestDistance = width < height ? width : height;
                    var magnitude = shortestDistance * main.follow.lookAhead.radius;
                    SceneTools.WireDisc(cameraPosition, Vector3.forward, magnitude, Color.blue);
                }
                else
                {
                    if (distanceX > 0)
                        SceneTools.Line(cameraPosition + V * height + H * width * distanceX,
                            cameraPosition - V * height + H * width * distanceX);
                    if (distanceX > 0)
                        SceneTools.Line(cameraPosition + V * height - H * width * distanceX,
                            cameraPosition - V * height - H * width * distanceX);
                    if (distanceY > 0)
                        SceneTools.Line(cameraPosition - V * height * distanceY - H * width,
                            cameraPosition - V * height * distanceY + H * width);
                    if (distanceY > 0)
                        SceneTools.Line(cameraPosition + V * height * distanceY - H * width,
                            cameraPosition + V * height * distanceY + H * width);
