...
namespace TwoBitMachines.TwoBitSprite
{
    ...
    public class SpriteEngineUA : SpriteEngineBase
    {
        ...
        [SerializeField] public List<UAPacket> animations = new();
        [NonSerialized] private int currentIndex = -1;

        [NonSerialized] private string playAnimation;
        ...
        public void Awake()
        {
            ...
            if (setToFirst && animations.Count > 0) SetNewAnimation(animations[0].name);
        }
        ...
        public override void SetFirstAnimation()
        {
            ...
            if (animations.Count > 0) SetNewAnimation(animations[0].name);
        }
        ...
        public override void SetNewAnimation(string newAnimation)
        {
            ...
            for (var i = 0; i < animations.Count; i++)
                if (animations[i].name == newAnimation)
                {
                    ...
                    var newAnim = animations[i].isRandom
                        ? GetRandom(animations[i], newAnimation, ref playAnimation)
                        : animations[i];

                    var oldIndex = currentIndex;
                    currentIndex = i;
                    if (newAnim.hasTransition &&
                        newAnim.Transition(this, currentAnimation, out var transitionAnimation))
                    {
                    ...
                    {
                        currentAnimation = newAnimation;
                        if (oldIndex >= 0 && oldIndex < animations.Count && animations[oldIndex].canSync &&
                            animations[oldIndex].syncID == newAnim.syncID)
                        {
                            var entryTime = animator.GetCurrentAnimatorStateInfo(0).normalizedTime;
        ...
        {
            if (currentIndex < 0 || currentIndex >= animations.Count || !uaPacket.changedDirection) return;
            if (tree.SignalTrue("changedDirection") &&
                uaPacket.Transition(this, currentAnimation, out var transitionAnimation))
            {
                animator?.Play(transitionAnimation);
                transition = true;
            }
        ...
        {
            if (signal.TryGetValue(signalName, out var value) && value && animator != null)
            {
    ...
    public class UAPacket
    {
        ...
        [SerializeField] public List<RandomAnimation> randomAnimations = new();
        ...
        public bool Transition(SpriteEngineUA spriteEngine, string currentAnimation, out string transitionAnimation)
        {
            ...
            {
                if (transition[i].from != currentAnimation) continue;
