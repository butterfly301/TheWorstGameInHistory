...
namespace TwoBitMachines.TwoBitSprite
{
    ...
    {
        [SerializeField] public List<CharacterSkin> characterSkin = new();
        [NonSerialized] public List<SpritePacket> sprites = new();
        ...
        {
            if (sprites != null) Swap(skinName, sprites);
        }

        public void Swap(string skinName, List<SpritePacket> sprites)
        {
            for (var i = 0; i < characterSkin.Count; i++)
            {
                if (characterSkin[i].name != skinName) continue;

                var skin = characterSkin[i].skin;
                for (var j = 0; j < skin.Count; j++)
                for (var z = 0; z < sprites.Count; z++)
                {
                    if (sprites[z].name != skin[j].name) continue;
                    for (var x = 0; x < skin[j].sprite.Count; x++)
...
#pragma warning disable 0414
    ...
    public class CharacterSkin
    {
        ...
        [SerializeField] public List<Skins> skin = new();
#if UNITY_EDITOR
#pragma warning disable 0414
    ...
    public class Skins
    {
        ...
        [SerializeField] public List<Sprite> sprite = new();
#if UNITY_EDITOR
#pragma warning disable 0414
        [SerializeField] [HideInInspector] public bool add;
        [SerializeField] [HideInInspector] public bool edit;
