...
namespace TwoBitMachines.TwoBitSprite
{
    ...
    public class SpriteEngine : SpriteEngineBase
    {
        ...
        [SerializeField] public FlipType flipType;
        [NonSerialized] private int currentIndex = -1;

        [NonSerialized] public SpritePlayer player = new();
        ...
        public void Awake()
        {
            ...
            if (setToFirst && sprites.Count > 0) SetNewAnimation(sprites[0].name);
        }
        ...
        {
            if (pause || !enabled || !gameObject.activeInHierarchy) return;
        ...
        public void SetNewAnimation(int index)
        {
            ...
            SetNewAnimation(sprites[index].name);
        }
        ...
        public override bool FlipAnimation(Dictionary<string, bool> signal, string signalName, string direction)
        {
            ...
            {
                if (flipType == FlipType.Sprite)
                    FlipAnimationSprite(player.renderer, direction);
                else
                    FlipAnimationObject(transform, flipType, direction);
            }
