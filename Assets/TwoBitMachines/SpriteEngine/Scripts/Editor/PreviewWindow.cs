...
namespace TwoBitMachines.TwoBitSprite.Editors
{
    ...
    {
        public static void Display(Editor editor, SerializedObject parent, ref Sprite currentSprite,
            List<SpritePacket> sprites, int spriteIndex)
        {
            ...
            var mainPreviewWindow = Layout.CreateRect(Layout.longInfoWidth, Mathf.Min(Layout.infoWidth, 350), -11);

            var scrollInfo = new ScrollWindowInfo(parent);
            var sprite = currentSprite = RetrieveSpriteToPreview(editor, sprites, spriteIndex, scrollInfo);
            var spriteInfo = new SpriteWindowInfo(sprite, mainPreviewWindow, scrollInfo.zoom.floatValue);

            MouseScrollEvents(mainPreviewWindow, scrollInfo, spriteInfo);
            DisplaySpriteInWindow(mainPreviewWindow, scrollInfo, spriteInfo);
        }

        private static Sprite RetrieveSpriteToPreview(Editor editor, List<SpritePacket> sprites, int spriteIndex,
            ScrollWindowInfo scrollInfo)
        {
            ...
            var timer = scrollInfo.timer.floatValue;
            var frame = scrollInfo.frame.intValue;
            Sprite sprite = null;
            for (var i = 0; i < sprites.Count; i++)
            {
                if (i != spriteIndex)
                    continue;
                var frameArray = sprites[i].frame;

                if (!scrollInfo.playInInspector
                        .boolValue) //                      if sprite is currently not playing in inspector
                {
                    var index = sprites[i].frameIndex;
                    if (index >= 0 && index < frameArray.Count)
                        sprite = frameArray[index].sprite;
                }
                else if
                    (frameArray.Count >
                     0) //                                  if play in inspector is true, then play sprite animation
                {
                    frame = Mathf.Clamp(frame, 0, frameArray.Count - 1);
                    if (Clock.TimerEditor(ref timer, frameArray[frame].rate))
        ...
        {
            if (viewRect
                .ContainsMouseRightDown()) //                           reset sprite position and zoom level with right click
            {
            ...
            {
                var scale = spriteInfo.scale;
                var baseScale = spriteInfo.baseScale;
                var spriteSize = spriteInfo.spriteSize;
                ...
                var newScale = baseScale * scrollInfo.zoom.floatValue;
                scrollInfo.offsetPosition.vector2Value +=
                    new Vector2((scale - newScale) * spriteSize.x, (scale - newScale) * spriteSize.y) /
                    2f; // adjust texture position for zoom
            }
        ...
        private static void DisplaySpriteInWindow(Rect viewRect, ScrollWindowInfo scrollInfo,
            SpriteWindowInfo spriteInfo)
        {
            var offset = ScrollView.Begin(viewRect, scrollInfo.scrollPosition, spriteInfo.spritePosition,
                scrollInfo.color.colorValue);
            {
                var baseOffsetX = Mathf.Abs(viewRect.width - spriteInfo.spriteSize.x * spriteInfo.baseScale) / 2f;
                var baseOffsetY = Mathf.Abs(viewRect.height - spriteInfo.spriteSize.y * spriteInfo.baseScale) / 2f;
                ...
                if (spriteInfo.spriteTexture != null)
                    GUI.DrawTextureWithTexCoords(spriteInfo.spritePosition, spriteInfo.spriteTexture,
                        spriteInfo.spriteRect);
            }
            ScrollView.End();
            var colorRect = Layout.GetLastRect(15, 13, -11); //place color field on top left corner of window
            scrollInfo.color.colorValue = EditorGUI.ColorField(colorRect, GUIContent.none, scrollInfo.color.colorValue,
                false, true, false);
        }
        ...
        public static void Display(SpriteEngineExtraEditor editor, SerializedObject parent, SpriteEngineExtra main)
        {
            ...
            var scrollInfo = new ScrollWindowInfo(parent);
            var sprite = editor.currentSprite = RetrieveSpriteToPreview(editor, main, scrollInfo);
            var spriteInfo = new SpriteWindowInfo(sprite, mainPreviewWindow, scrollInfo.zoom.floatValue);
        ...
        private static Sprite RetrieveSpriteToPreview(SpriteEngineExtraEditor editor, SpriteEngineExtra main,
            ScrollWindowInfo scrollInfo)
        {
            if (scrollInfo.resetPlayFrame.ReadBool())
            {
            ...
            for (var i = 0; i < main.sprites.Count; i++)
            {
                if (i != main.spriteIndex)
                    continue;
                var frameArray = main.sprites[i].frame;

                if (!scrollInfo.playInInspector
                        .boolValue) //                      if sprite is currently not playing in inspector
                {
                    var index = main.sprites[i].frameIndex;
                    if (index >= 0 && index < frameArray.Count)
                        sprite = frameArray[index].sprite;
                }
                else if
