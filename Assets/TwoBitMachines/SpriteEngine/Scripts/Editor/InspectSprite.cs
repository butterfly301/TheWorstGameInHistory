...
namespace TwoBitMachines.TwoBitSprite.Editors
{
    ...
    {
        public static void Display(Editor editor, SerializedObject parent, SerializedProperty frames,
            SerializedProperty frameIndex, SerializedProperty sprite, bool usingExtra = false)
        {
        ...
        {
            if (Slider.Set(Tint.White, Tint.SoftDark, globalRate, 0.25f, 60f, true))
                for (var i = 0; i < frames.arraySize; i++)
                    frames.Element(i).Get("rate").floatValue = 1f / globalRate.floatValue;
        }

        private static void DisplaySprites(Editor editor, SerializedProperty frames, SerializedProperty frameIndex,
            SerializedProperty sprite, bool usingExtra)
        {
            if (evt.downArrow || evt.upArrow)
            {
                var newIndex = frameIndex.intValue + (evt.downArrow ? 1 : -1);
            ...
            for (var i = 0; i < frames.arraySize; i++)
            {
                var frame = frames.Element(i);
                var rate = Mouse.holding ? frame.Get("rate") : null;
                var ratePrevious = Mouse.holding ? rate.floatValue : 0;
                ...
                {
                    sprite.Get("globalRate").floatValue =
                        Mathf.Clamp(rate.floatValue == 0 ? 0 : 1f / rate.floatValue, 0.25f, 60f);
                    for (var j = 0; j < frames.arraySize; j++)
                ...
                if (Header.SignalActive("GripUsed"))
                {
                    ...
                    SpriteProperty.ReorderNestedArrays(sprite.Get("property"), ListReorder.srcItemIndex,
                        ListReorder.dstItemIndex);
                }
                ...
                if (Block.boxRect.ContainsScrollWheel())
                {
                    var newIndex = frameIndex.intValue + evt.scrollDelta;
                    frameIndex.intValue = Compute.WrapArrayIndex(newIndex, frames.arraySize);
                }
            }
        }
        ...
        {
            var array = frame.Get("events");
            for (var i = 0; i < array.arraySize; i++)
            {
                var element = array.Element(i);
                if (Fields.EventFoldOutFloat(element.Get("frameEvent"), element.Get("atPercent"),
                        element.Get("eventFoldOut"), "       Frame Event", offsetX: 20, ySpace: false,
                        color: Tint.SoftDark))
                {
            ...
            if (FoldOut.CornerButton(Tint.Blue, ySpace: false)) array.arraySize++;
