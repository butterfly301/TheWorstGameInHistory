    ...
    public class SpriteEngineExtraEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            serializedObject.Update();
            {
                ...
                if (CreateSprite()) SetCurrentSprite(); // reset animation if sprites have been added or deleted
                if (spriteExists)
                {
        ...
        {
            var delete = parent.Bool("deleteAsk");
            FoldOut.Bar(parent, Tint.SoftDark, 4).BL("open", "ArrowRight").BR("deleteAsk", "Yes", execute: delete)
                .SR(8, delete).BR("delete", "Close", execute: delete).BR("deleteAsk", "Minus", execute: !delete).SR()
                .BR(execute: !delete).BR("sort", "Sort");
            {
                var labelRect = new Rect(Bar.barStart) { width = Layout.longInfoWidth - 70 };
                labelRect.DrawRect(texture: Skin.square, color: Color.white);
                if (spriteNames.Count > 0)
                    GUI.Label(labelRect.Adjust(5, labelRect.width - 4), spriteNames[spriteIndex.intValue],
                        FoldOut.boldStyle);
                if (spriteExists) sprite.FieldOnlyClear(labelRect, "name"); // change index string name
            }
            if (parent.Bool("open"))
            {
        ...
        {
            tempSprites.Clear();
            var dropArea = Layout.CreateRectAndDraw(Layout.longInfoWidth, 88, -11, texture: Skin.square,
                color: Tint.SoftDark);
            {
                ...
                if (ActiveEditorTracker.sharedTracker.isLocked)
                    Labels.Centered(dropArea, "Locked", color, 10, 28);
            }
        }
        ...
        public void Options()
        {
            ...
            {
                var array = sprite.Get("randomSprites");
                ...
                {
                    Fields.EventField(sprite.Get("onLoopOnce"), 10, Tint.SoftDark);
                }
                else
                {
                    Layout.VerticalSpacing(4);
                    Layout.CreateRectAndDraw(Layout.longInfoWidth, 42, -11, texture: Texture2D.whiteTexture,
                        color: Tint.SoftDark); // to keep all options height even
                }
            }
        }
        ...
        public void Buttons()
        {
            var menu = parent.Get("spriteMenu");
            var resetSprites = parent.Get("resetSprites");
            var propertyFoldOut = parent.Get("propertyFoldOut");

            Bar.Setup(Texture2D.whiteTexture, Tint.SoftDark, false, 20);
